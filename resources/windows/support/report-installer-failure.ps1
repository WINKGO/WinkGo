...
$log = $LogPath
if (-not $log) {
  $log = Join-Path $env:TEMP 'winkgo-installer-fallback-log.jsonl'
}
$logFileName = Split-Path -Leaf $log
$statusPath = Join-Path $env:TEMP 'winkgo-installer-report.json'
...
function Read-WinkGoAnalyticsId {
  $candidates = @()
  if ($env:APPDATA) {
    $candidates += (Join-Path $env:APPDATA 'WINK GO\analytics.json')
    $candidates += (Join-Path $env:APPDATA 'WinkGo\analytics.json')
  }
  if ($env:LOCALAPPDATA) {
    $candidates += (Join-Path $env:LOCALAPPDATA 'WINK GO\analytics.json')
    $candidates += (Join-Path $env:LOCALAPPDATA 'WinkGo\analytics.json')
  }
...
function Get-OptionalHandleDiagnostics {
  ...
  $outPath = Join-Path $env:TEMP ('winkgo-handle-' + [guid]::NewGuid().ToString('N') + '.out')
  $errPath = Join-Path $env:TEMP ('winkgo-handle-' + [guid]::NewGuid().ToString('N') + '.err')
...
function New-ReportDetailsText(
  ...
  $lines.Add('--------------------------------')
  $lines.Add('WINK GO installer failure ' + $code)
  $lines.Add('--------------------------------')
  $lines.Add('')
  if ($eventId) {
...
try {
  ...
  $userId = Read-WinkGoAnalyticsId
  $eventPayload = @{
    message = ('installer-failure ' + $Code)
    level = 'error'
    platform = 'other'
    release = $Release
    logger = 'winkgo.installer'
  ...
  Show-ReportMessage ('WINK GO installer report sent.' + [Environment]::NewLine + [Environment]::NewLine + 'Press [ Ctrl + C ] in this dialog to copy the diagnostic details, then contact the WINK GO team through:' + [Environment]::NewLine + 'https://winkgo.top/' + [Environment]::NewLine + [Environment]::NewLine + $reportDetails) 'Information'
} catch {
  ...
  Show-ReportMessage ('WINK GO installer report failed.' + [Environment]::NewLine + [Environment]::NewLine + 'Status: ' + $statusPath + [Environment]::NewLine + 'Log: ' + $log) 'Exclamation'
}
