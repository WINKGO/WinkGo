...
duplicate_versions="$(
    find crates/winkgo-db/migrations -maxdepth 1 -type f -name '*.sql' -print \
        | awk -F/ '
            {
                name = $NF
                if (name ~ /^[0-9]+_/) {
...
if [[ "${WINKGO_CORE_ALLOW_MAIN_MIGRATION_EDIT:-}" == "1" ]]; then
    echo "WINKGO_CORE_ALLOW_MAIN_MIGRATION_EDIT=1; skipping migration immutability check"
    exit 0
fi

base_ref="${WINKGO_CORE_MIGRATION_BASE_REF:-}"
if [[ -z "$base_ref" ]]; then
...
changed="$(
    git diff --name-status --diff-filter=DMR "$base_commit" -- 'crates/winkgo-db/migrations/*.sql'
)"
...
If this is an intentional high-risk exception, rerun with WINKGO_CORE_ALLOW_MAIN_MIGRATION_EDIT=1.
