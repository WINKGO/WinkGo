...
$migrationDir = Join-Path $repoRoot "crates/winkgo-db/migrations"
...
if ($env:WINKGO_CORE_ALLOW_MAIN_MIGRATION_EDIT -eq "1") {
    Write-Output "WINKGO_CORE_ALLOW_MAIN_MIGRATION_EDIT=1; skipping migration immutability check"
    exit 0
}

$baseRef = $env:WINKGO_CORE_MIGRATION_BASE_REF
...
$changed = git diff --name-status --diff-filter=DMR $baseCommit -- "crates/winkgo-db/migrations/*.sql"
if (-not [string]::IsNullOrWhiteSpace(($changed -join "`n"))) {
    ...
    [Console]::Error.WriteLine("If this is an intentional high-risk exception, rerun with WINKGO_CORE_ALLOW_MAIN_MIGRATION_EDIT=1.")
    [Console]::Error.WriteLine("")
