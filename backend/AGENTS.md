...
(claude, codex, gemini, opencode, hermes, winkgo_agent, …) — its wire protocol, message shapes, field
...
1. **Captured real data** — actual sampled wire traffic under
   `~/winkgo/protocols/samples/` (e.g. `codex-cli/<ver>/`, `claude-cli/<ver>/`, `codex-acp/`,
...
3. **An official adapter's code** — the codex `app-server` machine-generated JSON schema under
   `~/winkgo/protocols/samples/codex-cli/<ver>/schema-full/` (ground truth from the codex binary
...
See also the standing discipline in the root `AGENTS.md` / memory `winkgo_core-verification-blindspot-g6`: self-consistent-all-green ≠ correct — verify outward (against a real agent) AND against the old/reference implementation, not just against your own happy path.
...
- All request/response types defined in `winkgo-api-types`
- `winkgo-api-types` must NOT depend on axum/tower or any HTTP framework
- Use `winkgo_common::ApiError` only at API/HTTP boundaries such as routes and middleware. Service/domain code must prefer crate-owned errors (`ConversationError`, `TeamError`, etc.) and map them to `ApiError` in route modules.
...
- Repository traits in `winkgo-db`, prefixed with `I`
- Concrete implementations prefixed with `Sqlite`
- Row models in `winkgo-db/src/models/`
...
- All assembly happens in `winkgo-app`'s `build_*_state()` functions
...
New subprocess spawn sites must go through `winkgo_runtime`'s spawn Builder — never raw `tokio::process::Command`. See [ARCHITECTURE.md § Runtime Infrastructure](./ARCHITECTURE.md#runtime-infrastructure) for the correct constructor and details.
...
1. Request/response types → `winkgo-api-types/src/{domain}.rs`
2. Handler function → `crates/winkgo-{domain}/src/routes.rs`
3. Business logic → `crates/winkgo-{domain}/src/service.rs`
4. Register route in `domain_routes()` function
5. Add test → `crates/winkgo-{domain}/tests/` or `crates/winkgo-app/tests/`

### Add Migration

1. Next number → `ls crates/winkgo-db/migrations/`
2. Create `NNN_descriptive_name.sql` with `IF NOT EXISTS`

### Add WebSocket Event

1. Event type → `winkgo-api-types`
...
> ⚠️ **When to run what:**
> - During development: only test the crate you're working on → `cargo test -p winkgo-<crate>`
...
> - `cargo clippy -p winkgo-<crate>` and `cargo test -p winkgo-<crate>` typically complete in under 1 minute.

### During Development (fast feedback loop)

```bash
cargo test -p winkgo-<crate>                          # Test the crate you changed
cargo clippy -p winkgo-<crate> -- -D warnings         # Lint the crate you changed
```
...
cargo clippy -p winkgo-<crate1> -p winkgo-<crate2> -- -D warnings              # Lint affected crates
cargo test -p winkgo-<crate1> -p winkgo-<crate2>                               # Test affected crates
```
