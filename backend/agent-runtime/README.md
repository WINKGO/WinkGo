...
./target/release/winkgo_agent config init
# Edit the generated config (run `winkgo_agent config path` to find it)

# Single-shot mode
winkgo_agent "Read Cargo.toml and explain the dependencies"
...
```bash
winkgo_agent --max-turns 10 "Run the task"
winkgo_agent --max-tool-call-malformed-turns 2 "Run the task"
winkgo_agent --max-tool-call-failure-turns 2 "Run the task"
```
