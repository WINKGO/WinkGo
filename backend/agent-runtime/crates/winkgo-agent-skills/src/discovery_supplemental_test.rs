...
mod discovery_supplemental_tests {
    ...
    /// Create a `.winkgo_agent/skills/` directory inside `parent`.
    fn create_skill_dir(parent: &Path) -> PathBuf {
        let dir = parent.join(".winkgo_agent").join("skills");
    ...
    // TC-22: discovers `.winkgo_agent/skills/` inside a direct subdirectory of cwd.
    #[tokio::test]
    async fn tc22_discover_dirs_finds_winkgo_agent_skills_in_subdir() {
        let tmp = TempDir::new().unwrap();
        let cwd = tmp.path().to_str().unwrap().to_string();

        // Create /tmp/proj/module/.winkgo_agent/skills/
        ...
        assert!(found[0].ends_with(".winkgo_agent/skills"));
    }
    ...
    // TC-23: `.winkgo_agent/skills/` at cwd level is not returned (loaded at startup).
    #[tokio::test]
    ...
    // TC-25: directories without `.winkgo_agent/skills/` are still recorded to avoid
    // repeated stat calls.
    #[tokio::test]
    async fn tc25_discover_dirs_records_miss_dirs_in_checked() {
        ...
        // `b` does NOT have .winkgo_agent/skills/
