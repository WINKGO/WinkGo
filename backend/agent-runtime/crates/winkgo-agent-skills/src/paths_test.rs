...
mod tests {
    ...
    #[test]
    fn test_user_skills_dir_contains_winkgo_agent_skills() {
        if let Some(dir) = user_skills_dir() {
            let s = dir.to_string_lossy();
            assert!(s.contains("winkgo_agent"), "expected 'winkgo_agent' in path: {s}");
    ...
    #[test]
    fn test_user_commands_dir_contains_winkgo_agent_commands() {
        if let Some(dir) = user_commands_dir() {
            let s = dir.to_string_lossy();
            assert!(s.contains("winkgo_agent"));
    ...
    fn test_project_skills_dirs_finds_dirs() {
        ...
        make_dir(root, ".winkgo_agent/skills");
        let nested = root.join("sub").join("project");
        fs::create_dir_all(&nested).unwrap();
        make_dir(&nested, ".winkgo_agent/skills");
    ...
    fn test_project_skills_dirs_skips_missing() {
        let tmp = TempDir::new().unwrap();
        fs::create_dir(tmp.path().join(".git")).unwrap();
        // No .winkgo_agent/skills/ anywhere
    ...
    fn test_additional_skills_dirs_existing() {
        let tmp = TempDir::new().unwrap();
        make_dir(tmp.path(), ".winkgo_agent/skills");
    ...
    fn test_additional_skills_dirs_missing_silently_skipped() {
        let tmp = TempDir::new().unwrap();
        // No .winkgo_agent/skills/ under tmp
        let result = additional_skills_dirs(&[tmp.path().to_path_buf()]);
...
mod supplemental_tests {
    ...
    fn tc_2_1_user_skills_dir_ends_with_skills() {
        if let Some(dir) = user_skills_dir() {
            ...
            assert!(s.contains("winkgo_agent"), "path should contain 'winkgo_agent': {s}");
        }
    }
    ...
    fn tc_4_2_project_skills_dirs_nonexistent_subdir_not_returned() {
        let tmp = TempDir::new().unwrap();
        fs::create_dir(tmp.path().join(".git")).unwrap();
        // No .winkgo_agent/skills/ created
        let dirs = project_skills_dirs(tmp.path());
        assert!(
            dirs.is_empty(),
            "should be empty when .winkgo_agent/skills/ doesn't exist"
        );
    }
    ...
    fn tc_4_3_project_skills_dirs_deepest_first() {
        ...
        make_dir(&inner, ".winkgo_agent/skills");
    ...
    fn tc_4_4_project_skills_dirs_stops_at_git_root() {
        ...
        // .winkgo_agent/skills in grandparent (above git root) — should NOT be collected
        make_dir(grandparent, ".winkgo_agent/skills");

        let repo = grandparent.join("repo");
        fs::create_dir_all(&repo).unwrap();
        fs::create_dir(repo.join(".git")).unwrap();
        make_dir(&repo, ".winkgo_agent/skills");

        let sub = repo.join("sub");
        fs::create_dir_all(&sub).unwrap();

        let dirs = project_skills_dirs(&sub);
        // Only repo's .winkgo_agent/skills should be included
        assert!(
    ...
    fn tc_5_1_project_commands_dirs_finds_commands_dir() {
        ...
        let root = tmp.path();
        fs::create_dir(root.join(".git")).unwrap();
        make_dir(root, ".winkgo_agent/commands");

        let dirs = project_commands_dirs(root);
        assert_eq!(dirs.len(), 1);
        assert!(dirs[0].ends_with(".winkgo_agent/commands"));
    }
    ...
    fn tc_6_1_additional_skills_dirs_with_existing_subdir() {
        ...
        let result = additional_skills_dirs(&[tmp.path().to_path_buf()]);
        assert_eq!(result.len(), 1);
        assert!(result[0].ends_with(".winkgo_agent/skills"));
    }

    #[test]
    fn tc_6_2_additional_skills_dirs_no_subdir_skipped() {
        let tmp = TempDir::new().unwrap();
        // No .winkgo_agent/skills/ subdirectory
        let result = additional_skills_dirs(&[tmp.path().to_path_buf()]);
        assert!(result.is_empty());
    }
    ...
    fn tc_6_4_additional_skills_dirs_multiple_add_dirs() {
        ...
        make_dir(tmp1.path(), ".winkgo_agent/skills");
        make_dir(tmp2.path(), ".winkgo_agent/skills");
