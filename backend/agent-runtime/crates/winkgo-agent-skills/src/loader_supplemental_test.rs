...
async fn tc_11_1_bare_mode_only_loads_add_dirs() {
    ...
    // Put a skill in add_dir's .winkgo_agent/skills/
    let add_skills_dir = add_tmp.path().join(".winkgo_agent").join("skills");
...
async fn tc_11_5_empty_scenario_returns_empty_vec() {
    ...
    let tmp = TempDir::new().unwrap();
    // tmp exists but has no .winkgo_agent/skills
...
// matching TS skillRoot used for ${WINKGO_AGENT_SKILL_DIR} substitution.
// -----------------------------------------------------------------------
