...
// AC-5: $ARGUMENTS, $0, ${WINKGO_AGENT_SKILL_DIR} are correctly substituted
// ---------------------------------------------------------------------------
...
async fn tc_e2e_2_inline_variable_substitution() {
    ...
    let mut skill = make_skill(
        "var-skill",
        "Arguments: $ARGUMENTS\nFirst arg: $0\nSkill dir: ${WINKGO_AGENT_SKILL_DIR}",
    );
    ...
    assert!(
        ...
        "${{WINKGO_AGENT_SKILL_DIR}} should be replaced with skill root, got: {result}"
    );
    ...
    assert!(
        !result.contains("${WINKGO_AGENT_SKILL_DIR}"),
        "${{WINKGO_AGENT_SKILL_DIR}} literal should not remain, got: {result}"
    );
}
...
// AC-14: legacy command files from .winkgo_agent/commands/ are loaded as SkillDefinition
// ---------------------------------------------------------------------------
...
async fn tc_e2e_11_legacy_commands_loaded() {
    ...
    // load_all_skills looks for .winkgo_agent/commands/ relative to cwd.
    let commands_dir = tmp.path().join(".winkgo_agent").join("commands");
    ...
    let skill = skills
        ...
        .expect("legacy-cmd should be loaded from flat .md file in .winkgo_agent/commands/");
