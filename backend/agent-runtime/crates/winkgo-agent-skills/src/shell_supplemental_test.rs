...
async fn tc_4_5_cwd_used() {
    use winkgo_agent_config::shell::ShellKind;

    let tmp = std::env::temp_dir();
    let shell = winkgo_agent_config::shell::default_shell();
...
async fn tc_windows_skill_shell_powershell_stdout_preserved() {
    let result = run("before !`Write-Output winkgo_skill_stdout_probe` after")
        .await
        .unwrap();

    assert!(
        result.contains("winkgo_skill_stdout_probe"),
