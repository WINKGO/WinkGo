...
async fn load_skill_file(
    ...
    // used for ${WINKGO_AGENT_SKILL_DIR} variable substitution in skill content.
...
///
/// Examples:
/// - base=`<config_dir>/winkgo_agent/skills`, target=`<config_dir>/winkgo_agent/skills/db/migrate` → `"db:migrate"`
/// - base=`<config_dir>/winkgo_agent/skills`, target=`<config_dir>/winkgo_agent/skills/my-skill` → `"my-skill"`
