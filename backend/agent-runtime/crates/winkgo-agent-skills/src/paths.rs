...
// ---------------------------------------------------------------------------
// User-level directories (<config_dir>/winkgo_agent/)
// ---------------------------------------------------------------------------

/// Return the user-level skills directory: `<config_dir>/winkgo_agent/skills/`
///
...
/// Return the user-level legacy commands directory: `<config_dir>/winkgo_agent/commands/`
...
/// Find all project-level `.winkgo_agent/skills/` directories by walking up from
...
/// Find all project-level `.winkgo_agent/commands/` directories (legacy), same walk.
...
///
/// Each path in `add_dirs` is checked for a `.winkgo_agent/skills/` subdirectory.
/// Only directories that exist are included.
pub fn additional_skills_dirs(add_dirs: &[PathBuf]) -> Vec<PathBuf> {
    add_dirs
        .iter()
        .map(|d| d.join(".winkgo_agent").join("skills"))
        .filter(|p| p.is_dir())
        .collect()
}
...
/// `.winkgo_agent/<subdir>/` directories that exist. Returns deepest-first.
fn walk_up_dirs(cwd: &Path, subdir: &str) -> Vec<PathBuf> {
    ...
    loop {
        let candidate = current.join(".winkgo_agent").join(subdir);
        if candidate.is_dir() {
            dirs.push(candidate);
        }
