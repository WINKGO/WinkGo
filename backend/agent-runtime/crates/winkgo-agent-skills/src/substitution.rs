...
/// 5. Skill directory: `${WINKGO_AGENT_SKILL_DIR}` → `skill_root`
/// 6. Session ID: `${WINKGO_AGENT_SESSION_ID}` → `session_id`
...
pub fn substitute_arguments(
    ...
    // 5. ${WINKGO_AGENT_SKILL_DIR}
    if let Some(root) = skill_root {
        result = result.replace("${WINKGO_AGENT_SKILL_DIR}", root);
    }

    // 6. ${WINKGO_AGENT_SESSION_ID}
    if let Some(sid) = session_id {
        result = result.replace("${WINKGO_AGENT_SESSION_ID}", sid);
    }
