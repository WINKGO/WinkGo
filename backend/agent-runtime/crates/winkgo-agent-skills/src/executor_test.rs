...
mod tests {
    ...
    async fn test_prepare_inline_substitutes_skill_dir() {
        let skill = make_skill("Dir: ${WINKGO_AGENT_SKILL_DIR}", Some("/skills/mine"));
        let result = prepare_inline_content(&skill, None, None, Path::new("/tmp"))
            .await
            .unwrap();
    ...
    async fn test_prepare_inline_substitutes_session_id() {
        let skill = make_skill("Session: ${WINKGO_AGENT_SESSION_ID}", None);
...
mod supplemental_tests {
    ...
    async fn tc_10_3_skill_root_prepends_header() {
        let skill = make_skill_full(
            "s",
            "${WINKGO_AGENT_SKILL_DIR}/script.sh",
            Some("/path/to/skill"),
            vec![],
...
mod phase7_tests {
    ...
    use winkgo_agent_types::spawner::{ForkOverrides, Spawner, SubAgentConfig, SubAgentResult};
