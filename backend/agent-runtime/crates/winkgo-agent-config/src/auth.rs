...
impl OAuthManager {
    ...
    pub async fn get_token(&self) -> anyhow::Result<String> {
        ...
        anyhow::bail!("Token expired and no refresh token available. Run 'winkgo_agent auth login'")
    }
    ...
    fn load_credentials(&self) -> anyhow::Result<OAuthCredentials> {
        let json = std::fs::read_to_string(&self.credentials_path)
            .map_err(|_| anyhow::anyhow!("No saved credentials. Run 'winkgo_agent auth login'"))?;
