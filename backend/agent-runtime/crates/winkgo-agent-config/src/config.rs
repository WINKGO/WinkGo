...
use winkgo_agent_types::llm::ThinkingConfig;
...
fn default_session_dir() -> String {
    ".winkgo_agent/sessions".to_string()
}
...
impl Config {
    ...
    pub fn resolve(cli: &CliArgs) -> anyhow::Result<Self> {
        ...
        let project_path = cli
            .project_dir
            .as_ref()
            .map(|d| d.join(".winkgo_agent.toml"))
...
///
/// - Linux:   `~/.config/winkgo_agent`
/// - macOS:   `~/Library/Application Support/winkgo_agent`
/// - Windows: `%APPDATA%\winkgo_agent`
pub fn app_config_dir() -> Option<PathBuf> {
    dirs::config_dir().map(|d| d.join("winkgo_agent"))
}
...
pub fn global_config_path() -> PathBuf {
    app_config_dir()
        .unwrap_or_else(|| PathBuf::from("winkgo_agent"))
        .join("config.toml")
}

fn project_config_path() -> PathBuf {
    PathBuf::from(".winkgo_agent.toml")
}

fn load_config_file(path: &Path) -> ConfigFile {
    match std::fs::read_to_string(path) {
        Ok(content) => toml::from_str(&content).unwrap_or_else(|e| {
            tracing::warn!(target: "winkgo_agent_config", path = %path.display(), error = %e, "failed to parse config file");
            ConfigFile::default()
        }),
...
pub fn init_config() -> anyhow::Result<()> {
    ...
    if path.exists() {
        tracing::info!(target: "winkgo_agent_config", path = %path.display(), "config file already exists");
        return Ok(());
    }
    ...
    tracing::info!(target: "winkgo_agent_config", path = %path.display(), "config file created");
    Ok(())
}

const DEFAULT_CONFIG_TEMPLATE: &str = r#"# winkgo_agent configuration
...
[session]
enabled = true
directory = ".winkgo_agent/sessions"  # relative to project root
...
# dir = "~/Library/Logs/winkgo_agent"    # log directory (default: platform-specific)
