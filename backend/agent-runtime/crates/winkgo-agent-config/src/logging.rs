...
pub fn default_log_dir() -> PathBuf {
    ...
    {
        dirs::home_dir()
            .map(|h| h.join("Library").join("Logs").join("winkgo_agent"))
            .unwrap_or_else(|| PathBuf::from("winkgo_agent/logs"))
    }
    #[cfg(target_os = "linux")]
    {
        dirs::state_dir()
            .map(|d| d.join("winkgo_agent").join("logs"))
            .unwrap_or_else(|| {
                dirs::home_dir()
                    .map(|h| h.join(".local").join("state").join("winkgo_agent").join("logs"))
                    .unwrap_or_else(|| PathBuf::from("winkgo_agent/logs"))
            })
    }
    ...
    {
        PathBuf::from("winkgo_agent/logs")
    }
}
    ...
    S: Subscriber + for<'a> LookupSpan<'a>,
{
    ...
    let file_appender = tracing_appender::rolling::RollingFileAppender::builder()
        ...
        .filename_suffix("winkgo_agent.log")
        .build(&config.dir)
