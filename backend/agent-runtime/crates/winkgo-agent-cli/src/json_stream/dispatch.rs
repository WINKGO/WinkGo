...
use winkgo_agent_protocol::ToolApprovalResult;
...
pub(super) fn handle(cmd: ProtocolCommand, engine: &mut AgentEngine, ctx: &StreamContext) -> DispatchOutcome {
    match cmd {
        ...
        ProtocolCommand::InitHistory { text } => {
            tracing::debug!(target: "winkgo_agent_protocol", chars = text.len(), "InitHistory received");
        }
        ProtocolCommand::SetMode { mode } => {
            ...
            tracing::debug!(target: "winkgo_agent_protocol", mode = %mode_str, "SetMode applied");
        }
