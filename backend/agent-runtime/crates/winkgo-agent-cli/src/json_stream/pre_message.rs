...
use winkgo_agent_mcp::tool_proxy::register_single_server_tools;
use winkgo_agent_protocol::commands::ProtocolCommand;
use winkgo_agent_protocol::events::ProtocolEvent;
use winkgo_agent_protocol::writer::{ProtocolEmitter, ProtocolWriter};
    ...
    while let Some(cmd) = cmd_rx.recv().await {
        match cmd {
            ProtocolCommand::AddMcpServer {
                ...
                tracing::info!(target: "winkgo_agent_mcp", %name, %transport, ?command, "AddMcpServer received");
                ...
                tracing::info!(target: "winkgo_agent_mcp", %name, "connecting to mcp server");
                match McpManager::connect_all(&single_configs).await {
                    Ok(mgr) => {
                        ...
                        tracing::info!(target: "winkgo_agent_mcp", %name, tools = tool_names.len(), "mcp server connected");
                    ...
                    Err(e) => {
                        tracing::warn!(target: "winkgo_agent_mcp", %name, error = %e, "mcp server connection failed");
