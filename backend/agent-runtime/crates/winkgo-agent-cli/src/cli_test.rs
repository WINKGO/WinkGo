...
fn no_subcommand_parses_prompt_as_trailing_args() {
    let cli = Cli::try_parse_from(["winkgo_agent", "write", "a", "function"]).unwrap();
...
fn config_init_parses_to_config_action() {
    let cli = Cli::try_parse_from(["winkgo_agent", "config", "init"]).unwrap();
    assert!(matches!(
        cli.command,
...
fn deleted_flags_are_rejected() {
    assert!(Cli::try_parse_from(["winkgo_agent", "--config-path"]).is_err());
    assert!(Cli::try_parse_from(["winkgo_agent", "--login"]).is_err());
    assert!(Cli::try_parse_from(["winkgo_agent", "--list-sessions"]).is_err());
