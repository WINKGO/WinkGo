...
//!
//! Note: `winkgo_agent` also exposes a module named `bootstrap`
//! (`winkgo_agent::bootstrap::AgentBootstrap`). Call sites should keep using
//! that fully-qualified path for the engine builder itself; this module is
//! `crate::bootstrap` and only wraps it.

use std::sync::Arc;

use winkgo_agent::bootstrap::{AgentBootstrap, BootstrapResult};
use winkgo_agent::output::OutputSink;
use winkgo_agent::session::{Session, SessionManager};
use winkgo_agent_compact::CompactLevel;
use winkgo_agent_config::config::{CliArgs, Config};
use winkgo_agent_config::logging::{LoggingGuard, create_file_layer};
