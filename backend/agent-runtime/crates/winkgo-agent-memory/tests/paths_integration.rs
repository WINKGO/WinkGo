...
fn tc_2_1_default_base_dir_uses_platform_config() {
    ...
    // Should end with "winkgo_agent" (the brand, not "claude")
    assert!(
        base.to_string_lossy().contains("winkgo_agent"),
        "base dir should use winkgo_agent brand: {base:?}"
    );
