...
const MEMORY_DIR_ENV: &str = "WINKGO_AGENT_MEMORY_DIR";
...
///
/// Resolution order:
///   1. `WINKGO_AGENT_MEMORY_DIR` environment variable (explicit override)
///   2. `app_config_dir()` from `winkgo-agent-config` (platform-aware default)
///
...
pub fn memory_base_dir() -> Option<PathBuf> {
    ...
    winkgo_agent_config::config::app_config_dir()
}
