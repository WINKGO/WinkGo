...
use tokio::sync::mpsc;
use winkgo_agent::spawner::{AgentSpawner, SubAgentConfig};
use winkgo_agent::tool_policy::ToolPolicy;
...
async fn test_spawn_shares_provider() {
    ...
    let provider_dyn: Arc<dyn winkgo_agent_providers::LlmProvider> = provider;
    let spawner = AgentSpawner::new(
