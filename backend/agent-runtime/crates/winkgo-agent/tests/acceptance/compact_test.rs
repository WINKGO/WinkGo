...
use serde_json::json;
use winkgo_agent::compact::auto::{BOUNDARY_PREFIX, autocompact, should_autocompact};
use winkgo_agent::compact::emergency::is_at_emergency_limit;
use winkgo_agent::compact::micro::{CLEARED_TOOL_RESULT, microcompact};
...
async fn autocompact_triggers_llm_summary() {
    ...
    let config = {
        ...
        winkgo_agent_config::config::Config {
    ...
    let provider = winkgo_agent_providers::create_provider(&config);
