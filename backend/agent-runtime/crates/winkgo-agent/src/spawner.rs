...
use winkgo_agent_providers::LlmProvider;
...
use winkgo_agent_types::message::TokenUsage;
...
// Re-export from winkgo-agent-types — single source of truth
pub use winkgo_agent_types::spawner::{ForkOverrides, Spawner, SubAgentConfig, SubAgentResult};
...
impl AgentSpawner {
    ...
    pub async fn spawn_one(&self, sub_config: SubAgentConfig) -> SubAgentResult {
        ...
        tracing::info!(target: "winkgo_agent", cwd = %self.cwd.display(), "sub-agent spawned with workspace cwd");
...
fn build_tool_registry(policy: &ToolPolicy, cwd: &Path, runtime_env: &[(String, String)]) -> ToolRegistry {
    let all_tools: Vec<(&str, Box<dyn winkgo_agent_tools::Tool>)> = vec![
