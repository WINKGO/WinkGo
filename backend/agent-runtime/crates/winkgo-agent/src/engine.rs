...
use winkgo_agent_config::config::Config;
use winkgo_agent_config::hooks::HookEngine;
use winkgo_agent_protocol::ToolApprovalManager;
use winkgo_agent_protocol::events::ToolCategory;
use winkgo_agent_protocol::writer::ProtocolEmitter;
use winkgo_agent_providers::provider::{LlmProvider, create_provider};
use winkgo_agent_tools::registry::ToolRegistry;
use winkgo_agent_types::llm::{LlmEvent, LlmRequest, ThinkingConfig};
use winkgo_agent_types::message::{ContentBlock, ImageInputCapability, Message, Role, StopReason, TokenUsage};
use winkgo_agent_types::skill_types::{ContextModifier, PlanModeTransition, effort_to_string};
...
impl AgentEngine {
    ...
    async fn run_inner(&mut self, content_blocks: Vec<ContentBlock>, msg_id: &str) -> Result<AgentResult, AgentError> {
        ...
        loop {
            if let Some(limit) = guards.turn_budget_reached() {
                ...
                warn!(target: "winkgo_agent", limit, "stopping agent run at turn budget");
    ...
    async fn run_compaction(&mut self) -> Result<(), AgentError> {
        ...
        if should_compact {
            info!(target: "winkgo_agent", context_tokens = self.compact_state.last_input_tokens, "context compaction triggered");
...
impl AgentEngine {
    ...
    pub fn init_session(&mut self, provider_name: &str, cwd: &str, session_id: Option<&str>) -> AnyhowResult<()> {
        if let Some(mgr) = &self.session_manager {
            ...
            info!(target: "winkgo_agent", session_id = %session.id, provider = %provider_name, model = %self.model, "session started");
    ...
    pub async fn run_stop_hooks(&self) {
        if let Some(hook_engine) = &self.hooks {
            ...
            for msg in messages {
                info!(target: "winkgo_agent", hook_message = %msg, "stop hook output");
            }
        }
    }
}
