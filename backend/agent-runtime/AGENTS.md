...
winkgo_agent is a **multi-provider AI agent CLI** written in Rust. It connects to
...
protocol for host integration (e.g. Electron-based WINK GO).
...
|-------|-------|----------------|
| Bottom | `winkgo-agent-types` | Shared provider-neutral data types (LLM, message, tool) — zero internal deps |
| Bottom | `winkgo-agent-compact` | Context compression algorithms (folding, sanitization, tokenization) |
| Mid | `winkgo-agent-config` | Configuration, ProviderCompat, auth, hooks, logging (`create_file_layer`), **cross-platform shell helpers** |
| Mid | `winkgo-agent-protocol` | JSON stream protocol (events, commands, approval manager) for host integration |
| Mid | `winkgo-agent-providers` | LLM provider implementations (Anthropic, OpenAI, Bedrock, Vertex) |
| Mid | `winkgo-agent-tools` | Built-in agent tools (Read, Write, Edit, Bash, Grep, Glob, Spawn) |
| Mid | `winkgo-agent-mcp` | MCP (Model Context Protocol) client |
| Mid | `winkgo-agent-skills` | Skills system (prompt snippets, hooks, permissions, shell expansion) |
| Mid | `winkgo-agent-memory` | Long-term cross-session memory (user prefs, feedback, project context) |
| Top | `winkgo-agent` | Agent engine, session management, orchestration |
| Top | `winkgo-agent-cli` | CLI binary entry point |
...
- WinkGoCLI is the built-in SDK/library integration path. Do not model it as an
  external agent subprocess when changing WinkGo host integration code.
...
- All shell invocations must go through `winkgo_agent_config::shell` module
...
| [json-stream-protocol.md](docs/json-stream-protocol.md) | JSON Lines protocol spec for host integration (e.g. WINK GO) |
