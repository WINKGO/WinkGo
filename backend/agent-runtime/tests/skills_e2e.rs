...
//! `.winkgo_agent/skills/` and `.winkgo_agent/commands/` layout, then exercises the full
...
use winkgo_agent::context::{SystemPromptCache, build_system_prompt};
use winkgo_agent::skills::loader::load_all_skills;
use winkgo_agent::skills::permissions::SkillPermissionChecker;
use winkgo_agent::skills::types::SkillMetadata;
use winkgo_agent::tools::skill::SkillTool;
use winkgo_agent::tools::Tool;
use serde_json::json;
...
/// Create a project-like temp directory with `.git` marker and `.winkgo_agent/skills/` + `.winkgo_agent/commands/`.
...
fn make_project() -> (TempDir, PathBuf) {
    ...
    // Skills directory
    let skills_dir = root.join(".winkgo_agent").join("skills");
    fs::create_dir_all(&skills_dir).unwrap();

    // Commands directory (legacy)
    let commands_dir = root.join(".winkgo_agent").join("commands");
...
async fn e2_legacy_commands_discovered() {
    ...
    println!("E2 PASS: legacy command 'legacy-cmd' discovered from .winkgo_agent/commands/");
}
...
// ---------------------------------------------------------------------------
// E8: Full SkillTool execution (db:migrate with $ARGUMENTS + ${WINKGO_AGENT_SKILL_DIR})
// ---------------------------------------------------------------------------
...
async fn e8_full_execution() {
    ...
    assert!(
        !result.content.contains("${WINKGO_AGENT_SKILL_DIR}"),
        "E8 FAIL: ${{WINKGO_AGENT_SKILL_DIR}} not expanded. Got: {}",
        result.content
    );
    println!("E8 PASS: full execution with $ARGUMENTS and ${{WINKGO_AGENT_SKILL_DIR}} substitution");
}
