...
use winkgo_agent::provider::openai::OpenAIProvider;
use winkgo_agent::types::llm::{LlmEvent, LlmRequest};
use winkgo_agent::types::message::{ContentBlock, Message, Role, StopReason};
use serde_json::json;
...
async fn test_openai_api_error_non_success_status() {
    ...
    match result.unwrap_err() {
        winkgo_agent::provider::ProviderError::Api { status, .. } => {
...
async fn test_openai_rate_limited() {
    ...
    assert!(result.is_err());
    match result.unwrap_err() {
        winkgo_agent::provider::ProviderError::RateLimited { retry_after_ms } => {
