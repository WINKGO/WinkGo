...
# Binary location
./target/release/winkgo_agent
```

## Command Format

```
winkgo_agent [OPTIONS] [PROMPT]...
```

- With `PROMPT`: single-shot mode — completes the task and exits
- Without `PROMPT`: enters interactive REPL mode

> For the full list of CLI parameters, run `winkgo_agent --help`.
...
| `--log-level <filter>` | Log level filter (e.g. `debug`, `info`, `winkgo_agent_providers=debug`) |
...
```
<global config>                   (global, user-level; run `winkgo_agent config path` to find)
    ↓ overridden by
./.winkgo_agent.toml                  (project-level, working directory)
    ↓ overridden by
...
```bash
winkgo_agent config init
# Creates the global config file (run `winkgo_agent config path` to see the location)
```

### Config File Format

```toml
# Global config file (path varies by OS, use `winkgo_agent config path` to find)
...
[session]
enabled = true
directory = ".winkgo_agent/sessions"
max_sessions = 20
...
[plan]
enabled = true
plan_directory = ".winkgo_agent/plans"
...
```bash
winkgo_agent config init
# Edit the config file (run `winkgo_agent config path` to find it), add your API key
```

### 2. Single-Shot Mode

```bash
winkgo_agent "Read and explain crates/winkgo-agent/src/engine.rs"
```
...
```bash
winkgo_agent --profile deepseek "Fix the bug in main.rs"
winkgo_agent --profile ollama "Analyze code quality"
```
...
winkgo_agent "List all Rust files in this project"
```
...
Sessions auto-save to `.winkgo_agent/sessions/`.

```bash
# List saved sessions
winkgo_agent session list

# Resume the latest session
winkgo_agent --resume latest

# Resume a specific session
winkgo_agent --resume a1b2c3

# Create a session with a custom ID
winkgo_agent --session-id my-conv-123
```
