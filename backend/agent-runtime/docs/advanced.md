...
command = "echo \"$(date): ${TOOL_INPUT_COMMAND}\" >> .winkgo_agent/audit.log"
...
```bash
# CLI — logs to /tmp/winkgo_agent-logs at debug level
winkgo_agent --log-dir /tmp/winkgo_agent-logs --log-level debug "Read Cargo.toml"
```
...
| `"winkgo_agent_providers=debug,info"` | Debug for providers, info for everything else |
...
| Platform | Path |
|----------|------|
| macOS | `~/Library/Logs/winkgo_agent/` |
| Linux | `$XDG_STATE_HOME/winkgo_agent/logs/` or `~/.local/state/winkgo_agent/logs/` |
| Windows | `{data_local_dir}/winkgo_agent/logs/` |
...
```json
{"timestamp":"2026-05-13T12:12:52.431Z","level":"INFO","fields":{"message":"mcp server connected","server":"sentry","tools":20},"target":"winkgo_agent_mcp","spans":[{"name":"agent_run","session_id":"abc-123","msg_id":"msg-456"}]}
```
...
| Field | Description |
|-------|-------------|
| `target` | Source crate (`winkgo_agent`, `winkgo_agent_providers`, `winkgo_agent_mcp`, etc.) |
...
grep '"session_id":"abc-123"' 2026-05-13.winkgo_agent.log | jq .
```

### Library Integration

When winkgo_agent is used as a library (e.g. embedded in a backend server), the `create_file_layer()` API provides a composable tracing layer:

```rust
use winkgo_agent_config::logging::{ResolvedLogging, create_file_layer};

let resolved = ResolvedLogging {
    enabled: true,
    level: "winkgo_agent=debug,winkgo_agent_providers=debug".to_string(),
...
tracing_subscriber::registry()
    .with(your_app_layer)
    .with(layer)  // winkgo_agent logs → separate winkgo_agent.log file
    .init();
```

The host application owns the global subscriber; winkgo_agent library crates only emit tracing events and never initialize a subscriber themselves.
...
1. **Global**: `<config_dir>/winkgo_agent/AGENTS.md` — user-level instructions for all projects
...
Running winkgo in `packages/server/` produces a system prompt containing both files, workspace first, then server.
...
```
<config_dir>/winkgo_agent/projects/<sanitized-project-path>/memory/
...
```bash
export WINKGO_AGENT_MEMORY_DIR=/custom/path
```
...
plan_directory = ".winkgo_agent/plans"  # Where plan files are saved
```
