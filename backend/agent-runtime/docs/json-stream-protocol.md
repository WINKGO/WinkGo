...
┌──────────────┐   stdin (JSON Lines)     ┌──────────────────┐
│              │ ◄─────────────────────── │                  │
│    winkgo_agent    │                          │   Host Client    │
│  (Rust CLI)  │ ──────────────────────►  │   (WINK GO etc.)  │
│              │   stdout (JSON Lines)    │                  │
└──────────────┘                          └──────────────────┘
...
- **Encoding**: UTF-8
- **Activation**: `winkgo_agent --json-stream [other flags]`
...
winkgo_agent --json-stream --session-id my-conv-123 --provider openai --model gpt-4o

# Resume an existing session
winkgo_agent --json-stream --resume my-conv-123 --provider openai --model gpt-4o
```
