...
2. Substitutes variables (`$ARGUMENTS`, `$0`, `${WINKGO_AGENT_SKILL_DIR}`)
...
|----------|------|-------------|
| 1 | `.winkgo_agent/skills/` | Project-local skills (checked-in with the repo) |
| 2 | `<CONFIG_DIR>/winkgo_agent/skills/` | User-global skills (see below) |
| 3 | `.winkgo_agent/commands/` | Legacy flat `.md` files (backward compatibility) |
...
>
> Run `winkgo_agent skills path` to see the actual paths on your machine.
...
| `${WINKGO_AGENT_SKILL_DIR}` | Absolute path to the directory containing this skill's `SKILL.md` |
...
Working directory: ${WINKGO_AGENT_SKILL_DIR}
```
...
Use `winkgo_agent skills path` to see which directories are being scanned and whether
they exist on disk:

```
$ winkgo_agent skills path
User:    ~/Library/Application Support/winkgo_agent/skills  (exists)
Project: /path/to/repo/.winkgo_agent/skills                 (exists)
Legacy:  /path/to/repo/.winkgo_agent/commands                (not found)
```
