...
use winkgo_ai_agent::IWorkerTaskManager;
use winkgo_api_types::{AddAgentRequest, GetConfigOptionsResponse, TeamAgentInput, TeamToolTransport};
use winkgo_common::{AgentKillReason, AgentType, ProviderWithModel, generate_id};
use winkgo_db::models::{AgentMetadataRow, TeamRow};
use winkgo_db::{IAgentMetadataRepository, IProviderRepository, ITeamRepository, UpdateTeamParams};
...
impl TeamAgentProvisioner {
    ...
    async fn agent_capabilities(&self, backend: &str) -> Result<Option<serde_json::Value>, TeamError> {
        let Some(metadata) = cli_backend_metadata(&self.agent_metadata_repo, backend).await? else {
            return Ok(None);
        };
    ...
    pub(crate) async fn write_team_mcp_runtime_config(
        ...
        let cli_metadata = cli_backend_metadata(&self.agent_metadata_repo, &agent.backend).await?;
    ...
    async fn create_team_conversation_for_agent(
        ...
        let cli_metadata = cli_backend_metadata(&self.agent_metadata_repo, backend).await?;
        ...
        let provider_id = if agent_type == AgentType::WinkGoAgent {
        ...
        let (top_level_model, extra) = if agent_type == AgentType::WinkGoAgent {
            (
    ...
    fn build_team_extra(
        ...
        if agent_type != AgentType::WinkGoAgent {
            extra["current_model_id"] = serde_json::Value::String(model.to_owned());
        }
        if let Some(assistant_id) = assistant_id {
...
mod tests {
    ...
    use winkgo_ai_agent::{AgentError, AgentInstance};
    ...
    fn test_mcp_config() -> TeamMcpStdioConfig {
        TeamMcpStdioConfig {
            ...
            token: "token".into(),
            slot_id: "slot-1".into(),
            binary_path: "/tmp/winkgo_core".into(),
        }
    }

    #[tokio::test]
    async fn team_tool_transport_prefers_mcp_for_builtin_winkgo_agent_backend() {
        let provisioner = test_provisioner(Arc::new(Mutex::new(Vec::new())));
        let mut agent = test_agent();
        agent.backend = "winkgo_agent".into();
