...
use winkgo_common::{AgentKillReason, generate_id};
...
pub struct WakeInput {
    ...
    /// as left bubbles (matches WinkGo `TeammateManager.wake()`). These are
...
pub struct TeamSession {
    ...
    /// agent's conversation as left bubbles (WinkGo parity: see
...
impl TeamSession {
    ...
    pub async fn start_with_prompt_dump(
        ...
        let mailbox = Arc::new(Mailbox::new_with_broadcaster(repo.clone(), broadcaster.clone()));
        let task_board = Arc::new(TaskBoard::new_with_broadcaster(repo, broadcaster.clone()));
        ...
        let auth_token = winkgo_common::generate_id();
    ...
    pub async fn on_agent_finish(&self, conversation_id: &str, is_error: bool) -> Result<Option<String>, TeamError> {
        ...
        // `on_agent_finish` remains callable for winkgo_agent resume and test scenarios.
...
mod tests {
    ...
    impl TeamProjectionMessageStore for NoopProjectionStore {
        ...
        async fn find_projected_message(
            ...
            _msg_id: &str,
            _msg_type: &str,
        ) -> Result<Option<winkgo_db::models::MessageRow>, TeamError> {
            Ok(None)
        }

        async fn insert_projected_message(&self, _row: &winkgo_db::models::MessageRow) -> Result<(), TeamError> {
            Ok(())
        }
    }
    ...
    fn backend_path() -> Arc<PathBuf> {
        Arc::new(PathBuf::from("/tmp/winkgo_core-test"))
    }
    ...
    async fn stdio_spec_uses_fixed_name_and_binary_path() {
        ...
        assert_eq!(spec.command, "/tmp/winkgo_core-test");
    ...
    async fn recovery_scan_creates_one_wake_per_slot_with_non_self_unread() {
        ...
        assert_eq!(payload.source, winkgo_api_types::TeamRunSource::SystemLifecycle);
    ...
    async fn message_to_runtime_starting_member_returns_blocked_runtime_starting() {
        ...
        assert_eq!(
            ...
            Some(winkgo_api_types::TeamSlotBlockedReason::RuntimeStarting)
        );
        session.stop();
    }
