...
use winkgo_db::models::AgentMetadataRow;
use winkgo_db::{IAgentMetadataRepository, resolve_agent_binding_from_rows};
...
impl TeamSessionService {
    ...
    /// Returns a flat list of model IDs for use by internal agents (winkgo_agent).
...
mod tests {
    ...
    use std::sync::Arc;
    use winkgo_db::models::{AgentMetadataRow, AssistantDefinitionRow, AssistantOverlayRow, Provider};
    use winkgo_db::{
    ...
    impl IProviderRepository for SingleProviderRepo {
        ...
        async fn create(&self, _params: winkgo_db::CreateProviderParams<'_>) -> Result<Provider, DbError> {
            Err(DbError::NotFound("not implemented".into()))
        }

        async fn update(&self, _id: &str, _params: winkgo_db::UpdateProviderParams<'_>) -> Result<Provider, DbError> {
            Err(DbError::NotFound("not implemented".into()))
        }
    ...
    async fn create_team_rejects_assistant_missing_from_team_selectable_catalog() {
        let svc = service_with_selectable_catalog(vec![], vec![assistant_definition("word-creator", "winkgo_agent")]);
    ...
    async fn add_agent_rejects_assistant_missing_from_team_selectable_catalog() {
        ...
        let err = svc
            .add_agent(
                "user1",
                &created.id,
                winkgo_api_types::AddAgentRequest {
                    name: "Worker".into(),
                    role: "teammate".into(),
                    backend: None,
    ...
    async fn resolve_spawn_backend_and_model_prefers_assistant_identity_over_caller_backend() {
        ...
        let svc = TeamSessionService::new(
            ...
            Arc::new(RowsTeamAssistantCatalog {
                rows: vec![team_assistant_entry("word-creator", "Word Creator", "winkgo_agent")],
            }),
        ...
        assert_eq!(backend, "winkgo_agent");
