...
use winkgo_api_types::{TeamToolPermission, TeamToolRole};

// ---------------------------------------------------------------------------
// Tool descriptors (returned by tools/list)
// ---------------------------------------------------------------------------

pub type ToolDescriptor = winkgo_api_types::TeamToolDescriptor;
...
pub fn all_tool_descriptors_for_role(caller_role: TeammateRole) -> Vec<ToolDescriptor> {
    ...
    winkgo_api_types::team_tool_descriptors_for_role(role)
}
...
pub fn authorize_tool(caller_role: TeammateRole, tool_name: &str) -> Result<(), String> {
    let Some(spec) = winkgo_api_types::team_tool_descriptor(tool_name) else {
...
pub fn is_builtin_mcp_backend(backend: &str) -> bool {
    backend == winkgo_common::constants::WINKGO_AGENT_RUNTIME_BACKEND
}
...
mod tests {
    ...
    #[test]
    fn team_spawn_agent_description_is_winkgo_original() {
        ...
        assert!(
            ...
            "description must be the full WinkGo original, not the legacy one-liner"
        );
        assert!(
    ...
    fn builtin_mcp_backend_check() {
        assert!(is_builtin_mcp_backend("winkgo_agent"));
