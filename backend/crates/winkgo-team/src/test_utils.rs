...
use winkgo_db::{DbError, ITeamRepository, UpdateTaskParams, UpdateTeamParams};
...
pub(crate) mod workspace_harness {
    ...
    use winkgo_common::{AgentKillReason, AgentType, PaginatedResult, now_ms};
    ...
    impl ITeamRepository for FullMockTeamRepo {
        ...
        async fn write_message(&self, _row: &winkgo_db::models::MailboxMessageRow) -> Result<(), DbError> {
            Ok(())
        }
        ...
        async fn update_task(&self, _task_id: &str, _params: &winkgo_db::UpdateTaskParams) -> Result<(), DbError> {
            Ok(())
        }
    ...
    impl IWorkerTaskManager for NoopTaskManager {
        fn get_task(&self, _conversation_id: &str) -> Option<winkgo_ai_agent::AgentInstance> {
            None
        }
        ...
        async fn get_or_build_task(
            &self,
            _conversation_id: &str,
            _options: winkgo_ai_agent::types::BuildTaskOptions,
        ...
        fn collect_idle(&self, _idle_threshold_ms: winkgo_common::TimestampMs) -> Vec<String> {
            vec![]
        }
    }
    ...
    pub(crate) fn setup_with_factory_metadata_team_repo_conversation_repo_broadcaster_and_task_manager(
        ...
        let svc = TeamSessionService::new(
            ...
            Arc::new(std::path::PathBuf::from("/tmp/winkgo_core-test")),
        );
    ...
    pub(crate) fn single_agent_team_request(name: &str) -> CreateTeamRequest {
        CreateTeamRequest {
            name: name.into(),
            agents: vec![winkgo_api_types::TeamAgentInput {
                name: "Lead".into(),
                role: "lead".into(),
