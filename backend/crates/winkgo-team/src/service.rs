...
impl TeamSessionService {
    ...
    pub async fn get_run_state(&self, user_id: &str, team_id: &str) -> Result<TeamRunStateResponse, TeamError> {
        ...
        let active_run = session.team_run_manager().current_payload(&snapshot).filter(|run| {
            matches!(
                run.status,
                winkgo_api_types::TeamRunStatus::Accepted
                    | winkgo_api_types::TeamRunStatus::Running
                    | winkgo_api_types::TeamRunStatus::Cancelling
            )
        });
    ...
    pub async fn resolve_team_tool_context(
        ...
        let allowed_tools = winkgo_api_types::team_tool_descriptors_for_role(role_to_tool_role(agent.role))
            .into_iter()
...
async fn set_active_agent_session_mode(instance: &AgentInstance, mode: &str) -> Result<(), AgentError> {
    ...
    match instance {
        ...
        AgentInstance::WinkGoAgent(manager) => manager.set_mode(mode).await,
...
mod tests {
    ...
    use winkgo_api_types::{AddAgentRequest, ConfigOptionConfirmation, SetConfigOptionResponse};
    use winkgo_common::{AgentKillReason, AgentType, ConversationStatus, TimestampMs, now_ms};
    use winkgo_db::{IConversationRepository, ITeamRepository};
    ...
    impl ModeSettingAgent {
        ...
        fn idle_pending_winkgo_agent(conversation_id: &str) -> Self {
            Self::accepts_mode(conversation_id)
    ...
    impl IAgentTask for ModeSettingAgent {
        ...
        fn workspace(&self) -> &str {
            "/tmp/winkgo_core-team-mode-test"
        }
    ...
    impl MutableTaskManager {
        ...
        fn insert_idle_pending_winkgo_agent_agent(&self, conversation_id: &str) {
            self.tasks.lock().unwrap().insert(
                conversation_id.to_owned(),
                AgentInstance::Mock(Arc::new(ModeSettingAgent::idle_pending_winkgo_agent(conversation_id))),
            );
        }
    ...
    fn two_agent_team_request(name: &str) -> winkgo_api_types::CreateTeamRequest {
        winkgo_api_types::CreateTeamRequest {
            name: name.into(),
            agents: vec![
                winkgo_api_types::TeamAgentInput {
                    name: "Lead".into(),
                    role: "lead".into(),
    ...
    fn team_with_winkgo_agent_worker_request(name: &str) -> winkgo_api_types::CreateTeamRequest {
        let mut request = two_agent_team_request(name);
        request.agents.push(winkgo_api_types::TeamAgentInput {
            name: "Butler".into(),
            role: "teammate".into(),
    ...
    #[test]
    fn idle_collectable_team_member_accepts_idle_pending_winkgo_agent_runtime() {
        let task = AgentInstance::Mock(Arc::new(ModeSettingAgent::idle_pending_winkgo_agent(
            "winkgo_agent-idle",
        )));
    ...
    #[test]
    fn idle_collectable_team_member_rejects_running_winkgo_agent_runtime() {
        let task = AgentInstance::Mock(Arc::new(
            ModeSettingAgent::accepts_mode("winkgo_agent-running")
                .with_agent_type(AgentType::WinkGoAgent)
    ...
    #[tokio::test]
    async fn idle_cleanup_stops_team_session_when_winkgo_agent_member_is_idle_pending() {
        let task_manager = Arc::new(MutableTaskManager::new());
        let (svc, _repo, _task_manager, _conv_repo, _broadcaster) =
            setup_with_factory_metadata_team_repo_conversation_repo_broadcaster_and_task_manager(task_manager.clone());
        let created = svc
            .create_team(
                "user-test",
                team_with_winkgo_agent_worker_request("Idle WinkGoAgent Cleanup"),
            )
            .await
            .unwrap();
        let lead = created.assistants.iter().find(|agent| agent.role == "lead").unwrap();
        let acp_worker = created.assistants.iter().find(|agent| agent.name == "Worker").unwrap();
        let winkgo_agent_worker = created.assistants.iter().find(|agent| agent.name == "Butler").unwrap();
        task_manager.insert_idle_finished_agent(&lead.conversation_id);
        task_manager.insert_idle_finished_agent(&acp_worker.conversation_id);
        task_manager.insert_idle_pending_winkgo_agent_agent(&winkgo_agent_worker.conversation_id);
    ...
    async fn set_session_mode_does_not_persist_agent_seed_when_active_runtime_rejects_mode() {
        ...
        .unwrap();
        conv_repo
            .create(&winkgo_db::models::ConversationRow {
