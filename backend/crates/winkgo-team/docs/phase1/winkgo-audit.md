...
> - 只考虑 ACP agent type（范围内：claude / codex / winkgo_agent / 其他 ACP MCP stdio-capable backend；Gemini 非 ACP 但也出现在 team-capable 白名单里，本文档按原样记录不剔除）。
...
>
> **WinkGo 源码路径索引**（下文中所有路径均相对前端根目录 `/Volumes/Macintosh HD/Users/testuser/project/WinkGo/`）：
>
> | 模块 | 路径 | 行数 |
> |------|------|------|
...
| 创建 team（MCP spawn） | `TeamGuideMcpServer.handleCreateTeam(args, backend, callerConversationId)`：1) 从 `WINKGO_MCP_BACKEND` env 拿 agent type 2) 校验 team-capable 3) 复用 callerConversationId 作为 leader 4) 调 `teamSessionService.createTeam` 5) emit `team.listChanged` + `conversation.listChanged` + `deepLink.received` 6) 异步 `getOrStartSession` 并向 leader 发 summary message | `TeamGuideMcpServer.ts:165-261` |
...
solo agent (claude/codex)
   │ winkgo_create_team(summary, [name], [workspace])
   ▼
[team-guide-mcp-stdio.js]
   │ TCP → { tool:"winkgo_create_team", args, backend, conversation_id, auth_token }
   ▼
...
| Team Guide 工具集 | 2 个：`winkgo_create_team`、`winkgo_list_models`（共享 `handleListModels`） | `TeamGuideMcpServer.ts:155-163` |
| Team Guide 注入时机 | ACP agent `loadBuiltinSessionMcpServers()` 里：**不在 team 里** 且 `shouldInjectTeamGuideMcp(backend)` 通过时，取 `getTeamGuideStdioConfig()`，追加 env `WINKGO_MCP_BACKEND` + `WINKGO_MCP_CONVERSATION_ID` 然后 `buildTeamMcpServer` 包装注入 | `acp/index.ts:1623-1640` |
| Gemini / WinkGoAgent 路径 | Gemini 有独立 `GeminiAgentManager.getMcpServers`；WinkGoAgent 有 `WinkGoAgentManager` 额外字段 `awaitReady:true` + `notifyMcpReady(slotId)` | `GeminiAgentManager.ts:341-391`, `WinkGoAgentManager.ts:149-201` |
| Team Guide 能力判断 | `shouldInjectTeamGuideMcp(backend)` → `isTeamCapableBackend(backend, cachedInitResults)`：硬编码白名单 `{gemini, claude, codex, winkgo_agent}`，其他 backend 靠 `cachedInitResults.capabilities.mcpCapabilities.stdio===true` | `teamGuideCapability.ts:18-21`, `teamTypes.ts:16-30` |
| Team Guide Prompt 注入 | ACP / Gemini / WinkGoAgent 的 `AgentManager` 在构建 system instructions 时检查 `!isInTeam && shouldInjectTeamGuideMcp(backend)` → `getTeamGuidePrompt({ backend, leaderLabel })` 拼到 `instructions` 数组 | `AcpAgentManager.ts:1024-1052`, `GeminiAgentManager.ts:248-254`, `task/agentUtils.ts:61/165/219` |
...
| `StdioMcpConfig` 形状 | `{ name: 'winkgo-team-<teamId>', command: 'node', args: [scriptPath], env: [{name:'TEAM_MCP_PORT',value},{name:'TEAM_MCP_TOKEN',value},{name:'TEAM_AGENT_SLOT_ID',value}] }` | `TeamMcpServer.ts:44-49, 101-118` |
...
│  env: WINKGO_MCP_PORT/TOKEN       │  env: TEAM_MCP_PORT/TOKEN
│       WINKGO_MCP_BACKEND          │       TEAM_AGENT_SLOT_ID
│       WINKGO_MCP_CONVERSATION_ID  │
└───────▲────────┘                └──────▲────────┘
...
注意：同一个 agent 进入 team 后，**Team Guide MCP 不再注入**（`!this.extra.teamMcpStdioConfig` 才注入），避免 team 成员把 `winkgo_create_team` 当工具再次建团。
...
- `winkgo_create_team` description：**动态**由 `getCreateTeamToolDescription()` 返回（`teamGuidePrompt.ts:88-108`），被 stdio bridge 在 import 时引用。
- `winkgo_list_models` description：**静态**写在 `teamGuideMcpStdio.ts:109-112` inline。
...
> WinkGo 走 ipcBridge；后端要包掉这些 team 逻辑意味着提供等价 HTTP/WebSocket 端点。具体 endpoint 命名/路径由后端决定，此处只列能力。
...
| 创建 team（agent 调 MCP） | `winkgo_create_team(summary, name?, workspace?)` + system-injected `backend` + `conversation_id` | 复用 caller conversation 作 leader；`sessionMode='yolo'`, `workspaceMode='shared'`；返回 `{teamId, route, leadAgent, next_step}` |
| 列 team | `userId` | `TTeam[]` |
...
1. `winkgo_create_team(summary, name?, workspace?)` — `system.backend`、`system.conversation_id` 由 bridge 注入
2. `winkgo_list_models(agent_type?)`
...
- `getCreateTeamToolDescription()` — `winkgo_create_team` 工具描述（3 PRECONDITIONS + STRICT 流程）
...
- `shouldInjectTeamGuideMcp(backend)` — 复用 `isTeamCapableBackend` 白名单 `{gemini, claude, codex, winkgo_agent}` + `cachedInitResults.capabilities.mcpCapabilities.stdio`
...
这些是 WinkGo 代码里写死的、前后端分离必须保留的行为契约：
...
- WinkGo 没有显式 REST 端点做 "winkgo_create_team"——它完全依赖 MCP + ipcBridge。后端要暴露 REST 需要后端自己设计。
...
- WinkGo 源码 commit：`ed8a6bcd3 fix(bundled-bun): add baseline variant for linux-x64 to support non-AVX2 CPUs (#2654)`（main 分支，2026-04-29 拉取）
- winkgo-backend 所在 worktree：`/Users/testuser/.superset/worktrees/winkgo-backend/repeated-algebra`，分支 `docs/api-for-frontend`
