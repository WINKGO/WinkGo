...
use winkgo_ai_agent::task_manager::AgentFactory;
use winkgo_ai_agent::types::BuildTaskOptions;
use winkgo_ai_agent::{ActiveLeaseRegistry, AgentError, IWorkerTaskManager, WorkerTaskManagerImpl};
...
use winkgo_common::{AgentKillReason, AgentType, PaginatedResult, ProviderWithModel};
...
use winkgo_team::session::SpawnAgentRequest;
use winkgo_team::{
...
impl FakeConversationPorts {
    fn new(repo: Arc<MockConversationRepo>) -> Self {
        let workspace_root =
            std::env::temp_dir().join(format!("winkgo-team-fake-workspaces-{}", winkgo_common::generate_id()));
        Self {
            repo,
            workspace_root,
...
impl TeamConversationProvisioningPort for FakeConversationPorts {
    ...
    ) -> Result<TeamConversationCreateResult, winkgo_team::TeamError> {
        if request.assistant_id.is_some() && request.agent_type.is_some() {
            return Err(winkgo_team::TeamError::InvalidRequest(
                "assistant-backed team conversations must not provide agent_type".into(),
            ));
        }
        let id = winkgo_common::generate_id();
        let now = winkgo_common::now_ms();
    ...
    async fn conversation_workspace(&self, conversation_id: &str) -> Result<Option<String>, winkgo_team::TeamError> {
    ...
    async fn conversation_assistant_id(&self, conversation_id: &str) -> Result<Option<String>, winkgo_team::TeamError> {
        Ok(self.repo.get_extra(conversation_id).and_then(|extra| {
            extra
                .get("assistant_id")
                .or_else(|| extra.get("preset_assistant_id"))
    ...
    async fn create_team_temp_workspace(&self, team_id: &str) -> Result<String, winkgo_team::TeamError> {
        if self
        ...
        self.repo
            .update(
                ...
                &ConversationRowUpdate {
                    ...
                    status: None,
                    updated_at: Some(winkgo_common::now_ms()),
                    project_id: None,
                    folder_id: None,
                },
            )
    ...
    async fn save_acp_runtime_mode(&self, conversation_id: &str, mode: &str) -> Result<(), winkgo_team::TeamError> {
    ...
    async fn get_config_options(
        &self,
        conversation_id: &str,
    ) -> Result<GetConfigOptionsResponse, winkgo_team::TeamError> {
        let extra = self
            .repo
            .get_extra(conversation_id)
            .ok_or_else(|| winkgo_team::TeamError::AgentNotFound(conversation_id.to_owned()))?;
        let model = extra
    ...
    async fn warmup_agent_process(
        ...
        let row = self
            ...
            .ok_or_else(|| {
                winkgo_team::TeamError::InvalidRequest(format!("conversation not found: {conversation_id}"))
            })?;
        let extra: serde_json::Value = serde_json::from_str(&row.extra)?;
        let team = winkgo_api_types::TeamSessionBinding::from_extra_value(&extra)?;
        ...
        task_manager
            ...
            .await
            .map_err(|error| {
                winkgo_team::TeamError::InvalidRequest(format!("failed to warm up agent process: {error}"))
            })?;
        Ok(())
    }
...
impl TeamProjectionMessageStore for FakeConversationPorts {
    ...
    async fn find_projected_message(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, winkgo_team::TeamError> {
        Ok(self
            .repo
    ...
    async fn insert_projected_message(&self, row: &MessageRow) -> Result<(), winkgo_team::TeamError> {
...
impl TeamConversationLookupPort for FakeConversationPorts {
    async fn lookup_team_binding_by_conversation(
        &self,
        conversation_id: &str,
    ) -> Result<Option<TeamConversationBindingLookup>, winkgo_team::TeamError> {
...
struct FullMockTeamRepo {
    inner: MockTeamRepo,
    teams: std::sync::Mutex<Vec<winkgo_db::models::TeamRow>>,
...
impl ITeamRepository for FullMockTeamRepo {
    async fn create_team(&self, row: &winkgo_db::models::TeamRow) -> Result<(), DbError> {
        self.teams.lock().unwrap().push(row.clone());
        Ok(())
    }
    async fn list_teams(&self) -> Result<Vec<winkgo_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().clone())
    }
    async fn list_teams_by_user(&self, user_id: &str) -> Result<Vec<winkgo_db::models::TeamRow>, DbError> {
        Ok(self
            .teams
            .lock()
            .unwrap()
    ...
    async fn get_team(&self, id: &str) -> Result<Option<winkgo_db::models::TeamRow>, DbError> {
        Ok(self.teams.lock().unwrap().iter().find(|t| t.id == id).cloned())
    }
    async fn update_team(&self, id: &str, params: &winkgo_db::UpdateTeamParams) -> Result<(), DbError> {
        ...
        team.updated_at = winkgo_common::now_ms();
        Ok(())
    }
    ...
    async fn write_message(&self, row: &winkgo_db::models::MailboxMessageRow) -> Result<(), DbError> {
    ...
    async fn read_unread_and_mark(
        ...
        team_id: &str,
        to_agent_id: &str,
    ) -> Result<Vec<winkgo_db::models::MailboxMessageRow>, DbError> {
    ...
    async fn create_task(&self, row: &winkgo_db::models::TeamTaskRow) -> Result<(), DbError> {
    ...
    async fn find_task_by_id(
        ...
        team_id: &str,
        task_id: &str,
    ) -> Result<Option<winkgo_db::models::TeamTaskRow>, DbError> {
        self.inner.find_task_by_id(team_id, task_id).await
    }
    async fn update_task(&self, task_id: &str, params: &winkgo_db::UpdateTaskParams) -> Result<(), DbError> {
        self.inner.update_task(task_id, params).await
    }
    async fn list_tasks(&self, team_id: &str) -> Result<Vec<winkgo_db::models::TeamTaskRow>, DbError> {
...
impl IWorkerTaskManager for CountingTaskManager {
    fn get_task(&self, conversation_id: &str) -> Option<winkgo_ai_agent::AgentInstance> {
    ...
    fn collect_idle(&self, idle_threshold_ms: winkgo_common::TimestampMs) -> Vec<String> {
...
mod mock_agent {
    ...
    use winkgo_ai_agent::AgentError;
    use winkgo_ai_agent::agent_task::{IAgentTask, IMockAgent};
    use winkgo_ai_agent::protocol::events::AgentStreamEvent;
    use winkgo_ai_agent::types::SendMessageData;
    use winkgo_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs};
    ...
    impl IAgentTask for MockAgent {
        ...
        async fn send_message(&self, _data: SendMessageData) -> Result<(), winkgo_ai_agent::AgentSendError> {
            Ok(())
        }
...
fn success_factory() -> AgentFactory {
    ...
    Arc::new(|opts: BuildTaskOptions| {
        async move {
            Ok(winkgo_ai_agent::AgentInstance::Mock(Arc::new(
...
impl IProviderRepository for EmptyProviderRepo {
    async fn list(&self) -> Result<Vec<winkgo_db::models::Provider>, DbError> {
        Ok(vec![])
    }
    async fn find_by_id(&self, _id: &str) -> Result<Option<winkgo_db::models::Provider>, DbError> {
        Ok(None)
    }
    async fn create(
        &self,
        _params: winkgo_db::CreateProviderParams<'_>,
    ) -> Result<winkgo_db::models::Provider, DbError> {
        Err(DbError::NotFound("not implemented".into()))
    }
    async fn update(
        &self,
        _id: &str,
        _params: winkgo_db::UpdateProviderParams<'_>,
    ) -> Result<winkgo_db::models::Provider, DbError> {
        Err(DbError::NotFound("not implemented".into()))
    }
...
fn setup_with_factory_metadata_team_repo_and_conversation_repo(
    ...
    let backend_binary_path = Arc::new(std::path::PathBuf::from("/tmp/winkgo_core-test"));
...
async fn recovery_creates_system_run_intents_without_restoring_old_memory_run() {
    ...
    team_repo
        .write_message(&winkgo_db::models::MailboxMessageRow {
    ...
    let created = svc
        .create_team(
            ...
            CreateTeamRequest {
                ...
                agents: winkgo_agent_two_agent_input(),
                workspace: None,
            },
        )
        .await
...
fn winkgo_agent_two_agent_input() -> Vec<TeamAgentInput> {
    two_agent_input()
        .into_iter()
        .map(|mut agent| {
            agent.backend = Some("winkgo_agent".into());
            agent
        })
        .collect()
}
...
async fn renew_active_lease_allows_empty_team_without_unrelated_lease() {
    ...
    team_repo
        .create_team(&winkgo_db::models::TeamRow {
            id: "team-empty".into(),
            user_id: "user1".into(),
            name: "Empty".into(),
            ...
            created_at: winkgo_common::now_ms(),
            updated_at: winkgo_common::now_ms(),
            project_id: None,
            folder_id: None,
        })
        .await
...
async fn create_team_with_workspace_writes_same_workspace_to_team_and_initial_agents() {
    ...
    let workspace_dir =
        std::env::temp_dir().join(format!("winkgo-team-user-workspace-{}", winkgo_common::generate_id()));
...
async fn create_team_side_branch_backfills_project_binding_when_injected() {
    ...
    let store: Arc<dyn winkgo_db::IProjectStore> = Arc::new(winkgo_db::SqliteProjectStore::new(db.pool().clone()));
    std::mem::forget(db);
    svc.with_project_service(Arc::new(winkgo_project::ProjectService::new(
        store,
        std::env::temp_dir().join("winkgo-team-bind-test-conversations"),
    )));

    let workspace_dir = std::env::temp_dir().join(format!("winkgo-team-bind-{}", winkgo_common::generate_id()));
    std::fs::create_dir_all(&workspace_dir).unwrap();
...
async fn tg3_get_team_rejects_cross_user_access() {
    ...
    assert!(matches!(result, Err(winkgo_team::TeamError::Forbidden(_))));
}
...
async fn aa_add_agent_inherits_team_workspace() {
    ...
    let (svc, _, conv_repo) =
        setup_with_factory_and_metadata_and_conversation_repo(success_factory(), agent_metadata_repo);
    let workspace = std::env::temp_dir().join(format!("winkgo-team-workspace-{}", winkgo_common::generate_id()));
...
async fn add_agent_uses_team_temp_workspace_when_team_and_leader_workspaces_are_unusable() {
    ...
    conv_repo
        .patch_extra(
            ...
            serde_json::json!({ "workspace": "/tmp/winkgo-team-missing-leader-workspace" }),
        )
        .unwrap();
...
async fn d31b2_ensure_session_broadcasts_failed_loading_team_for_missing_team() {
    ...
    assert!(matches!(err, winkgo_team::TeamError::TeamNotFound(_)));
