...
use winkgo_team::events::TeamEventEmitter;
use winkgo_team::message_projection::{
    ProjectedTeamMessage, TeamMessageProjection, TeamProjectionMessageStore, TeamProjectionRequest,
    TeamProjectionSource,
};
use winkgo_team::prompts::{AvailableAssistant, build_lead_prompt, build_teammate_prompt, build_wake_payload};
use winkgo_team::types::{
    MailboxMessage, MailboxMessageType, TaskStatus, TeamAgent, TeamTask, TeammateRole, TeammateStatus,
};
use winkgo_team::visibility::TeamVisibilityPolicy;
use winkgo_team::{Mailbox, TaskBoard, TeammateManager};
...
fn lp2_lead_prompt_contains_tool_descriptions() {
    ...
    // WinkGo lead prompt references the `team_*` coordination tools that the
