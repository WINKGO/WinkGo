...
use winkgo_team::{Mailbox, TaskBoard, TeamAgent, TeamMcpServer, TeamPromptDumpConfig, TeammateManager, TeammateRole};
...
async fn sb1_bridge_config_generation() {
    use winkgo_team::{TeamMcpStdioConfig, TeamMcpStdioServerSpec};
    ...
    let config = TeamMcpStdioConfig {
        ...
        slot_id: "lead-1".into(),
        binary_path: "/bin/winkgo_core".into(),
    };

    let spec = TeamMcpStdioServerSpec::from_config("/bin/winkgo_core", &config);
