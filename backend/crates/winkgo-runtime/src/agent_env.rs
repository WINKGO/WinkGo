...
mod tests {
    ...
    const CHILD_MARKER: &str = "WINKGO_RUNTIME_AGENT_ENV_TEST_CHILD";
...
"#,
            ...
            let output = std::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env("WINKGO_CURRENT_ONLY", "from-current")
                .env("WINKGO_OVERLAY", "from-current")
        ...
        assert_eq!(value("WINKGO_CURRENT_ONLY").as_deref(), Some("from-current"));
        assert_eq!(value("WINKGO_SHELL_ONLY").as_deref(), Some("from-shell"));
        assert_eq!(value("WINKGO_OVERLAY").as_deref(), Some("from-shell"));
