...
pub const RESERVED_NAME_PREFIXES: &[&str] = &["winkgo-", "internal-", "builtin-", "system-"];
...
/// Well-known skill source name for the winkgo skills market.
pub const SKILLS_MARKET_NAME: &str = "winkgo-skills";

/// Well-known skill source path for the winkgo skills market.
///
...
// Keep the trailing slash: release binaries may place adjacent string constants
// back-to-back, and the slash preserves an unambiguous repository boundary for
// the privacy audit without changing Git's remote semantics.
...
mod tests {
    ...
    fn test_manifest_file_name() {
        assert_eq!(EXTENSION_MANIFEST_FILE, "winkgo-extension.json");
    }

    #[test]
    fn test_reserved_prefixes_contains_expected() {
        assert!(RESERVED_NAME_PREFIXES.contains(&"winkgo-"));
