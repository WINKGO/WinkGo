...
pub enum ExtensionError {
    ...
    #[error("Engine incompatible: extension '{name}' requires winkgo {required}, got {actual}")]
    EngineIncompatible {
        name: String,
        required: String,
        actual: String,
...
mod tests {
    ...
    fn test_reserved_name_prefix_error_display() {
        let err = ExtensionError::ReservedNamePrefix {
            name: "winkgo-test".into(),
            prefix: "winkgo-".into(),
        };
        assert_eq!(
            err.to_string(),
            "Extension name 'winkgo-test' uses reserved prefix 'winkgo-'"
        );
    }
