...
//!
//! These traits live in `winkgo-extension` (not `winkgo-assistant`) so
//! `skill_routes` can depend on them without pulling `winkgo-assistant` into
//! the dependency graph; the concrete implementation ships from
//! `winkgo-assistant::AssistantService`.

use winkgo_api_types::AssistantSource;
...
///
/// Implemented by `winkgo_assistant::AssistantService`; depended on by
