...
use winkgo_db::{CreateSkillImportRecordParams, ISkillRepository, SkillRow, UpsertSkillParams};
...
///
/// Mirrors the strategy used by `winkgo-assistant::builtin`: the corpus is
/// authoritative at build time; an optional on-disk override
/// (`WINKGO_BUILTIN_SKILLS_PATH`) is consulted at runtime for rapid
/// iteration and E2E fixtures.
static BUILTIN_SKILLS: Dir<'static> = include_dir!("$CARGO_MANIFEST_DIR/../winkgo-app/assets/builtin-skills");
...
pub const BUILTIN_SKILLS_ENV_VAR: &str = "WINKGO_BUILTIN_SKILLS_PATH";
...
pub struct SkillPaths {
    /// Root data directory (~/.winkgo/).
    pub data_dir: PathBuf,
    /// User-created skills directory (~/.winkgo/skills/).
    pub user_skills_dir: PathBuf,
    /// Per-job cron skills directory (~/.winkgo/cron/skills/).
    ...
    /// Assistant-level rules directory (~/.winkgo/assistant-rules/).
    pub assistant_rules_dir: PathBuf,
    /// Assistant-level skills directory (~/.winkgo/assistant-skills/).
...
///
/// `data_dir` is the user-level data root (e.g. `~/.winkgo/`) and
...
pub async fn import_skills(paths: &SkillPaths, source_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    let operation_id = winkgo_common::generate_prefixed_id("skill_import_op");
...
async fn import_skills_from_zip(paths: &SkillPaths, archive_path: &Path) -> Result<SkillImportOutcome, ExtensionError> {
    ...
    let result = async {
        ...
        import_skill_dirs_batch(
            paths,
            skill_dirs,
            winkgo_common::generate_prefixed_id("skill_import_op"),
        )
        .await
    }
    .await;
...
mod tests {
    ...
    use winkgo_db::{ISkillRepository, SqliteSkillRepository, init_database_memory};
