...
mod tests {
    ...
    fn write_test_hook(dir: &Path, stem: &str, unix_body: &str, windows_body: &str) -> String {
        ...
        #[cfg(windows)]
        let (file_name, content) = (format!("{stem}.cmd"), format!("@echo off\r\n{windows_body}\r\n"));
        #[cfg(not(windows))]
        let (file_name, content) = (format!("{stem}.sh"), format!("#!/bin/sh\n{unix_body}\n"));
    ...
    async fn test_execute_hook_success() {
        let dir = tempfile::tempdir().unwrap();
        let hook_path = write_test_hook(dir.path(), "hook", "exit 0", "exit /b 0");
    ...
    async fn test_execute_hook_working_directory() {
        ...
        let hook_path = write_test_hook(dir.path(), "check_cwd", "pwd > cwd_marker.txt", "cd > cwd_marker.txt");
