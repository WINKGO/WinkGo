...
use winkgo_extension::{ExtensionError, ExtensionManifest, parse_manifest, validate_manifest};
...
#[test]
fn mv2_reserved_prefix_winkgo_rejected() {
    let json = serde_json::json!({"name": "winkgo-my-ext", "version": "1.0.0"});
    let bytes = serde_json::to_vec(&json).unwrap();
    let err = parse_manifest(&bytes).unwrap_err();
    assert!(matches!(err, ExtensionError::ReservedNamePrefix { ref prefix, .. } if prefix == "winkgo-"));
}
