...
use winkgo_api_types::{ApiResponse, AssistantSource, SkillImportLimitsResponse};
use winkgo_extension::classifier::{AssistantClassifier, AssistantRuleDispatcher};
use winkgo_extension::error::ExtensionError;
use winkgo_extension::external_paths::ExternalPathsManager;
use winkgo_extension::skill_routes::{SkillRouterState, skill_routes};
use winkgo_extension::skill_service::SkillPaths;
...
async fn router_with_dispatcher(dispatcher: Arc<FakeDispatcher>) -> axum::Router {
    ...
    let skill_repo = Arc::new(winkgo_db::SqliteSkillRepository::new(db.pool().clone()));
    std::mem::forget(tmp);
