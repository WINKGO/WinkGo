...
use winkgo_extension::hub::{HubIndexManager, HubInstaller};
use winkgo_extension::registry::ExtensionRegistry;
use winkgo_extension::state::ExtensionStateStore;
use winkgo_extension::types::HubExtensionStatus;
...
async fn hm5_check_updates_detects_newer_version() {
    ...
    let scan_paths = vec![winkgo_extension::loader::ScanPath {
        path: h.hub_dir.path().to_path_buf(),
        source: winkgo_extension::types::ExtensionSource::Env,
    }];
...
async fn verify_installation_catches_invalid_manifest() {
    ...
    std::fs::create_dir_all(&ext_dir).unwrap();
    std::fs::write(ext_dir.join("winkgo-extension.json"), b"not valid json").unwrap();
...
async fn verify_installation_catches_reserved_name() {
    let h = setup();
    let ext_dir = h.hub_dir.path().join("winkgo-bad");
    write_extension_manifest(&ext_dir, "winkgo-bad", "1.0.0");
