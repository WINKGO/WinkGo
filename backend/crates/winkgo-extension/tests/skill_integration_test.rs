...
//!
//! These tests exercise the public API surface of `winkgo_extension::skill_service`
//! and `winkgo_extension::external_paths` against the functional requirements in
...
use winkgo_extension::skill_service::{
...
///
/// Production mode: no `WINKGO_BUILTIN_SKILLS_PATH` set — the built-in
...
async fn sm11_get_skill_paths() {
    ...
    let data_dir = Path::new("/home/user/.winkgo");
...
async fn cp4_enable_skills_market() {
    ...
    let paths = mgr.get_custom_external_paths().await;
    assert_eq!(paths.len(), 1);
    assert_eq!(paths[0].name, "winkgo-skills");
}
