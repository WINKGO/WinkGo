...
use winkgo_ai_agent::agent_task::AgentInstance;
...
use winkgo_common::{PaginatedResult, ProviderWithModel, TimestampMs, now_ms};
use winkgo_conversation::ConversationService;
use winkgo_cron::{CronRouterState, cron_routes};
use winkgo_db::{
...
use winkgo_cron::events::CronEventEmitter;
use winkgo_cron::executor::JobExecutor;
use winkgo_cron::scheduler::CronScheduler;
use winkgo_cron::service::{CronService, CronServiceDeps};
use winkgo_cron::types::CronAgentConfig;
use winkgo_cron::types::JobStatus;
...
impl winkgo_ai_agent::task_manager::IWorkerTaskManager for StubTaskManager {
    ...
    fn kill(&self, _: &str, _: Option<winkgo_common::AgentKillReason>) -> Result<(), winkgo_ai_agent::AgentError> {
        Ok(())
    }
    fn kill_and_wait(
        &self,
...
struct StubConvRepo {
    ...
    artifacts: Mutex<Vec<winkgo_db::ConversationArtifactRow>>,
    rows: Mutex<HashMap<String, winkgo_db::models::ConversationRow>>,
...
impl StubConvRepo {
    ...
    fn upsert_artifact_row(&self, artifact: winkgo_db::ConversationArtifactRow) {
    ...
    fn artifacts(&self) -> Vec<winkgo_db::ConversationArtifactRow> {
    ...
    fn set_conversation_extra(&self, conversation_id: &str, extra: serde_json::Value) {
        ...
        let row = rows
            ...
            .or_insert_with(|| winkgo_db::models::ConversationRow {
...
impl IConversationRepository for StubConvRepo {
    async fn get(&self, id: &str) -> Result<Option<winkgo_db::models::ConversationRow>, winkgo_db::DbError> {
        ...
        let row = if id == "conv_mode" {
            winkgo_db::models::ConversationRow {
                id: id.into(),
                user_id: "u1".into(),
                ...
                extra: serde_json::json!({
                    "backend": "gemini",
                    "agent_name": "Gemini",
                    "workspace": ensure_named_workspace_path("winkgo-cron-service-gemini-workspace"),
                    "session_mode": "yolo",
            ...
            winkgo_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "hermes",
                    "agent_name": "Hermes",
                    "workspace": ensure_named_workspace_path("winkgo-cron-service-hermes-workspace"),
            ...
            winkgo_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "gemini",
                    "agent_name": "Gemini",
                    "workspace": ensure_named_workspace_path("winkgo-cron-service-gemini-default-workspace"),
                    "session_mode": "default",
                    "current_model_id": "gemini-2.5-pro"
                })
                .to_string(),
            ...
            winkgo_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "codex",
                    "agent_name": "Codex",
                    "workspace": ensure_named_workspace_path("winkgo-cron-service-codex-workspace"),
                    "session_mode": "default",
            ...
            winkgo_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "claude",
                    "agent_name": "Claude",
                    "workspace": ensure_named_workspace_path("winkgo-cron-service-claude-workspace"),
                    "session_mode": "default",
            ...
            winkgo_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    "backend": "claude",
                    "agent_name": "Gemini",
                    "workspace": ensure_named_workspace_path("winkgo-cron-service-stale-backend-workspace"),
                    "session_mode": "yolo",
                    "current_model_id": "gemini-2.5-pro"
                })
                .to_string(),
        ...
        } else if id == "conv_mode_winkgo_agent" {
            winkgo_db::models::ConversationRow {
                id: id.into(),
                user_id: "u1".into(),
                name: "WinkGoAgent Chat".into(),
                ...
                extra: serde_json::json!({
                    ...
                    "workspace": ensure_named_workspace_path("winkgo-cron-service-winkgo_agent-workspace"),
                    "session_mode": "default",
                    "current_model_id": "claude-sonnet-4-20250514"
                })
                .to_string(),
            ...
            winkgo_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    ...
                    "workspace": ensure_named_workspace_path("winkgo-cron-service-assistant-stale-backend-workspace"),
            ...
            winkgo_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    ...
                    "workspace": ensure_named_workspace_path("winkgo-cron-service-missing-assistant-stale-backend-workspace")
                })
                .to_string(),
                pinned: false,
                pinned_at: None,
            ...
            winkgo_db::models::ConversationRow {
                ...
                extra: serde_json::json!({
                    ...
                    "workspace": ensure_named_workspace_path("winkgo-cron-service-assistant-snapshot-workspace")
                })
                .to_string(),
                pinned: false,
                pinned_at: None,
    ...
    async fn create(&self, row: &winkgo_db::models::ConversationRow) -> Result<(), winkgo_db::DbError> {
    ...
    async fn update(&self, id: &str, updates: &ConversationRowUpdate) -> Result<(), winkgo_db::DbError> {
        if self.update_failures.lock().unwrap().iter().any(|item| item == id) {
            return Err(winkgo_db::DbError::Init(format!("forced update failure for {id}")));
        }
    ...
    async fn list_artifacts(
        &self,
        conversation_id: &str,
    ) -> Result<Vec<winkgo_db::ConversationArtifactRow>, winkgo_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<winkgo_db::ConversationArtifactRow>, winkgo_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn upsert_artifact(
        &self,
        artifact: &winkgo_db::ConversationArtifactRow,
    ) -> Result<winkgo_db::ConversationArtifactRow, winkgo_db::DbError> {
...
async fn setup_with_conv_runtime_and_agent_metadata() -> (
    ...
    let data_dir = std::env::temp_dir().join(format!("winkgo-cron-test-{}", now_ms()));
    ...
    let task_manager: Arc<dyn winkgo_ai_agent::task_manager::IWorkerTaskManager> = Arc::new(StubTaskManager);
...
fn make_create_req(name: &str, schedule: CronScheduleDto) -> CreateCronJobRequest {
    CreateCronJobRequest {
        ...
        execution_mode: None,
        queue_enabled: false,
        agent_config: Some(winkgo_api_types::CronAgentConfigWriteDto {
...
async fn seed_bare_assistant_definitions(repo: &Arc<dyn IAssistantDefinitionRepository>) {
    for (definition_id, assistant_id, agent_backend) in [
        ...
        ("asstdef_bare_winkgo_agent", "bare:632f31d2", "winkgo_agent"),
    ] {
...
async fn create_job_strips_legacy_agent_ids_when_assistant_id_present() {
    ...
    req.agent_config = Some(winkgo_api_types::CronAgentConfigWriteDto {
        name: "Helper".into(),
        cli_path: None,
...
async fn create_job_requires_assistant_id_for_new_jobs() {
    ...
    assert!(matches!(err, winkgo_cron::error::CronError::InvalidAgentConfig(_)));
    assert!(err.to_string().contains("assistant_id is required for new cron jobs"));
}

#[tokio::test]
async fn create_job_derives_runtime_type_from_winkgo_agent_assistant() {
    let (svc, _, _, _, definition_repo, _) = setup_with_assistant_repos().await;
    seed_assistant_definition(
        &definition_repo,
        "asstdef_runtime_winkgo_agent",
        "assistant-winkgo_agent",
        "winkgo_agent",
    )
    .await;

    let mut req = make_create_req("Assistant WinkGoAgent", every_60s());
    req.agent_config = Some(winkgo_api_types::CronAgentConfigWriteDto {
        name: "WinkGoAgent Assistant".into(),
        cli_path: None,
        assistant_id: Some("assistant-winkgo_agent".into()),
        mode: Some("yolo".into()),
    ...
    assert_eq!(job.agent_type, "winkgo_agent");
}
...
async fn create_job_derives_runtime_type_from_assistant_overlay_override() {
    ...
    .await;
    seed_assistant_overlay(&overlay_repo, "asstdef_runtime_override", Some("winkgo_agent")).await;
...
async fn cj5_get_nonexistent_job() {
    ...
    assert!(matches!(err, winkgo_cron::error::CronError::JobNotFound(_)));
}
...
async fn update_existing_conversation_job_rejects_agent_config_changes() {
    ...
    assert!(
        matches!(err, winkgo_cron::error::CronError::InvalidAgentConfig(message) if message.contains("ongoing conversation"))
    );
}
...
async fn update_existing_job_to_new_conversation_preserves_custom_workspace() {
    ...
    let custom_workspace = ensure_named_workspace_path(&format!("winkgo-cron-switch-custom-{conversation_id}"));
    conv_repo.set_conversation_extra(
        &conversation_id,
        serde_json::json!({
...
async fn update_team_conversation_job_rejects_execution_mode_change() {
    ...
    conv_repo.set_conversation_extra(
        ...
        serde_json::json!({
            "team_id": "team-1",
            "workspace": ensure_named_workspace_path("winkgo-cron-service-team-workspace")
        }),
    );
    let created = svc.add_job(create_req).await.unwrap();
    ...
    let err = svc.update_job(&created.id, req).await.unwrap_err();
    assert!(matches!(err, winkgo_cron::error::CronError::InvalidExecutionMode(message) if message.contains("Team")));
}
...
async fn cj9_update_schedule_type() {
    ...
    assert!(matches!(
        updated.schedule,
        winkgo_cron::types::CronSchedule::Cron { .. }
    ));
...
async fn cj12_delete_nonexistent() {
    ...
    assert!(matches!(
        err,
        winkgo_cron::error::CronError::Database(winkgo_db::DbError::NotFound(_))
    ));
}
...
async fn sk1_1_save_skill_marks_related_skill_suggest_artifacts_saved() {
    ...
    conv_repo.upsert_artifact_row(winkgo_db::ConversationArtifactRow {
...
async fn sk4_save_empty_skill() {
    ...
    let err = svc
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, winkgo_cron::error::CronError::InvalidSkillContent(_)));
}
...
async fn sc5_invalid_cron_expression() {
    ...
    let err = svc.add_job(req).await.unwrap_err();
    assert!(matches!(err, winkgo_cron::error::CronError::InvalidCronExpression(_)));
}
...
async fn sc7_every_zero_interval() {
    ...
    let err = svc.add_job(req).await.unwrap_err();
    assert!(matches!(err, winkgo_cron::error::CronError::InvalidSchedule(_)));
}
...
async fn create_for_conversation_helper_fails_when_conversation_binding_fails() {
    ...
    assert!(matches!(err, winkgo_cron::error::CronError::Database(_)));
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("x-winkgo-user-id", "u1")
                .header("x-winkgo-conversation-id", "conv_1")
                .body(Body::from(
...
async fn conversation_cron_routes_reject_missing_headers_unclaimed_and_wrong_user() {
    ...
    assert!(body.contains("x-winkgo-user-id"));
    ...
    let wrong_user = app
        .oneshot(
            Request::builder()
                ...
                .uri("/api/internal/conversation-cron/create")
                .header("content-type", "application/json")
                .header("x-winkgo-user-id", "other_user")
                .header("x-winkgo-conversation-id", "conv_1")
                .body(Body::from(
                    serde_json::to_vec(&conversation_cron_request("hello")).unwrap(),
                ))
...
async fn create_for_conversation_helper_rejects_wrong_user() {
    ...
    assert!(matches!(err, winkgo_cron::error::CronError::Conversation(_)));
}
...
async fn sr1_system_resume_missed_job() {
    ...
    let params = winkgo_db::UpdateCronJobParams {
...
async fn cd3c_on_conversation_delete_preserves_custom_workspace_on_jobs() {
    ...
    let custom_workspace = ensure_named_workspace_path(&format!("winkgo-cron-custom-workspace-{conversation_id}"));
    conv_repo.set_conversation_extra(
        &conversation_id,
        serde_json::json!({
            "workspace": custom_workspace,
