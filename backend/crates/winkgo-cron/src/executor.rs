...
use winkgo_ai_agent::AgentRegistry;
use winkgo_ai_agent::task_manager::IWorkerTaskManager;
use winkgo_api_types::{AssistantConversationRequest, CreateConversationRequest};
use winkgo_common::{
    AgentType, ProviderWithModel, WorkspacePathValidationError, now_ms, validate_workspace_path_availability,
};
use winkgo_conversation::{
...
use winkgo_db::models::MessageRow;
use winkgo_db::{ConversationRowUpdate, IConversationRepository};
...
impl JobExecutor {
    ...
    pub async fn get_conversation_row(
        &self,
        conversation_id: &str,
    ) -> Result<Option<winkgo_db::models::ConversationRow>, CronError> {
    ...
    pub async fn get_assistant_snapshot(
        &self,
        conversation_id: &str,
    ) -> Result<Option<winkgo_db::models::ConversationAssistantSnapshotRow>, CronError> {
        self.conversation_repo
    ...
    pub async fn insert_tips_message(
        ...
        let row = MessageRow {
            ...
            hidden: false,
            created_at: winkgo_common::now_ms(),
        };
...
/// Only winkgo_agent conversations carry meaningful model info in `conversations.model`;
...
fn resolve_model(job: &CronJob) -> Option<ProviderWithModel> {
    if job.agent_type != "winkgo_agent" {
        return None;
    }
...
mod tests {
    ...
    use winkgo_ai_agent::AgentStreamEvent;
    use winkgo_ai_agent::agent_task::{AgentInstance, IAgentTask, IMockAgent};
    use winkgo_ai_agent::protocol::events::{FinishEventData, TextEventData};
    use winkgo_ai_agent::types::{BuildTaskOptions, SendMessageData};
    use winkgo_api_types::{AgentModeResponse, ConfigOptionConfirmation, SetConfigOptionResponse, WebSocketMessage};
    use winkgo_common::{AgentKillReason, ConversationStatus, PaginatedResult, TimestampMs};
    use winkgo_db::{
    ...
    fn sample_job() -> CronJob {
        CronJob {
            ...
            agent_config: Some(CronAgentConfig {
                ...
                model: None,
                config_options: None,
                workspace: Some(ensure_named_workspace_path("winkgo-cron-sample-job-workspace")),
            }),
    ...
    async fn hydrated_registry() -> Arc<AgentRegistry> {
        ...
        let repo = Arc::new(winkgo_db::SqliteAgentMetadataRepository::new(db.pool().clone()));
    ...
    async fn parse_agent_type_known_types() {
        ...
        assert_eq!(
            parse_agent_type(&registry, "winkgo_agent").await.unwrap(),
    ...
    fn resolve_model_returns_none_for_acp() {
        // Model info only applies to winkgo_agent; ACP ignores it.
        let job = sample_job();
    ...
    #[test]
    fn resolve_model_returns_none_for_non_winkgo_agent_type() {
        let job = CronJob {
    ...
    #[test]
    fn resolve_model_winkgo_agent_with_full_config() {
        let job = CronJob {
            agent_type: "winkgo_agent".into(),
            agent_config: Some(CronAgentConfig {
                name: "OpenAI".into(),
                cli_path: None,
                is_preset: None,
        ...
        let model = resolve_model(&job).expect("winkgo_agent + full config returns Some");
        assert_eq!(model.provider_id, "4056cdea");
        assert_eq!(model.model, "gpt-5");
    }

    #[test]
    fn resolve_model_winkgo_agent_uses_model_payload() {
        let job = CronJob {
            agent_type: "winkgo_agent".into(),
            agent_config: Some(CronAgentConfig {
                name: "OpenAI".into(),
        ...
        let model = resolve_model(&job).expect("winkgo_agent model payload returns Some");
        assert_eq!(model.provider_id, "4056cdea");
        assert_eq!(model.model, "gpt-5");
    }

    #[test]
    fn resolve_model_winkgo_agent_without_config_returns_none() {
    ...
    #[test]
    fn resolve_model_winkgo_agent_without_model_returns_none() {
        let job = CronJob {
            agent_type: "winkgo_agent".into(),
            agent_config: Some(CronAgentConfig {
                name: "Bogus".into(),
    ...
    async fn build_conversation_extra_preserves_agent_workspace() {
        ...
        assert_eq!(
            extra["workspace"],
            ensure_named_workspace_path("winkgo-cron-sample-job-workspace")
        );
    }
    ...
    async fn execute_inner_uses_conversation_workspace_when_job_workspace_missing() {
        ...
        assert_eq!(
            ...
            ensure_named_workspace_path("winkgo-cron-existing-conversation-workspace")
        );
    ...
    async fn execute_inner_persists_agent_workspace_when_conversation_workspace_missing() {
        ...
        assert_eq!(
            value["workspace"],
            ensure_named_workspace_path("winkgo-cron-agent-workspace")
        );
    }
    ...
    async fn execute_inner_inserts_right_side_user_message_for_cron_prompt() {
        ...
        let repo = Arc::new(MissingWorkspaceConversationRepo::new(
            "conv_1",
            serde_json::json!({
                "workspace": ensure_named_workspace_path("winkgo-cron-existing-conversation-workspace")
            }),
        ));
    ...
    fn make_executor_for_busy_tests() -> JobExecutor {
        ...
        impl IWorkerTaskManager for StubTaskManager {
            ...
            async fn get_or_build_task(
                ...
                _: &str,
                _: BuildTaskOptions,
            ) -> Result<AgentInstance, winkgo_ai_agent::AgentError> {
                Err(winkgo_ai_agent::AgentError::internal("stub"))
            }
            fn kill(
                &self,
                _: &str,
                _: Option<winkgo_common::AgentKillReason>,
            ) -> Result<(), winkgo_ai_agent::AgentError> {
                Ok(())
            }
            fn kill_and_wait(
                &self,
            ...
            fn collect_idle(&self, _: winkgo_common::TimestampMs) -> Vec<String> {
                vec![]
            }
        }
        ...
        impl IConversationRepository for StubConvRepo {
            async fn get(&self, _id: &str) -> Result<Option<winkgo_db::models::ConversationRow>, winkgo_db::DbError> {
                Ok(None)
            }
            async fn create(&self, _row: &winkgo_db::models::ConversationRow) -> Result<(), winkgo_db::DbError> {
                Ok(())
            }
            async fn update(&self, _id: &str, _updates: &ConversationRowUpdate) -> Result<(), winkgo_db::DbError> {
                Ok(())
            }
            ...
            ) -> Result<PaginatedResult<winkgo_db::models::ConversationRow>, winkgo_db::DbError> {
                Ok(PaginatedResult {
                    items: vec![],
                    total: 0,
                    has_more: false,
            ...
            async fn find_by_source_and_chat(
                ...
                _chat_id: &str,
                _agent_type: &str,
            ) -> Result<Option<winkgo_db::models::ConversationRow>, winkgo_db::DbError> {
                Ok(None)
            }
            async fn list_by_cron_job(
                ...
                _user_id: &str,
                _cron_job_id: &str,
            ) -> Result<Vec<winkgo_db::models::ConversationRow>, winkgo_db::DbError> {
                Ok(vec![])
            }
            ...
            async fn insert_message(&self, _message: &winkgo_db::models::MessageRow) -> Result<(), winkgo_db::DbError> {
                Ok(())
            }
            async fn update_message(&self, _id: &str, _updates: &MessageRowUpdate) -> Result<(), winkgo_db::DbError> {
                Ok(())
            }
            async fn delete_messages_by_conversation(&self, _conv_id: &str) -> Result<(), winkgo_db::DbError> {
                Ok(())
            }
            async fn get_message_by_msg_id(
                ...
                _msg_id: &str,
                _msg_type: &str,
            ) -> Result<Option<winkgo_db::models::MessageRow>, winkgo_db::DbError> {
                Ok(None)
            }
        ...
        impl winkgo_realtime::EventBroadcaster for StubBroadcaster {
            fn broadcast(&self, _: WebSocketMessage<serde_json::Value>) {}
        }

        struct StubSkillResolver;
        #[async_trait::async_trait]
        impl winkgo_conversation::skill_resolver::SkillResolver for StubSkillResolver {
            ...
            async fn resolve_skills(
                &self,
                _names: &[String],
            ) -> Vec<winkgo_conversation::skill_resolver::ResolvedAgentSkill> {
                Vec::new()
            }
            ...
            async fn link_workspace_skills(
                ...
                _rel_dirs: &[&str],
                _skills: &[winkgo_conversation::skill_resolver::ResolvedAgentSkill],
            ) -> usize {
                0
            }
        }

        let stub_broadcaster: Arc<dyn winkgo_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
        let stub_repo: Arc<dyn IConversationRepository> = Arc::new(StubConvRepo);
        let agent_metadata_repo: Arc<dyn winkgo_db::IAgentMetadataRepository> = Arc::new(StubAgentMetadataRepo);
        let acp_session_repo: Arc<dyn winkgo_db::IAcpSessionRepository> = Arc::new(StubAcpSessionRepo);
    ...
    impl RecordingAgent {
        fn new(conversation_id: &str, mode: &str, initialized: bool) -> Self {
            ...
            Self {
                conversation_id: conversation_id.to_owned(),
                workspace: ensure_named_workspace_path("winkgo-cron-agent-workspace"),
                event_tx,
    ...
    impl IAgentTask for RecordingAgent {
        ...
        async fn send_message(&self, data: SendMessageData) -> Result<(), winkgo_ai_agent::AgentSendError> {
        ...
        async fn cancel(&self) -> Result<(), winkgo_ai_agent::AgentError> {
            Ok(())
        }

        fn kill(&self, _reason: Option<AgentKillReason>) -> Result<(), winkgo_ai_agent::AgentError> {
            Ok(())
        }
    }
    ...
    impl IMockAgent for RecordingAgent {
        async fn mode(&self) -> Result<AgentModeResponse, winkgo_ai_agent::AgentError> {
        ...
        async fn set_config_option(
            ...
            option_id: &str,
            value: &str,
        ) -> Result<SetConfigOptionResponse, winkgo_ai_agent::AgentError> {
            if option_id != "mode" {
                return Err(winkgo_ai_agent::AgentError::bad_request(format!(
    ...
    impl IWorkerTaskManager for FailingTaskManager {
        ...
        async fn get_or_build_task(
            ...
            Err(winkgo_ai_agent::AgentError::bad_gateway(self.message.clone()))
        }
    ...
    impl IConversationRepository for ExistingConversationRepo {
        async fn get(&self, id: &str) -> Result<Option<winkgo_db::models::ConversationRow>, winkgo_db::DbError> {
            Ok(Some(winkgo_db::models::ConversationRow {
                id: id.to_owned(),
                user_id: "cron".into(),
    ...
    struct MissingWorkspaceConversationRepo {
        row: winkgo_db::models::ConversationRow,
        updates: Mutex<Vec<ConversationRowUpdate>>,
        inserted_messages: Mutex<Vec<winkgo_db::models::MessageRow>>,
    ...
    impl MissingWorkspaceConversationRepo {
        ...
        fn inserted_messages(&self) -> Vec<winkgo_db::models::MessageRow> {
    ...
    impl winkgo_realtime::EventBroadcaster for RecordingBroadcaster {
    ...
    impl IConversationRepository for MissingWorkspaceConversationRepo {
        ...
        async fn update(&self, _id: &str, updates: &ConversationRowUpdate) -> Result<(), winkgo_db::DbError> {
        ...
        async fn insert_message(&self, message: &winkgo_db::models::MessageRow) -> Result<(), winkgo_db::DbError> {
    ...
    fn make_executor_with_task_manager_and_repo(
        ...
        let broadcaster: Arc<dyn winkgo_realtime::EventBroadcaster> = Arc::new(StubBroadcaster);
    ...
    #[async_trait::async_trait]
    impl winkgo_db::IAcpSessionRepository for StubAcpSessionRepo {
        async fn get(
            &self,
            _conversation_id: &str,
        ) -> Result<Option<winkgo_db::models::AcpSessionRow>, winkgo_db::DbError> {
            Ok(None)
        }
        async fn create(
            &self,
            _params: &winkgo_db::CreateAcpSessionParams<'_>,
        ) -> Result<winkgo_db::models::AcpSessionRow, winkgo_db::DbError> {
            Err(winkgo_db::DbError::Init("stub".into()))
        }
        async fn update_session_id(
            &self,
            _conversation_id: &str,
            _session_id: &str,
        ) -> Result<bool, winkgo_db::DbError> {
            Ok(false)
        }
        async fn clear_session_id(&self, _conversation_id: &str) -> Result<bool, winkgo_db::DbError> {
            Ok(false)
        }
        async fn delete(&self, _conversation_id: &str) -> Result<bool, winkgo_db::DbError> {
            Ok(false)
        }
        async fn load_runtime_state(
            &self,
            _conversation_id: &str,
        ) -> Result<Option<winkgo_db::PersistedSessionState>, winkgo_db::DbError> {
            Ok(None)
        }
        async fn save_runtime_state(
            &self,
            _conversation_id: &str,
            _params: &winkgo_db::SaveRuntimeStateParams<'_>,
        ) -> Result<bool, winkgo_db::DbError> {
            Ok(false)
        }
    }
    ...
    #[async_trait::async_trait]
    impl winkgo_db::IAgentMetadataRepository for StubAgentMetadataRepo {
        async fn list_all(&self) -> Result<Vec<winkgo_db::models::AgentMetadataRow>, winkgo_db::DbError> {
            Ok(Vec::new())
        }
        async fn get(&self, _id: &str) -> Result<Option<winkgo_db::models::AgentMetadataRow>, winkgo_db::DbError> {
            Ok(None)
        }
        async fn find_by_source_and_name(
            ...
            _agent_source: &str,
            _name: &str,
        ) -> Result<Option<winkgo_db::models::AgentMetadataRow>, winkgo_db::DbError> {
            Ok(None)
        }
        ...
        async fn upsert(
            &self,
            _params: &winkgo_db::models::UpsertAgentMetadataParams<'_>,
        ) -> Result<winkgo_db::models::AgentMetadataRow, winkgo_db::DbError> {
            Err(winkgo_db::DbError::Init("stub".into()))
        }
        async fn apply_handshake(
            &self,
            _id: &str,
            _params: &winkgo_db::models::UpdateAgentHandshakeParams<'_>,
        ) -> Result<Option<winkgo_db::models::AgentMetadataRow>, winkgo_db::DbError> {
            Ok(None)
        }
        async fn update_availability_snapshot(
            &self,
            _id: &str,
            _params: &winkgo_db::models::UpdateAgentAvailabilitySnapshotParams<'_>,
        ) -> Result<Option<winkgo_db::models::AgentMetadataRow>, winkgo_db::DbError> {
            Ok(None)
        }
        ...
        async fn set_enabled(&self, _id: &str, _enabled: bool) -> Result<bool, winkgo_db::DbError> {
            Ok(false)
        }
        async fn delete(&self, _id: &str) -> Result<bool, winkgo_db::DbError> {
            Ok(false)
        }
    }
}
