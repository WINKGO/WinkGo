...
use serde_json::json;
use winkgo_api_types::{ConversationArtifactResponse, WebSocketMessage};
use winkgo_common::generate_id;
use winkgo_db::ConversationArtifactRow;
...
mod tests {
    ...
    fn builds_skill_suggest_response() {
        ...
        assert_eq!(response.kind, winkgo_api_types::ConversationArtifactKind::SkillSuggest);
        assert_eq!(response.status, winkgo_api_types::ConversationArtifactStatus::Pending);
    ...
    fn builds_cron_trigger_payload() {
        ...
        let response = artifact_response_from_row(&row).unwrap();
        assert_eq!(response.kind, winkgo_api_types::ConversationArtifactKind::CronTrigger);
