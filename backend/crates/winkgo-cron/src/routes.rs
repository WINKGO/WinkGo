...
async fn system_resume(
    ...
    let is_internal = headers.get("x-winkgo-internal").and_then(|value| value.to_str().ok()) == Some("1");
    if !is_internal {
...
async fn create_conversation_cron(
    ...
    let user_id = header_value(&headers, "x-winkgo-user-id")?;
    let conversation_id = header_value(&headers, "x-winkgo-conversation-id")?;
    let Json(req) = body.map_err(ApiError::from)?;
    let resp = state
        .cron_service
...
mod tests {
    ...
    fn header_value_rejects_missing_or_blank_required_header() {
        ...
        assert!(matches!(
            header_value(&headers, "x-winkgo-user-id"),
            Err(ApiError::BadRequest(message)) if message.contains("x-winkgo-user-id")
        ));

        headers.insert("x-winkgo-user-id", "   ".parse().unwrap());
    ...
    fn header_value_trims_required_header() {
        let mut headers = HeaderMap::new();
        headers.insert("x-winkgo-user-id", " user_1 ".parse().unwrap());

        assert_eq!(header_value(&headers, "x-winkgo-user-id").unwrap(), "user_1");
    }
    ...
    fn conversation_error_maps_through_boundary_mapper() {
        let err: ApiError =
            CronError::Conversation(winkgo_conversation::ConversationError::NotFound { id: "conv-1".into() }).into();
