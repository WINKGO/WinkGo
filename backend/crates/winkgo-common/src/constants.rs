...
pub const WINKGO_FILES_MARKER: &str = "[[WINKGO_FILES]]";
...
pub const COOKIE_NAME: &str = "winkgo-session";
pub const COOKIE_MAX_AGE_DAYS: u32 = 30;
pub const CSRF_COOKIE_NAME: &str = "winkgo-csrf-token";
...
pub const WINKGO_AGENT_RUNTIME_BACKEND: &str = "winkgo_agent";
...
pub fn supports_team_mcp(backend: &str, agent_capabilities: Option<&serde_json::Value>) -> bool {
    if backend == WINKGO_AGENT_RUNTIME_BACKEND {
        return true;
    }
...
mod tests {
    ...
    fn team_mcp_builtin_backend_does_not_require_capability_metadata() {
        assert!(supports_team_mcp("winkgo_agent", None));
        assert!(supports_team_mcp("winkgo_agent", Some(&json!({}))));
    }
