...
//! backends. It lives in `winkgo-common` (below every consumer: `winkgo-session`,
//! `winkgo-conversation`, the agent-* crates all depend on common) so the
//! consuming backends gain no upward crate dependency. Ported verbatim from the
//! deleted `winkgo-ai-agent::manager::acp::stderr_error_extractor` (git
