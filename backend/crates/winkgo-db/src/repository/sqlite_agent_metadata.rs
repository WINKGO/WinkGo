...
struct AgentMetadataSafeRow {
    ...
    last_check_at: Option<winkgo_common::TimestampMs>,
    last_success_at: Option<winkgo_common::TimestampMs>,
    last_failure_at: Option<winkgo_common::TimestampMs>,
    command_override: Option<String>,
    env_override: Option<String>,
    created_at: winkgo_common::TimestampMs,
    updated_at: winkgo_common::TimestampMs,
}
...
mod tests {
    ...
    async fn seed_rows_populated_after_migrations() {
        ...
        assert!(
            rows.iter()
                .any(|r| r.name == "WinkGo CLI" && r.agent_source == "internal")
        );
    ...
    async fn seed_rows_include_icon_backfill() {
        ...
        let rows = repo.list_all().await.unwrap();
        let winkgo_agent = rows
            .iter()
            .find(|row| row.agent_type == "winkgo_agent" && row.agent_source == "internal")
            .expect("seeded winkgo cli row");
        assert_eq!(winkgo_agent.icon.as_deref(), Some("/api/assets/logos/brand/winkgo.svg"));
        let winkgo_agent_modes: serde_json::Value = serde_json::from_str(
            winkgo_agent
                .available_modes
                .as_deref()
                .expect("winkgo_agent modes catalog"),
        )
        .unwrap();
        assert_eq!(winkgo_agent_modes["current_mode_id"].as_str(), Some("default"));
        assert_eq!(
            winkgo_agent_modes["available_modes"]
                .as_array()
                .expect("winkgo_agent available modes")
                .iter()
                .filter_map(|item| item.get("id").and_then(serde_json::Value::as_str))
                .collect::<Vec<_>>(),
            vec!["default", "auto_edit", "yolo"]
        );
        let winkgo_agent_config_options: serde_json::Value = serde_json::from_str(
            winkgo_agent
                .config_options
                .as_deref()
                .expect("winkgo_agent config options"),
        )
        .unwrap();
        assert_eq!(
            winkgo_agent_config_options["config_options"][0]["options"][1]["value"].as_str(),
            Some("auto_edit")
        );
