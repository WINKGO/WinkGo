...
async fn resolves_internal_agent_type_when_backend_is_null() {
    ...
    let resolved = resolve_agent_binding(db.pool(), "winkgo_agent")
        .await
        .unwrap()
        .expect("winkgo_agent should resolve");

    assert_eq!(resolved.agent_id, "632f31d2");
    assert_eq!(resolved.agent_source, "internal");
    assert_eq!(resolved.agent_type, "winkgo_agent");
    assert_eq!(resolved.runtime_backend, "winkgo_agent");
}
