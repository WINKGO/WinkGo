...
async fn seed_legacy_assistant_identity(pool: &sqlx::SqlitePool) {
    ...
    for (id, backend, agent_type, name, source, sort_order) in [
        ("agent-winkgo_agent", "", "winkgo_agent", "WinkGo CLI", "internal", 100),
...
#[tokio::test]
async fn migration_015_populates_winkgo_agent_catalog_by_agent_type() {
    ...
    sqlx::query(
        "INSERT INTO agent_metadata (
            id, name, backend, command, agent_type, enabled, agent_source, sort_order, created_at, updated_at
         ) VALUES ('agent-winkgo_agent', 'WinkGo CLI', NULL, '', 'winkgo_agent', 1, 'internal', 100, 1, 1)",
    )
    .execute(&pool)
    .await
    .unwrap();
...
#[tokio::test]
async fn migration_016_clears_internal_winkgo_cli_overrides_only() {
    let pool = SqlitePoolOptions::new()
        .max_connections(1)
        .connect("sqlite::memory:")
        .await
...
async fn migration_013_normalizes_legacy_cron_agent_identity() {
    ...
    insert_legacy_cron(
        ...
        "conv_winkgo_agent",
        "winkgo_agent",
        r#"{"backend":"provider-1","name":"WinkGo","assistant_id":"winkgo-assistant","model_id":"gpt-5"}"#,
    )
    .await;
    insert_legacy_cron(
        &pool,
    ...
    let winkgo_agent = sqlx::query(
        "SELECT
            ...
            enabled
         FROM cron_jobs WHERE id = 'cron_winkgo_agent'",
    )
    .fetch_one(&pool)
    .await
    .unwrap();
    assert_eq!(winkgo_agent.get::<String, _>("assistant_id"), "winkgo-assistant");
    assert_eq!(winkgo_agent.get::<String, _>("provider_id"), "provider-1");
    assert_eq!(winkgo_agent.get::<String, _>("model"), "gpt-5");
    assert_eq!(winkgo_agent.get::<i64, _>("enabled"), 1);
