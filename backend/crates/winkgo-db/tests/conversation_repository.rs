...
async fn setup() -> (SqliteConversationRepository, winkgo_db::Database) {
...
fn make_conversation(suffix: &str) -> ConversationRow {
    ...
    ConversationRow {
        id: winkgo_common::generate_prefixed_id("conv"),
...
fn make_message(conv_id: &str, content: &str) -> MessageRow {
    let now = winkgo_common::now_ms();
    MessageRow {
        id: winkgo_common::generate_prefixed_id("msg"),
        conversation_id: conv_id.to_string(),
        msg_id: Some(winkgo_common::generate_prefixed_id("cmsg")),
...
fn make_artifact(conv_id: &str, artifact_id: &str) -> winkgo_db::ConversationArtifactRow {
    winkgo_db::ConversationArtifactRow {
...
async fn filter_by_source_and_pinned_combined() {
    ...
    let mut c1 = make_conversation("winkgo-pinned");
    c1.source = Some("winkgo".to_string());
    c1.pinned = true;
    c1.pinned_at = Some(winkgo_common::now_ms());
    repo.create(&c1).await.unwrap();

    let mut c2 = make_conversation("telegram-pinned");
    c2.source = Some("telegram".to_string());
    c2.pinned = true;
    c2.pinned_at = Some(winkgo_common::now_ms());
    repo.create(&c2).await.unwrap();

    let mut c3 = make_conversation("winkgo-unpinned");
    c3.source = Some("winkgo".to_string());
    c3.pinned = false;
    repo.create(&c3).await.unwrap();

    // Filter: source=winkgo AND pinned=true
    let result = repo
        .list_paginated(
            USER_ID,
            &ConversationFilters {
...
async fn concurrent_insert_message_does_not_require_sequence_allocation() {
    ...
    let db = winkgo_db::init_database(&dir.path().join("messages.db")).await.unwrap();
    let repo = SqliteConversationRepository::new(db.pool().clone());
...
async fn anchor_rejects_legacy_artifact_rows() {
    ...
    assert!(matches!(err, winkgo_db::DbError::NotFound(_)));
}
...
async fn pin_and_unpin_conversation() {
    ...
    // Pin
    let pin_time = winkgo_common::now_ms();
    repo.update(
        &conv.id,
        &ConversationRowUpdate {
            pinned: Some(true),
