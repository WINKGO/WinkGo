...
async fn repo() -> (Arc<dyn IRemoteAgentRepository>, winkgo_db::Database) {
...
async fn full_crud_lifecycle() {
    ...
    // Update status
    r.update_status(&created.id, "connected", Some(winkgo_common::now_ms()))
        .await
        .unwrap();
