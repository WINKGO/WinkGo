...
use winkgo_ai_agent::{AgentStreamEvent, IWorkerTaskManager};
use winkgo_api_types::{AssistantConversationRequest, CreateConversationRequest, SendMessageRequest};
use winkgo_common::{AgentType, ConversationSource};
use winkgo_conversation::ConversationService;
use winkgo_db::models::AssistantSessionRow;
...
impl ChannelMessageService {
    ...
    async fn create_conversation_for_session(
        ...
        // Top-level `model` is only accepted for winkgo_agent; other types pass via `extra`.
        let top_level_model = if agent_type == AgentType::WinkGoAgent {
            Some(model)
        } else {
...
fn platform_to_source(platform: PluginType) -> ConversationSource {
    match platform {
        ...
        // Reserved variants default to WinkGoui
        PluginType::Slack | PluginType::Discord => ConversationSource::WinkGoui,
    }
}
...
fn parse_agent_type(s: &str) -> Result<AgentType, ChannelError> {
    let agent_type = match s {
        ...
        "winkgo_agent" => AgentType::WinkGoAgent,
...
mod tests {
    use super::*;
    use winkgo_ai_agent::protocol::events::{
    ...
    #[test]
    fn platform_to_source_reserved_defaults_to_winkgo() {
        assert_eq!(platform_to_source(PluginType::Slack), ConversationSource::WinkGoui);
        assert_eq!(platform_to_source(PluginType::Discord), ConversationSource::WinkGoui);
    }
    ...
    fn parse_known_agent_types() {
        ...
        assert_eq!(parse_agent_type("winkgo_agent").unwrap(), AgentType::WinkGoAgent);
    }
    ...
    // ── model placement by agent_type (regression: non-winkgo_agent must not
    //    use top-level model) ──────────────────────────────────────────
    ...
    #[test]
    fn winkgo_agent_model_stays_at_top_level() {
        let agent_type = AgentType::WinkGoAgent;
        let model = ProviderWithModel {
        ...
        assert!(top_level_model.is_some(), "winkgo_agent must use top-level model");
    ...
    #[test]
    fn conv_name_telegram_winkgo_agent() {
        let name = channel_conversation_name(PluginType::Telegram, "winkgo_agent", None, Some("70880480"));
        assert_eq!(name, "tg-winkgo_agent-70880480");
    }
    ...
    fn conv_name_non_acp_ignores_backend() {
        let name = channel_conversation_name(PluginType::Telegram, "winkgo_agent", Some("claude"), Some("70880480"));
        assert_eq!(name, "tg-winkgo_agent-70880480");
    }
}
