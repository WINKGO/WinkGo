...
const DEFAULT_AGENT_TYPE: &str = "winkgo_agent";
...
/// Non-ACP agent types (winkgo_agent, nanobot, remote, …) have `backend = None`.
#[derive(Debug, Clone)]
...
impl ChannelSettingsService {
    ...
    ///
    /// Falls back to `agent_type=winkgo_agent, backend=None` when no config exists.
    ...
    async fn resolve_default_assistant_identity(&self) -> Result<Option<String>, ChannelError> {
        ...
        let mut any_winkgo_agent = None;
        for definition in &definitions {
            if self.effective_assistant_backend(definition, &overlays).await? == DEFAULT_AGENT_TYPE {
                any_winkgo_agent = Some(definition);
                break;
            }
        }
        if let Some(definition) = any_winkgo_agent {
            return Ok(Some(definition.assistant_id.clone()));
        }
    ...
    async fn effective_assistant_backend(
        &self,
        definition: &winkgo_db::models::AssistantDefinitionRow,
        overlays: &[winkgo_db::models::AssistantOverlayRow],
...
fn backend_to_agent_type(backend: &str) -> String {
    match backend {
        "winkgo_agent" | "winkgo-cli" => "winkgo_agent".to_owned(),
...
mod tests {
    ...
    use winkgo_db::{IAssistantDefinitionRepository, IAssistantOverlayRepository};
    ...
    #[test]
    fn winkgo_agent_backends_map_to_winkgo_agent() {
        assert_eq!(backend_to_agent_type("winkgo_agent"), "winkgo_agent");
        assert_eq!(backend_to_agent_type("winkgo-cli"), "winkgo_agent");
    }
    ...
    async fn agent_config_returns_default_when_no_pref() {
        ...
        assert_eq!(config.agent_type, "winkgo_agent");
    ...
    #[tokio::test]
    async fn agent_config_winkgo_agent_has_no_backend() {
        let repo = Arc::new(MockPrefRepo::with_data(vec![(
            "assistant.lark.agent",
            r#"{"backend":"winkgo_agent","name":"WinkGo CLI"}"#,
        )]));
        let svc = ChannelSettingsService::new(repo);
    ...
    #[tokio::test]
    async fn agent_config_reads_new_format_winkgo_agent() {
        let repo = Arc::new(MockPrefRepo::with_data(vec![(
            "assistant.lark.agent",
            r#"{"agent_type":"winkgo_agent","name":"WinkGo CLI"}"#,
        )]));
        let svc = ChannelSettingsService::new(repo);
    ...
    #[tokio::test]
    async fn get_assistant_setting_defaults_to_generated_winkgo_agent_assistant() {
        let repo = Arc::new(MockPrefRepo::new());
        let definition_repo = Arc::new(MockAssistantDefinitionRepo {
            rows: vec![
                make_definition("bare-claude", "claude"),
                make_definition("bare-winkgo_agent", "winkgo_agent"),
            ],
        });
        ...
        assert_eq!(setting.assistant_id.as_deref(), Some("bare-winkgo_agent"));
        assert!(setting.custom_agent_id.is_none());
        assert!(setting.backend.is_none());
        assert!(setting.agent_type.is_none());
    ...
    #[tokio::test]
    async fn get_platform_settings_defaults_to_generated_winkgo_agent_assistant() {
        let repo = Arc::new(MockPrefRepo::new());
        let definition_repo = Arc::new(MockAssistantDefinitionRepo {
            rows: vec![
                make_definition("bare-claude", "claude"),
        ...
        assert_eq!(assistant.assistant_id.as_deref(), Some("bare-winkgo_agent"));
        assert!(assistant.custom_agent_id.is_none());
        assert!(assistant.backend.is_none());
        assert!(assistant.agent_type.is_none());
