...
use winkgo_ai_agent::agent_task::{AgentInstance, IAgentTask};
use winkgo_ai_agent::protocol::events::FinishEventData;
use winkgo_ai_agent::types::{BuildTaskOptions, SendMessageData};
use winkgo_ai_agent::{AgentError, AgentSendError, AgentStreamEvent, IMockAgent, IWorkerTaskManager};
use winkgo_api_types::WebSocketMessage;
use winkgo_channel::channel_settings::ChannelSettingsService;
use winkgo_channel::error::ChannelError;
use winkgo_channel::message_service::ChannelMessageService;
...
use winkgo_conversation::skill_resolver::{ResolvedAgentSkill, SkillResolver};
use winkgo_db::models::AssistantSessionRow;
use winkgo_db::models::UpsertAssistantDefinitionParams;
use winkgo_db::{
...
#[tokio::test]
async fn send_to_agent_without_saved_binding_defaults_to_bare_winkgo_agent_assistant() {
    let db = init_database_memory().await.unwrap();
    let pool = db.pool().clone();
    ...
    let session = AssistantSessionRow {
        id: "session-assisted-default-winkgo_agent".to_owned(),
    ...
    assert_eq!(snapshot.assistant_id, "bare-winkgo_agent");
    assert_eq!(snapshot.agent_id, "632f31d2");
    assert_eq!(conversation.r#type, AgentType::WinkGoAgent.serde_name());
    assert_eq!(conversation.name, "tg-winkgo_agent-70880480");
}
