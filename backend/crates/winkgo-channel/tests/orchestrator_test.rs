...
async fn unauthorized_user_gets_pairing_response() {
    ...
    let repo: Arc<dyn winkgo_db::IChannelRepository> = Arc::new(winkgo_db::SqliteChannelRepository::new(pool.clone()));
    let bus = Arc::new(winkgo_realtime::BroadcastEventBus::new(64));

    let pref_repo: Arc<dyn winkgo_db::IClientPreferenceRepository> =
        Arc::new(winkgo_db::SqliteClientPreferenceRepository::new(pool));
