...
use winkgo_db::models::AssistantUserRow;
use winkgo_db::{IChannelRepository, SqliteChannelRepository, init_database_memory};
use winkgo_realtime::EventBroadcaster;

use winkgo_channel::action::{ActionExecutor, MessageResult};
use winkgo_channel::channel_settings::ChannelSettingsService;
use winkgo_channel::pairing::PairingService;
use winkgo_channel::session::SessionManager;
use winkgo_channel::types::{
...
async fn setup() -> (
    ...
    let pref_repo: Arc<dyn winkgo_db::IClientPreferenceRepository> =
        Arc::new(winkgo_db::SqliteClientPreferenceRepository::new(db.pool().clone()));
...
async fn action_session_new() {
    ...
    match result {
        MessageResult::Action(resp) => {
            ...
            // With no client_preferences, defaults to "winkgo_agent"
            assert!(text.contains("winkgo_agent"));
        }
        _ => panic!("Expected Action result"),
    }
}
...
// winkgo-db sqlite_channel.rs::update_session_conversation_persists.
