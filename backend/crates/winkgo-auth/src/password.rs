...
pub fn dummy_password_hash() -> &'static str {
    DUMMY_HASH.get_or_init(|| {
        ...
        bcrypt::hash("__winkgo_dummy_password__", BCRYPT_COST).expect("bcrypt hash of constant input must succeed")
    })
}
...
/// of `uuid::Uuid::now_v7()` used in `winkgo-common`.
...
mod tests {
    ...
    fn dummy_hash_matches_dummy_input() {
        let hash = dummy_password_hash();
        assert!(verify_password("__winkgo_dummy_password__", hash).unwrap());
    }
