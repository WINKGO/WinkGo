...
use winkgo_db::{Database, IProjectStore, SqliteProjectStore, init_database_memory};
use winkgo_project::ProjectService;
use winkgo_project::canonical::to_file_uri;
use winkgo_project::scm::{ScmActor, ScmInbound, ScmWirePush};
use winkgo_project::types::AttachInput;
...
async fn fixture() -> Fixture {
    ...
    let created = service
        .create_standard(to_file_uri(repo_dir.path()).expect("uri"))
        .await
...
async fn pe_name_is_carried_when_the_entry_has_a_name() {
    let fx = fixture().await;
    fx.service
        .rename_entry(&fx.pe_id, Some("My Repo".to_owned()))
        .await
        .expect("rename");
...
async fn a_blank_entry_name_is_dropped_rather_than_carried() {
    let fx = fixture().await;
    fx.service
        .rename_entry(&fx.pe_id, Some("   ".to_owned()))
        .await
        .expect("rename");
...
async fn attaching_and_detaching_a_repo_pushes_repositories_changed() {
    ...
    let before = fx.frame_count();
    fx.service.remove_attached(&row.pe_id).await.expect("detach");
    let params = fx
        .wait_for_notification("conn-1", "scm/repositoriesChanged", before)
        .await;
...
// Windows normalizes a directory named only with spaces to its parent, so this
// filesystem fixture cannot exist there. The label filter itself is covered by
// the entry-name tests on every platform.
#[cfg(not(windows))]
#[tokio::test]
