...
impl ScmActor {
    ...
    async fn authorize_file(&self, _user_id: &str, file: &FileRef, op: FileOp) -> Result<FileRef, ScmError> {
        self.project
    ...
    async fn roots_of(&self, _user_id: &str, project_id: &str) -> Result<Vec<ResolvedRoot>, ScmError> {
        let view = self
            .project
