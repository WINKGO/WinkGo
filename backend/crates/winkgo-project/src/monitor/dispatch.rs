...
impl FsMonitorActor {
    ...
    /// Resolve a project file reference for previewers that still require a
    /// local path (PDF and Office). The same identity and realpath containment
    /// checks as `fs/read` are applied before either path is returned.
    async fn handle_resolve(&mut self, session: &str, id: Option<Value>, params: Value) {
    ...
    async fn handle_search(&mut self, session: &str, id: Option<Value>, params: Value) {
        let Some(search_id) = id else {
            tracing::warn!(session, "fs/search missing id; ignoring notification-shaped request");
            return;
        };
        ...
        for root in &p.roots {
            match self.resolve(root, FileOp::Browse).await {
