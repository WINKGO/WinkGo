...
//! existing `CliAgentProcess` / process registry in `winkgo-ai-agent`.
//!
//! "Bytes not semantics": it never parses agent output, holds no session
//! state, and never mutates `std::env`. It depends only on `winkgo-common`
//! and `winkgo-runtime`.
//!
//! ## Isolation contract (why two mechanisms coexist without conflict)
//! All shared resources are namespaced under `{data_dir}/runtime/winkgo-process/`
