...
use crate::session_context::{SessionContextBuilder, WinkGoAgentRuntimePermissionSeed};
...
fn assistant_snapshot_modes<'a>(
    ...
    definition: &'a winkgo_db::AssistantDefinitionRow,
...
impl ConversationService {
    ...
    /// `delete()` removes the conversation row. Used by `winkgo-app` to wire up `WorkerTaskManagerImpl`
...
impl ConversationService {
    ...
    async fn assistant_identity_from_snapshot(
    ...
    ) -> Result<winkgo_api_types::ConversationAssistantIdentityResponse, ConversationError> {
        ...
        Ok(winkgo_api_types::ConversationAssistantIdentityResponse {
    ...
    /// to `winkgo`, and broadcasts `conversation.listChanged(created)`.
    ...
    pub async fn create(
        ...
        let now = now_ms();
        let source = req.source.unwrap_or(ConversationSource::WinkGoui);
        ...
        // Type-aware rule: top-level `model` is winkgo_agent-only. Other agent types
        ...
        if effective_type != AgentType::WinkGoAgent && req.model.is_some() {
            return Err(ConversationError::BadRequest {
                reason: format!(
                    "top-level `model` is only accepted for winkgo_agent conversations; pass model via `extra` for {}",
                    effective_type.serde_name()
                ),
            });
        }

        // winkgo_agent source-of-truth rule: top-level `model` wins. If an older client
        // still packs `extra.model`, strip it before persist so the stored row
        // has a single canonical model representation.
        if effective_type == AgentType::WinkGoAgent
            && let Some(obj) = extra.as_object_mut()
            && obj.remove("model").is_some()
        {
            warn!("winkgo_agent create: stripped legacy `extra.model`; top-level `model` is canonical");
        }
        ...
        let row = winkgo_db::models::ConversationRow {
            id: id.clone(),
        ...
        if let Some(snapshot) = assistant_snapshot.as_ref() {
            response.assistant = Some(winkgo_api_types::ConversationAssistantIdentityResponse {
                id: snapshot.assistant_id.clone(),
    ...
    async fn persist_assistant_preferences_from_snapshot(
        ...
        preference_repo
            .upsert(&winkgo_db::UpsertAssistantPreferenceParams {
                assistant_definition_id: &snapshot.assistant_definition_id,
    ...
    pub async fn update(
        ...
        // Type-aware rule: top-level `model` is winkgo_agent-only. For non-winkgo_agent
        // conversations, model/mode must be updated via `extra` (see spec
        // 2026-05-12).
        if existing_type != AgentType::WinkGoAgent && req.model.is_some() {
            return Err(ConversationError::BadRequest {
                reason: format!(
                    "top-level `model` is only accepted for winkgo_agent conversations; pass model via `extra` for {}",
                    existing.r#type
        ...
        // Merge extra if provided. For winkgo_agent, strip `extra.model` post-merge
        ...
        let merged_extra = if let Some(new_extra) = &req.extra {
            ...
            if existing_type == AgentType::WinkGoAgent
                && let Some(obj) = existing_extra.as_object_mut()
                && obj.remove("model").is_some()
            {
                warn!("winkgo_agent update: stripped legacy `extra.model` from merged extra");
            }
...
impl ConversationService {
    ...
    pub async fn send_message(
        ...
        let user_msg = winkgo_db::models::MessageRow {
...
impl ConversationService {
    /// Loads the persisted runtime permission gate inputs for an winkgo_agent
    /// rebuild. Returns `None` for non-winkgo_agent conversations and for winkgo_agent
    ...
    async fn load_winkgo_agent_permission_seed(
        &self,
        row: &winkgo_db::models::ConversationRow,
    ) -> Result<Option<WinkGoAgentRuntimePermissionSeed>, ConversationError> {
        if row.r#type != AgentType::WinkGoAgent.serde_name() {
            return Ok(None);
        }
        let snapshot = self
            .conversation_repo
            ...
            .map_err(|e| {
                ConversationError::internal(format!(
                    "Failed to load assistant snapshot for winkgo_agent permission seed: {e}"
                ))
            })?;
        Ok(snapshot.map(|snapshot| WinkGoAgentRuntimePermissionSeed {
            default_permission_mode: snapshot.default_permission_mode,
    ...
    pub(crate) async fn build_task_options(
        ...
        reject_deprecated_runtime_row(row)?;
        let seed = self.load_winkgo_agent_permission_seed(row).await?;
    ...
    /// workspaces are constrained to WinkGo's generated path; custom
...
/// name (e.g. `"winkgo_agent"`). Falls back to the agent type's serde name
...
fn classify_repo_mcp_status(row: &winkgo_db::models::McpServerRow, support: McpSupportPolicy) -> ConversationMcpStatus {
...
mod tests {
    ...
    fn enum_to_db_source() {
        assert_eq!(enum_to_db(&ConversationSource::WinkGoui).unwrap(), "winkgo");
    ...
    #[test]
    fn assistant_lineage_extracts_winkgo_agent_preset_id() {
        use winkgo_common::AgentType;
        let response = response_with_type(AgentType::WinkGoAgent);
        let extra = json!({ "preset_assistant_id": "preset-xyz" });
        let lineage = AssistantLineage::from_response_and_extra(&response, &extra);
        assert_eq!(lineage.agent_type, "winkgo_agent");
        ...
        let status = classify_session_mcp_status(
            &SessionMcpServer {
                ...
                transport: SessionMcpTransport::Stdio {
                    command: "__definitely_missing_winkgo_mcp_command__".into(),
                    args: Vec::new(),
                    env: HashMap::new(),
                },
            },
            McpSupportPolicy::WINKGO_AGENT,
        );
