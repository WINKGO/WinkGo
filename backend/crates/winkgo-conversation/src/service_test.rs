...
use winkgo_ai_agent::agent_task::{AgentInstance, IAgentTask, IMockAgent};
use winkgo_ai_agent::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
use winkgo_ai_agent::protocol::events::{AgentStreamEvent, ErrorEventData, FinishEventData, TextEventData};
use winkgo_ai_agent::types::{
    BuildTaskOptions, CONVERSATION_RUNTIME_CONTEXT_VERSION, SendMessageData, WINKGO_BASE_URL_ENV,
    WINKGO_HELPER_BIN_ENV, WINKGO_RUNTIME_TOKEN_ENV,
};
use winkgo_ai_agent::{
...
use winkgo_db::{MessagePageCursor, MessagePageDirection, MessagePageParams, MessagePageResult};
...
impl IConversationRepository for MockRepo {
    async fn get(&self, id: &str) -> Result<Option<ConversationRow>, winkgo_db::DbError> {
        let rows = self.rows.lock().unwrap();
        Ok(rows.iter().find(|r| r.id == id).cloned())
    }

    async fn create(&self, row: &ConversationRow) -> Result<(), winkgo_db::DbError> {
        self.rows.lock().unwrap().push(row.clone());
        Ok(())
    }

    async fn update(&self, id: &str, updates: &ConversationRowUpdate) -> Result<(), winkgo_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let row = rows
            .iter_mut()
            .find(|r| r.id == id)
            .ok_or_else(|| winkgo_db::DbError::NotFound(format!("Conversation {id}")))?;
    ...
    async fn delete(&self, id: &str) -> Result<(), winkgo_db::DbError> {
        let mut rows = self.rows.lock().unwrap();
        let len_before = rows.len();
        rows.retain(|r| r.id != id);
        if rows.len() == len_before {
            return Err(winkgo_db::DbError::NotFound(format!("Conversation {id}")));
        }
        Ok(())
    }
    ...
    ) -> Result<PaginatedResult<ConversationRow>, winkgo_db::DbError> {
        let rows = self.rows.lock().unwrap();
    ...
    async fn find_by_source_and_chat(
        ...
        _chat_id: &str,
        _agent_type: &str,
    ) -> Result<Option<ConversationRow>, winkgo_db::DbError> {
        Ok(None)
    }
    ...
    async fn list_by_cron_job(
        ...
        _user_id: &str,
        _cron_job_id: &str,
    ) -> Result<Vec<ConversationRow>, winkgo_db::DbError> {
        Ok(vec![])
    }
    ...
    async fn get_assistant_snapshot(
        &self,
        conversation_id: &str,
    ) -> Result<Option<ConversationAssistantSnapshotRow>, winkgo_db::DbError> {
    ...
    async fn delete_assistant_snapshot(&self, conversation_id: &str) -> Result<bool, winkgo_db::DbError> {
        let mut rows = self.assistant_snapshots.lock().unwrap();
        let before = rows.len();
    ...
    ) -> Result<MessagePageResult, winkgo_db::DbError> {
        ...
        let items = match &params.direction {
            ...
            MessagePageDirection::Anchor { message_id } => {
                let anchor = matched
                    ...
                    .cloned()
                    .ok_or_else(|| winkgo_db::DbError::NotFound(format!("Message {message_id}")))?;
    ...
    async fn update_message(&self, id: &str, updates: &MessageRowUpdate) -> Result<(), winkgo_db::DbError> {
        ...
        let message = messages
            ...
            .ok_or_else(|| winkgo_db::DbError::NotFound(format!("Message {id}")))?;
    ...
    async fn delete_messages_by_conversation(&self, conv_id: &str) -> Result<(), winkgo_db::DbError> {
        self.messages
            .lock()
            .unwrap()
    ...
    async fn get_message_by_msg_id(
        ...
        msg_id: &str,
        msg_type: &str,
    ) -> Result<Option<MessageRow>, winkgo_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
            .find(|message| {
    ...
    async fn list_stale_runtime_messages(&self) -> Result<Vec<MessageRow>, winkgo_db::DbError> {
        let messages = self.messages.lock().unwrap();
        Ok(messages
            .iter()
            .filter(|message| {
    ...
    async fn search_messages(
        ...
        _page: u32,
        _page_size: u32,
    ) -> Result<PaginatedResult<MessageSearchRow>, winkgo_db::DbError> {
        Ok(PaginatedResult {
            items: vec![],
            total: 0,
            has_more: false,
    ...
    async fn list_artifacts(&self, conversation_id: &str) -> Result<Vec<ConversationArtifactRow>, winkgo_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    async fn get_artifact(
        ...
        conversation_id: &str,
        artifact_id: &str,
    ) -> Result<Option<ConversationArtifactRow>, winkgo_db::DbError> {
        Ok(self
            .artifacts
            .lock()
            .unwrap()
    ...
    ) -> Result<ConversationArtifactRow, winkgo_db::DbError> {
    ...
    async fn mark_skill_suggest_artifacts_saved(
        ...
        cron_job_id: &str,
        updated_at: TimestampMs,
    ) -> Result<Vec<ConversationArtifactRow>, winkgo_db::DbError> {
        let mut artifacts = self.artifacts.lock().unwrap();
    ...
    async fn delete_artifacts_by_conversation(&self, conversation_id: &str) -> Result<(), winkgo_db::DbError> {
        self.artifacts
            .lock()
            .unwrap()
    ...
    async fn list_legacy_cron_trigger_messages(
        &self,
        conversation_id: &str,
    ) -> Result<Vec<MessageRow>, winkgo_db::DbError> {
        Ok(self
            .messages
            .lock()
            .unwrap()
...
fn stub_agent_metadata_rows() -> Vec<AgentMetadataRow> {
    [
        ...
        ("632f31d2", None, "winkgo_agent", "WinkGo CLI", 200),
...
fn ensure_test_workspace_path() -> String {
    let workspace = std::env::temp_dir().join("winkgo-conversation-service-test-project");
...
fn unique_test_workspace_path(label: &str) -> PathBuf {
    let workspace = std::env::temp_dir()
        .join(format!("winkgo-conversation-service-test-{label}"))
...
async fn create_assistant_backed_conversation(
    ...
    if conversation_type == Some("winkgo_agent") {
        payload["model"] = json!({
...
async fn insert_conversation_with_type(repo: &Arc<MockRepo>, user_id: &str, agent_type: AgentType) -> ConversationRow {
    let id = format!(
        ...
        winkgo_common::generate_short_id()
    );
...
async fn create_returns_conversation_with_defaults() {
    ...
    assert_eq!(resp.source, Some(ConversationSource::WinkGoui));
    assert!(!resp.pinned);
    ...
    assert_eq!(events[0].data["source"], "winkgo");
}

// ── Project-bind side branch tests ─────────────────────────────────

async fn make_injected_project_service(temp_root: &std::path::Path) -> std::sync::Arc<winkgo_project::ProjectService> {
    ...
    let db = winkgo_db::init_database_memory().await.unwrap();
    let store: std::sync::Arc<dyn winkgo_db::IProjectStore> =
        std::sync::Arc::new(winkgo_db::SqliteProjectStore::new(db.pool().clone()));
    std::mem::forget(db);
    std::sync::Arc::new(winkgo_project::ProjectService::new(
        store,
...
async fn create_auto_provisions_workspace_under_date_partition() {
    ...
    let workspace_root = temp.path().join("winkgo-data");
...
async fn create_stores_model_as_json() {
    ...
    // Top-level model is only valid for winkgo_agent conversations.
    let req: CreateConversationRequest = serde_json::from_value(json!({
        "type": "winkgo_agent",
...
#[tokio::test]
async fn create_derives_winkgo_agent_type_from_assistant_backend_when_type_is_missing() {
    ...
    upsert_test_assistant_definition(
        &definition_repo,
        "asstdef_winkgo_agent_missing_type",
        "assistant-winkgo_agent-missing-type",
        "winkgo_agent",
        "auto",
        "auto",
    )
    ...
    overlay_repo
        .upsert(&UpsertAssistantOverlayParams {
            assistant_definition_id: "asstdef_winkgo_agent_missing_type",
            enabled: true,
            sort_order: 0,
    ...
    let req: CreateConversationRequest = serde_json::from_value(json!({
        "assistant": {
            "id": "assistant-winkgo_agent-missing-type",
            "locale": "en-US"
        },
        "model": {
            "provider_id": "provider-1",
    ...
    let resp = svc.create("user_1", req).await.unwrap();
    assert_eq!(resp.r#type, AgentType::WinkGoAgent);
...
async fn get_reports_idle_runtime_when_only_persisted_status_is_running() {
    ...
    assert_eq!(runtime.state, winkgo_api_types::ConversationRuntimeStateKind::Idle);
...
async fn update_extra_merge() {
    ...
    let dir = std::env::temp_dir().join(format!(
        "winkgo-conversation-update-extra-merge-{}",
        winkgo_common::generate_short_id()
    ));
...
async fn update_model() {
    ...
    // Top-level model updates are only valid on winkgo_agent conversations
    // (Task 8 enforces the winkgo_agent-only rule in update).
...
async fn delete_invokes_registered_hook() {
    use winkgo_common::OnConversationDelete;
...
impl IAgentTask for MockAgent {
    ...
    async fn send_message(&self, _data: SendMessageData) -> Result<(), AgentSendError> {
        ...
        let _ = self.event_tx.send(AgentStreamEvent::Finish(
            winkgo_ai_agent::protocol::events::FinishEventData::default(),
        ));
        Ok(())
    }
...
fn assert_conversation_runtime_context(options: &BuildTaskOptions, user_id: &str, conversation_id: &str) {
    assert!(
        options
            .context
            .runtime_env
            .contains(&("WINKGO_USER_ID".to_owned(), user_id.to_owned())),
        "runtime env should include WINKGO_USER_ID"
    );
    assert!(
        options
            .context
            .runtime_env
            .contains(&("WINKGO_CONVERSATION_ID".to_owned(), conversation_id.to_owned())),
        "runtime env should include WINKGO_CONVERSATION_ID"
    );
    assert_eq!(
...
async fn send_message_injects_configured_runtime_helper_context() {
    ...
    let svc = svc.with_runtime_helper_context(
        "/Applications/WinkGo/winkgo_core".to_owned(),
    ...
    assert!(
        options[0].context.runtime_env.contains(&(
            WINKGO_HELPER_BIN_ENV.to_owned(),
            "/Applications/WinkGo/winkgo_core".to_owned()
        )),
        "runtime env should include WINKGO_HELPER_BIN"
    );
    assert!(
        options[0]
            .context
            .runtime_env
            .contains(&(WINKGO_BASE_URL_ENV.to_owned(), "http://127.0.0.1:51234".to_owned())),
        "runtime env should include WINKGO_BASE_URL"
    );
}
...
#[tokio::test]
async fn update_winkgo_agent_model_updates_assistant_preference_only_when_snapshot_model_mode_is_auto() {
    let task_mgr = Arc::new(MockTaskManager::new());
    let (svc, _broadcaster, repo, definition_repo, overlay_repo, preference_repo) =
        make_service_with_mock_task_manager_and_assistant_support(task_mgr.clone()).await;
    ...
    upsert_test_assistant_definition(
        &definition_repo,
        "asstdef_winkgo_agent_auto",
        "assistant-winkgo_agent-auto",
        "winkgo_agent",
        "auto",
        "auto",
    )
    ...
    overlay_repo
        .upsert(&UpsertAssistantOverlayParams {
            assistant_definition_id: "asstdef_winkgo_agent_auto",
            enabled: true,
            sort_order: 0,
            agent_id_override: None,
            last_used_at: None,
    ...
    preference_repo
        .upsert(&UpsertAssistantPreferenceParams {
            assistant_definition_id: "asstdef_winkgo_agent_auto",
            last_model_id: Some("legacy-winkgo_agent-model"),
    ...
    let auto_conv = create_assistant_backed_conversation(
        &svc,
        "user_1",
        Some("winkgo_agent"),
        "winkgo_agent",
    ...
    let auto_pref = preference_repo.get("asstdef_winkgo_agent_auto").await.unwrap().unwrap();
    ...
    upsert_test_assistant_definition(
        &definition_repo,
        "asstdef_winkgo_agent_fixed",
        "assistant-winkgo_agent-fixed",
        "winkgo_agent",
        "fixed",
        "auto",
    )
    ...
    overlay_repo
        .upsert(&UpsertAssistantOverlayParams {
            assistant_definition_id: "asstdef_winkgo_agent_fixed",
            enabled: true,
            sort_order: 0,
            agent_id_override: None,
            last_used_at: None,
    ...
    preference_repo
        .upsert(&UpsertAssistantPreferenceParams {
            assistant_definition_id: "asstdef_winkgo_agent_fixed",
            last_model_id: Some("legacy-winkgo_agent-fixed-model"),
            last_permission_value: None,
            last_thought_level_value: None,
            last_skill_ids: "[]",
            last_disabled_builtin_skill_ids: "[]",
    ...
    let fixed_conv = create_assistant_backed_conversation(
        &svc,
        "user_1",
        Some("winkgo_agent"),
        "winkgo_agent",
    ...
    let fixed_pref = preference_repo
        .get("asstdef_winkgo_agent_fixed")
        .await
        .unwrap()
        .unwrap();
    assert_eq!(
        fixed_pref.last_model_id.as_deref(),
        Some("legacy-winkgo_agent-fixed-model")
    );
...
async fn send_message_missing_workspace_persists_message_and_failure_tip() {
    ...
    let conv = svc.create("user_1", make_create_req()).await.unwrap();
    broadcaster.take_events();
    let legacy_workspace = format!("/tmp/does-not-exist-{}", winkgo_common::generate_short_id());
    repo.update(
        &conv.id,
        &ConversationRowUpdate {
    ...
    let scripted_agent = Arc::new(
        ScriptedAgent::new(
            ...
            vec![vec![AgentStreamEvent::Error(ErrorEventData::legacy(
                "winkgo_agent terminal error",
                Some(AgentErrorCode::UnknownUpstreamError),
            ))]],
        )
        .with_agent_type(AgentType::WinkGoAgent),
    );
    task_mgr.insert_agent(&conv.id, AgentInstance::Mock(scripted_agent));
...
async fn send_message_records_agent_availability_feedback_on_send_failure() {
    ...
    create_req.source = Some(ConversationSource::WinkGoui);
...
async fn cancel_timeout_does_not_kill_non_acp_task() {
    ...
    let conv = svc.create("user_1", make_create_req()).await.unwrap();
    let agent = Arc::new(BlockingCancelAgent::new_with_type(&conv.id, AgentType::WinkGoAgent));
    task_mgr.insert_agent(&conv.id, AgentInstance::Mock(agent));
...
async fn warmup_injects_runtime_token_for_mcp_team_conversation() {
    ...
    assert!(
        options[0]
            ...
            .runtime_env
            .iter()
            .any(|(key, value)| key == WINKGO_RUNTIME_TOKEN_ENV && !value.is_empty()),
        "MCP Team conversations should receive WINKGO_RUNTIME_TOKEN for CLI fallback"
    );
}
...
async fn confirm_removes_confirmation_and_broadcasts() {
    ...
    let req = winkgo_api_types::ConfirmRequest {
        msg_id: "msg-1".into(),
...
async fn create_resolves_assistant_snapshot_and_updates_preferences() {
    ...
    assert_eq!(
        resp.assistant,
        Some(winkgo_api_types::ConversationAssistantIdentityResponse {
            id: "preset-1".into(),
            source: "builtin".into(),
            name: "Preset".into(),
            avatar: "🤖".into(),
...
#[tokio::test]
async fn assistant_backed_winkgo_agent_build_options_include_snapshot_rule_as_preset_rules() {
    let resolver = Arc::new(FixedSkillResolver { names: vec![] });
    let dispatcher = Arc::new(StaticAssistantDispatcher {
        rules: std::collections::HashMap::from([(
            "preset-winkgo_agent-rule".to_string(),
    ...
    upsert_test_assistant_definition(
        &definition_repo,
        "asstdef_preset_winkgo_agent_rule",
        "preset-winkgo_agent-rule",
        "winkgo_agent",
        "auto",
        "auto",
    )
    ...
    state_repo
        .upsert(&UpsertAssistantOverlayParams {
            assistant_definition_id: "asstdef_preset_winkgo_agent_rule",
            enabled: true,
            sort_order: 0,
            agent_id_override: None,
            last_used_at: None,
...
async fn warmup_restores_skill_links_for_recreated_auto_workspace() {
    ...
    assert!(workspace.join(".winkgo_agent/skills/cron").is_dir());
    ...
    assert_eq!(calls.len(), 1);
    assert_eq!(calls[0].workspace, workspace);
    assert_eq!(calls[0].rel_dirs, vec![".winkgo_agent/skills"]);
    assert_eq!(calls[0].skill_names, vec!["cron"]);
}
...
// ── winkgo_agent rebuild permission seed (Sentry 135525584) ──────────────

/// Inserts an winkgo_agent conversation whose `extra.session_mode` is the create-time
...
async fn seed_winkgo_agent_conversation_with_snapshot(
    repo: &Arc<MockRepo>,
    session_mode: &str,
    default_permission_mode: &str,
    ...
    let row = ConversationRow {
        id: format!("winkgo_agent-seed-{}", winkgo_common::generate_short_id()),
        user_id: "user_1".into(),
        name: "winkgo_agent seed".into(),
        r#type: "winkgo_agent".into(),
        extra: json!({
...
fn winkgo_agent_session_mode(options: &BuildTaskOptions) -> Option<String> {
    match &options.context.kind {
...
#[tokio::test]
async fn winkgo_agent_rebuild_auto_mode_preserves_runtime_yolo() {
    // AC#1: an `auto` winkgo_agent session that was switched to yolo at runtime must
    // keep yolo after a rebuild (model switch / agent restart).
    let (svc, _broadcaster, repo, _task_mgr) = make_service();
    let row = seed_winkgo_agent_conversation_with_snapshot(&repo, "default", "auto", Some("yolo")).await;

    let options = svc.build_task_options(&row).await.unwrap();

    assert_eq!(winkgo_agent_session_mode(&options).as_deref(), Some("yolo"));
}

#[tokio::test]
async fn winkgo_agent_rebuild_existing_data_auto_overrides_create_time_seed() {
    // AC#2: existing data — create-time non-yolo seed is overridden by the
    // authoritative resolved runtime value.
    let (svc, _broadcaster, repo, _task_mgr) = make_service();
    let row = seed_winkgo_agent_conversation_with_snapshot(&repo, "auto_edit", "auto", Some("yolo")).await;
...
#[tokio::test]
async fn winkgo_agent_rebuild_fixed_mode_blocks_runtime_escalation() {
    ...
    let (svc, _broadcaster, repo, _task_mgr) = make_service();
    let row = seed_winkgo_agent_conversation_with_snapshot(&repo, "default", "fixed", Some("yolo")).await;

    let options = svc.build_task_options(&row).await.unwrap();

    assert_eq!(winkgo_agent_session_mode(&options).as_deref(), Some("default"));
}
...
async fn cron_required_runtime_mode_wins_over_resolved_permission_seed() {
    ...
    // an `auto` winkgo_agent session whose snapshot resolved value is yolo, a cron
