...
//! given conversation; otherwise an winkgo_agent job that runs fine
...
//! we return an empty `ProviderWithModel`; non-winkgo_agent factory branches
//! ignore the field, and the winkgo_agent branch surfaces a clear "provider
...
/// `provider_id: ""`, `model: ""`, `use_model: None`. Non-winkgo_agent factories
/// ignore the field, while the winkgo_agent factory surfaces a clear "Provider
...
mod tests {
    ...
    /// label `"winkgo_agent"` when the conversation's model JSON was an older
    /// shape) and `send_message` parsed the row directly, so the cron
    /// path would emit `Provider 'winkgo_agent' not found` while the
    ...
    fn interactive_and_cron_paths_agree_on_provider_id() {
        ...
        assert_ne!(canonical_resolved.provider_id, "winkgo_agent");
        assert_ne!(legacy_resolved.provider_id, "winkgo_agent");
    }
}
