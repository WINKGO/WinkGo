...
use winkgo_api_types::{ConversationArtifactResponse, ConversationResponse, MessageResponse, MessageSearchItem};
use winkgo_common::{
    AgentType, ConversationSource, ConversationStatus, MessagePosition, MessageStatus, MessageType, ProviderWithModel,
};
use winkgo_db::MessageSearchRow;
use winkgo_db::models::{ConversationArtifactRow, ConversationRow, MessageRow};
...
///
/// WinkGo stores the full provider object (`TProviderWithModel`) which includes
...
mod tests {
    use super::*;
    use serde_json::json;
    use winkgo_common::{AgentType, ConversationSource, ConversationStatus};
    ...
    #[test]
    fn parse_provider_with_model_winkgo_format() {
    ...
    fn row_to_response_marks_workspace_inside_data_dir_as_temporary() {
        let row = make_row(
            ...
            Some("winkgo"),
            None,
            r#"{"workspace":"/srv/winkgo-data/conversations/claude-temp-abc"}"#,
        );
        let resp = row_to_response(row, Path::new("/srv/winkgo-data")).unwrap();
    ...
    fn row_to_response_marks_missing_workspace_as_non_temporary() {
        let row = make_row("acp", "pending", Some("winkgo"), None, r#"{}"#);
        let resp = row_to_response(row, Path::new("/srv/winkgo-data")).unwrap();
        assert_eq!(resp.extra["is_temporary_workspace"], false);
    }
    ...
    fn test_search_row_to_item_builds_nested_conversation() {
        let row = MessageSearchRow {
            ...
            conversation_source: Some("winkgo".into()),
        ...
        assert_eq!(item.conversation.source, Some(ConversationSource::WinkGoui));
