...
use winkgo_ai_agent::protocol::events::{ErrorEventData, TipType};
use winkgo_ai_agent::{AgentSendError, AgentStreamEvent, protocol::events::ThinkingEventData};
...
use winkgo_api_types::{AgentErrorCode, WebSocketMessage};
use winkgo_common::{ErrorChain, normalize_keys_to_snake_case, now_ms};
...
use winkgo_db::IConversationRepository;
                ...
                Err(broadcast::error::RecvError::Closed) => {
                    ...
                    let mut outcome = if deleting {
                        ...
                        self.finalize(
                            &full_text_buffer,
                            &text_segments,
                            &AgentStreamEvent::Finish(winkgo_ai_agent::protocol::events::FinishEventData::default()),
...
mod tests {
    ...
    use winkgo_ai_agent::protocol::events::{ErrorEventData, FinishEventData, TextEventData, ThinkingEventData};
    use winkgo_db::DbError;
    ...
    async fn run_text_then_finish_persists_message() {
        ...
        let bus = Arc::new(winkgo_realtime::BroadcastEventBus::new(64));
    ...
    async fn needs_auth_tip_sets_summary_flag_on_finish() {
        use winkgo_ai_agent::protocol::events::{TipType, TipsEventData};
    ...
    async fn run_warning_tip_with_finish_persists_warning_tip() {
        ...
        tx.send(AgentStreamEvent::Tips(
            winkgo_ai_agent::protocol::events::TipsEventData {
                content: String::new(),
                tip_type: winkgo_ai_agent::protocol::events::TipType::Warning,
                code: Some("ACP_EMPTY_TURN".into()),
                params: None,
            },
        ))
    ...
    async fn run_send_error_injects_error_and_completes_turn() {
        ...
        assert_eq!(
            outcome.terminal,
            RelayTerminal::Error {
                code: Some(winkgo_api_types::AgentErrorCode::UserLlmProviderAuthFailed),
    ...
    async fn run_acp_tool_call_inserts_then_updates() {
        use winkgo_ai_agent::protocol::events::tool_call::{
    ...
    async fn run_tool_group_persists_message() {
        use winkgo_ai_agent::protocol::events::tool_call::{ToolCallStatus, ToolGroupEntry};
    ...
    async fn complete_conversation_ignores_not_found_after_delete() {
        ...
        let bus: Arc<dyn EventBroadcaster> = Arc::new(winkgo_realtime::BroadcastEventBus::new(64));
    ...
    struct RecordingRepo {
        ...
        updates: Mutex<Vec<(String, winkgo_db::MessageRowUpdate)>>,
    ...
    impl RecordingRepo {
        ...
        #[allow(dead_code)]
        fn take_updates(&self) -> Vec<(String, winkgo_db::MessageRowUpdate)> {
    ...
    impl IConversationRepository for RecordingRepo {
        async fn get(&self, _id: &str) -> Result<Option<winkgo_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn create(&self, _row: &winkgo_db::models::ConversationRow) -> Result<(), DbError> {
            Ok(())
        }
        async fn update(&self, _id: &str, _updates: &winkgo_db::ConversationRowUpdate) -> Result<(), DbError> {
        ...
        async fn list_paginated(
            &self,
            _user_id: &str,
            _filters: &winkgo_db::ConversationFilters,
        ) -> Result<winkgo_common::PaginatedResult<winkgo_db::models::ConversationRow>, DbError> {
            Ok(winkgo_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
            })
        ...
        async fn find_by_source_and_chat(
            ...
            _chat_id: &str,
            _agent_type: &str,
        ) -> Result<Option<winkgo_db::models::ConversationRow>, DbError> {
            Ok(None)
        }
        async fn list_by_cron_job(
            ...
            _user_id: &str,
            _cron_job_id: &str,
        ) -> Result<Vec<winkgo_db::models::ConversationRow>, DbError> {
            Ok(vec![])
        }
        ...
        async fn list_messages_page(
            &self,
            _conv_id: &str,
            _params: &winkgo_db::MessagePageParams,
        ) -> Result<winkgo_db::MessagePageResult, DbError> {
            Ok(winkgo_db::MessagePageResult {
                items: vec![],
        ...
        async fn update_message(&self, id: &str, updates: &winkgo_db::MessageRowUpdate) -> Result<(), DbError> {
            if self.not_found.load(Ordering::Acquire) {
        ...
        async fn search_messages(
            ...
            _page: u32,
            _page_size: u32,
        ) -> Result<winkgo_common::PaginatedResult<winkgo_db::MessageSearchRow>, DbError> {
            Ok(winkgo_common::PaginatedResult {
                items: vec![],
                total: 0,
                has_more: false,
