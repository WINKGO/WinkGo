...
use winkgo_api_types::{AcpBuildExtra, TeamSessionBinding, WinkGoAgentBuildExtra};
use winkgo_common::{AgentType, WorkspacePathValidationError, validate_workspace_path_availability};
use winkgo_db::models::ConversationRow;
use winkgo_db::{IAcpSessionRepository, IAgentMetadataRepository};
...
impl<'a> SessionContextBuilder<'a> {
    ...
    pub(crate) async fn build_options(
        &self,
        row: &ConversationRow,
        seed: Option<WinkGoAgentRuntimePermissionSeed>,
    ...
    async fn resolve_acp_identity(
        ...
        extra: &serde_json::Value,
        session_row: Option<&winkgo_db::models::AcpSessionRow>,
...
/// Runtime permission gate inputs for an winkgo_agent rebuild, loaded from the
/// conversation's persisted assistant snapshot in the service layer
/// (`ConversationService::load_winkgo_agent_permission_seed`) and threaded down so
...
///   sessions.
#[derive(Debug, Clone)]
pub(crate) struct WinkGoAgentRuntimePermissionSeed {
    pub default_permission_mode: String,
    pub resolved_permission_value: Option<String>,
}

fn build_winkgo_agent_context(
    row: &ConversationRow,
    extra: serde_json::Value,
    team: Option<TeamSessionBinding>,
    permission_seed: Option<WinkGoAgentRuntimePermissionSeed>,
) -> WinkGoAgentSessionBuildContext {
    let mut config: WinkGoAgentBuildExtra = match serde_json::from_value(extra.clone()) {
        Ok(config) => config,
        Err(err) => {
            warn!(
                conversation_id = %row.id,
                error = %err,
                "session_context: invalid winkgo_agent extra; using defaults"
            );
            WinkGoAgentBuildExtra::default()
        }
    };
    config.user_id.get_or_insert_with(|| row.user_id.clone());
    apply_team_seed_to_winkgo_agent_config(&team, &mut config);
    let belongs_to_team = team.is_some();
...
fn apply_runtime_permission_seed(
    seed: WinkGoAgentRuntimePermissionSeed,
    row: &ConversationRow,
    config: &mut WinkGoAgentBuildExtra,
) {
    ...
    if let Some(resolved) = seed.resolved_permission_value.filter(|value| !value.is_empty()) {
        debug!(
            conversation_id = %row.id,
            "session_context: winkgo_agent rebuild seeded from resolved runtime permission"
        );
...
fn apply_team_seed_to_winkgo_agent_config(team: &Option<TeamSessionBinding>, config: &mut WinkGoAgentBuildExtra) {
    let Some(team) = team else {
        return;
    };
    if config.backend.as_deref().is_none_or(str::is_empty) {
...
fn decode_persisted_session_state(state: winkgo_db::PersistedSessionState) -> PersistedSessionState {
...
mod tests {
    ...
    fn winkgo_agent_context(context: AgentSessionContext) -> WinkGoAgentSessionBuildContext {
        match context.kind {
            AgentSessionKind::WinkGoAgent(winkgo_agent) => *winkgo_agent,
            other => panic!("expected WinkGoAgent context, got {other:?}"),
        }
    }
    ...
    #[tokio::test]
    async fn winkgo_agent_team_extra_is_exposed_as_typed_context() {
        let repos = setup().await;
        let row = row(
            "winkgo_agent",
            serde_json::json!({
        ...
        assert_eq!(team.runtime_seed.backend.as_deref(), Some("winkgo_agent"));
        assert_eq!(team.mcp.as_ref().unwrap().stdio.port, 5252);

        let winkgo_agent = winkgo_agent_context(context);
        assert!(winkgo_agent.belongs_to_team);
        assert_eq!(winkgo_agent.config.team_mcp_stdio_config.unwrap().port, 5252);
    }

    #[tokio::test]
    async fn winkgo_agent_uses_conversation_model_and_ignores_legacy_extra_model() {
        let repos = setup().await;
        let row = row(
            "winkgo_agent",
            serde_json::json!({
    ...
    async fn workspace_empty_uses_auto_path_and_is_not_custom() {
        let repos = setup().await;
        let row = row("winkgo_agent", serde_json::json!({}), None);

        let context = repos.builder().build(&row).await.unwrap();
        assert!(!context.workspace.is_custom);
        assert!(context.workspace.stored_path.is_empty());
        assert!(context.workspace.path.ends_with("winkgo_agent-temp-conv-1"));
    }
    ...
    fn workspace_validation_failure_logs_redacted_runtime_check() {
        let runtime = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap();
        let raw_path = "/tmp/winkgo-secret-workspace-token-12345";
        let captured = capture_logs(Level::WARN, || {
            runtime.block_on(async {
                let repos = setup().await;
                let row = row("winkgo_agent", serde_json::json!({ "workspace": raw_path }), None);
        ...
        assert!(captured.contains("winkgo_feedback_diagnostics"), "{captured}");
    ...
    async fn legacy_agent_types_are_archived_before_runtime_context() {
        ...
        for (agent_type, extra) in [
            ...
            ("codex", serde_json::json!({ "workspace": "/tmp/winkgo-codex-history" })),
            (
                "openclaw-gateway",
    ...
    fn winkgo_agent_seed(mode: &str, resolved: Option<&str>) -> WinkGoAgentRuntimePermissionSeed {
        WinkGoAgentRuntimePermissionSeed {
    ...
    #[test]
    fn winkgo_agent_auto_mode_rebuild_adopts_resolved_permission_value() {
        // AC#1: auto happy path — runtime yolo survives rebuild.
        let row = row("winkgo_agent", serde_json::json!({ "session_mode": "default" }), None);
        let ctx = build_winkgo_agent_context(
            &row,
            serde_json::json!({ "session_mode": "default" }),
            None,
            Some(winkgo_agent_seed("auto", Some("yolo"))),
        );
        assert_eq!(ctx.config.session_mode.as_deref(), Some("yolo"));
    }

    #[test]
    fn winkgo_agent_auto_mode_resolved_overrides_create_time_seed() {
        // AC#2: existing-data compat — create-time non-yolo seed is overridden.
        let row = row("winkgo_agent", serde_json::json!({ "session_mode": "auto_edit" }), None);
        let ctx = build_winkgo_agent_context(
            &row,
    ...
    #[test]
    fn winkgo_agent_fixed_mode_ignores_resolved_permission_value() {
        ...
        let ctx = build_winkgo_agent_context(
            ...
            serde_json::json!({ "session_mode": "default" }),
            None,
            Some(winkgo_agent_seed("fixed", Some("yolo"))),
        );
        assert_eq!(ctx.config.session_mode.as_deref(), Some("default"));
    }

    #[test]
    fn winkgo_agent_team_bound_session_ignores_resolved_permission_value() {
