...
use winkgo_common::{AgentKillReason, ConversationStatus, TimestampMs, generate_prefixed_id, now_ms};
...
use winkgo_db::{IConversationRepository, SqliteConversationRepository, init_database_memory};
...
fn ensure_test_workspace_path() -> String {
    ensure_named_workspace_path("winkgo-conversation-extended-test-project")
}
...
async fn t9_6_search_result_includes_conversation_model() {
    ...
    // Search surfaces conversation.model only for winkgo_agent (the only type that
    // carries a top-level model under the winkgo_agent-only rule).
    let winkgo_agent_req: CreateConversationRequest = serde_json::from_value(json!({
        "type": "winkgo_agent",
        "model": { "provider_id": "p1", "model": "claude-sonnet-4-20250514" },
        "extra": { "workspace": workspace }
    }))
    .unwrap();
    let conv = svc.create(USER_ID, winkgo_agent_req).await.unwrap();
...
async fn t10_1_same_workspace() {
    let (svc, _repo, _b) = setup().await;
    let shared_workspace = ensure_named_workspace_path("winkgo-conversation-extended-shared-workspace");
    let other_workspace = ensure_named_workspace_path("winkgo-conversation-extended-other-workspace");
...
async fn t10_2_no_associated() {
    let (svc, _repo, _b) = setup().await;
    let workspace = ensure_named_workspace_path("winkgo-conversation-extended-unique-workspace");
