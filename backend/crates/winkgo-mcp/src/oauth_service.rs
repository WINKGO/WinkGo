...
use winkgo_api_types::{OAuthLoginResponse, OAuthStatusResponse};
use winkgo_common::{TimestampMs, now_ms};
use winkgo_db::{IOAuthTokenRepository, UpsertOAuthTokenParams};
...
const DEFAULT_CLIENT_ID: &str = "winkgo";
...
impl McpOAuthService {
    ...
    pub async fn logout(&self, server_url: &str) -> Result<(), McpError> {
        match self.token_repo.delete(server_url).await {
            ...
            Err(winkgo_db::DbError::NotFound(_)) => {
    ...
    async fn handle_callback_connection(
        ...
        let response = "HTTP/1.1 200 OK\r\n\
            ...
            <p>You can close this window and return to WinkGo.</p>\
            </body></html>";
...
mod tests {
    ...
    impl IOAuthTokenRepository for MockTokenRepo {
        async fn get_by_url(&self, _: &str) -> Result<Option<winkgo_db::models::OAuthTokenRow>, winkgo_db::DbError> {
            Ok(None)
        }
        ...
        ) -> Result<winkgo_db::models::OAuthTokenRow, winkgo_db::DbError> {
            unimplemented!()
        }

        async fn delete(&self, _: &str) -> Result<(), winkgo_db::DbError> {
            Ok(())
        }

        async fn list_authenticated_urls(&self) -> Result<Vec<String>, winkgo_db::DbError> {
            Ok(vec![])
        }
    }
    ...
    impl IOAuthTokenRepository for IdempotentDeleteRepo {
        ...
        async fn delete(&self, url: &str) -> Result<(), winkgo_db::DbError> {
            Err(winkgo_db::DbError::NotFound(format!(
    ...
    impl IOAuthTokenRepository for ValidTokenRepo {
        async fn get_by_url(&self, _: &str) -> Result<Option<winkgo_db::models::OAuthTokenRow>, winkgo_db::DbError> {
            Ok(Some(winkgo_db::models::OAuthTokenRow {
