...
use winkgo_db::{CreateMcpServerParams, IMcpServerRepository, UpdateMcpServerParams};
...
mod tests {
    ...
    use std::sync::Mutex;
    use winkgo_api_types::{ImportMcpServerRequest, McpTransport};
    use winkgo_common::{McpServerStatus, TimestampMs};
    use winkgo_db::models::McpServerRow;
    use winkgo_db::{CreateMcpServerParams, DbError, UpdateMcpServerParams};
    ...
    async fn persist_test_result_records_success_status_and_tools() {
        ...
        let result = McpConnectionTestResult {
            success: true,
            tools: Some(vec![winkgo_api_types::McpToolResponse {
                name: "read_file".into(),
        ...
        assert_eq!(updated.last_test_status, winkgo_common::McpServerStatus::Connected);
    ...
    async fn persist_test_result_records_error_and_clears_tools() {
        ...
        let updated = svc.get_server(&created.id).await.unwrap();
        assert_eq!(updated.last_test_status, winkgo_common::McpServerStatus::Error);
