...
pub fn build_builtin_image_gen_server(
    ...
    Some(AcpSessionMcpServer::Stdio {
        name: "winkgo-image-generation".into(),
...
fn build_image_gen_env(config: &ImageGenConfig) -> Vec<NameValuePair> {
    let entries: [(&str, &Option<String>); 6] = [
        ("WINKGO_IMG_API_KEY", &config.api_key),
        ("WINKGO_IMG_API_URL", &config.api_url),
        ("WINKGO_IMG_MODEL", &config.model),
        ("WINKGO_IMG_QUALITY", &config.quality),
        ("WINKGO_IMG_SIZE", &config.size),
        ("WINKGO_IMG_STYLE", &config.style),
    ];
    ...
    fn builtin_image_gen_with_full_config() {
        ...
        match result.unwrap() {
            AcpSessionMcpServer::Stdio {
                ...
                assert_eq!(env.len(), 6);
                assert_eq!(env[0].name, "WINKGO_IMG_API_KEY");
    ...
    fn builtin_image_gen_with_partial_config() {
        ...
        match result.unwrap() {
            AcpSessionMcpServer::Stdio { env, .. } => {
                ...
                assert_eq!(env[0].name, "WINKGO_IMG_API_URL");
                assert_eq!(env[0].value, "https://api.openai.com");
                assert_eq!(env[1].name, "WINKGO_IMG_MODEL");
