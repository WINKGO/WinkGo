...
const CLI_NAME: &str = "winkgo_agent";

/// MCP Agent adapter for WinkGoAgent.
///
/// WinkGoAgent stores MCP configuration in a TOML config file. The config path
/// is obtained via `winkgo_agent --config-path`.
///
/// # Config Format (TOML)
///
/// ```toml
...
/// ```
pub struct WinkGoAgentAdapter;

#[async_trait::async_trait]
impl McpAgentAdapter for WinkGoAgentAdapter {
    fn source(&self) -> McpSource {
        McpSource::WinkGoAgent
    }
...
/// Resolve the TOML config file path from the `winkgo_agent` CLI.
///
/// Newer `winkgo_agent` exposes this via the `config path` subcommand; older
...
async fn get_config_path() -> Result<String, McpError> {
    ...
    // Legacy fallback for `winkgo_agent` versions predating the `config` subcommand.
    ...
    if path.is_empty() {
        return Err(McpError::AgentOperationFailed(
            "winkgo_agent config path returned empty output".into(),
        ));
    }
    Ok(path)
}
