...
//!
//! For WinkGoui, we use a mock repository since it reads from the DB.

use std::collections::HashMap;
use std::sync::Arc;

use winkgo_common::McpSource;
use winkgo_mcp::{McpAgentAdapter, McpServerTransport, WinkGouiAdapter};

// ===========================================================================
// WinkGoui adapter (DB-backed)
// ===========================================================================

mod winkgo {
    use super::*;
    use winkgo_db::models::McpServerRow;
    use winkgo_db::{CreateMcpServerParams, DbError, IMcpServerRepository, UpdateMcpServerParams};
    ...
    impl IMcpServerRepository for MockRepo {
        ...
        async fn update_status(
            ...
            _id: &str,
            _s: &str,
            _lc: Option<winkgo_common::TimestampMs>,
        ) -> Result<(), DbError> {
            unimplemented!()
        }
    ...
    #[tokio::test]
    async fn source_is_winkgo() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter = WinkGouiAdapter::new(repo);
        assert_eq!(adapter.source(), McpSource::WinkGoui);
    }
    ...
    async fn install_is_noop() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter = WinkGouiAdapter::new(repo.clone());
    ...
    async fn trait_object_safety() {
        let repo = Arc::new(MockRepo::new(vec![]));
        let adapter: Arc<dyn McpAgentAdapter> = Arc::new(WinkGouiAdapter::new(repo));
        assert_eq!(adapter.source(), McpSource::WinkGoui);
        assert!(adapter.is_installed().await.unwrap());
    }
}
...
mod opencode {
    use super::*;
    use winkgo_mcp::OpencodeAdapter;
...
// ===========================================================================
// WinkGoAgent adapter (CLI + TOML-backed)
// ===========================================================================

// Note: Full lifecycle tests for WinkGoAgent require the `winkgo_agent` CLI to be
// installed (for `--config-path`). The unit tests in winkgo_agent.rs thoroughly
// cover TOML parsing, serialization, and roundtrip behavior. Here we
// verify the public API surface.

mod winkgo_agent {
    use super::*;
    use winkgo_mcp::WinkGoAgentAdapter;

    #[test]
    fn source_is_winkgo_agent() {
        assert_eq!(WinkGoAgentAdapter.source(), McpSource::WinkGoAgent);
    }

    #[test]
    fn trait_object_safety() {
        let adapter: Box<dyn McpAgentAdapter> = Box::new(WinkGoAgentAdapter);
        assert_eq!(adapter.source(), McpSource::WinkGoAgent);
    }
}
