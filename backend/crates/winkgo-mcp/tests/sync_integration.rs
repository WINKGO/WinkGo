...
use winkgo_db::SqliteMcpServerRepository;
use winkgo_mcp::{DetectedServer, McpAgentAdapter, McpError, McpServerTransport, McpSyncService};
...
async fn make_service(adapters: Vec<Arc<dyn McpAgentAdapter>>) -> McpSyncService {
    ...
    let repo: Arc<dyn winkgo_db::IMcpServerRepository> = Arc::new(SqliteMcpServerRepository::new(db.pool().clone()));
