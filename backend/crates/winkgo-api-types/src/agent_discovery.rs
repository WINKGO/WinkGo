...
use winkgo_common::{AgentType, TimestampMs};

/// How an agent row was sourced.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum AgentSource {
    /// Ships with the backend binary (no CLI install required — e.g. `winkgo_agent`).
    Internal,
...
/// the frontend verbatim, and typed-decoded inside `winkgo-ai-agent`
...
pub struct AgentMetadata {
    ...
    /// Native mode id that WinkGo's legacy `yolo` / `yoloNoSandbox`
