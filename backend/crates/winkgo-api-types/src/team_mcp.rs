...
//!
//! These are promoted from `winkgo-team::mcp::bridge` so that downstream
//! crates (`winkgo-ai-agent` deserializing `AcpBuildExtra`, etc.) can reference
//! the same shape without depending on `winkgo-team`.
...
pub const TEAM_MCP_SERVER_NAME: &str = "winkgo-team";
...
mod tests {
    ...
    fn deserialization_tolerates_unknown_fields() {
        ...
        let json = r#"{"team_id":"t-1","port":1,"token":"t","slot_id":"s","binary_path":"/usr/bin/winkgo_core","future_field":42}"#;
        let parsed: TeamMcpStdioConfig = serde_json::from_str(json).unwrap();
