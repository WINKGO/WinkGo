...
/// WinkGoAgent-specific fields extracted from `extra` in build task options.
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct WinkGoAgentBuildExtra {
    #[serde(default)]
...
mod tests {
    ...
    fn acp_build_extra_ignores_legacy_guide_config_field() {
        ...
        let parsed: AcpBuildExtra = serde_json::from_value(serde_json::json!({
            "backend": "claude",
            legacy_key: {"port": 1234, "token": "legacy", "binary_path": "/bin/winkgo_core"}
        }))
        .unwrap();
    ...
    #[test]
    fn winkgo_agent_build_extra_ignores_legacy_fields() {
        let legacy_key = concat!("guide", "_mcp_config");
        let parsed: WinkGoAgentBuildExtra = serde_json::from_value(serde_json::json!({
        ...
        assert_eq!(parsed.backend.as_deref(), Some("winkgo_agent"));
        let serialized = serde_json::to_value(&parsed).unwrap();
        assert!(
            serialized.get(legacy_key).is_none(),
            "legacy guide config must be ignored, not re-serialized"
