...
fn to_zip_entry(e: winkgo_api_types::ZipFileEntry) -> ZipEntry {
...
fn to_snapshot_info_response(info: SnapshotInfo) -> SnapshotInfoResponse {
    let mode = match info.mode {
        SnapshotMode::GitRepo => winkgo_api_types::SnapshotMode::GitRepo,
        SnapshotMode::Snapshot => winkgo_api_types::SnapshotMode::Snapshot,
    };
...
mod tests {
    ...
    fn zip_entry_conversion_text() {
        let e = winkgo_api_types::ZipFileEntry {
            name: "a.txt".into(),
    ...
    fn snapshot_info_response_git_repo() {
        ...
        assert_eq!(r.mode, winkgo_api_types::SnapshotMode::GitRepo);
    ...
    fn snapshot_info_response_snapshot_mode() {
        ...
        let r = to_snapshot_info_response(info);
        assert_eq!(r.mode, winkgo_api_types::SnapshotMode::Snapshot);
