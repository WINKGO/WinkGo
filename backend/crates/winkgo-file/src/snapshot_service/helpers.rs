...
pub(super) const SNAPSHOT_DIR_PREFIX: &str = "winkgo-snapshot-";
...
const SNAPSHOT_SIG_NAME: &str = "winkgo";
/// Signature email used for snapshot commits.
const SNAPSHOT_SIG_EMAIL: &str = "snapshot@winkgo.local";
...
pub(super) struct WorkspaceState {
    ...
    /// - snapshot mode: `/tmp/winkgo-snapshot-{hash}` (bare-style git dir).
