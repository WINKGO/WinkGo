---
name: winkgo-troubleshooting
description: >-
  Diagnose a running WINK GO installation: inspect stuck or errored conversations, read provider health, scheduled task state, MCP server health, team member state, backend health, and winkgo_core logs. Use when the user reports WINK GO is misbehaving, a conversation is stuck, an LLM/provider call is failing, a scheduled task did not run, an MCP server has no tools, a team member is hung, or they ask to troubleshoot WINK GO.
---
<!-- Modified from AionCore by WINK GO contributors in 2026. -->

# WINK GO Troubleshooting

Use the bundled `winkgo_core diagnose` CLI for read-only troubleshooting. It uses
...
1. Use only `"$WINKGO_HELPER_BIN" diagnose ...`.
...
4. Treat every command as read-only. To change WINK GO configuration, use the
   separate `winkgo-config` skill.
...
```bash
"$WINKGO_HELPER_BIN" diagnose capabilities
```
...
The CLI resolves `"current"` from `WINKGO_CONVERSATION_ID`. Commands that read
the backend also use `WINKGO_BASE_URL` and `WINKGO_USER_ID` from the same
runtime context.

## Start Wide

For a vague "WINK GO is broken" report, run:

```bash
"$WINKGO_HELPER_BIN" diagnose overview
```
...
```bash
"$WINKGO_HELPER_BIN" diagnose conversations get <<'JSON'
{
  "conversation_id": "current"
}
JSON
...
```bash
"$WINKGO_HELPER_BIN" diagnose conversations messages <<'JSON'
{
  "conversation_id": "current",
  "limit": 30,
  "errors_only": true
...
```bash
"$WINKGO_HELPER_BIN" diagnose providers summary
```
...
```bash
"$WINKGO_HELPER_BIN" diagnose cron summary
```
...
```bash
"$WINKGO_HELPER_BIN" diagnose mcp summary
```
...
```bash
"$WINKGO_HELPER_BIN" diagnose teams summary
```
...
```bash
"$WINKGO_HELPER_BIN" diagnose health
```
...
Tail logs when the runtime provides `WINKGO_LOG_DIR`:

```bash
"$WINKGO_HELPER_BIN" diagnose logs tail <<'JSON'
{
  "lines": 100,
  "errors_only": true,
  "conversation_id": "current"
...
If `WINKGO_LOG_DIR` is unavailable and the user gives a log directory, pass it
explicitly:

```bash
"$WINKGO_HELPER_BIN" diagnose logs tail <<'JSON'
{
  "log_dir": "/Users/alex/Library/Logs/WinkGo",
  "lines": 100,
  "errors_only": true,
  "conversation_id": "conv_123"
}
...
```bash
"$WINKGO_HELPER_BIN" diagnose http get <<'JSON'
{
  "path": "/api/teams",
...
- For configuration changes, switch to `winkgo-config`.
- For scheduled task creation or updates, use the `cron` skill or
  `winkgo-config` cron commands.
