...
1. Use only `"$WINKGO_HELPER_BIN" config ...`.
2. Never pass, inline, export, echo, or set any `WINKGO_...` environment variable.
3. Put all command input in stdin JSON.
4. Do not use flags for business fields.
5. Use `"$WINKGO_HELPER_BIN" config capabilities` when unsure which config command or stdin fields are supported.
...
Ask winkgo_core what this version supports:

```bash
"$WINKGO_HELPER_BIN" config capabilities
```
...
```bash
"$WINKGO_HELPER_BIN" config context
```
...
```bash
"$WINKGO_HELPER_BIN" config assistants list
```

Inspect the current assistant:

```bash
"$WINKGO_HELPER_BIN" config assistants get <<'JSON'
{
...
```bash
"$WINKGO_HELPER_BIN" config assistants create <<'JSON'
{
...
```bash
"$WINKGO_HELPER_BIN" config assistants update <<'JSON'
{
  "assistant_id": "current",
  "locale": "en-US",
...
```bash
"$WINKGO_HELPER_BIN" config assistants state <<'JSON'
{
  "assistant_id": "current",
  "enabled": true,
  "sort_order": 10
...
```bash
"$WINKGO_HELPER_BIN" config assistants rule read <<'JSON'
{
  "assistant_id": "current",
  "locale": "en-US"
}
...
```bash
"$WINKGO_HELPER_BIN" config assistants rule write <<'JSON'
{
  "assistant_id": "current",
  "locale": "en-US",
  "content": "# Role\nYou are..."
...
```bash
"$WINKGO_HELPER_BIN" config skills list
```

Inspect a skill directory before importing:

```bash
"$WINKGO_HELPER_BIN" config skills info <<'JSON'
{
...
```bash
"$WINKGO_HELPER_BIN" config skills import <<'JSON'
{
...
{
  "assistant_id": "current",
  "enabled_skills": ["winkgo-config", "cron"]
}
JSON
```
...
```bash
"$WINKGO_HELPER_BIN" config skills external-paths list
```

```bash
"$WINKGO_HELPER_BIN" config skills external-paths add <<'JSON'
{
  "name": "Team Skills",
...
```bash
"$WINKGO_HELPER_BIN" config skills external-paths remove <<'JSON'
{
  "path": "/absolute/path/to/team-skills"
}
JSON
...
```bash
"$WINKGO_HELPER_BIN" config skills market enable
```

```bash
"$WINKGO_HELPER_BIN" config skills market disable
```
...
```bash
"$WINKGO_HELPER_BIN" config mcp servers list
```

Create an MCP server:

```bash
"$WINKGO_HELPER_BIN" config mcp servers create <<'JSON'
{
  "name": "Local Tools",
  "transport": {
    "type": "stdio",
...
```bash
"$WINKGO_HELPER_BIN" config mcp servers update <<'JSON'
{
  "server_id": "mcp_123",
...
```bash
"$WINKGO_HELPER_BIN" config mcp test-connection <<'JSON'
{
  "name": "Local Tools",
  "transport": {
    "type": "stdio",
...
```bash
"$WINKGO_HELPER_BIN" config mcp oauth check-status <<'JSON'
{
...
```bash
"$WINKGO_HELPER_BIN" config providers list
```

Create a provider:

```bash
"$WINKGO_HELPER_BIN" config providers create <<'JSON'
{
  "name": "OpenAI",
  "platform": "openai",
...
```bash
"$WINKGO_HELPER_BIN" config providers update <<'JSON'
{
...
```bash
"$WINKGO_HELPER_BIN" config providers detect-protocol <<'JSON'
{
...
```bash
"$WINKGO_HELPER_BIN" config providers models fetch <<'JSON'
{
  "provider_id": "provider_123"
}
JSON
...
```bash
"$WINKGO_HELPER_BIN" config providers health-check <<'JSON'
{
  "provider_id": "provider_123",
  "model": "gpt-4.1"
}
...
```bash
"$WINKGO_HELPER_BIN" config settings get
```

Patch backend settings:

```bash
"$WINKGO_HELPER_BIN" config settings patch <<'JSON'
{
  "language": "en-US",
...
```bash
"$WINKGO_HELPER_BIN" config settings client get
```

```bash
"$WINKGO_HELPER_BIN" config settings client put <<'JSON'
{
  "ui.zoomFactor": 1.2
}
JSON
...
```bash
"$WINKGO_HELPER_BIN" config agents list
```

Enable or disable an agent:

```bash
"$WINKGO_HELPER_BIN" config agents enable <<'JSON'
{
  "agent_id": "codex",
  "enabled": true
}
...
```bash
"$WINKGO_HELPER_BIN" config agents overrides get <<'JSON'
{
  "agent_id": "codex"
}
JSON
...
```bash
"$WINKGO_HELPER_BIN" config agents overrides set <<'JSON'
{
  "agent_id": "codex",
...
```bash
"$WINKGO_HELPER_BIN" config agents custom create <<'JSON'
{
  "name": "Custom Agent",
...
```bash
"$WINKGO_HELPER_BIN" config agents custom update <<'JSON'
{
...
```bash
"$WINKGO_HELPER_BIN" config agents custom try-connect <<'JSON'
{
  "command": "/absolute/path/to/agent-cli"
}
JSON
...
```bash
"$WINKGO_HELPER_BIN" config cron current list
```

Create a task:

```bash
"$WINKGO_HELPER_BIN" config cron current create <<'JSON'
{
  "name": "Daily Summary",
...
```bash
"$WINKGO_HELPER_BIN" config cron current update <<'JSON'
{
  "job_id": "cron_123",
  "name": "Daily Summary",
  "schedule": "0 18 * * MON-FRI",
...
```bash
"$WINKGO_HELPER_BIN" config cron jobs list
```

Create a cron job:

```bash
"$WINKGO_HELPER_BIN" config cron jobs create <<'JSON'
{
  "name": "Weekly Report",
...
```bash
"$WINKGO_HELPER_BIN" config cron jobs update <<'JSON'
{
  "job_id": "cron_123",
  "name": "Weekly Report",
...
```bash
"$WINKGO_HELPER_BIN" config cron jobs run <<'JSON'
{
  "job_id": "cron_123"
}
JSON
...
```bash
"$WINKGO_HELPER_BIN" config cron jobs skill save <<'JSON'
{
  "job_id": "cron_123",
