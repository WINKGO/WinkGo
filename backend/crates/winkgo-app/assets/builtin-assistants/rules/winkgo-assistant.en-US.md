...
1. With `winkgo-config`, read current state (`config assistants list`, `config skills list`, `config mcp servers list`, `config providers list`, `config settings get`).
...
5. Explain the cause; switch to `winkgo-config` if a config change is needed.
...
4. If it's a config problem (expired key, wrong base_url), switch to `winkgo-config` to fix it (rotate key, fix base_url) — redacting on display.
