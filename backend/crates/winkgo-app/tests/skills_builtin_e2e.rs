...
//! `winkgo_app::create_router_with_states` against an in-memory DB.
...
use winkgo_app::{ModuleStates, build_module_states, create_router_with_states};
use winkgo_db::init_database_memory;
...
async fn fixture_embedded() -> Fixture {
    ...
    unsafe {
        std::env::remove_var("WINKGO_BUILTIN_SKILLS_PATH");
    }
    ...
    winkgo_extension::materialize_if_needed(&data_dir, winkgo_extension::builtin_skills_corpus(), "test-fixture")
        .await
    ...
    // `build_module_states` builds a state pointing at `~/.winkgo/`,
    ...
    let skill_repo = Arc::new(winkgo_db::SqliteSkillRepository::new(services.database.pool().clone()));
    ...
    winkgo_extension::sync_skill_catalog_into_repo(&states.skill.skill_paths, skill_repo.as_ref())
        .await
