...
//! mirroring `winkgo-shell/tests/stt_stream_deepgram_integration.rs`).
//!
...
use winkgo_app::{AppConfig, AppServices, create_router};
...
async fn seed_stt_prefs(app: &TestApp, config: Value) {
    let repo = Arc::new(winkgo_db::SqliteClientPreferenceRepository::new(
        app.services.database.pool().clone(),
    ));
    let service = winkgo_system::ClientPrefService::new(repo);
    let mut req = winkgo_api_types::UpdateClientPreferencesRequest::new();
...
// ---------------------------------------------------------------------------
// Mock Deepgram live server (minimal copy of the winkgo-shell integration
