...
use winkgo_ai_agent::{AgentInstance, IAgentTask, IMockAgent, WorkerTaskManagerImpl};
use winkgo_app::{AppConfig, AppServices, build_module_states, create_router, create_router_with_states};
use winkgo_extension::{ExternalPathsManager, SkillPaths, SkillRouterState};
use winkgo_file::FileService;
use winkgo_system::VersionCheckService;
...
pub async fn build_app_with_skill_paths(root: &std::path::Path) -> (axum::Router, AppServices, SkillPaths) {
    ...
    states.skill = SkillRouterState {
        ...
        skill_repo: std::sync::Arc::new(winkgo_db::SqliteSkillRepository::new(services.database.pool().clone())),
...
pub async fn sync_skill_catalog_for_test(services: &AppServices, paths: &SkillPaths) {
    let repo = winkgo_db::SqliteSkillRepository::new(services.database.pool().clone());
    winkgo_extension::sync_skill_catalog_into_repo(paths, &repo)
        .await
...
pub async fn build_app_with_noop_opener() -> (axum::Router, AppServices) {
    ...
    let services = AppServices::from_config(db, &AppConfig::default()).await.unwrap();
    let (mut states, _) = build_module_states(&services).await.expect("build module states");
    states.shell.shell_service = std::sync::Arc::new(winkgo_shell::ShellService::new(std::sync::Arc::new(
        winkgo_shell::NoopSystemOpener,
    )));
    let router = create_router_with_states(&services, states);
    (router, services)
}
...
pub async fn build_app_with_mock_agents() -> (axum::Router, AppServices) {
    ...
    let factory: std::sync::Arc<
        dyn Fn(
                winkgo_ai_agent::types::BuildTaskOptions,
            )
                -> futures_util::future::BoxFuture<'static, Result<AgentInstance, winkgo_ai_agent::AgentError>>
            + Send
            + Sync,
    ...
    let wtm: std::sync::Arc<dyn winkgo_ai_agent::IWorkerTaskManager> =
...
impl IAgentTask for NoopMockAgent {
    fn agent_type(&self) -> winkgo_common::AgentType {
        winkgo_common::AgentType::Acp
    }
    ...
    fn status(&self) -> Option<winkgo_common::ConversationStatus> {
        None
    }
    fn last_activity_at(&self) -> winkgo_common::TimestampMs {
        winkgo_common::now_ms()
    }
    fn subscribe(&self) -> tokio::sync::broadcast::Receiver<winkgo_ai_agent::AgentStreamEvent> {
        let (tx, _) = tokio::sync::broadcast::channel(1);
        tx.subscribe()
    }
    async fn send_message(
        &self,
        _data: winkgo_ai_agent::types::SendMessageData,
    ) -> Result<(), winkgo_ai_agent::AgentSendError> {
        Ok(())
    }
    async fn cancel(&self) -> Result<(), winkgo_ai_agent::AgentError> {
        Ok(())
    }
    fn kill(&self, _reason: Option<winkgo_common::AgentKillReason>) -> Result<(), winkgo_ai_agent::AgentError> {
        Ok(())
    }
}
...
impl IMockAgent for NoopMockAgent {
    async fn get_config_options(
        &self,
    ) -> Result<winkgo_api_types::GetConfigOptionsResponse, winkgo_ai_agent::AgentError> {
        Ok(winkgo_api_types::GetConfigOptionsResponse {
            config_options: vec![winkgo_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                ...
                options: vec![winkgo_api_types::AcpConfigSelectOptionDto {
    ...
    async fn set_config_option(
        ...
        _option_id: &str,
        _value: &str,
    ) -> Result<winkgo_api_types::SetConfigOptionResponse, winkgo_ai_agent::AgentError> {
        Ok(winkgo_api_types::SetConfigOptionResponse {
            confirmation: winkgo_api_types::ConfigOptionConfirmation::Observed,
            config_options: Some(vec![winkgo_api_types::AcpConfigOptionDto {
                id: "model".to_owned(),
                name: Some("Model".to_owned()),
                label: None,
                description: None,
...
pub fn extract_csrf_token(resp: &axum::response::Response) -> Option<String> {
    resp.headers()
        ...
        .find(|s| s.starts_with("winkgo-csrf-token="))
        .map(|s| {
            s.strip_prefix("winkgo-csrf-token=")
                .unwrap()
                .split(';')
                .next()
                .unwrap()
...
pub fn json_with_token(method_str: &str, uri: &str, body: serde_json::Value, token: &str, csrf: &str) -> Request<Body> {
    Request::builder()
        ...
        .header("cookie", format!("winkgo-csrf-token={csrf}"))
...
pub async fn setup_and_login(
    ...
    let hash = winkgo_auth::hash_password(password).unwrap();
    if username == "admin" {
        services
            .user_repo
