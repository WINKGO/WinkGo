...
async fn second_start_with_same_version_is_noop() {
    ...
    let first =
        winkgo_extension::materialize_if_needed(data_dir, winkgo_extension::builtin_skills_corpus(), "test-1.0.0")
            .await
            .unwrap();
...
async fn version_bump_triggers_rewrite() {
    ...
    let second =
        winkgo_extension::materialize_if_needed(data_dir, winkgo_extension::builtin_skills_corpus(), "test-2.0.0")
            .await
            .unwrap();
