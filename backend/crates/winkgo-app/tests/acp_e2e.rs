...
async fn management_list_returns_array() {
    ...
    assert!(agents.iter().any(|a| a["agent_type"] == "winkgo_agent"));
}
...
async fn management_list_includes_missing_custom_agents() {
    ...
    repo.upsert(&UpsertAgentMetadataParams {
        ...
        agent_type: "acp",
        agent_source: "custom",
        agent_source_info: Some(r#"{"binary_name":"winkgo-missing-agent-binary"}"#),
        enabled: true,
        command: Some("winkgo-missing-agent-binary"),
        args: Some("[]"),
        env: Some("[]"),
