...
//! integration level in `winkgo-office/tests/`.
...
use winkgo_app::{AppConfig, AppServices, build_module_states, create_router_with_states};
use winkgo_office::{ConversionService, OfficeRouterState, OfficecliWatchManager, ProxyService, SnapshotService};
...
fn build_test_office_state(data_dir: &std::path::Path, allowed_roots: Vec<std::path::PathBuf>) -> OfficeRouterState {
    use winkgo_office::error::OfficeError;
    use winkgo_office::types::DocType;
    use winkgo_office::{ProcessHandle, ProcessSpawner};
    ...
    impl winkgo_realtime::EventBroadcaster for NoopBroadcaster {
        fn broadcast(&self, _msg: winkgo_api_types::WebSocketMessage<serde_json::Value>) {}
    }

    let spawner: Arc<dyn ProcessSpawner> = Arc::new(NoopSpawner);
    let bc: Arc<dyn winkgo_realtime::EventBroadcaster> = Arc::new(NoopBroadcaster);
