...
async fn ensure_default_team_agent_installed(services: &winkgo_app::AppServices) {
...
async fn mark_claude_backend_team_mcp_stdio_capable(services: &winkgo_app::AppServices) {
...
async fn tc6c_create_team_rejects_missing_workspace_path() {
    ...
    let missing_workspace =
        std::env::temp_dir().join(format!("winkgo-team-missing-{}", winkgo_common::generate_short_id()));
