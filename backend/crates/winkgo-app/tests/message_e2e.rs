...
async fn insert_message(
    services: &winkgo_app::AppServices,
    conv_id: &str,
    msg_id: &str,
    content: &str,
    created_at: i64,
) {
    let repo = winkgo_db::SqliteConversationRepository::new(services.database.pool().clone());
    let msg = winkgo_db::models::MessageRow {
        id: msg_id.into(),
    ...
    winkgo_db::IConversationRepository::insert_message(&repo, &msg)
        .await
        .unwrap();
}

async fn update_conversation_workspace(services: &winkgo_app::AppServices, conv_id: &str, workspace: &str) {
    let repo = winkgo_db::SqliteConversationRepository::new(services.database.pool().clone());
...
async fn upsert_artifact(services: &winkgo_app::AppServices, artifact: winkgo_db::ConversationArtifactRow) {
    let repo = winkgo_db::SqliteConversationRepository::new(services.database.pool().clone());
    winkgo_db::IConversationRepository::upsert_artifact(&repo, &artifact)
        .await
        .unwrap();
}
