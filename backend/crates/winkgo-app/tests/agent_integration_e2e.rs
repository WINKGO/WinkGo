...
use winkgo_ai_agent::protocol::events::TextEventData;
use winkgo_ai_agent::types::{BuildTaskOptions, SendMessageData};
use winkgo_ai_agent::{AgentError, AgentStreamEvent, IWorkerTaskManager};
use winkgo_api_types::AgentSource;
use winkgo_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, TimestampMs, now_ms};
use winkgo_db::UpsertAgentMetadataParams;
...
async fn build_app_with_mock_tasks() -> (axum::Router, winkgo_app::AppServices, Arc<MockTaskManager>) {
    let db = winkgo_db::init_database_memory().await.unwrap();
    let services = winkgo_app::AppServices::from_config(db, &winkgo_app::AppConfig::default())
        .await
        .unwrap();
    ...
    let router = winkgo_app::create_router(&services).await.expect("build router");
...
async fn upsert_visible_agent_metadata(services: &winkgo_app::AppServices, id: &str, agent_type: &str) {
    services
        .agent_registry
        .repo_handle()
...
async fn management_endpoint_keeps_deprecated_runtime_rows_for_diagnostics() {
    ...
    for (id, agent_type) in [
        ...
        ("test-visible-winkgo_agent", "winkgo_agent"),
    ...
    assert!(types.contains(&"winkgo_agent"));
...
async fn agent_logos_endpoint_returns_backend_to_logo_catalog() {
    ...
    // WinkGo CLI has no vendor `backend` (NULL); it must still be keyed by its
    // agent_type ("winkgo_agent") so winkgo_agent conversations resolve a logo.
    assert_eq!(
        logo_for("winkgo_agent").as_deref(),
        Some("/api/assets/logos/brand/winkgo.svg")
    );
...
async fn agent_logos_endpoint_includes_disabled_and_missing_rows() {
    ...
    services
        ...
        .upsert(&UpsertAgentMetadataParams {
            id: "logo-only-row",
            icon: Some("/api/assets/logos/brand/winkgo.svg"),
            name: "Logo Only",
            name_i18n: None,
            description: None,
            description_i18n: None,
    ...
    assert_eq!(entry.unwrap()["logo"], "/api/assets/logos/brand/winkgo.svg");
}
...
#[tokio::test]
async fn internal_winkgo_cli_rejects_overrides() {
    let (mut app, services, _mock_tm) = build_app_with_mock_tasks().await;
    let (token, csrf) = setup_and_login(&mut app, &services, "admin", "Pass123!").await;
    upsert_visible_agent_metadata(&services, "632f31d2", "winkgo_agent").await;
    services.agent_registry.hydrate().await.unwrap();
    services.agent_registry.refresh_availability().await;
