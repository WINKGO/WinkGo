...
async fn t1_5c_create_rejects_missing_workspace_path() {
    ...
    let missing_workspace =
        std::env::temp_dir().join(format!("winkgo-conv-missing-{}", winkgo_common::generate_short_id()));
...
async fn t2_4_list_source_filter() {
    ...
    // Create 2 winkgo + 1 telegram
    for _ in 0..2 {
        let req = json_with_token(
            "POST",
            "/api/conversations",
            create_body("WinkGoui Conv"),
            &token,
            &csrf,
        );
        app.clone().oneshot(req).await.unwrap();
...
async fn t4_4_update_model() {
    ...
    // winkgo_agent — only type that allows top-level model updates
    let create = json!({
        "type": "winkgo_agent",
        "name": "Model Test",
        "model": { "provider_id": "p1", "model": "m1" },
