...
use winkgo_db::models::{AssistantSessionRow, AssistantUserRow};
use winkgo_db::{IChannelRepository, SqliteChannelRepository};
...
#[tokio::test]
async fn get_channel_settings_defaults_to_generated_winkgo_agent_assistant() {
    let (mut app, services) = build_app().await;
    let (token, _csrf) = setup_and_login(&mut app, &services, "admin", "StrongP@ss1").await;
    ...
    // winkgo_agent bare assistant (see channel "default to bare assistant bindings");
    // only the assistant_id is canonical, legacy fields are omitted.
    let assistant_id = json["data"]["assistant"]["assistant_id"]
        .as_str()
        .expect("default channel assistant should be the generated winkgo_agent bare assistant");
    assert!(
...
async fn pairing_approve_creates_user() {
    ...
    let repo: std::sync::Arc<dyn winkgo_db::IChannelRepository> =
        std::sync::Arc::new(winkgo_db::SqliteChannelRepository::new(pool));
    let pairing_svc = winkgo_channel::pairing::PairingService::new(repo.clone(), services.event_bus.clone());
