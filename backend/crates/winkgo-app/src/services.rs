...
use winkgo_auth::{CookieConfig, JwtService, QrTokenStore, resolve_jwt_secret};
use winkgo_common::OnConversationDelete;
use winkgo_conversation::{ConversationService, runtime_state::ConversationRuntimeStateService};
use winkgo_db::{
...
use winkgo_project::ProjectService;
use winkgo_realtime::{BroadcastEventBus, WebSocketManager};
...
pub struct AppServices {
    ...
    pub skill_paths: Arc<winkgo_extension::SkillPaths>,
...
impl AppServices {
    ...
    pub async fn from_config(database: Database, config: &AppConfig) -> anyhow::Result<Self> {
        ...
        let skill_paths = Arc::new(winkgo_extension::resolve_skill_paths(&app_resource_dir, &data_dir));
        winkgo_extension::sync_skill_catalog_into_repo(skill_paths.as_ref(), skill_repo.as_ref())
            .await
        ...
        let process_registry = Arc::new(winkgo_process::FileRegistryStore::new(&data_dir));
        let machine_id = winkgo_process::local_machine_id(&data_dir);
...
struct ConversationServiceDeps<'a> {
    ...
    skill_paths: Arc<winkgo_extension::SkillPaths>,
