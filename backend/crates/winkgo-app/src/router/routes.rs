...
use winkgo_ai_agent::{agent_routes, remote_agent_routes};
use winkgo_api_types::ErrorResponse;
use winkgo_assets::{AssetRouterState, asset_routes};
use winkgo_assistant::assistant_routes;
use winkgo_auth::{
    AuthRouterState, AuthState, auth_middleware, auth_routes, csrf_middleware, security_headers_middleware,
};
use winkgo_channel::channel_routes;
#[cfg(feature = "weixin")]
use winkgo_channel::weixin_login_route;
use winkgo_common::ApiErrorLogContext;
use winkgo_conversation::{conversation_ops_routes, conversation_routes};
use winkgo_cron::cron_routes;
use winkgo_extension::{extension_routes, hub_routes, skill_routes};
use winkgo_file::file_routes;
use winkgo_mcp::mcp_routes;
use winkgo_office::{office_proxy_routes, office_routes};
use winkgo_project::project_routes;
use winkgo_realtime::{NoopMessageRouter, WsHandlerState, ws_upgrade_handler};
use winkgo_shell::shell_routes;
use winkgo_system::{ClientPrefService, connection_test_routes, system_routes};
use winkgo_team::{TeamSessionService, team_routes};
...
pub fn create_router_with_all_state(services: &AppServices, states: ModuleStates, ws_state: WsHandlerState) -> Router {
    ...
    let router = router.layer(DefaultBodyLimit::max(winkgo_common::constants::BODY_LIMIT));
