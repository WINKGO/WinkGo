...
//!
//! Adapts the transport-agnostic [`ScmActor`] (in `winkgo-project`) to the
...
use winkgo_realtime::{ConnectionId, MessageRouter, WebSocketManager};
...
impl MessageRouter for ScmMessageRouter {
    fn route(&self, conn_id: ConnectionId, name: &str, data: Value) -> bool {
        ...
        let _ = self.inbound.send(ScmInbound::Frame {
            ...
            // Current WINK GO desktop sessions are local/single-user. Keep the
            // protocol field for forward compatibility without coupling SCM to
            // the later multi-account router signature.
            user_id: String::new(),
            frame: data,
        });
        true
...
impl MessageRouter for CompositeMessageRouter {
    fn route(&self, conn_id: ConnectionId, name: &str, data: Value) -> bool {
        for router in &self.routers {
            ...
            if router.route(conn_id, name, data.clone()) {
                return true;
            }
        }
        false
