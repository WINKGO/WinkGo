...
use serde_json::json;
use tokio::sync::mpsc;
use winkgo_realtime::{PER_CONNECTION_BUFFER, WsOutbound};
...
fn router_claims_scm_frames_with_a_stringified_session() {
    ...
    let handled = router.route(ConnectionId(5), "scm", json!({"method": "scm/status"}));
    ...
    match rx.try_recv().expect("frame forwarded") {
        ScmInbound::Frame {
            ...
            assert_eq!(user_id, "", "local WINK GO desktop uses the unscoped user identity");
...
fn router_declines_other_envelopes() {
    ...
    assert!(!router.route(ConnectionId(1), "fs", json!({})));
...
impl MessageRouter for SpyRouter {
    fn route(&self, _conn_id: ConnectionId, name: &str, _data: Value) -> bool {
...
fn composite_routes_each_envelope_to_its_owner() {
    ...
    assert!(composite.route(ConnectionId(1), "scm", json!({"a": 1})));
    assert!(composite.route(ConnectionId(1), "fs", json!({})));
    assert_eq!(
...
fn composite_reports_unclaimed_envelopes_as_unhandled() {
    ...
    assert!(!composite.route(ConnectionId(1), "something-else", json!({})));
}
