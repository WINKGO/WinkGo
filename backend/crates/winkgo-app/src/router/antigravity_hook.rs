...
//!
//! agy cannot prompt for tool permission in headless mode, so WINK GO registers
...
use winkgo_ai_agent::antigravity_hook::HookTokenRegistry;
...
async fn handle_hook(
    ...
    if !state.tokens.verify(&conversation_id, presented) {
        ...
        return Json(AntigravityHookOutput::deny(
            "winkgo rejected an unauthenticated hook callback",
        ));
    }
    ...
    let Some(AgentInstance::Session(task)) = state.task_manager.get_task(&conversation_id) else {
        ...
        return Json(AntigravityHookOutput::deny(
            "winkgo has no live session for this conversation",
        ));
    };
    ...
    tracing::info!(
        ...
        approved = matches!(
            decision,
            winkgo_session::PermissionDecision::Approved | winkgo_session::PermissionDecision::AllowAlways
        ),
    ...
    Json(match decision {
        winkgo_session::PermissionDecision::Approved | winkgo_session::PermissionDecision::AllowAlways => {
            AntigravityHookOutput::allow("approved in WINK GO")
        }
        winkgo_session::PermissionDecision::Denied => AntigravityHookOutput::deny("rejected in WINK GO"),
    })
}
...
mod tests {
    ...
    use winkgo_ai_agent::WorkerTaskManagerImpl;
    use winkgo_ai_agent::task_manager::AgentFactory;
    use winkgo_api_types::{AntigravityHookDecision, AntigravityHookToolCall};
