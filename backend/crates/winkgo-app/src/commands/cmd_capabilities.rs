...
fn data() -> Value {
    json!({
        "schema_version": 1,
        "contract": "agent-facing-winkgo_core-cli",
        "stability": "stable",
        "entrypoint": "winkgo_core capabilities",
        "purpose": "Top-level index for agent-facing WinkGoCore CLI domains.",
        "output": {
        ...
        "runtime_context": {
            "primary": "WINKGO_CONVERSATION_ID",
            "environment": RUNTIME_ENV,
            "selectors": {
                "conversation_id": {
                    "current": "resolve from WINKGO_CONVERSATION_ID"
                },
                "assistant_id": {
                    "current": "resolve via current conversation"
                },
                "user_id": {
                    "current": "resolve from WINKGO_USER_ID"
                }
            }
        },
        "input": {
        ...
        "domains": [
            {
                "name": "config",
                "mode": "read-write",
                "description": "Manage WinkGo configuration: assistants, assistant rules, skills, MCP servers, providers, settings, agents, and scheduled tasks.",
                "contract": "agent-facing-config-cli",
                "contract_command": "config capabilities",
                "invocation": "winkgo_core config capabilities",
                "runtime_required": ["WINKGO_BASE_URL", "WINKGO_CONVERSATION_ID", "WINKGO_USER_ID"],
                "safety": {
                    "can_write": true,
            ...
            },
            {
                "name": "diagnose",
                "mode": "read-only",
                "description": "Diagnose a running WinkGo installation: backend health, conversations, provider health, MCP, cron, teams, logs, and controlled GET reads.",
                "contract": "agent-facing-diagnose-cli",
                "contract_command": "diagnose capabilities",
                "invocation": "winkgo_core diagnose capabilities",
                "runtime_required": ["WINKGO_BASE_URL", "WINKGO_CONVERSATION_ID", "WINKGO_USER_ID"],
                "optional_runtime": ["WINKGO_LOG_DIR"],
                "safety": {
                    "can_write": false,
                    "read_only": true,
                    "redacted_by_default": true,
            ...
            },
            {
                ...
                "invocation": "winkgo_core team capabilities",
                "runtime_required": ["WINKGO_BASE_URL", "WINKGO_CONVERSATION_ID", "WINKGO_USER_ID", "WINKGO_RUNTIME_TOKEN"],
