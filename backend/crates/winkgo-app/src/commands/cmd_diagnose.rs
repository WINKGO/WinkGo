...
const ENV_BASE_URL: &str = "WINKGO_BASE_URL";
const ENV_CONVERSATION_ID: &str = "WINKGO_CONVERSATION_ID";
const ENV_USER_ID: &str = "WINKGO_USER_ID";
const ENV_LOG_DIR: &str = "WINKGO_LOG_DIR";
    ...
    let log_dir = optional_string_field(&payload, "log_dir")
        ...
        .ok_or_else(|| {
            DiagnoseError::new(
                ...
                command,
                "missing log directory; set WINKGO_LOG_DIR or pass log_dir in stdin JSON",
            )
...
async fn request_json(
    ...
    let response = client
        ...
        .header("x-winkgo-conversation-id", &env.conversation_id)
        .header("x-winkgo-user-id", &env.user_id)
        .send()
        .await
        .map_err(|_| {
            DiagnoseError::new(
    ...
    let text = response.text().await.map_err(|_| {
        DiagnoseError::new(
            ...
            command,
            "failed to read WinkGo backend response",
        )
        .field("path", path)
    })?;
    ...
    if !status.is_success() {
        return Err(DiagnoseError::new(
            ...
            command,
            "WinkGo backend returned an error status",
        )
        .field("path", path)
    ...
    let value: Value = serde_json::from_str(&text).map_err(|_| {
        DiagnoseError::new(
            ...
            command,
            "WinkGo backend returned invalid JSON",
        )
        .field("path", path)
    })?;
...
fn extract_api_data(value: Value, command: &str) -> Result<Value, DiagnoseError> {
    ...
    Err(DiagnoseError::new(
        DiagnoseErrorCode::HttpStatusError,
        command,
        "WinkGo backend returned an unsuccessful response",
    ))
}
...
fn tail_latest_log(
    ...
    collect_winkgo_core_logs(log_dir, 0, &mut files).map_err(|_| {
        DiagnoseError::new(
    ...
    let latest = files
        ...
        .ok_or_else(|| {
            DiagnoseError::new(
                ...
                command,
                "no *.winkgo_core.log files found",
            )
            .field("path", log_dir.display().to_string())
        })?;
...
fn collect_winkgo_core_logs(dir: &Path, depth: usize, out: &mut Vec<PathBuf>) -> io::Result<()> {
    ...
    for entry in std::fs::read_dir(dir)? {
        ...
        let path = entry.path();
        if path.is_dir() {
            collect_winkgo_core_logs(&path, depth + 1, out)?;
        } else if path
            .file_name()
            .and_then(|name| name.to_str())
            .is_some_and(|name| name.ends_with(".winkgo_core.log"))
        {
            out.push(path);
        }
    }
...
mod tests {
    ...
    fn diagnose_error_renders_quoted_stable_contract() {
        ...
        assert_eq!(
            error.stderr_line(),
            "DIAGNOSE_ENV_MISSING command=\"diagnose context\" field=\"WINKGO_CONVERSATION_ID\": missing required environment variable"
        );
    }
