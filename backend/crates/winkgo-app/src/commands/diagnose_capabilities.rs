...
//! Agent-readable capability contract for `winkgo_core diagnose`.

use serde_json::{Value, json};

const RUNTIME_ENV: [&str; 3] = ["WINKGO_BASE_URL", "WINKGO_CONVERSATION_ID", "WINKGO_USER_ID"];
...
pub(crate) fn data() -> Value {
    json!({
        ...
        "runtime_context": {
            ...
            "optional_environment": ["WINKGO_LOG_DIR"]
        },
        "safety": {
            "read_only": true,
            "redacted_by_default": [
        ...
        "domains": [
            ...
            domain("logs", &[
                command(CommandDescriptor {
                    path: &["logs", "tail"],
                    description: "Tail winkgo_core logs from WINKGO_LOG_DIR or stdin log_dir.",
                    input: "stdin_json",
