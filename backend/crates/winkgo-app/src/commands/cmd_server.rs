...
use winkgo_api_types::{RuntimeStatusScope, RuntimeStatusScopeKind};
use winkgo_app::{AppConfig, AppServices, RouterBuildError, create_router_with_runtime};
use winkgo_system::RuntimePrepareService;
use winkgo_team::TeamIdleCleanupCoordinator;

use crate::bootstrap::{BootstrapError, BootstrapErrorCode, ParentExitSignal, ServerEnvironment};

const LISTENING_EVENT_PREFIX: &str = "WINKGO_CORE_LISTENING";
...
pub(crate) async fn run_server(
    ...
    // 10 min / 30 min / 60 s and are overridable via WINKGO_IDLE_TIMEOUT_SECS,
    // WINKGO_TEAM_IDLE_TIMEOUT_SECS, and WINKGO_IDLE_SCAN_INTERVAL_SECS. The
    ...
    let idle_cleanup_coordinator: Arc<dyn winkgo_ai_agent::IdleCleanupCoordinator> =
        Arc::new(TeamIdleCleanupCoordinator::new(
            router_runtime.team_service.clone(),
            services.active_lease_registry.clone(),
        ));
    let (solo_timeout_secs, team_timeout_secs, scan_interval_secs) = winkgo_ai_agent::resolve_idle_config_from_env();
    let idle_scanner_handle = winkgo_ai_agent::start_idle_scanner_with_coordinator(
...
mod tests {
    ...
    fn listening_event_line_is_machine_readable() {
        ...
        let payload = line
            .strip_prefix("WINKGO_CORE_LISTENING ")
