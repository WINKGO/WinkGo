...
pub fn init_environment(cli: &Cli, merged_path: &str) -> Result<ServerEnvironment, BootstrapError> {
    ...
    unsafe {
        std::env::set_var("WINKGO_WORK_DIR", &work_dir);
    }
...
pub async fn init_data_layer(config: &AppConfig) -> Result<Database, BootstrapError> {
    ...
    winkgo_db::maybe_copy_legacy_database(&db_path).map_err(|e| {
        BootstrapError::new(
            BootstrapErrorCode::DataInitFailed,
            "data.legacy_db",
            "failed to initialize application data",
    ...
    let database = winkgo_db::init_database_staged_with_options(
        &db_path,
        winkgo_db::DatabaseInitOptions {
...
mod tests {
    ...
    fn database_stage_comes_from_db_boundary_error() {
        let err = winkgo_db::DatabaseInitError::new(
            "database.migration",
            winkgo_db::DbError::Migration(sqlx::migrate::MigrateError::VersionMismatch(42)),
        );
    ...
    fn database_schema_repair_stage_comes_from_db_boundary_error() {
        let err = winkgo_db::DatabaseInitError::new(
            ...
            winkgo_db::DbError::Init("repair failed".into()),
        );
    ...
    fn database_recoverable_corruption_stage_comes_from_db_boundary_error() {
        let err = winkgo_db::DatabaseInitError::new(
            ...
            winkgo_db::DbError::Migration(sqlx::migrate::MigrateError::ExecuteMigration(
