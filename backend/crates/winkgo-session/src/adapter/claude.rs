...
use winkgo_process::{BoxedStdin, ProcessError, Spawner};
...
/// leak), mirroring the ACP/winkgo_agent per-kind id convention. `Other` has no
        ...
        for b in &blocks {
            match b.get("type").and_then(Value::as_str).unwrap_or("") {
                ...
                "tool_use" => {
                    ...
                    // #486 (parity with winkgo_agent output_sink): DROP a malformed empty-name
...
impl BackendAdapter for ClaudeAdapter {
    async fn start_turn(
        ...
        let proc = spawner.spawn(spec, &[], "winkgo-session").await?;
...
async fn write_ndjson_line(stdin: &mut BoxedStdin, value: &Value) -> Result<(), ProcessError> {
    ...
    bytes.push(b'\n');
    // DIAGNOSTIC (env-gated, default OFF via WINKGO_CLAUDE_WIRE_DUMP): log the RAW
    ...
    if std::env::var("WINKGO_CLAUDE_WIRE_DUMP").is_ok_and(|v| v != "0" && !v.is_empty()) {
...
mod tests {
    ...
    async fn captured_frame_with_id(content: &[crate::ContentBlock], client_msg_id: Option<&str>) -> String {
        ...
        let mut stdin: winkgo_process::BoxedStdin = Box::new(CaptureStdin(buf.clone()));
        ClaudeAdapter::new()
    ...
    /// persists as a ghost `tool_step{name:""}` row. Mirrors winkgo_agent's empty-name
