...
pub enum SessionEvent {
    ...
    // (No clock/timeout lower: WinkGoCore does NOT auto-time-out a turn. A user may
    ...
    Detached {
        ...
        /// the 14-keyword allowlist in `winkgo_common::error_extract` survive,
    ...
    Permission {
        ...
        /// `{ "server_name": "winkgo-team"?, "options": [{"option_id","kind"}] }`.
    ...
    /// fields Option — only claude fills them; codex/ACP/winkgo_agent never emit this.
    SubagentDetail {
        r#ref: String,
        parent_ref: Option<String>,
        label: Option<String>,
    ...
    /// projects it to a live tool-output pane. Claude/ACP/winkgo_agent never emit it today.
...
mod additive_tests {
    ...
    fn additive_defaults_keep_old_frames_deserializable() {
        ...
        let new_p = r#"{"Permission":{"request_id":"r","kind":"Tool","metadata":{"server_name":"winkgo-team"}}}"#;
