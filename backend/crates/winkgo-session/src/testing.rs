...
use winkgo_process::{BoxedStdin, BoxedStdout, ManagedProcess, ProcessError, Spawner};
...
/// `winkgo-winkgo_agent` crate's own invariant test (winkgo_agent depends on session, so it
/// cannot live in session's `tests/` and be reached from winkgo_agent — it lives here,
