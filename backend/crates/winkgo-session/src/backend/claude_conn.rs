...
/// Pure `serde_json`, no ACP SDK — `winkgo-session` stays SDK-free.
...
/// DIAGNOSTIC (env-gated, default OFF): when `WINKGO_CLAUDE_WIRE_DUMP` is set, log
...
fn claude_wire_dump_enabled() -> bool {
    std::env::var("WINKGO_CLAUDE_WIRE_DUMP").is_ok_and(|v| v != "0" && !v.is_empty())
}
...
fn dump_wire(direction: &str, session_id: &str, turn_gen: u64, bytes: &[u8]) {
    ...
    tracing::info!(
        target: "winkgo_session::claude_wire",
        direction,
                        ...
                        match tokio::time::timeout(std::time::Duration::from_millis(200), stdout.read(&mut chunk)).await {
                            ...
                            Ok(Ok(n)) if n > 0 => {
                                ...
                                match parsed {
                                    ...
                                    Err(_) => {
                                        tracing::error!(
                                            target: "winkgo_session::backend::claude_conn",
                                            conversation_id = %session_id,
...
mod tests {
    ...
    async fn f4_turn_in_flight_blocks_idle_suspend() {
        ...
        let suspended = backend
            .suspend
            .suspend_if_idle(winkgo_common::now_ms() + 10_000, true)
            .await;
