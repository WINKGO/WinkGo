...
/// (winkgo-agent-rest/src/protocol/acp.rs:55, an `oneshot` + `tokio::time::timeout`)
...
/// Restore the legacy 30s budget (env-overridable via `WINKGO_HANDSHAKE_TIMEOUT_SECS`
...
pub(crate) fn handshake_budget() -> std::time::Duration {
    let secs = std::env::var("WINKGO_HANDSHAKE_TIMEOUT_SECS")
        .ok()
...
/// → `session/new`|`session/load` `mcpServers[]`; codex → `thread/start`; winkgo_agent →
/// its `McpServerConfig` map). Deliberately crate-local — `winkgo-session` stays
/// self-contained (no `winkgo-api-types`/ACP-SDK dep, §Cargo.toml). The app
...
pub struct SessionInit {
    ...
    /// Skill ids/names to surface to the agent on the first turn (acp/winkgo_agent
    ...
    /// The field is carried for the winkgo_agent/acp first-prompt delivery path only.
    pub skills: Vec<String>,
    /// Composed system prompt / preset context (the `compose_preset_context`
    /// output + winkgo_agent `preset_rules`-merged prompt). Delivered first-message.
    pub preset_context: Option<String>,
    /// Opaque persisted session snapshot for resume (winkgo_agent `SessionManager` JSON
    /// blob). Kept as `serde_json::Value` so `winkgo-session` need not depend on
    /// the `winkgo-agent` `Session` type; the winkgo_agent backend deserializes it.
    pub session_snapshot: Option<serde_json::Value>,
    /// Explicit resume flag. acp/codex resume via `SessionSpec::Resume`; this is the
    /// winkgo_agent equivalent + the single source of truth for "sanitize on resume".
    pub resume: bool,
}
