...
/// Do not let a slow account/model probe hold the first prompt indefinitely.
/// A normal cold `agy models` finishes in roughly three seconds. If it has not
/// completed by this deadline, the turn still starts, but without forwarding a
/// possibly stale model id from another Agent.
const FIRST_TURN_MODEL_PROBE_WAIT: std::time::Duration = std::time::Duration::from_secs(6);
...
pub fn antigravity_modes() -> Vec<ModeInfo> {
    // `yolo` is NOT one of agy's modes — it is WINK GO's sentinel for "run
...
impl BackendConnection for AntigravityConnection {
    async fn open_session(
        ...
        let backend = Arc::new(AntigravitySessionBackend {
            ...
            model_probe_complete: Arc::new(AtomicBool::new(false)),
            model_probe_notify: Arc::new(Notify::new()),
...
pub struct AntigravitySessionBackend {
    ...
    /// Separates "the probe has not answered yet" from "agy answered with no
    /// models". The first prompt waits briefly for the former, but never treats
    /// the latter as permission to pass an unrelated model id through.
    model_probe_complete: Arc<AtomicBool>,
    model_probe_notify: Arc<Notify>,
    /// Model chosen at runtime, overriding the create-time seed for the next
    /// per-turn agy process.
    ...
    current: Arc<Mutex<Option<Arc<winkgo_process::ManagedProcess>>>>,
...
impl AntigravitySessionBackend {
    ...
    fn spawn_model_probe(self: &Arc<Self>) {
        ...
        let complete = Arc::clone(&self.model_probe_complete);
        let notify = Arc::clone(&self.model_probe_notify);
    ...
    fn effective_model(&self) -> Option<String> {
        let requested = self.requested_model()?;
    ...
    /// Resolve the spawn-time model after giving the asynchronous catalog probe
    /// a short head start. Without this gate, a prompt sent immediately after
    /// opening the session can beat `agy models` and forward a model persisted
    /// from Codex/Gemini. agy then spends several seconds before rejecting the
    /// whole turn as an invalid model selection.
    async fn effective_model_for_turn(&self) -> Option<String> {
        self.requested_model()?;

        if self.models.read().is_ok_and(|models| !models.is_empty()) {
            return self.effective_model();
        }

        if !self.model_probe_complete.load(Ordering::Acquire) {
            // Create the waiter before the second completion check so a probe
            // finishing between the two cannot leave us sleeping on a missed
            // notification.
            let notified = self.model_probe_notify.notified();
            if !self.model_probe_complete.load(Ordering::Acquire) {
                let _ = tokio::time::timeout(FIRST_TURN_MODEL_PROBE_WAIT, notified).await;
            }
        }
        ...
        let requested = self.requested_model().unwrap_or_default();
        tracing::warn!(
            session_id = %self.session_id,
            requested = %requested,
            probe_complete = self.model_probe_complete.load(Ordering::Acquire),
            "antigravity: model catalog unavailable before turn; omitting the stale model and using agy's default"
        );
        None
    }
    ...
    async fn start_turn(&self, content: Vec<ContentBlock>) -> Result<u64, BackendError> {
        ...
        let model = self.effective_model_for_turn().await;
        let input = ArgvInput {
    ...
    fn spawn_reader(&self, proc: Arc<winkgo_process::ManagedProcess>, turn_gen: u64) {
...
impl SessionBackend for AntigravitySessionBackend {
    async fn dispatch(&self, command: Command) -> Result<CommandReceipt, BackendError> {
        match command {
            ...
            Command::SetModel { ref model } => {
                if let Ok(mut guard) = self.model_override.write() {
    ...
    fn capabilities(&self) -> Capabilities {
        let available_models = self.models.read().map(|m| m.clone()).unwrap_or_default();
        let current_model = self
            .requested_model()
            .filter(|requested| available_models.is_empty() || available_models.iter().any(|m| m.id == *requested));
        Capabilities {
            available_models,
...
mod tests {
    ...
    async fn backend_for_permissions() -> Arc<AntigravitySessionBackend> {
        ...
        Arc::new(AntigravitySessionBackend {
            session_id: "conv-1".into(),
            models: Arc::new(std::sync::RwLock::new(Vec::new())),
            model_probe_complete: Arc::new(AtomicBool::new(true)),
            model_probe_notify: Arc::new(Notify::new()),
            model_override: Arc::new(std::sync::RwLock::new(None)),
    ...
    #[tokio::test]
    async fn a_runtime_model_switch_reaches_the_next_turn() {
        let b = backend_with_models(
            &["gemini-3.6-flash-low", "claude-sonnet-4-6"],
            Some("gemini-3.6-flash-low"),
        );

        b.dispatch(Command::SetModel {
            model: "claude-sonnet-4-6".into(),
        })
        .await
        .expect("model switch accepted");

        assert_eq!(b.effective_model_for_turn().await.as_deref(), Some("claude-sonnet-4-6"));
        assert_eq!(b.capabilities().current_model.as_deref(), Some("claude-sonnet-4-6"));
    }
    ...
    fn modes_are_agys_three_plus_the_full_auto_sentinel() {
        // agy's own axis is default / accept-edits / plan. `yolo` is WINK GO's
    ...
    fn an_empty_model_list_means_unknown_not_invalid() {
        // The synchronous helper cannot tell whether discovery has finished;
        // the async turn resolver below owns that distinction.
        let b = backend_with_models(&[], Some("gemini-3.1-pro-low"));
        assert_eq!(b.effective_model().as_deref(), Some("gemini-3.1-pro-low"));
    }

    #[tokio::test]
    async fn a_completed_empty_probe_does_not_forward_a_stale_cross_agent_model() {
        let b = backend_with_models(&[], Some("gpt-5.5"));

        assert_eq!(b.effective_model_for_turn().await, None);
    }

    #[tokio::test]
    async fn the_first_turn_waits_for_model_discovery_before_validating() {
        let b = Arc::new(backend_with_models(&[], Some("claude-sonnet-4-6")));
        b.model_probe_complete.store(false, Ordering::Release);
        let updater = Arc::clone(&b);
        tokio::spawn(async move {
            tokio::task::yield_now().await;
            updater.models.write().unwrap().push(ModelInfo {
                id: "claude-sonnet-4-6".into(),
                name: "Claude Sonnet 4.6".into(),
                description: None,
                reasoning_efforts: Vec::new(),
            });
            updater.model_probe_complete.store(true, Ordering::Release);
            updater.model_probe_notify.notify_waiters();
        });
