...
/// WINK GO's sentinel mode id meaning "run without approval prompts".
///
...
pub(crate) fn build_argv(input: &ArgvInput) -> Vec<String> {
    ...
    // "allow" cannot override that (hooks can tighten, not loosen). WINK GO
    ...
    // `yolo` is WINK GO's sentinel for "no approval prompts", not one of agy's
...
mod tests {
    ...
    fn the_full_auto_sentinel_is_never_sent_as_agys_mode() {
        // `yolo` is WINK GO's marker, not one of agy's modes. agy accepts unknown
    ...
    fn permission_gate_is_opened_for_the_hook_bridge() {
        ...
        // "allow" cannot override that. WINK GO opens the gate here and makes its
