...
async fn read_to_end(stdout: winkgo_process::BoxedStdout) -> String {
...
mod tests {
    ...
    fn parses_one_id_per_line() {
        // Real `agy models` output from agy 1.1.10 (2026-08-05).
    ...
    async fn probing_closes_stdin_so_agy_can_exit() {
        ...
        let spawner: Arc<dyn Spawner> = Arc::new(winkgo_process::RealSpawner::new(
            Arc::new(winkgo_process::FileRegistryStore::new(tmp.path())),
            uuid::Uuid::now_v7(),
            "test-machine",
        ));
