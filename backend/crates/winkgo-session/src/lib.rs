...
//!
//! ## What this crate is
//! It spawns the claude CLI directly (via 001 `winkgo-process`, no ACP/SDK),
...
//! `winkgo-conversation` + WebSocket — all Non-Goal here, untouched. This crate
...
//!
//! ## Layering
//! Domain layer, parallel to `winkgo-ai-agent`. Depends ONLY on
//! `winkgo-process` (001) + `winkgo-common`. NO production caller this
...
//!   acp/winkgo_agent impls in `backend/`) — the ONLY backend-aware code: startup+prompt
