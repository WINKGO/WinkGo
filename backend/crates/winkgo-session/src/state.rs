...
pub struct RequiresActionSet {
    ...
    /// Pending structured questions (`Ask` +1 / `AskResolved` -1).
...
/// codex fills only label, and ACP/winkgo_agent have no workflow concept (empty roster).
...
///       queue affordance (MX-QUEUE-3). ACP/winkgo_agent lack the path → degrade to false.
///
/// Orthogonal to `can_send` (Idle): `can_send || can_queue` is the input-box gate.
/// Truth table (× backend `accepts_proactive_input`):
/// | state                              | claude(true) | codex/acp/winkgo_agent(false) |
/// |------------------------------------|--------------|-------------------------|
...
/// with a Workflow and was WRONG. Re-measured on claude 2.1.176 (WinkGoCore's exact
...
mod tests {
    ...
    fn can_queue_truth_table_capability_gated() {
        ...
        // codex/acp/winkgo_agent (accepts_proactive_input=false): degrades to false in
