...
//! still can, via the `WINKGO_BUILTIN_ASSISTANTS_PATH` env var → disk
//! fallback path.
...
/// `crates/winkgo-app/assets/builtin-assistants/`.
static BUILTIN_ASSETS: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/../winkgo-app/assets/builtin-assistants");
...
/// `WINKGO_BUILTIN_ASSISTANTS_PATH` at a fixture directory.
enum Source {
    Embedded,
    Disk(PathBuf),
}
...
impl BuiltinAssistantRegistry {
    ...
    ///
    /// If `WINKGO_BUILTIN_ASSISTANTS_PATH` is set and points to a readable
    /// directory, read from disk (test-only override). Otherwise use the
    /// assets embedded at compile time.
    pub fn load() -> Self {
        if let Ok(env) = std::env::var("WINKGO_BUILTIN_ASSISTANTS_PATH") {
            let p = PathBuf::from(env);
            if p.exists() {
                return Self::load_from_dir(p);
            }
            warn!(
                "WINKGO_BUILTIN_ASSISTANTS_PATH points to missing directory; \
    ...
    /// `WINKGO_BUILTIN_ASSISTANTS_PATH` E2E fixtures — the three
...
mod tests {
    ...
    // WINKGO_BUILTIN_ASSISTANTS_PATH). Graceful-degradation semantics must
    // stay intact.
    // -----------------------------------------------------------------------
    ...
    fn load_respects_env_var_disk_override() {
        ...
        let key = "WINKGO_BUILTIN_ASSISTANTS_PATH";
