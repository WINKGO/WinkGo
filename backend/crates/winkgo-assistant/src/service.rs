...
use winkgo_extension::{AssistantClassifier, AssistantRuleDispatcher, ExtensionError};
...
fn assistant_error_is_busy(error: &AssistantError) -> bool {
    matches!(error, AssistantError::Internal(message) if winkgo_db::message_indicates_busy(message))
}
...
fn assistant_error_is_unique(error: &AssistantError) -> bool {
    match error {
        ...
        AssistantError::Internal(message) => winkgo_db::message_indicates_unique_violation(message),
        _ => false,
    }
}
...
pub struct AssistantService {
    ...
    /// Defaults to `~/.winkgo/` but can be overridden for tests.
...
impl AssistantService {
    ...
    /// directory.
    ///
    /// There is no implicit `~/.winkgo` fallback on purpose: an earlier
    /// version had one, and dev builds silently wrote rule files to the
    /// release directory while the db lived under `~/.winkgo-dev/`,
    ...
    ///    Bedrock, Vertex, …), return `"winkgo_agent"`. WinkGoRS speaks both
    ...
    pub async fn resolve_default_agent_id(&self) -> Result<String, AssistantError> {
        ...
        if providers.iter().any(|p| p.enabled) {
            self.resolve_agent_id_for_agent_ref("winkgo_agent").await
        } else {
    ...
    pub async fn import(&self, req: ImportAssistantsRequest) -> Result<ImportAssistantsResult, AssistantError> {
        ...
        for entry in req.assistants {
            ...
            match self.repo.create(&params).await {
                ...
                Err(winkgo_db::DbError::Conflict(_)) => {
    ...
    ///   override when `WINKGO_BUILTIN_ASSISTANTS_PATH` is set).
...
impl AssistantService {
    ...
    fn definition_to_detail_response(
        ...
        definition: &AssistantDefinitionRow,
        state: Option<&AssistantOverlayRow>,
        preference: Option<&winkgo_db::AssistantPreferenceRow>,
        rules_content: &str,
        projection: &AssistantRuntimeProjection,
...
fn assistant_projection_for_definition(
    ...
    // agents, but winkgo_agent (the built-in Rust agent) has a NULL `backend` and is
    // keyed by its `agent_type` ("winkgo_agent") instead. Match on either so winkgo_agent
    // assistants resolve to the winkgo_agent row rather than falling back to Missing.
...
mod tests {
    ...
    struct Fixture {
        ...
        override_repo: Arc<dyn IAssistantOverrideRepository>,
        provider_repo: Arc<dyn IProviderRepository>,
        agent_rows: Arc<Mutex<Vec<winkgo_api_types::AgentManagementRow>>>,
        _tmp: TempDir,
        _db: winkgo_db::Database,
    }

    #[derive(Clone, Default)]
    struct StubAgentCatalog {
        rows: Arc<Mutex<Vec<winkgo_api_types::AgentManagementRow>>>,
    }
    ...
    /// `resolve_default_agent_type` returns `"winkgo_agent"`. Tests that need to
    ...
    struct FixtureOpts {
        ...
        /// Defaults to `"openai"` so existing tests get an `"winkgo_agent"`
        /// default agent type.
        seed_platform: Option<&'static str>,
        agent_rows: Vec<winkgo_api_types::AgentManagementRow>,
    }
    ...
    fn mk_agent_row(
        id: &str,
        backend: &str,
        status: winkgo_api_types::AgentManagementStatus,
    ) -> winkgo_api_types::AgentManagementRow {
        winkgo_api_types::AgentManagementRow {
            id: id.into(),
            ...
            agent_source: winkgo_api_types::AgentSource::Builtin,
            ...
            behavior_policy: winkgo_api_types::BehaviorPolicy {
                supports_team: true,
                ..Default::default()
            },
            yolo_id: None,
            ...
            team_capable: true,
            status,
            last_check_status: Some(winkgo_api_types::AgentSnapshotCheckStatus::Online),
            last_check_kind: Some(winkgo_api_types::AgentSnapshotCheckKind::Manual),
    ...
    fn mk_uninstalled_agent_row(id: &str, backend: &str) -> winkgo_api_types::AgentManagementRow {
        let mut row = mk_agent_row(id, backend, winkgo_api_types::AgentManagementStatus::Unchecked);
        row.installed = false;
    ...
    async fn bootstrap_skips_dirty_generated_assistant_definitions() {
        let fx = fixture().await;
        {
            ...
            *rows = vec![
                mk_agent_row("agent-dirty", "dirty", winkgo_api_types::AgentManagementStatus::Online),
                mk_agent_row("agent-valid", "valid", winkgo_api_types::AgentManagementStatus::Online),
            ];
        }
    ...
    async fn builtin_listing_uses_manifest_default_enabled_and_sort_order() {
        ...
        let mut butler = mk_builtin("winkgo-assistant", "Butler");
        ...
        let butler_resp = list.iter().find(|a| a.id == "winkgo-assistant").unwrap();
        assert!(butler_resp.enabled, "butler defaults to enabled");
        assert_eq!(butler_resp.sort_order, 0);
        let butler_idx = list.iter().position(|a| a.id == "winkgo-assistant").unwrap();
    ...
    async fn list_maps_generated_definition_to_generated_source() {
        ...
        fx.agent_rows
            ...
            .push(mk_agent_row(
                "agent-claude",
                "claude",
                winkgo_api_types::AgentManagementStatus::Online,
            ));
        fx.definition_repo
            .upsert(&UpsertAssistantDefinitionParams {
    ...
    async fn bootstrap_materializes_generated_assistant_from_available_agent() {
        ...
        assert_eq!(bare.agent_status, winkgo_api_types::AgentManagementStatus::Online);
    ...
    async fn bootstrap_materializes_generated_assistant_from_unchecked_agent() {
        let mut unchecked_row = mk_agent_row(
            "agent-cursor",
            "cursor",
            winkgo_api_types::AgentManagementStatus::Unchecked,
        );
        ...
        assert_eq!(bare.agent_status, winkgo_api_types::AgentManagementStatus::Unchecked);
        assert!(bare.team_selectable);
    ...
    async fn list_hides_existing_generated_assistant_when_agent_is_uninstalled_until_installed() {
        ...
        uninstalled_row.status = winkgo_api_types::AgentManagementStatus::Offline;
        let fx = fixture_with_options(FixtureOpts {
...

        {
            ...
            rows[0].status = winkgo_api_types::AgentManagementStatus::Unchecked;
        }
    ...
    async fn bootstrap_materializes_generated_assistant_from_available_custom_agent() {
        ...
        custom_row.agent_source = winkgo_api_types::AgentSource::Custom;
    ...
    async fn bootstrap_falls_back_to_agent_type_when_backend_is_empty() {
        // Engines like WinkGo CLI carry their identity in `agent_type` and leave
        ...
        agent_row.agent_type = winkgo_common::AgentType::WinkGoAgent;
        ...
        let bare = list
            .iter()
            .find(|assistant| assistant.id == "bare:agent-winkgo_agent")
            .unwrap();
        assert_eq!(bare.agent_id, "agent-winkgo_agent");
    }

    #[tokio::test]
    async fn winkgo_agent_assistant_resolves_agent_status_via_agent_type_not_backend() {
        // Regression: an assistant whose engine is winkgo_agent must match the winkgo_agent
        // agent row by `agent_type` ("winkgo_agent"), since that row's `backend` is
        // NULL. Matching on `backend` alone left the row unresolved and
        // mislabelled every winkgo_agent assistant as Missing/unavailable.
        let mut winkgo_agent_row = mk_agent_row(
            "agent-winkgo_agent",
            "winkgo_agent",
            winkgo_api_types::AgentManagementStatus::Online,
        );
        winkgo_agent_row.backend = None;
        winkgo_agent_row.agent_type = winkgo_common::AgentType::WinkGoAgent;

        let mut builtin = mk_builtin("builtin-winkgo_agent", "WinkGo Assistant");
        builtin.agent_ref = "winkgo_agent".into();

        let fx = fixture_with_options(FixtureOpts {
            builtins: vec![builtin],
            agent_rows: vec![winkgo_agent_row],
            ..Default::default()
        })
        .await;
        ...
        let assistant = list
            .iter()
            .find(|assistant| assistant.id == "builtin-winkgo_agent")
            .expect("winkgo_agent builtin assistant should be listed");
        assert_eq!(
            assistant.agent_status,
            winkgo_api_types::AgentManagementStatus::Online,
            "winkgo_agent assistant should resolve to the online winkgo_agent agent row, not Missing"
        );
    }
    ...
    async fn bootstrap_places_new_generated_assistants_before_existing_assistants() {
        let fx = fixture_with_options(FixtureOpts {
            ...
            agent_rows: vec![
                ...
                mk_agent_row("agent-codex", "codex", winkgo_api_types::AgentManagementStatus::Online),
            ],
            ..Default::default()
        })
        .await;
    ...
    async fn bootstrap_reactivates_soft_deleted_builtin_definition_by_source_ref() {
        let mut builtin = mk_builtin("winkgo-assistant", "WinkGo Butler");
        builtin.rule_file = Some("rules/winkgo-assistant.{locale}.md".into());
        let fx = fixture_with_builtins(vec![builtin]).await;

        let original = fx
            .definition_repo
            .get_by_assistant_id("winkgo-assistant")
            .await
            .unwrap()
        ...
        assert_eq!(restored.rule_resource_ref.as_deref(), Some("winkgo-assistant"));
    ...
    /// Anthropic provider routes to WinkGoRS, not the Claude Code CLI:
    /// having an Anthropic API key does not imply the user has
    /// `claude` on `PATH`. CLI-based agents must be opted into
    /// explicitly.
    #[tokio::test]
    async fn resolve_default_agent_id_routes_anthropic_provider_to_winkgo_agent() {
        let fx = fixture_with_options(FixtureOpts {
    ...
    /// OpenAI / custom provider falls back to WinkGoRS, the only WinkGo
    /// agent that doesn't require a third-party CLI.
    #[tokio::test]
    async fn resolve_default_agent_id_falls_back_to_winkgo_agent_for_openai_provider() {
        let fx = fixture_with_options(FixtureOpts {
    ...
    /// Custom (non-anthropic, non-openai) platform also routes to WinkGoRS,
    /// which handles OpenAI-compatible custom URLs.
    #[tokio::test]
    async fn resolve_default_agent_id_handles_custom_platform_as_winkgo_agent() {
        let fx = fixture_with_options(FixtureOpts {
    ...
    /// or otherwise — should resolve to `"winkgo_agent"`, the only built-in
    ...
    async fn create_without_agent_id_does_not_default_to_gemini_when_provider_exists() {
        for platform in ["anthropic", "openai"] {
            ...
            assert_eq!(
                ...
                "{platform} provider should resolve to winkgo_agent"
            );
        }
    }
