...
use winkgo_system::ClientPrefService;
...
///
/// Key mismatch fix: the WinkGo frontend stores the config under
...
pub(crate) async fn load_stt_config(
...
) -> Result<SpeechToTextConfig, winkgo_system::SystemError> {
    ...
    Ok(prefs
        ...
        .unwrap_or(SpeechToTextConfig {
            enabled: false,
            provider: winkgo_api_types::SpeechToTextProvider::Openai,
            auto_send: None,
            openai: None,
            deepgram: None,
        }))
...
mod tests {
    ...
    fn make_state() -> ShellRouterState {
        ...
        let repo = Arc::new(winkgo_db::SqliteClientPreferenceRepository::new(pool));
        let client_pref_service = winkgo_system::ClientPrefService::new(repo);
