...
- Add a model only when the provider's own documentation confirms image input on the API protocol used by WinkGoCore.
...
- Keep an empty `models` array when the preset endpoint is known but no stable model ID can be positively verified for that endpoint. This still records the WinkGo preset without claiming image support.
- Aggregator entries may be refreshed manually from that aggregator's own catalog. The reviewed result must be committed as a static snapshot; WinkGoCore never fetches it at runtime.

Poe bot names and Ctyun deployment model IDs are account- or deployment-specific, so they intentionally have no static model entries. DeepSeek does not currently expose a positively verified image-input chat model on the corresponding WinkGo preset endpoint.
...
- WinkGo provider presets: https://github.com/xuweihafeichangniu-lab/WinkGo/blob/main/packages/desktop/src/renderer/utils/model/modelPlatforms.ts
