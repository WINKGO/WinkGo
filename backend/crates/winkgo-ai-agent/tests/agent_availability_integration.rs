...
use winkgo_ai_agent::{AgentRegistry, AgentService};
use winkgo_api_types::{AgentManagementStatus, AgentSnapshotCheckKind, AgentSnapshotCheckStatus};
use winkgo_db::{
...
async fn management_rows_derive_missing_available_and_unavailable_statuses() {
    ...
    repo.upsert(&custom_params(
        "agent-missing",
        "Missing Agent",
        "winkgo-missing-agent-binary",
        r#"{"binary_name":"winkgo-missing-agent-binary"}"#,
    ))
    .await
    .unwrap();
    repo.upsert(&custom_params(
...
async fn manual_health_check_does_not_refresh_unrelated_agents() {
    ...
    repo.upsert(&custom_params(
        ...
        "winkgo-definitely-missing-health-check-target",
        r#"{"binary_name":"winkgo-definitely-missing-health-check-target"}"#,
    ))
    .await
    .unwrap();
...
async fn custom_enabled_toggle_does_not_refresh_unrelated_agents() {
    ...
    repo.upsert(&custom_params(
        "agent-target-toggle",
        "Target Toggle Agent",
        "winkgo-target-toggle-command",
        r#"{"binary_name":"winkgo-target-toggle-command"}"#,
    ))
    .await
    .unwrap();
...
async fn custom_delete_does_not_refresh_unrelated_agents() {
    ...
    repo.upsert(&custom_params(
        "agent-target-delete",
        "Target Delete Agent",
        "winkgo-target-delete-command",
        r#"{"binary_name":"winkgo-target-delete-command"}"#,
    ))
    .await
    .unwrap();
