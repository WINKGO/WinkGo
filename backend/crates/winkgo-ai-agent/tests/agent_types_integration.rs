...
//! - WinkGoAgent stub returns appropriate errors
...
use winkgo_ai_agent::manager::winkgo_agent::WinkGoAgentAgentManager;
use winkgo_ai_agent::task_manager::AgentFactory;
use winkgo_ai_agent::types::{BuildTaskOptions, SendMessageData, WinkGoAgentResolvedConfig};
use winkgo_ai_agent::*;
use winkgo_ai_agent::{SkillIndex, build_system_instructions_with_skills_index};
use winkgo_common::{AgentKillReason, AgentType, ConversationStatus, ProviderWithModel, TimestampMs, now_ms};
...
// ---------------------------------------------------------------------------
// WinkGoAgent agent tests (real implementation with AgentEngine)
// ---------------------------------------------------------------------------

fn make_winkgo_agent_config() -> WinkGoAgentResolvedConfig {
    WinkGoAgentResolvedConfig {
        ...
        session_directory: std::env::temp_dir().join("winkgo_agent-test-sessions"),
        session_mode: None,
        skills: Vec::new(),
...
async fn winkgo_agent_agent_kill_succeeds() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/proj".into(), make_winkgo_agent_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn winkgo_agent_agent_confirm_succeeds() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/proj".into(), make_winkgo_agent_config(), None)
        .await
        .unwrap();
    // `confirm` is an inherent method on `WinkGoAgentAgentManager` (reached via
    // `AgentInstance::WinkGoAgent(..)` in production); the test calls it
...
#[tokio::test]
async fn winkgo_agent_agent_metadata() {
    let agent = WinkGoAgentAgentManager::new("conv-abc".into(), "/work".into(), make_winkgo_agent_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn collect_idle_ignores_winkgo_agent_agent_type() {
    ...
    let make_opts = |agent_type: AgentType, id: &str| {
        let kind = match agent_type {
            ...
            AgentType::WinkGoAgent => AgentSessionKind::WinkGoAgent(Box::new(WinkGoAgentSessionBuildContext {
                config: Default::default(),
                team: None,
                belongs_to_team: false,
            })),
    ...
    mgr.get_or_build_task("acp-1", make_opts(AgentType::Acp, "acp-1"))
        .await
        .unwrap();
    mgr.get_or_build_task("winkgo_agent-1", make_opts(AgentType::WinkGoAgent, "winkgo_agent-1"))
        .await
        .unwrap();
