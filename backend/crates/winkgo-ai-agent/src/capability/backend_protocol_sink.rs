...
use tracing::debug;
use winkgo_agent_protocol::events::{ProtocolEvent, ToolCategory};
use winkgo_agent_protocol::writer::ProtocolEmitter;
use winkgo_common::{Confirmation, ConfirmationOption, generate_id};

use crate::protocol::events::{AcpPermissionEventData, AgentStreamEvent, ToolCallEventData, ToolCallStatus};

/// Implements `ProtocolEmitter` for the winkgo_core context.
///
/// Bridges winkgo_agent `ProtocolEvent` emissions to `AgentStreamEvent` on a
...
mod tests {
    use super::*;
    use winkgo_agent_protocol::events::ToolInfo;
