    ...
    pub fn apply_conversation_runtime_context(
        ...
        self.context.runtime_env.retain(|(key, _)| {
            !matches!(
                key.as_str(),
                WINKGO_USER_ID_ENV
                    | WINKGO_CONVERSATION_ID_ENV
                    | WINKGO_HELPER_BIN_ENV
                    | WINKGO_BASE_URL_ENV
                    | WINKGO_RUNTIME_TOKEN_ENV
            )
        });
        self.context
            .runtime_env
            .push((WINKGO_USER_ID_ENV.to_owned(), user_id.to_owned()));
        self.context
            .runtime_env
            .push((WINKGO_CONVERSATION_ID_ENV.to_owned(), conversation_id.to_owned()));
        if let Some(helper_bin) = helper_bin {
            self.context
                .runtime_env
                .push((WINKGO_HELPER_BIN_ENV.to_owned(), helper_bin.to_owned()));
        }
        if let Some(base_url) = base_url {
            self.context
                .runtime_env
                .push((WINKGO_BASE_URL_ENV.to_owned(), base_url.to_owned()));
        }
        if let Some(runtime_token) = runtime_token {
            self.context
                .runtime_env
                .push((WINKGO_RUNTIME_TOKEN_ENV.to_owned(), runtime_token.to_owned()));
        }
        self.runtime_capabilities.conversation_runtime_context_version = Some(CONVERSATION_RUNTIME_CONTEXT_VERSION);
    }
}

pub const WINKGO_USER_ID_ENV: &str = "WINKGO_USER_ID";
pub const WINKGO_CONVERSATION_ID_ENV: &str = "WINKGO_CONVERSATION_ID";
pub const WINKGO_HELPER_BIN_ENV: &str = "WINKGO_HELPER_BIN";
pub const WINKGO_BASE_URL_ENV: &str = "WINKGO_BASE_URL";
pub const WINKGO_RUNTIME_TOKEN_ENV: &str = "WINKGO_RUNTIME_TOKEN";
...
pub struct WinkGoAgentCompatOverrides {
...
/// Fully resolved WinkGoAgent configuration passed to the agent manager.
#[derive(Debug, Clone)]
pub struct WinkGoAgentResolvedConfig {
    ...
    /// Normal WinkGo conversations leave this unset.
    ...
    pub compat_overrides: WinkGoAgentCompatOverrides,
    /// Directory for winkgo_agent session persistence files.
    ...
    pub extra_mcp_servers: HashMap<String, winkgo_agent_config::config::McpServerConfig>,
    /// AWS Bedrock credentials (region + access key or profile).
    pub bedrock_config: Option<winkgo_agent_config::config::BedrockConfig>,
...
mod tests {
    use super::*;
    use serde_json::json;
    use winkgo_api_types::{AcpBuildExtra, AcpModelInfo, SlashCommandItem, WinkGoAgentBuildExtra};
    ...
    fn build_task_options_applies_conversation_runtime_context_once() {
        ...
        use winkgo_common::{AgentType, ProviderWithModel};
        ...
        let context = AgentSessionContext {
            ...
            skills: vec![],
            runtime_env: vec![
                (WINKGO_USER_ID_ENV.into(), "old-user".into()),
                (WINKGO_CONVERSATION_ID_ENV.into(), "old-conv".into()),
                (WINKGO_RUNTIME_TOKEN_ENV.into(), "old-token".into()),
        ...
        options.apply_conversation_runtime_context(
            "user-1",
            "conv-1",
            Some("/Applications/WinkGo/winkgo_core"),
        ...
        assert_eq!(
            options
                ...
                .runtime_env
                .iter()
                .filter(|(key, _)| key == WINKGO_USER_ID_ENV)
                .count(),
            1
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(WINKGO_USER_ID_ENV.to_owned(), "user-1".to_owned()))
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(WINKGO_CONVERSATION_ID_ENV.to_owned(), "conv-1".to_owned()))
        );
        ...
        assert!(
            options
                .context
                .runtime_env
                .contains(&(WINKGO_BASE_URL_ENV.to_owned(), "http://127.0.0.1:25808".to_owned()))
        );
        assert!(
            options
                .context
                .runtime_env
                .contains(&(WINKGO_RUNTIME_TOKEN_ENV.to_owned(), "runtime-token-1".to_owned()))
        );
        assert_eq!(
            options
                .context
                .runtime_env
                .iter()
                .filter(|(key, _)| key == WINKGO_RUNTIME_TOKEN_ENV)
                .count(),
            1
        );
    ...
    #[test]
    fn winkgo_agent_build_extra_serde_defaults() {
        let json = json!({});
        let extra: WinkGoAgentBuildExtra = serde_json::from_value(json).unwrap();
    ...
    #[test]
    fn winkgo_agent_build_extra_serde_with_overrides() {
        let json = json!({
    ...
    #[test]
    fn winkgo_agent_build_extra_serde_with_preset_rules() {
        let json = json!({
    ...
    #[test]
    fn winkgo_agent_build_extra_accepts_frozen_skills_snapshot() {
        let json = json!({
            "preset_rules": "Rules",
            "skills": ["pdf", "cron"]
        });
