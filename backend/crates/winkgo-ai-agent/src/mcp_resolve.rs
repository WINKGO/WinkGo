...
//! `Vec<agent_client_protocol::McpServer>`, `factory::winkgo_agent::load_user_mcp_servers`
//! → `HashMap<String, McpServerConfig>`) emit SDK/engine-specific types. This
//! module emits the NEUTRAL `winkgo_api_types::SessionMcpServer` so the app
//! boundary (`winkgo-app`) can convert it once into the crate-local
//! `winkgo_session::McpServerSpec`, and each backend serializes that into its own
...
use winkgo_runtime::ensure_runtime_command;
