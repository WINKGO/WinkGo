...
//! `winkgo-conversation::ConversationService`, which dispatches through
...
use winkgo_api_types::{AgentLogoEntry, AgentManagementRow, ProviderHealthCheckRequest, ProviderHealthCheckResponse};
use winkgo_db::IProviderRepository;
...
impl AgentService {
    ...
    pub async fn list_agent_logos(&self) -> Result<Vec<AgentLogoEntry>, AgentError> {
        ...
        for agent in self.registry.list_all_including_hidden().await {
            ...
            // for backends without a vendor label (e.g. winkgo_agent, where `backend`
    ...
    pub async fn set_agent_overrides(
        &self,
        id: &str,
        req: winkgo_api_types::SetAgentOverridesRequest,
        ...
        if (command_override.is_some() || has_env_override) && is_internal_winkgo_cli_row(&row) {
            return Err(AgentError::bad_request(
                "Internal WinkGo CLI does not support overrides",
            ));
        }
    ...
    pub async fn get_agent_overrides(&self, id: &str) -> Result<winkgo_api_types::AgentOverridesResponse, AgentError> {
        let row = self
            .registry
            .repo_handle()
            .get(id)
        ...
        let env_override = row
            .env_override
            .as_deref()
            .and_then(|s| serde_json::from_str::<Vec<winkgo_api_types::AgentEnvEntry>>(s).ok())
            .unwrap_or_default();

        Ok(winkgo_api_types::AgentOverridesResponse {
            command_override: if is_internal_winkgo_cli_row(&row) {
                None
            } else {
...
fn is_bridge_launched_row(row: &winkgo_db::AgentMetadataRow) -> bool {
    let Some(raw) = row.agent_source_info.as_deref() else {
        return false;
    };
    let Ok(info) = serde_json::from_str::<winkgo_api_types::AgentSourceInfo>(raw) else {
        return false;
    };
...
fn is_internal_winkgo_cli_row(row: &winkgo_db::AgentMetadataRow) -> bool {
    row.agent_type.eq_ignore_ascii_case("winkgo_agent") && row.agent_source.eq_ignore_ascii_case("internal")
}
