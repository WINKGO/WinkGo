...
use winkgo_agent::output::null_sink::NullSink;
use winkgo_agent_config::config::{CliArgs, Config};
use winkgo_api_types::{
    HealthStatus, ProviderHealthCheckErrorKind, ProviderHealthCheckRequest, ProviderHealthCheckResponse,
};
use winkgo_db::{IProviderRepository, models::Provider};

use crate::factory::winkgo_agent::{
    map_winkgo_agent_provider, resolve_bedrock_config, resolve_model_compat_overrides,
    resolve_winkgo_agent_url_and_compat_with_mode,
};
...
impl ProviderHealthCheckService {
    ...
    fn resolve_probe_config(&self, row: &Provider, model_id: &str) -> Result<WinkGoAgentResolvedConfig, AgentError> {
        let api_key = winkgo_common::decrypt_string(&row.api_key_encrypted, &self.encryption_key)
        ...
        Ok(WinkGoAgentResolvedConfig {
            ...
            compat_overrides,
            session_directory: self.data_dir.join("winkgo_agent-health-check-sessions"),
            session_mode: None,
            skills: Vec::new(),
...
async fn run_probe(
    ...
    let mut engine = match build_probe_engine(config_extra).await {
        Ok(engine) => engine,
        Err(error) => {
            let message = format!("WinkGoAgent probe bootstrap failed: {error}");
...
async fn build_probe_engine(config_extra: WinkGoAgentResolvedConfig) -> Result<AgentEngine, AgentError> {
...
mod tests {
    ...
    use winkgo_common::encrypt_string;
    use winkgo_db::{CreateProviderParams, DbError, UpdateProviderParams};
    ...
    fn test_service() -> ProviderHealthCheckService {
        ProviderHealthCheckService {
            ...
            data_dir: PathBuf::from("/tmp/winkgo_core-provider-health-test"),
        }
    }
