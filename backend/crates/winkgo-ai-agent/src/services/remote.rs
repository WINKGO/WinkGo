...
use winkgo_common::{RemoteAgentAuthType, RemoteAgentProtocol, RemoteAgentStatus, decrypt_string, encrypt_string};
use winkgo_db::models::RemoteAgentRow;
use winkgo_db::{IRemoteAgentRepository, UpdateRemoteAgentParams};
...
impl RemoteAgentService {
    ...
    pub async fn create(&self, req: CreateRemoteAgentRequest) -> Result<RemoteAgentResponse, AgentError> {
        ...
        let row = self
            .repo
            .create(winkgo_db::CreateRemoteAgentParams {
                name: &req.name,
    ...
    pub async fn update(&self, id: &str, req: UpdateRemoteAgentRequest) -> Result<RemoteAgentResponse, AgentError> {
        ...
        let row = self.repo.update(id, params).await.map_err(|e| match e {
            winkgo_db::DbError::NotFound(msg) => AgentError::not_found(msg),
...
fn generate_device_keypair(encryption_key: &[u8; 32]) -> Result<(String, String, String), AgentError> {
    ...
    let device_id = winkgo_common::generate_prefixed_id("dev");
...
fn db_err(e: winkgo_db::DbError) -> AgentError {
