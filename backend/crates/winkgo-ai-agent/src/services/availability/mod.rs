...
use winkgo_db::{IProviderRepository, UpdateAgentAvailabilitySnapshotParams};
...
pub struct AgentAvailabilityService {
    ...
    // Used to decide winkgo_agent (built-in, no external CLI) availability: it is
...
async fn run_probe(
    ...
    let (status, error_code, error_message) = if meta.agent_source == AgentSource::Builtin
                ...
                let drift = (meta.backend.as_deref() == Some("antigravity"))
                    .then(|| {
                        success
                            .reported_version
                            .as_deref()
                            .and_then(winkgo_session::version_drift)
                    })
                    .flatten();
                match drift {
    ...
    } else if meta.agent_type == AgentType::WinkGoAgent {
        // winkgo_agent is the built-in Rust agent: there is no external CLI to probe,
        ...
        probe_winkgo_agent_provider_readiness(provider_repo).await
    } else {
...
fn explicit_probe_args(meta: &AgentMetadata) -> Result<Vec<String>, String> {
    if meta.agent_source == AgentSource::Builtin && meta.agent_source_info.bridge_binary.as_deref() == Some("npx") {
        ...
        return winkgo_runtime::pin_registry_npx_args(backend, &meta.args).map_err(|error| error.to_string());
    }
    Ok(meta.args.clone())
}

/// Readiness check for the built-in winkgo_agent agent.
///
/// winkgo_agent has no external CLI; it runs models through configured providers.
/// Mirrors `AssistantService::resolve_default_agent_type`, which treats winkgo_agent
...
async fn probe_winkgo_agent_provider_readiness(
    provider_repo: &Arc<dyn IProviderRepository>,
...
mod tests {
    ...
    use super::{AgentAvailabilityService, explicit_probe_args, probe_winkgo_agent_provider_readiness, run_probe};
    ...
    #[tokio::test]
    async fn winkgo_agent_is_offline_without_an_enabled_provider() {
        let db = init_database_memory().await.unwrap();
        let provider_repo: Arc<dyn IProviderRepository> = Arc::new(SqliteProviderRepository::new(db.pool().clone()));

        let (status, code, _msg) = probe_winkgo_agent_provider_readiness(&provider_repo).await;
    ...
    #[tokio::test]
    async fn winkgo_agent_is_online_when_a_provider_is_enabled() {
        let db = init_database_memory().await.unwrap();
        let provider_repo: Arc<dyn IProviderRepository> = Arc::new(SqliteProviderRepository::new(db.pool().clone()));
    ...
    async fn manual_check_restores_previously_offline_agent() {
        ...
        repo.update_availability_snapshot(
            ...
            &winkgo_db::UpdateAgentAvailabilitySnapshotParams {
