...
async fn assert_no_stop_signal(agent: &WinkGoAgentAgentManager) {
...
fn make_test_config() -> WinkGoAgentResolvedConfig {
    WinkGoAgentResolvedConfig {
        ...
        session_directory: env::temp_dir().join("winkgo_agent-test-sessions"),
        session_mode: None,
        skills: Vec::new(),
...
#[test]
fn resolve_winkgo_agent_config_discards_standalone_max_token_settings() {
    let project = tempfile::tempdir().unwrap();
    fs::write(
        project.path().join(".winkgo_agent.toml"),
        r#"
[default]
max_tokens = 1234
...
"#,
    ...
    let embedded = resolve_winkgo_agent_config(&cli_args).unwrap();
    assert_eq!(embedded.max_tokens, None);
    assert_eq!(embedded.compat.default_max_tokens_for_model("gpt-test"), None);
}

#[test]
fn resolve_winkgo_agent_config_keeps_builtin_provider_max_token_policy() {
    let project = tempfile::tempdir().unwrap();
    fs::write(
        project.path().join(".winkgo_agent.toml"),
        r#"
...
#[test]
fn winkgo_agent_final_input_dump_value_contains_raw_split_input_and_context() {
    ...
    let context = WinkGoAgentFinalInputDumpContext {
        ...
        skills: vec!["winkgo-config".to_owned()],
        mcp_servers,
        runtime_env: vec![("WINKGO_RAW".to_owned(), "raw-env-value".to_owned())],
    };
    let data = SendMessageData {
        content: "team wake raw content".to_owned(),
        msg_id: "msg-winkgo_agent-final".to_owned(),
        turn_id: Some("turn-winkgo_agent-final".to_owned()),
        files: Vec::new(),
        inject_skills: Vec::new(),
    };

    let value = build_winkgo_agent_final_input_dump_value("conv-winkgo_agent", "/workspace", &context, &data);

    assert_eq!(value["kind"], "winkgo_agent-final-input");
    assert_eq!(value["backend"], "winkgo_agent");
    assert_eq!(value["conversation_id"], "conv-winkgo_agent");
    assert_eq!(value["msg_id"], "msg-winkgo_agent-final");
    assert_eq!(value["turn_id"], "turn-winkgo_agent-final");
    ...
    assert_eq!(value["resolved_context"]["skills"][0], "winkgo-config");
    assert_eq!(
...
#[tokio::test]
async fn winkgo_agent_agent_returns_correct_type() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
    assert_eq!(agent.agent_type(), AgentType::WinkGoAgent);
    assert_eq!(agent.workspace(), "/project");
    assert_eq!(agent.conversation_id(), "conv-1");
}

#[tokio::test]
async fn winkgo_agent_agent_initial_status_is_pending() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn winkgo_agent_agent_subscribe_returns_receiver() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn winkgo_agent_agent_kill_succeeds() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn winkgo_agent_agent_kill_with_reason_succeeds() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn winkgo_agent_agent_kill_running_turn_sends_stop_signal() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn winkgo_agent_agent_kill_and_wait_waits_for_running_turn_terminal() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
    agent.runtime.reset_for_new_turn(ConversationStatus::Running);
...
#[tokio::test]
async fn winkgo_agent_agent_kill_idle_turn_does_not_leave_stale_stop_signal() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn winkgo_agent_agent_confirmations_initially_empty() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn winkgo_agent_agent_get_slash_commands_does_not_wait_for_engine_lock() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
...
#[tokio::test]
async fn winkgo_agent_agent_check_approval_returns_false_by_default() {
    let agent = WinkGoAgentAgentManager::new("conv-1".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
...
async fn stop_only_signals_in_flight_run() {
    let agent = WinkGoAgentAgentManager::new("conv-stop".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
...
async fn runtime_can_emit_error_and_finish() {
    let agent = WinkGoAgentAgentManager::new("conv-err".into(), "/project".into(), make_test_config(), None)
        .await
        .unwrap();
    let mut rx = agent.subscribe();
