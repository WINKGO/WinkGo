...
use winkgo_agent::bootstrap::AgentBootstrap;
use winkgo_agent::engine::AgentEngine;
use winkgo_agent::output::OutputSink;
use winkgo_agent::session::Session;
use winkgo_agent_config::compat::ProviderCompat;
use winkgo_agent_config::config::{CliArgs, Config, McpServerConfig, ProviderType};
use winkgo_agent_mcp::manager::McpManager;
use winkgo_agent_protocol::commands::{ApprovalScope, SessionMode};
use winkgo_agent_protocol::{ToolApprovalManager, ToolApprovalResult};
...
use winkgo_common::{AgentKillReason, AgentType, Confirmation, ConversationStatus, ErrorChain, TimestampMs, now_ms};
...
use crate::types::{SendMessageData, WinkGoAgentResolvedConfig};

use super::content::build_content_blocks;
use super::error::{winkgo_agent_engine_error_to_send_error, winkgo_agent_runtime_error_summary};

fn resolve_winkgo_agent_config(cli_args: &CliArgs) -> Result<Config, AgentError> {
    let mut config =
        Config::resolve(cli_args).map_err(|e| AgentError::internal(format!("Config resolve failed: {e}")))?;

    // WinkGo owns the embedded runtime policy. Standalone winkgo_agent max-token
...
#[derive(Clone, Debug)]
struct WinkGoAgentFinalInputDumpContext {
    dump_dir: PathBuf,
    provider: String,
    model: String,
...
fn build_winkgo_agent_final_input_dump_value(
    conversation_id: &str,
    workspace: &str,
    context: &WinkGoAgentFinalInputDumpContext,
    data: &SendMessageData,
) -> Value {
    serde_json::json!({
        "kind": "winkgo_agent-final-input",
...
pub struct WinkGoAgentAgentManager {
    ...
    final_input_dump: Option<WinkGoAgentFinalInputDumpContext>,
...
impl Drop for WinkGoAgentAgentManager {
    fn drop(&mut self) {
...
impl WinkGoAgentAgentManager {
    pub async fn new(
        conversation_id: String,
        workspace: String,
        config_extra: WinkGoAgentResolvedConfig,
        ...
        info!(
            ...
            "Resolved image input capability for WinkGoAgent model"
        );
        let final_input_dump = config_extra
            .prompt_dump_dir
            .clone()
            .map(|dump_dir| WinkGoAgentFinalInputDumpContext {
                dump_dir,
        ...
        let mut config = resolve_winkgo_agent_config(&cli_args)?;
        ...
        if let Some(mode_str) = &config_extra.session_mode {
            ...
            info!(
                ...
                "WinkGoAgent initial session mode applied"
            );
        }
    ...
    fn request_stop(&self, reason: Option<AgentKillReason>, operation: &'static str) -> bool {
        ...
        info!(
            ...
            was_running,
            operation,
            "WinkGoAgent stop signal requested"
        );

        was_running
    }

    fn dump_winkgo_agent_final_input(&self, data: &SendMessageData) {
        let Some(context) = self.final_input_dump.as_ref() else {
            return;
        };

        let value = build_winkgo_agent_final_input_dump_value(
...
impl IAgentTask for WinkGoAgentAgentManager {
    ...
    async fn send_message(&self, data: SendMessageData) -> Result<(), AgentSendError> {
        ...
        info!(
            ...
            msg_id = %data.msg_id,
            turn_id = data.turn_id.as_deref().unwrap_or("none"),
            "WinkGoAgent send_message started"
        );
        self.runtime.bump_activity();
        self.runtime.reset_for_new_turn(ConversationStatus::Running);
        self.dump_winkgo_agent_final_input(&data);

        // Keep attachment paths in the provider-independent history. Images
        // are loaded on demand by winkgo_agent's ViewImage tool.
        debug!(
            attachment_count = data.files.len(),
            "Building structured WinkGoAgent content blocks"
        );
        let content_blocks = build_content_blocks(&data.content, &data.files);
        debug!(
            block_count = content_blocks.len(),
            "Built structured WinkGoAgent content blocks"
        );
        ...
        let result = tokio::select! {
            ...
            _ = self.cancel_notify.notified() => {
                info!(
                    conversation_id = %self.runtime.conversation_id(),
                    "WinkGoAgent engine.run() cancelled by stop signal"
                );
        ...
        let send_result = match result {
            Some(Ok(_)) => {
                info!(
                    conversation_id = %self.runtime.conversation_id(),
                    elapsed_ms,
                    "WinkGoAgent engine.run() completed, emitting Finish"
                );
                self.runtime.emit_finish(None);
                Ok(())
            }
            Some(Err(e)) => {
                let summary = winkgo_agent_runtime_error_summary(&e);
                error!(
                    conversation_id = %self.runtime.conversation_id(),
                    elapsed_ms,
                    error = %ErrorChain(&e),
                    "WinkGoAgent engine.run() failed, emitting Error"
                );
                error!(
                    target: "winkgo_feedback_diagnostics",
                    diagnostic_event = "feedback.runtime.winkgo_agent_error",
                    conversation_id = %self.runtime.conversation_id(),
                    msg_id = %data.msg_id,
                    turn_id = data.turn_id.as_deref().unwrap_or("none"),
                    elapsed_ms,
                    ...
                    "feedback.runtime.winkgo_agent_error"
                );
                let send_error = winkgo_agent_engine_error_to_send_error(&e);
    ...
    pub fn kill_and_wait(&self, reason: Option<AgentKillReason>) -> Pin<Box<dyn Future<Output = ()> + Send>> {
        ...
        Box::pin(async move {
                ...
                .is_err()
            {
                warn!(
                    conversation_id,
                    "Timed out waiting for winkgo_agent turn to finish after kill"
                );
            }
        })
    }
...
/// WinkGoAgent-specific operations reached through `AgentInstance::WinkGoAgent(..)`
...
impl WinkGoAgentAgentManager {
    ...
    pub async fn set_mode(&self, mode: &str) -> Result<(), AgentError> {
        ...
        info!(
            ...
            from = prev,
            to = mode,
            "WinkGoAgent session mode switched"
        );
        Ok(())
    }
    ...
    pub async fn config_options(&self) -> Result<GetConfigOptionsResponse, AgentError> {
        Ok(GetConfigOptionsResponse {
            config_options: vec![winkgo_agent_mode_config_option(self.approval_manager.current_mode())],
        })
    }
    ...
    pub async fn set_config_option(&self, option_id: &str, value: &str) -> Result<SetConfigOptionResponse, AgentError> {
        ...
        if option_id != WINKGO_AGENT_MODE_OPTION_ID {
            return Err(AgentError::bad_request(format!(
                "Config option '{option_id}' is not available"
            )));
        }
        if !is_winkgo_agent_session_mode(value) {
            return Err(AgentError::bad_request(format!(
...
const WINKGO_AGENT_MODE_OPTION_ID: &str = "mode";

fn is_winkgo_agent_session_mode(s: &str) -> bool {
    matches!(s, "default" | "auto_edit" | "yolo")
}

fn winkgo_agent_mode_config_option(current_value: String) -> AcpConfigOptionDto {
    AcpConfigOptionDto {
        id: WINKGO_AGENT_MODE_OPTION_ID.to_owned(),
        ...
        options: vec![
            winkgo_agent_mode_select_option("default", "Default"),
            winkgo_agent_mode_select_option("auto_edit", "Auto Edit"),
            winkgo_agent_mode_select_option("yolo", "YOLO"),
        ],
    }
}

fn winkgo_agent_mode_select_option(value: &str, name: &str) -> AcpConfigSelectOptionDto {
