...
#[test]
fn winkgo_agent_structured_malformed_tool_call_error_is_provider_error() {
    let error = WinkGoAgentAgentError::ToolCallMalformed { count: 3, limit: 3 };
    let send_error = winkgo_agent_engine_error_to_send_error(&error);
...
#[test]
fn winkgo_agent_provider_rate_limited_appends_response_body_to_detail() {
    let error = WinkGoAgentAgentError::Provider(ProviderError::RateLimited {
        retry_after_ms: 5000,
...
#[test]
fn winkgo_agent_provider_rate_limited_without_body_falls_back_to_bare_detail() {
    let error = WinkGoAgentAgentError::Provider(ProviderError::RateLimited {
        retry_after_ms: 5000,
        body: None,
    });
...
#[test]
fn winkgo_agent_provider_rate_limited_ignores_whitespace_only_body() {
    let error = WinkGoAgentAgentError::Provider(ProviderError::RateLimited {
        retry_after_ms: 5000,
        body: Some("   \n\t  ".to_owned()),
    });
...
#[test]
fn winkgo_agent_provider_connection_error_is_user_llm_provider_error() {
    let error = WinkGoAgentAgentError::Provider(ProviderError::Connection(
...
fn provider_error_summary_classifies_network_without_body() {
    let error = WinkGoAgentAgentError::Provider(ProviderError::Connection("connect failed".into()));
    let summary = winkgo_agent_runtime_error_summary(&error);
...
fn tool_call_failure_summary_classifies_loop() {
    let error = WinkGoAgentAgentError::ToolCallFailures { count: 3, limit: 3 };
    let summary = winkgo_agent_runtime_error_summary(&error);
...
#[test]
fn winkgo_agent_api_connection_error_is_user_llm_provider_network_error() {
    let error = WinkGoAgentAgentError::Provider(ProviderError::Connection("error decoding response body".to_owned()));
    let send_error = winkgo_agent_engine_error_to_send_error(&error);
...
#[test]
fn winkgo_agent_provider_status_error_uses_status_instead_of_message_text() {
    let error = WinkGoAgentAgentError::Provider(ProviderError::Api {
        status: 401,
...
#[test]
fn winkgo_agent_context_too_long_is_provider_context_error() {
    let error = WinkGoAgentAgentError::ContextTooLong {
...
#[test]
fn winkgo_agent_repeated_malformed_tool_call_is_user_llm_provider_error() {
    let error = WinkGoAgentAgentError::ToolCallMalformed { count: 3, limit: 3 };
    let send_error = winkgo_agent_engine_error_to_send_error(&error);
...
#[test]
fn winkgo_agent_tool_call_failures_are_unknown_upstream_error() {
    let error = WinkGoAgentAgentError::ToolCallFailures { count: 3, limit: 3 };
    let send_error = winkgo_agent_engine_error_to_send_error(&error);
