        ...
        match err {
            AgentError::WorkspacePathRuntimeUnavailable(path) => Self {
                stream_error: AgentStreamErrorData {
                    ...
                    ownership: Some(AgentErrorOwnership::WinkGoui),
            ...
            AgentError::Internal(_) => Self::new(
                "WinkGo failed while sending the message",
                AgentErrorCode::WinkGouiInternalError,
                AgentErrorOwnership::WinkGoui,
                Some(detail),
                true,
                true,
                resolution(
            ...
            AgentError::Forbidden(_) => Self::new(
                "WinkGo blocked the request before it reached the Agent",
                AgentErrorCode::WinkGouiPermissionError,
                AgentErrorOwnership::WinkGoui,
                Some(detail),
                false,
                true,
    ...
    fn from_acp_non_internal(err: &AcpError, detail: String) -> Self {
        match err {
            ...
            AcpError::NotConnected => Self::new(
                "WinkGo lost its Agent protocol connection",
                AgentErrorCode::UserAgentDisconnected,
                AgentErrorOwnership::UserAgent,
                Some(detail),
                true,
            ...
            let classified = acp_agent_internal_texts(message, data.as_ref())
                ...
                .find_map(|text| {
                    ...
                    classify_agent_lifecycle(&lower)
                        ...
                        .or_else(|| classify_winkgo_state(&lower))
...
fn classify_winkgo_state(lower: &str) -> Option<ClassifiedError> {
    if lower.contains("conversation is already processing") {
        return Some(ClassifiedError {
            message: "The current response is still running",
            code: AgentErrorCode::WinkGouiConversationBusy,
            ownership: AgentErrorOwnership::WinkGoui,
            retryable: true,
            feedback_recommended: false,
...
mod tests {
    use super::*;
    use serde_json::json;
    use winkgo_api_types::{AgentErrorResolutionKind, AgentErrorResolutionTarget};
    ...
    #[test]
    fn preserves_runtime_workspace_validation_as_structured_winkgo_error() {
        let err =
        ...
        assert_eq!(err.ownership(), Some(AgentErrorOwnership::WinkGoui));
        assert_eq!(
    ...
    fn classifies_provider_504_html_body_as_timeout_with_stripped_detail() {
        let raw = "WinkGoAgent agent error: Provider error: API error 504: <html>\r\n<head><title>504 Gateway Time-out</title></head>\r\n<body>\r\n<center><h1>504 Gateway Time-out</h1></center>\r\n<hr><center>openresty</center>\r\n</body>\r\n</html>";
    ...
    fn classifies_agent_setup_failures() {
        ...
        for managed_binary_missing in [
            "expected managed Claude ACP platform binary missing: C:\\Users\\user\\AppData\\Roaming\\WinkGo\\winkgo\\runtime\\acp\\claude-agent-acp\\0.58.1\\win32-x64\\node_modules\\@anthropic-ai\\claude-agent-sdk-win32-x64\\claude.exe",
            "expected managed Codex ACP platform binary missing: C:\\Users\\user\\AppData\\Roaming\\WinkGo\\winkgo\\runtime\\acp\\codex-acp\\1.1.2\\win32-x64\\node_modules\\@openai\\codex-win32-x64\\vendor\\x86_64-pc-windows-msvc\\bin\\codex.exe",
        ] {
            assert_classification(
    ...
    fn classifies_provider_billing_auth_and_rate_limit() {
        assert_classification(
            "WinkGoAgent agent error: Provider error: API error 402: {\"error\":{\"message\":\"Insufficient Balance\"}}",
            AgentErrorCode::UserLlmProviderBillingRequired,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::CheckProviderBilling,
        );
        assert_classification(
            "WinkGoAgent agent error: Provider error: API error 400: {\"type\":\"error\",\"error\":{\"type\":\"invalid_request_error\",\"message\":\"Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits.\"}}",
            AgentErrorCode::UserLlmProviderBillingRequired,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::CheckProviderBilling,
        );
        assert_classification(
            "WinkGoAgent agent error: Provider error: API error 401: invalid x-api-key",
            AgentErrorCode::UserLlmProviderAuthFailed,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::CheckProviderCredentials,
        );
        assert_classification(
            "WinkGoAgent agent error: Provider error: Rate limited, retry after 5000ms",
            AgentErrorCode::UserLlmProviderRateLimited,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::Retry,
        );
    ...
    fn classifies_provider_request_model_and_context_errors() {
        ...
        assert_classification(
            "WinkGoAgent agent error: provider repeatedly returned malformed tool calls (3/3); stopped to avoid wasting tokens",
            AgentErrorCode::UserLlmProviderInvalidRequest,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::ChangeModel,
        );
    ...
    fn classifies_bedrock_invalid_model_identifier_as_model_not_found() {
        assert_classification(
            "WinkGoAgent agent error: Provider error: API error 400: {\"message\":\"The provided model identifier is invalid.\"}",
            AgentErrorCode::UserLlmProviderModelNotFound,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::ChangeModel,
        );
    ...
    fn classifies_provider_endpoint_network_timeout_and_empty_response() {
        ...
        assert_classification(
            "WinkGoAgent agent error: API error: Connection error: error decoding response body",
            AgentErrorCode::UserLlmProviderNetworkError,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::Retry,
        );
        assert_classification(
            "WinkGoAgent agent error: API error: error sending request for url",
            AgentErrorCode::UserLlmProviderNetworkError,
            AgentErrorOwnership::UserLlmProvider,
            AgentErrorResolutionKind::Retry,
        );
    ...
    fn classifies_bare_provider_404_as_endpoint_not_found() {
        let detail = "WinkGoAgent agent error: Provider error: API error 404: {\"detail\":\"Not Found\"}";
        assert_classification(
            detail,
            AgentErrorCode::UserLlmProviderEndpointNotFound,
            AgentErrorOwnership::UserLlmProvider,
    ...
    #[test]
    fn classifies_winkgo_conversation_busy_after_agent_and_provider_checks() {
        assert_classification(
            "Conflict: Conversation is already processing a message",
            AgentErrorCode::WinkGouiConversationBusy,
            AgentErrorOwnership::WinkGoui,
