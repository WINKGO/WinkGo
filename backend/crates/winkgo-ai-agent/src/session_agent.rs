...
use winkgo_common::{AgentKillReason, ConversationStatus, TimestampMs, now_ms};
use winkgo_session::{
...
fn resolve_current_model_efforts(models: &[winkgo_session::ModelInfo], current_model: Option<&str>) -> Vec<String> {
...
/// handshake, in the SAME `winkgo_session` shape the getters read off live
...
struct CatalogPreload {
    available_models: Vec<winkgo_session::ModelInfo>,
    current_model: Option<String>,
    available_modes: Vec<winkgo_session::ModeInfo>,
...
impl CatalogPreload {
    ...
    fn from_handshake(handshake: &winkgo_api_types::AgentHandshake) -> Self {
        ...
        let (available_models, current_model) = handshake
            ...
            .map(|state| {
                let models = state
                    .available_models
                    .iter()
                    .map(|m| winkgo_session::ModelInfo {
...
impl SessionAgentTask {
    ...
    pub fn new_with_preload(
        ...
        backend: Arc<dyn SessionBackend>,
        session_repo: Option<Arc<dyn IAcpSessionRepository>>,
        handshake: &winkgo_api_types::AgentHandshake,
        prompt_dump: Option<SessionPromptDump>,
    ) -> Arc<Self> {
        Self::build(
            agent_type,
    ...
    /// generic allow/deny.
    pub fn get_confirmations(&self) -> Vec<winkgo_common::Confirmation> {
        self.backend
    ...
    pub fn confirm(
        ...
        use winkgo_session::PermissionDecision;
    ...
    fn effective_catalog(
        ...
        Vec<winkgo_session::ModelInfo>,
        Option<String>,
        Vec<winkgo_session::ModeInfo>,
        Option<String>,
    ) {
    ...
    pub async fn mode(&self) -> Result<winkgo_api_types::AgentModeResponse, AgentError> {
        let caps = self.backend.capabilities();
        ...
        let current_mode = caps.current_mode.or_else(|| self.catalog_preload.current_mode.clone());
        Ok(winkgo_api_types::AgentModeResponse {
    ...
    pub async fn get_model(&self) -> Result<winkgo_api_types::GetModelInfoResponse, AgentError> {
        ...
        if models.is_empty() && current_model.is_none() && override_model.is_none() {
            return Ok(winkgo_api_types::GetModelInfoResponse { model_info: None });
        }
        let available_models: Vec<winkgo_api_types::ModelInfoEntry> = models
            .iter()
            .map(|m| winkgo_api_types::ModelInfoEntry {
                id: m.id.clone(),
                label: m.name.clone(),
            })
            .collect();
        ...
        Ok(winkgo_api_types::GetModelInfoResponse {
            model_info: Some(winkgo_api_types::ModelInfoPayload {
    ...
    pub async fn get_config_options(&self) -> Result<winkgo_api_types::GetConfigOptionsResponse, AgentError> {
        // Live catalog wins; cold-start resume falls back to the persisted-handshake
        ...
        if !modes.is_empty() {
            config_options.push(winkgo_api_types::AcpConfigOptionDto {
                id: "mode".into(),
                name: Some("Mode".into()),
                label: None,
                description: None,
                ...
                options: modes
                    .iter()
                    .map(|m| winkgo_api_types::AcpConfigSelectOptionDto {
                        value: m.id.clone(),
        ...
        // path's `thought_level` config option (category-keyed so WinkGo's
        ...
        if !efforts.is_empty() {
            config_options.push(winkgo_api_types::AcpConfigOptionDto {
                ...
                options: efforts
                    .iter()
                    .map(|e| winkgo_api_types::AcpConfigSelectOptionDto {
                        value: e.clone(),
                        name: Some(e.clone()),
                        label: None,
                        description: None,
        ...
        Ok(winkgo_api_types::GetConfigOptionsResponse { config_options })
    }
    ...
    pub async fn set_config_option(
        ...
        option_id: &str,
        value: &str,
    ) -> Result<winkgo_api_types::SetConfigOptionResponse, AgentError> {
        ...
        match option_id {
            ...
            _ => {
                return Ok(winkgo_api_types::SetConfigOptionResponse {
                    confirmation: winkgo_api_types::ConfigOptionConfirmation::CommandAck,
                    config_options: None,
                });
            }
        }
        ...
        let observed = snapshot
            ...
            == Some(value);
        Ok(winkgo_api_types::SetConfigOptionResponse {
            confirmation: if observed {
                winkgo_api_types::ConfigOptionConfirmation::Observed
            } else {
                winkgo_api_types::ConfigOptionConfirmation::CommandAck
            },
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<winkgo_api_types::SlashCommandItem>, AgentError> {
        let caps = self.backend.capabilities();
        Ok(caps
            .slash_commands
            .iter()
            .map(|c| winkgo_api_types::SlashCommandItem {
    ...
    async fn cancel(&self) -> Result<(), AgentError> {
        ...
        self.backend
            .dispatch(Command::Cancel {
                target: winkgo_session::CancelTarget::Turn,
            })
            .await
            .map(|_| ())
...
pub struct SessionBuildInputs<'a> {
    ...
    pub metadata: &'a winkgo_api_types::AgentMetadata,
    ...
    /// The conversation runtime context env (`WINKGO_USER_ID` /
    /// `WINKGO_CONVERSATION_ID` / `WINKGO_HELPER_BIN` / `WINKGO_BASE_URL` /
    /// `WINKGO_RUNTIME_TOKEN`, filled by `apply_conversation_runtime_context`).
...
fn spec_mode_model(
    ...
    session_snapshot: Option<&PersistedSessionState>,
    metadata: &winkgo_api_types::AgentMetadata,
) -> (winkgo_session::SessionSpec, Option<String>, Option<String>) {
    ...
    // transform the ACP path applies in `initial_mode_from_params`. WinkGo persists
...
/// assembly (`crates/winkgo-app/src/session_runtime/mod.rs`): it resolves the
    ...
    if backend_label == "claude" {
        ...
        if !provider_env.is_empty() {
            ...
            session_config.spawn_env.extend(
                provider_env
                    .into_iter()
                    .map(|(name, value)| winkgo_common::EnvVar { name, value }),
            );
...
///     path injected these via `resolve_agent_command_spec`; `WINKGO_*`/`PATH`/…
///     keys are already filtered at the registry (`is_blocked_override_env_key`),
///     so they cannot shadow the runtime context below.
///  2. the `WINKGO_*` conversation runtime context (`WINKGO_USER_ID` /
///     `WINKGO_CONVERSATION_ID` / `WINKGO_HELPER_BIN` / `WINKGO_BASE_URL` /
///     `WINKGO_RUNTIME_TOKEN`) — the legacy path appended these via
///     `apply_acp_launch_policy` for every agent spawn.
fn assemble_spawn_env(
    agent_env: &[winkgo_api_types::AgentEnvEntry],
    runtime_env: &[(String, String)],
) -> Vec<winkgo_common::EnvVar> {
    let mut env: Vec<winkgo_common::EnvVar> = agent_env
        .iter()
        ...
        .collect();
    env.extend(runtime_env.iter().map(|(name, value)| winkgo_common::EnvVar {
        name: name.clone(),
        value: value.clone(),
    }));
    env
...
fn build_session_cli_config_dump_value(backend: &str, cfg: &winkgo_session::SessionConfig) -> serde_json::Value {
    use winkgo_session::McpTransport;
...
fn session_server_to_spec(server: &winkgo_api_types::SessionMcpServer) -> winkgo_session::McpServerSpec {
    use winkgo_api_types::SessionMcpTransport as T;
    use winkgo_session::{McpServerSpec, McpTransport};
...
fn team_mcp_server_spec(cfg: &winkgo_api_types::TeamMcpStdioConfig) -> winkgo_session::McpServerSpec {
    use winkgo_api_types::TeamMcpStdioConfig as C;
    winkgo_session::McpServerSpec {
        name: winkgo_api_types::TEAM_MCP_SERVER_NAME.to_owned(),
...
/// going empty).
fn catalog_partial_from_caps(caps: &winkgo_session::Capabilities) -> Option<winkgo_api_types::AgentHandshake> {
    let mut config_options = Vec::new();
    ...
    Some(winkgo_api_types::AgentHandshake {
        config_options,
        available_modes,
        available_models,
        available_commands,
...
fn spawn_event_pump(
    ...
    tokio::spawn(async move {
        ...
        while let Some(env) = events.next().await {
            ...
            {
                let mut config_options: Vec<winkgo_api_types::AcpConfigOptionDto> = Vec::new();
                if !modes.is_empty() {
                    config_options.push(winkgo_api_types::AcpConfigOptionDto {
                        id: "mode".into(),
                        name: Some("Mode".into()),
            ...
            if let SessionEvent::SubagentUpdate { r#ref, status, .. } = &env.event {
                use winkgo_session::SubagentStatus;
                match status {
            ...
            let suppress_intermediate_finish = matches!(&env.event, SessionEvent::TurnResult { is_error, outcome, .. }
                if !workflow_inflight.is_empty()
                    && !*is_error
                    && !matches!(outcome, winkgo_session::TurnOutcome::Cancelled { .. }));
...
/// `Orchestrator` uses (`winkgo_session::is_unrecoverable_resume_error`), so a
...
fn is_dead_resume_anchor(event: &SessionEvent) -> bool {
    use winkgo_session::{TurnOutcome, is_unrecoverable_resume_error};
    ...
    let reason = winkgo_session::ErrorReason::Backend {
...
fn empty_turn_tip(outcome: &winkgo_session::TurnOutcome) -> Option<TipsEventData> {
    use winkgo_session::{StopReason, TruncationKind, TurnOutcome};
...
fn translate_event(event: SessionEvent, conversation_id: &str, terminal_result_seen: bool) -> Vec<AgentStreamEvent> {
    match event {
        ...
        SessionEvent::TurnResult {
            ...
            let is_cancel = matches!(outcome, winkgo_session::TurnOutcome::Cancelled { .. });
        ...
        SessionEvent::Detached { exit, redacted_summary } => {
            ...
            match winkgo_session::crash_outcome(terminal_result_seen, exit) {
                winkgo_session::Outcome::Crashed => {
                ...
                winkgo_session::Outcome::CleanNoResult | winkgo_session::Outcome::FollowResult => {
                    vec![AgentStreamEvent::Finish(FinishEventData::default())]
                }
            }
        }
            ...
            let entries: Vec<serde_json::Value> = entries
                ...
                .map(|e| {
                    let status = match e.status {
                        winkgo_session::PlanStatus::Pending => "pending",
                        winkgo_session::PlanStatus::InProgress => "in_progress",
                        winkgo_session::PlanStatus::Completed => "completed",
                    };
...
mod build_mapping_tests {
    ...
    fn assemble_spawn_env_orders_agent_overrides_before_runtime_context() {
        let agent_env = vec![
            winkgo_api_types::AgentEnvEntry {
        ...
        let runtime_env = vec![
            ("WINKGO_USER_ID".to_owned(), "user-1".to_owned()),
            ("WINKGO_RUNTIME_TOKEN".to_owned(), "tok".to_owned()),
        ];
        ...
        assert_eq!(
            names,
            ["AWS_PROFILE", "HTTPS_PROXY", "WINKGO_USER_ID", "WINKGO_RUNTIME_TOKEN"]
        );
    ...
    fn session_cli_config_dump_value_captures_resolved_surface() {
        use winkgo_session::{McpServerSpec, McpTransport, SessionConfig, SessionInit};
        let cfg = SessionConfig {
            ...
            spawn_env: vec![winkgo_common::EnvVar {
    ...
    fn test_metadata(backend: Option<&str>, yolo_id: Option<&str>) -> winkgo_api_types::AgentMetadata {
        use winkgo_api_types::{AgentHandshake, AgentMetadata, AgentSource, AgentSourceInfo, BehaviorPolicy};
        use winkgo_common::AgentType;
        AgentMetadata {
            id: "test".into(),
            icon: None,
    ...
    fn catalog_partial_projects_discovered_modes_models_commands() {
        use winkgo_session::{ModeInfo, ModelInfo, SlashCommandInfo};
        let caps = winkgo_session::Capabilities {
        ...
        let empty = winkgo_session::Capabilities::default();
        assert!(
...
mod translate_tests {
    use super::*;
    use crate::protocol::events::tool_call::{ToolCallEventData, ToolCallStatus};
    use winkgo_session::PermissionKind;
    ...
    fn plan_surfaces_with_snake_case_status() {
        use winkgo_session::{PlanEntry, PlanStatus};
        let events = translate_event(
            SessionEvent::Plan {
                entries: vec![
                    PlanEntry {
    ...
    fn cancelled_turn_emits_finish_without_error() {
        use winkgo_session::{CancelReason, TurnOutcome};
        let events = translate_event(
            SessionEvent::TurnResult {
                is_error: true,
                api_error_status: None,
    ...
    fn errored_turn_emits_rich_error_terminal() {
        use winkgo_session::{StopReason, TurnOutcome};
        let events = translate_event(
            SessionEvent::TurnResult {
                is_error: true,
    ...
    fn detached_crash_surfaces_as_rich_disconnect_error() {
        use winkgo_session::ExitStatusLite;
        let events = translate_event(
    ...
    fn tip_code(outcome: winkgo_session::TurnOutcome) -> Option<(TipType, String)> {
...
mod persist_tests {
    ...
    use super::*;
    use winkgo_db::{CreateAcpSessionParams, IAcpSessionRepository, SqliteAcpSessionRepository, init_database_memory};
    ...
    async fn seeded_repo() -> (Arc<dyn IAcpSessionRepository>, winkgo_db::Database) {
    ...
    async fn send_dispatch_session_not_found_clears_anchor_and_classifies() {
        ...
        assert_eq!(
            err.code(),
            Some(winkgo_api_types::AgentErrorCode::UserAgentSessionNotFound),
    ...
    async fn set_config_option_effort_returns_observed_via_override() {
        ...
        assert!(
            matches!(resp.confirmation, winkgo_api_types::ConfigOptionConfirmation::Observed),
    ...
    fn is_dead_resume_anchor_matrix() {
        use winkgo_session::{CancelReason, StopReason, TurnOutcome};
...
mod pump_tests {
    ...
    async fn config_change_emits_no_frame_and_bound_id_reaches_finish() {
        let script = vec![
            ...
            env(SessionEvent::TurnResult {
                ...
                epoch: 0,
                outcome: winkgo_session::TurnOutcome::EndTurn,
            }),
        ];
    ...
    async fn cancelled_turn_closes_open_tool_calls_as_canceled_before_finish() {
        use winkgo_session::{CancelReason, SubagentKind, TurnOutcome};
        let script = vec![
    ...
    async fn completed_tool_call_is_not_recanceled_at_turn_end() {
        use winkgo_session::SubagentKind;
        let script = vec![
            env(SessionEvent::TurnStarted { epoch: 1 }),
            env(SessionEvent::ToolCall {
                tool_use_id: "call-1".into(),
    ...
    async fn catalog_updated_projects_config_option_with_model_and_mode() {
        use winkgo_session::{ModeInfo, ModelInfo};
    ...
    async fn catalog_updated_projects_available_commands_frame() {
        use winkgo_session::SlashCommandInfo;
        let script = vec![env(SessionEvent::CatalogUpdated {
            models: Vec::new(),
            modes: Vec::new(),
            slash_commands: vec![
    ...
    async fn workflow_launch_result_finish_is_suppressed_until_workflow_completes() {
        ...
        let script = vec![
            env(SessionEvent::ToolCall {
                ...
                name: "Task".into(),
                subagent: winkgo_session::SubagentKind::Workflow,
                input: serde_json::Value::Null,
                parent_tool_use_id: None,
            }),
    ...
    struct PendingPermBackend(winkgo_session::PendingPermissionView);
    ...
    impl SessionBackend for PendingPermBackend {
        ...
        fn pending_permission_requests(&self) -> Vec<winkgo_session::PendingPermissionView> {
            vec![self.0.clone()]
        }
    }
    ...
    fn handshake_with_catalog() -> winkgo_api_types::AgentHandshake {
        winkgo_api_types::AgentHandshake {
    ...
    async fn live_capabilities_overwrite_stale_preload() {
        ...
        let stale = winkgo_api_types::AgentHandshake {
            available_models: Some(serde_json::json!({
