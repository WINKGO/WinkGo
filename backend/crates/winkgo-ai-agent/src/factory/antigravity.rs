...
pub(super) async fn build(
    ...
    let meta = crate::factory::acp::resolve_catalog_metadata(&deps.agent_registry, &config).await?;
    if config.agent_id.is_some() || config.backend.is_none() {
        config.backend.clone_from(&meta.backend);
    }

    // Register WINK GO as agy's PreToolUse gate for THIS workspace, and mint the
    ...
    let hook_env = match deps.antigravity_hook_base_url.as_deref() {
        ...
        Some(base_url) => {
            ...
            vec![
                (
                    winkgo_api_types::AntigravityHookConfig::ENV_BASE_URL.to_owned(),
                    base_url.to_owned(),
                ),
                (winkgo_api_types::AntigravityHookConfig::ENV_TOKEN.to_owned(), token),
                (
                    winkgo_api_types::AntigravityHookConfig::ENV_CONVERSATION_ID.to_owned(),
