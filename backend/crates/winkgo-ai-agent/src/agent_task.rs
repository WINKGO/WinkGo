...
pub trait IMockAgent: IAgentTask {
    ...
    async fn mode(&self) -> Result<winkgo_api_types::AgentModeResponse, AgentError> {
...
pub enum AgentInstance {
    ...
    WinkGoAgent(Arc<WinkGoAgentAgentManager>),
    /// clean-slate direct-CLI session model (claude/codex only). Wraps an
    /// `winkgo_session::SessionBackend` via [`SessionAgentTask`]. Every other
...
impl AgentInstance {
    ...
    pub fn kill_and_wait(
        ...
        match self {
            ...
            Self::WinkGoAgent(m) => m.kill_and_wait(reason),
    ...
    /// router. WinkGoAgent maintains inline confirmation lists.
    pub fn get_confirmations(&self) -> Vec<winkgo_common::Confirmation> {
        match self {
            Self::Acp(m) => m.get_confirmations(),
            Self::WinkGoAgent(m) => m.get_confirmations(),
    ...
    pub fn confirm(
        ...
        match self {
            ...
            Self::WinkGoAgent(m) => m.confirm(msg_id, call_id, data, always_allow),
    ...
    pub fn check_approval(&self, action: &str, command_type: Option<&str>) -> bool {
        match self {
            Self::Acp(_) => false,
            Self::WinkGoAgent(m) => m.check_approval(action, command_type),
            // Session (claude/codex) has no winkgo_agent-style auto-approve list.
    ...
    pub fn get_session_key(&self) -> Option<String> {
        match self {
            Self::Acp(_) | Self::WinkGoAgent(_) | Self::Session(_) => None,
            #[cfg(any(test, feature = "test-support"))]
    ...
    pub async fn get_mode(&self) -> Result<winkgo_api_types::AgentModeResponse, AgentError> {
        match self {
            Self::Acp(m) => m.mode().await,
            Self::WinkGoAgent(m) => m.mode().await,
    ...
    pub async fn get_model(&self) -> Result<GetModelInfoResponse, AgentError> {
        match self {
            ...
            Self::WinkGoAgent(_) => Ok(GetModelInfoResponse { model_info: None }),
    ...
    pub async fn get_config_options(&self) -> Result<GetConfigOptionsResponse, AgentError> {
        match self {
            ...
            Self::WinkGoAgent(m) => m.config_options().await,
    ...
    pub async fn set_config_option(&self, option_id: &str, value: &str) -> Result<SetConfigOptionResponse, AgentError> {
        ...
        match self {
            ...
            Self::WinkGoAgent(m) => m.set_config_option(option_id, value).await,
    ...
    /// [`winkgo_common::normalize_keys_to_snake_case`] so keys land as
    /// `used` / `size` / `cost` to match the WinkGo wire convention —
    ...
    pub async fn get_usage(&self) -> Result<Option<serde_json::Value>, AgentError> {
        match self {
            Self::Acp(m) => {
                ...
                winkgo_common::normalize_keys_to_snake_case(&mut value);
                Ok(Some(value))
            }
            Self::WinkGoAgent(_) => Ok(None),
    ...
    pub async fn get_slash_commands(&self) -> Result<Vec<SlashCommandItem>, AgentError> {
        match self {
            ...
            Self::WinkGoAgent(m) => m.get_slash_commands().await,
    ...
    pub async fn handle_side_question(&self, req: SideQuestionRequest) -> Result<SideQuestionResponse, AgentError> {
        ...
        match self {
            ...
            Self::WinkGoAgent(_) => Ok(SideQuestionResponse {
                status: "unsupported".into(),
                answer: None,
            }),
...
#[cfg(test)]
mod winkgo_agent_config_option_tests {
    use std::sync::Arc;

    use winkgo_api_types::ConfigOptionConfirmation;

    use super::*;
    use crate::types::WinkGoAgentResolvedConfig;
    ...
    fn make_test_config() -> WinkGoAgentResolvedConfig {
        WinkGoAgentResolvedConfig {
            ...
            session_directory: std::env::temp_dir().join("winkgo_agent-agent-task-test-sessions"),
            session_mode: None,
            skills: Vec::new(),
    ...
    async fn winkgo_agent_instance() -> AgentInstance {
        let manager = WinkGoAgentAgentManager::new(
            "conv-winkgo_agent-config".into(),
            "/project".into(),
            make_test_config(),
            None,
        )
        .await
        .expect("winkgo_agent manager should start in tests");
        AgentInstance::WinkGoAgent(Arc::new(manager))
    }

    #[tokio::test]
    async fn winkgo_agent_exposes_mode_as_config_option() {
        let instance = winkgo_agent_instance().await;
        ...
        let mode = response
            ...
            .expect("winkgo_agent should expose a mode config option");
    ...
    #[tokio::test]
    async fn winkgo_agent_set_config_option_mode_switches_session_mode() {
        let instance = winkgo_agent_instance().await;
    ...
    #[tokio::test]
    async fn winkgo_agent_set_config_option_rejects_invalid_mode() {
        let instance = winkgo_agent_instance().await;
    ...
    #[tokio::test]
    async fn winkgo_agent_set_config_option_rejects_unavailable_option() {
        let instance = winkgo_agent_instance().await;
