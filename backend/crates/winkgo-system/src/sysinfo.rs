...
/// Resolve the cache directory for WinkGo.
///
/// Priority: `WINKGO_CACHE_DIR` env → `dirs::cache_dir()/winkgo`.
fn resolve_cache_dir() -> String {
    if let Ok(v) = std::env::var("WINKGO_CACHE_DIR")
        && !v.is_empty()
    {
        return v;
    }
    dirs::cache_dir()
        .map(|p| p.join("winkgo").to_string_lossy().into_owned())
        .unwrap_or_default()
}

/// Resolve the work (data) directory for WinkGo.
///
/// Priority: `WINKGO_WORK_DIR` env → `dirs::data_dir()/winkgo`.
fn resolve_work_dir() -> String {
    if let Ok(v) = std::env::var("WINKGO_WORK_DIR")
        && !v.is_empty()
    {
        return v;
    }
...
/// Resolve the log directory for WinkGo.
///
/// Priority: `WINKGO_LOG_DIR` env →
///   macOS: `~/Library/Logs/winkgo`
///   Linux: `dirs::state_dir()/winkgo/logs` (XDG_STATE_HOME)
///   Windows: `dirs::data_dir()/winkgo/logs`
fn resolve_log_dir() -> String {
    if let Ok(v) = std::env::var("WINKGO_LOG_DIR")
        && !v.is_empty()
    {
        return v;
    }
    ...
    {
        return home.join("Library/Logs/winkgo").to_string_lossy().into_owned();
    }
    // Linux: XDG state dir
    if let Some(state) = dirs::state_dir() {
        return state.join("winkgo/logs").to_string_lossy().into_owned();
    }
    // Fallback: data_dir/winkgo/logs
    dirs::data_dir()
        .map(|p| p.join("winkgo/logs").to_string_lossy().into_owned())
        .unwrap_or_default()
}
...
mod tests {
    ...
    fn test_env_override_cache_dir() {
        ...
        // the default path contains "winkgo".
        let dir = resolve_cache_dir();
        assert!(dir.contains("winkgo"), "cache_dir should contain 'winkgo': {dir}");
    }

    #[test]
    fn test_env_override_work_dir() {
        let dir = resolve_work_dir();
        assert!(dir.contains("winkgo"), "work_dir should contain 'winkgo': {dir}");
    }

    #[test]
    fn test_env_override_log_dir() {
        let dir = resolve_log_dir();
        assert!(dir.contains("winkgo"), "log_dir should contain 'winkgo': {dir}");
    }
}
