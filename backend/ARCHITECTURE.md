...
It provides HTTP REST APIs and WebSocket real-time events for the WinkGo desktop client.
...
│  tion    │          │          │                 │
├──────────┴──────────┴──────────┴────────────────┤
│   winkgo-auth          winkgo-realtime           │
│  (JWT, CSRF, middleware) (WebSocket, events)     │
├─────────────────────────────────────────────────┤
│  winkgo-db    winkgo-api-types   winkgo-runtime  │
│ (repositories) (API contracts)  (runtime/process)│
├─────────────────────────────────────────────────┤
│       winkgo-common          winkgo-assets       │
│  (error types, enums, crypto)  (embedded data)   │
└─────────────────────────────────────────────────┘
```

Dependencies flow strictly downward. Domain crates must not depend on winkgo-app,
and winkgo-common has zero internal dependencies.
...
|-------|----------------|
| `winkgo-common` | Shared error types (ApiError), enums, ID generation, crypto utilities, timestamps, pagination |
| `winkgo-api-types` | All HTTP/WebSocket request and response types — the single source of truth for API contracts |
| `winkgo-db` | SQLite database layer, defines Repository traits and implementations |
| `winkgo-assets` | Embedded static assets (agent metadata, prompts) |
| `winkgo-runtime` | Managed Node, subprocess spawning, PATH enhancement |
...
| Crate | Responsibility |
|-------|----------------|
| `winkgo-auth` | JWT authentication, password hashing, CSRF protection, cookie management, auth middleware |
| `winkgo-realtime` | WebSocket connection management, event broadcasting (BroadcastEventBus), message routing |
...
| Crate | Responsibility |
|-------|----------------|
| `winkgo-conversation` | Conversation management, messaging, confirmations, streaming responses |
| `winkgo-channel` | Multi-channel integration (WeChat, DingTalk, Lark), plugin system, pairing sessions |
| `winkgo-team` | Team collaboration, task scheduling, mailbox system |
| `winkgo-cron` | Scheduled job execution, cron expressions, event triggering |
| `winkgo-file` | File operations, watching, snapshots, git operations, compression |
| `winkgo-office` | Office document handling (Excel, PPT, Word), preview, conversion |
| `winkgo-system` | System settings, provider management, version checking, model fetching |
| `winkgo-mcp` | MCP protocol integration, OAuth, multi-platform adapters |
| `winkgo-ai-agent` | Agent lifecycle management, worker task queues, ACP/auxiliary skills |
| `winkgo-extension` | Extension registry, hub management, skill discovery and installation |
| `winkgo-shell` | Shell command execution, speech-to-text |
| `winkgo-assistant` | Assistant configuration and management |

### Composition

| Crate | Responsibility |
|-------|----------------|
| `winkgo-app` | Top-level binary entry point, assembles all crates into the Axum server |
...
Every domain crate follows a consistent internal organization. Using winkgo-conversation as a reference:

### Standard Directory Structure

```
crates/winkgo-conversation/src/
...
All response types are defined in `winkgo-api-types` — the single source of truth for API contracts.
...
All database access goes through trait abstractions defined in `winkgo-db`:
...
|------|----------|---------|---------|
| Row models | `winkgo-db/src/models/` | Database row mapping | `ConversationRow` |
| Params objects | `winkgo-db/src/repository/` | Database write parameters | `UpdateConversationParams` |
| Request/response types | `winkgo-api-types` | API contracts and shared DTOs | `CreateConversationRequest`, `ConversationResponse` |

**The service layer may directly use types from `winkgo-api-types`.** This crate contains
pure data structure definitions with no HTTP framework dependencies, essentially serving as a shared DTO layer.

⚠️ **Critical constraint: `winkgo-api-types` must not depend on axum, tower, or any HTTP framework.
...
- **Repository (winkgo-db):** Pure database operations, no business logic
...
- Migration files are located in `crates/winkgo-db/migrations/`
...
DbError (database layer)
  ↓ From trait implementation (winkgo-db/src/error.rs)
...
`winkgo-app` defines `AppServices`, which holds all shared dependencies centrally:
...
- **Domain crates do not construct their own dependencies** — they only define what they need (RouterState), `winkgo-app` handles assembly
...
- Validity: 24 hours
- Payload: `user_id`, `username`, `iat`, `exp`, `iss` ("winkgo"), `aud` ("winkgo-webui")
- Secret source priority: environment variable → database → random generation (64 bytes, getrandom)
- Token extraction priority: `Authorization: Bearer` header → `winkgo-session` cookie
- Supports token blacklist (SHA-256 hash, DashMap storage)

### CSRF Protection

Uses the Double Submit Cookie pattern:
- Cookie name: `winkgo-csrf-token` (not HttpOnly — JavaScript must read it)
...
|--------|----------|--------|----------|---------|
| `winkgo-session` | ✅ | When HTTPS | Strict(HTTPS) / Lax(HTTP) | 30 days |
| `winkgo-csrf-token` | ❌ | When HTTPS | Strict(HTTPS) / Lax(HTTP) | 30 days |
...
| E2E tests | `crates/winkgo-app/tests/` | In-memory SQLite | Full HTTP request chain verification |
...
`winkgo-app/tests/common/mod.rs` provides shared test utilities:
...
**Step 3: If database access is needed, add to winkgo-db**
...
**Step 4: If API types are needed, add to winkgo-api-types**

Define request/response types in `winkgo-api-types` to keep API contracts centrally managed.

**Step 5: Wire into winkgo-app**

1. Add dependency in `winkgo-app/Cargo.toml`:
   ```toml
   winkgo-my-feature.workspace = true
   ```
...
- [ ] Repository trait defined in winkgo-db, implementation uses Sqlite prefix
- [ ] API types defined in winkgo-api-types
...
`crates/winkgo-runtime/src/node_runtime/`. Packaged builds activate Node
...
`fn main()` calls `winkgo_runtime::enhance_process_path()` **before** the
...
`winkgo_runtime::Builder::agent(program)` (for long-running agent CLIs
whose stdio the caller owns) or `winkgo_runtime::Builder::clean_cli(program)`
