...
function emitListening(child: ChildProcess, port: number): void {
  child.stdout?.emit('data', Buffer.from(`WINKGO_CORE_LISTENING {"host":"127.0.0.1","port":${port}}\n`));
}
...
describe('buildSpawnArgs', () => {
  ...
  it('passes prompt dump flag in development only when WINKGO_DUMP_PROMPTS is enabled', () => {
    const prev = process.env.WINKGO_DUMP_PROMPTS;
    process.env.WINKGO_DUMP_PROMPTS = '1';
    try {
      const args = buildSpawnArgs({
        port: 1,
        dbPath: '/d',
      ...
      if (prev === undefined) delete process.env.WINKGO_DUMP_PROMPTS;
      else process.env.WINKGO_DUMP_PROMPTS = prev;
    }
  });
  ...
  it('respects WINKGO_LOG_LEVEL override', () => {
    const prev = process.env.WINKGO_LOG_LEVEL;
    process.env.WINKGO_LOG_LEVEL = 'trace';
    try {
      const args = buildSpawnArgs({
        port: 1,
        dbPath: '/d',
      ...
      if (prev === undefined) delete process.env.WINKGO_LOG_LEVEL;
      else process.env.WINKGO_LOG_LEVEL = prev;
    }
  });
});

describe('buildSpawnEnv', () => {
  it('merges process.env with WINKGO_* dir vars', () => {
    const env = buildSpawnEnv({
      cacheDir: '/c',
      workDir: '/w',
      logDir: '/l',
    });
    expect(env.WINKGO_CACHE_DIR).toBe('/c');
    expect(env.WINKGO_WORK_DIR).toBe('/w');
    expect(env.WINKGO_LOG_DIR).toBe('/l');
...
describe('findAvailablePort', () => {
  ...
  it('skips ports blocked by Fetch so health checks can use the selected port', async () => {
    ...
    try {
      ...
      expect(infoSpy).toHaveBeenCalledWith('[winkgo_core] skipped fetch-blocked backend port 1720');
      expect(infoSpy).toHaveBeenCalledWith('[winkgo_core] selected backend port 40404 after 2 attempts');
    } finally {
      infoSpy.mockRestore();
    }
  });
...
describe('BackendLifecycleManager.start (success path)', () => {
  it('lets winkgo_core choose the backend port and waits for the reported listening event', async () => {
    ...
    child.stdout?.emit('data', Buffer.from('WINKGO_CORE_LISTENING {"host":"127.0.0.1","port":55555}\n'));
  ...
  it('spawns with correct args, waits for /health, reports running', async () => {
    ...
    const resolveBackend = vi.fn(() => '/abs/path/winkgo_core');
    ...
    try {
      ...
      expect(spawnCall[0]).toBe('/abs/path/winkgo_core');
      ...
      expect(opts.env.WINKGO_CACHE_DIR).toBe('/c');
      expect(opts.env.WINKGO_WORK_DIR).toBe('/w');
      expect(opts.env.WINKGO_LOG_DIR).toBe('/l');
      ...
      expect(infoSpy).toHaveBeenCalledWith(
        expect.stringContaining('[winkgo_core] health ready on port 55555 after 1 attempts, elapsed_ms=')
      );
    } finally {
      fetchSpy.mockRestore();
      infoSpy.mockRestore();
...
describe('BackendLifecycleManager.start (health timeout)', () => {
  ...
  it('captures backend boundary code and stage from early-exit stderr', async () => {
    ...
    child.stderr?.emit(
      'data',
      Buffer.from(
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.open databasePath=/db/path/winkgo-backend.db: failed to initialize application data\n'
      )
    );
  ...
  it('captures backend boundary code when stderr drains after exit but before close', async () => {
    ...
    child.stderr?.emit(
      'data',
      Buffer.from(
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.migration databasePath=/db/path/winkgo-backend.db: failed to initialize application data\n'
      )
    );
    child.emit('close', 1, null);
  ...
  it('kills child and reports listen_timeout when winkgo_core never reports a port', async () => {
    vi.useFakeTimers();
  ...
  it('records when server listening appears before health check times out', async () => {
    ...
    const expectedRejection = expect(startPromise).rejects.toMatchObject({
      ...
      details: expect.objectContaining({
        ...
        serverListeningLine: expect.stringContaining('WINKGO_CORE_LISTENING'),
      }),
    });
...
describe('BackendLifecycleManager crash restart', () => {
  ...
  it('logs crash restart scheduling details', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith('[winkgo_core] child exited unexpectedly; scheduling restart', {
      exitCode: 1,
      signal: 'SIGABRT',
      port: 65303,
      restartCount: 1,
  ...
  it('logs when crash restart limit is exceeded', () => {
    ...
    expect(mgr.status).toBe('error');
    expect(errorSpy).toHaveBeenCalledWith('[winkgo_core] child exited unexpectedly; restart limit exceeded', {
      exitCode: 1,
      signal: 'SIGABRT',
      port: 0,
      restartCount: 4,
...
function makePeerAlreadyRunningError(): BackendStartupError {
  return new BackendStartupError('winkgo_core exited before health check passed', {
    stage: 'early_exit',
...
describe('BackendLifecycleManager.start peer retry', () => {
  it('retries with bounded backoff and succeeds once the peer releases the data dir', async () => {
    vi.useFakeTimers();
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const mgr = new BackendLifecycleManager(APP_META, () => '/abs/path/winkgo_core');
    ...
    const started = mgr.start('/data/winkgo-backend.db');
  ...
  it('does not retry a non-peer startup failure', async () => {
    ...
    await expect(mgr.start('/data/winkgo-backend.db')).rejects.toBe(nonPeerError);
