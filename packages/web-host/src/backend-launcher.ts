...
export type BackendLaunchOptions = {
  ...
  logDir?: string;
  /**
   * System dirs exposed to the backend via WINKGO_{CACHE,WORK,LOG}_DIR env.
...
export class BackendStartupCancelledError extends Error {
  constructor(message = 'winkgo_core startup cancelled') {
    super(message);
...
export function buildSpawnArgs(config: SpawnConfig): string[] {
  const logLevel = process.env.WINKGO_LOG_LEVEL || (config.isPackaged ? 'info' : 'debug');
  const args = [
    '--port',
    String(config.port),
    '--data-dir',
  ...
  if (!config.isPackaged && process.env.WINKGO_DUMP_PROMPTS === '1') args.push('--dump-prompts');
...
/**
 * Backend reads WINKGO_{CACHE,WORK,LOG}_DIR env vars to report system dirs
 * (see backend/crates/winkgo-system/src/sysinfo.rs). Inject them so the
 * backend's `/api/system/info` matches what Electron main persists in
 * ProcessEnv('winkgo.dir').
 */
export function buildSpawnEnv(dirs: BackendDirConfig): NodeJS.ProcessEnv {
  return {
    ...process.env,
    WINKGO_CACHE_DIR: dirs.cacheDir,
    WINKGO_WORK_DIR: dirs.workDir,
    WINKGO_LOG_DIR: dirs.logDir,
  };
}
...
const WINKGO_CORE_LISTENING_PREFIX = 'WINKGO_CORE_LISTENING ';
const BACKEND_PORT_REPORT_TIMEOUT_MS = 30_000;

// Benign boundary code emitted by an winkgo_core instance that yielded the
...
export function findAvailablePort(
  ...
  if (preferredPort && firstRequestedPort === 0) {
    console.info(`[winkgo_core] skipped fetch-blocked backend port ${preferredPort}`);
  }
      ...
      server.listen(requestedPort, '127.0.0.1', () => {
        ...
        server.close(() => {
          ...
          if (resolvedPort > 0 && !isFetchForbiddenPort(resolvedPort)) {
            console.info(`[winkgo_core] selected backend port ${resolvedPort} after ${attempt} attempts`);
            resolve(resolvedPort);
            return;
          }
          if (resolvedPort > 0 && remainingAttempts > 1) {
            console.info(`[winkgo_core] skipped fetch-blocked backend port ${resolvedPort}`);
...
function parseWinkGoCoreListeningPort(line: string): number | undefined {
  if (!line.startsWith(WINKGO_CORE_LISTENING_PREFIX)) return undefined;
  try {
    const parsed = JSON.parse(line.slice(WINKGO_CORE_LISTENING_PREFIX.length)) as { port?: unknown };
...
export class BackendLifecycleManager {
  ...
  async start(
    ...
    // Bounded retry loop for the transient "a peer winkgo_core already owns this
    ...
    for (let attempt = 0; attempt < PEER_RETRY_MAX_ATTEMPTS; attempt += 1) {
      try {
        ...
        console.warn(
          `[winkgo_core] a peer already owns the data directory; retrying startup in ${backoff}ms (attempt ${attempt + 1}/${PEER_RETRY_MAX_ATTEMPTS})`
        );
    ...
    throw lastPeerError ?? new Error('winkgo_core startup failed after peer retries');
  }
  ...
  private async attemptStart(
    ...
    try {
      ...
      throw new BackendStartupError(
        'winkgo_core startup failed while resolving backend binary',
        {
    ...
    console.log(`[winkgo_core] starting: ${binaryPath} ${args.join(' ')}`);
    ...
    try {
      ...
      this._status = 'error';
      throw makeStartupError('spawn', 'winkgo_core startup directory preparation failed', error);
    }
    ...
    try {
      ...
      this._status = 'error';
      throw makeStartupError('spawn', 'winkgo_core process spawn threw before startup', error);
    }
    ...
    const startupFailure = new Promise<never>((_resolve, reject) => {
      ...
      this.childProcess?.once('error', (error) => {
        ...
        this._status = 'error';
        rejectOnce(makeStartupError('spawn_error', 'winkgo_core process emitted an error before startup', error));
      });
      ...
      this.childProcess?.once('close', (code, signal) => {
        ...
        if (!pendingStartupExit.startupSettledAtExit) {
          if (pendingStartupExit.statusAtExit === 'stopped') {
            rejectOnce(new BackendStartupCancelledError('winkgo_core startup cancelled before health check passed'));
            return;
          }
          rejectOnce(
            makeStartupError('early_exit', 'winkgo_core exited before health check passed', undefined, {
        ...
        if (pendingStartupExit.statusAtExit === 'starting') {
          void Promise.resolve(
            options?.onPendingExit?.(
              makeStartupError('early_exit', 'winkgo_core exited after startup health timeout', undefined, {
                exitCode: exitCode ?? undefined,
                signal: exitSignal ?? undefined,
              })
            )
          ).catch((error) => {
            console.error('[winkgo_core] pending exit handler failed:', error);
          });
        }
      });
    });
    ...
    const reportedPort = new Promise<number>((resolve, reject) => {
      ...
      reportedPortTimer = setTimeout(() => {
        rejectReportedPort(
          makeStartupError(
            'listen_timeout',
            'winkgo_core did not report its listening port before timeout',
            undefined,
            {
    ...
    this.childProcess.stdout?.on('data', (data: Buffer) => {
      ...
      for (const line of data.toString().split('\n')) {
        ...
        const port = parseWinkGoCoreListeningPort(trimmed);
        if (port !== undefined) {
          this._port = port;
        ...
        if (trimmed) console.log(`[winkgo_core] ${line}`);
      }
    });
    ...
    this.childProcess.stderr?.on('data', (data: Buffer) => {
      ...
      for (const line of data.toString().split('\n')) {
        if (line.trim()) console.error(`[winkgo_core] ${line}`);
      }
    });
    ...
    if (!health.ok) {
      ...
      if (healthTimeoutKeptAlive && this.childProcess) {
        ...
        void Promise.resolve(options.onHealthTimeout?.(healthTimeoutError)).catch((error) => {
          console.error('[winkgo_core] health timeout handler failed:', error);
        });
    ...
    this.restartCount = 0;
    console.info(
      `[winkgo_core] health ready on port ${this._port} after ${health.diagnostics.healthCheckAttempts} attempts, elapsed_ms=${health.diagnostics.healthCheckElapsedMs}, data-dir: ${dbPath}`
    );
    return this._port;
  }
  ...
  private continueWaitingForHealth(
    ...
    void (async () => {
      ...
      console.info(
        `[winkgo_core] late health ready on port ${port} after ${health.diagnostics.healthCheckAttempts} attempts, elapsed_ms=${elapsedMs}, data-dir: ${this._lastDbPath}`
      );
      await onReady?.(port);
    })().catch((error) => {
      console.error('[winkgo_core] background health wait failed:', error);
    });
  }
  ...
  private handleCrash(code: number | null, signal?: NodeJS.Signals | string | null): void {
    ...
    if (this.restartCount > this.maxRestarts) {
      this._status = 'error';
      console.error('[winkgo_core] child exited unexpectedly; restart limit exceeded', crashContext);
      return;
    }

    const delay = Math.pow(2, this.restartCount - 1) * 1000;
    console.warn('[winkgo_core] child exited unexpectedly; scheduling restart', {
      ...crashContext,
      delayMs: delay,
    });
    ...
    setTimeout(() => {
      ...
      this.start(this._lastDbPath, this._lastLogDir, this._lastDirs, this._lastOptions, this._port)
        ...
        .catch((error) => {
          this._status = 'error';
          console.error('[winkgo_core] restart after crash failed', {
            port: this._port,
            restartCount: this.restartCount,
            maxRestarts: this.maxRestarts,
            delayMs: delay,
