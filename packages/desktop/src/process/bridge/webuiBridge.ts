 ...
 * generate-qr-token) are NOT handled here — those are HTTP routes on winkgo_core's
 ...
 * WebUI instance requires Electron's app.* / Node child_process — winkgo_core
...
import {
  ...
  authorizeWebUIFirewall,
  isWebUIFirewallAuthorized,
...
export function initWebuiBridge(): void {
  ipcBridge.webui.getStatus.provider(async () => {
    ...
    const firewallAuthorized = !snapshot.allowRemote || (await isWebUIFirewallAuthorized(snapshot.port));
    return { ...snapshot, adminUsername, firewallAuthorized };
  });

  ipcBridge.webui.start.provider(async (params) => {
    if (params?.allowRemote) requireWinkGoCapability('remote.desktop');
    await maybeSeedInitialPassword();
    const handle = await startDesktopWebUI({
      port: params?.port,
      allowRemote: params?.allowRemote,
    });
    const firewallAuthorized = !params?.allowRemote || (await authorizeWebUIFirewall(handle.port));
    ...
    return { ...handle, firewallAuthorized };
  });

  ipcBridge.webui.authorizeFirewall.provider(async (params) => {
    requireWinkGoCapability('remote.desktop');
    return {
      authorized: await authorizeWebUIFirewall(params.port),
    };
  });
