...
function getProviderFromEnv(): TProviderWithModel | null {
  const platform = process.env.WINKGO_IMG_PLATFORM;
  const base_url = process.env.WINKGO_IMG_BASE_URL;
  const api_key = process.env.WINKGO_IMG_API_KEY;
  const model = process.env.WINKGO_IMG_MODEL;
