 ...
 * 只认自己进程树继承下来的端口。
 *
 * 端口由 Electron 主进程写进 WINKGO_CDP_ACTIVE_PORT，经 WINK GO Core 继承到这里，所以
 ...
 * writes it into WINKGO_CDP_ACTIVE_PORT and WINK GO Core passes it down, so failing to
...
export const resolveBrowserUrl = (deps: ResolveBrowserUrlDeps): string | null => {
  ...
  const rawPort = env.WINKGO_CDP_ACTIVE_PORT?.trim();
  if (rawPort) {
...
export const resolveBridgeToken = (deps: ResolveBrowserUrlDeps): string | null => {
  const raw = deps.env.WINKGO_CDP_BRIDGE_TOKEN?.trim();
  return raw ? raw : null;
};
