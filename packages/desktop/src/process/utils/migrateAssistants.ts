 ...
 * every assistant (built-in and user). The current backend ships `'winkgo_agent'` as
...
async function backendSupportsAssistantDefinitions(): Promise<boolean> {
  try {
    ...
    console.warn('[WinkGo] Failed to probe unified assistant detail support:', error);
    return false;
  }
}
...
async function markAssistantsMigrationDone(configFile: ConfigFile): Promise<void> {
  ...
  try {
    ...
    console.warn('[WinkGo] failed to persist assistants migration flag', err);
  }
}
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      ...
      if (isBackendHttpError(reason) && reason.status === 404) {
        skipped += 1;
        console.warn(
          `[WinkGo] Skipped override for retired built-in '${overrides[i].id}' (no longer in backend manifest)`
        );
        return;
      }
      failed += 1;
      console.error(`[WinkGo] Failed to apply builtin override for ${overrides[i].id}:`, reason);
    }
  });
  const applied = overrides.length - failed - skipped;
  if (failed === 0) {
    console.log(`[WinkGo] Applied ${applied} builtin disabled-state override(s) (skipped ${skipped} retired id(s))`);
  } else {
    console.error(
      `[WinkGo] Builtin override partial: ${failed}/${overrides.length} failed, ${skipped} skipped, ${applied} applied`
    );
  }
  return failed;
}
 ...
 * whatever manifest the running backend ships (e.g. current is `winkgo_agent`, but
 ...
 * on built-in rows (see `winkgo-assistant/src/service.rs`). 404 is treated as
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      ...
      if (isBackendHttpError(reason) && reason.status === 404) {
        skipped += 1;
        console.warn(
          `[WinkGo] Skipped agent_id override for retired built-in '${overrides[i].id}' (no longer in backend manifest)`
        );
        return;
      }
      failed += 1;
      console.error(`[WinkGo] Failed to apply agent_id override for ${overrides[i].id}:`, reason);
    }
  });
  const applied = overrides.length - failed - skipped;
  if (failed === 0) {
    console.log(`[WinkGo] Applied ${applied} builtin agent_id override(s) (skipped ${skipped} retired id(s))`);
  } else {
    console.error(
      `[WinkGo] Builtin agent_id override partial: ${failed}/${overrides.length} failed, ${skipped} skipped, ${applied} applied`
    );
  }
  return failed;
}
...
async function fetchCurrentBuiltinAgentIds(): Promise<Map<string, string>> {
  try {
    ...
    console.error('[WinkGo] Failed to fetch current builtin agent_id map:', error);
    return new Map();
  }
}
...
async function fetchAgentIdByRuntimeKey(): Promise<Map<string, string>> {
  try {
    ...
    console.error('[WinkGo] Failed to fetch agent runtime identity map:', error);
    return new Map();
  }
}
...
async function uploadLegacyAssistantRules(legacyAssistantIds: Set<string>): Promise<number> {
  ...
  try {
    ...
    console.error('[WinkGo] Failed to read legacy assistant rules dir:', error);
    return 1;
  }
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      failed += 1;
      console.error(
        `[WinkGo] Failed to upload legacy rule for '${ruleEntries[i].id}' (${ruleEntries[i].locale}):`,
        r.reason
      );
      return;
    }
  ...
  if (failed === 0) {
    if (uploaded > 0 || skipped > 0) {
      console.log(`[WinkGo] Legacy rule upload: ${uploaded} uploaded, ${skipped} skipped`);
    }
  } else {
    console.error(`[WinkGo] Legacy rule upload partial: ${failed}/${ruleEntries.length} failed`);
  }
  return failed;
}
 ...
 *      across the 'gemini' → 'winkgo_agent' default migration.
 ...
 *
 * Honors `WINKGO_SKIP_ELECTRON_MIGRATION=1` so E2E fixtures can seed via
...
export async function migrateAssistantsToBackend(configFile: ConfigFile): Promise<boolean> {
  if (process.env.WINKGO_SKIP_ELECTRON_MIGRATION === '1') {
    console.log('[WinkGo] Assistant migration skipped (env flag set)');
    return false;
  }
  ...
  if (userAssistants.length > 0) {
    try {
      ...
      if (result.failed !== 0) {
        console.error(`[WinkGo] Assistant migration partial: ${result.failed} failed`, result.errors);
        return false;
      }
      if (result.imported > 0 || result.skipped > 0) {
        console.log(`[WinkGo] migrated ${result.imported} assistants (skipped ${result.skipped})`);
      }
    } catch (error) {
      console.error('[WinkGo] Assistant migration failed:', error);
      return false;
    }
  }
