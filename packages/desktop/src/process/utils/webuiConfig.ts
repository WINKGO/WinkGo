...
import { getLanIP, startWebHost, type WebHostHandle } from '@winkgo/web-host';
...
const WEBUI_FIREWALL_RULE = 'WINK GO WebUI';
...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.WINKGO_PORT ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(process.env.WINKGO_ALLOW_REMOTE || process.env.WINKGO_REMOTE);
  const hostHint = process.env.WINKGO_HOST?.trim();
...
export async function isWebUIFirewallAuthorized(port: number): Promise<boolean> {
  if (process.platform !== 'win32') return true;
  const script =
    `$ErrorActionPreference='SilentlyContinue';` +
    `$policy=New-Object -ComObject HNetCfg.FwPolicy2;` +
    `$ok=$false;foreach($rule in $policy.Rules){` +
    `if($rule.Enabled -and $rule.Direction -eq 1 -and $rule.Action -eq 1 -and $rule.Protocol -eq 6 ` +
    `-and $rule.Name -eq '${WEBUI_FIREWALL_RULE}' -and [string]$rule.LocalPorts -eq '${port}'){$ok=$true;break}};` +
...
export async function authorizeWebUIFirewall(port: number): Promise<boolean> {
  if (process.platform !== 'win32') return true;
  const safePort = parsePortValue(port);
  if (!safePort) throw new Error('WebUI 端口无效。');
  if (await isWebUIFirewallAuthorized(safePort)) return true;

  const scriptPath = path.join(app.getPath('temp'), `winkgo-webui-firewall-${process.pid}-${Date.now()}.ps1`);
  const elevatedScript = [
    "$ErrorActionPreference='Stop'",
    `Get-NetFirewallRule -DisplayName '${WEBUI_FIREWALL_RULE}' -ErrorAction SilentlyContinue | Remove-NetFirewallRule -ErrorAction SilentlyContinue`,
    // The active network can legitimately be Public (mobile hotspot, hotel
    // Wi-Fi, or Windows misclassification). Authentication still protects the
    // WebUI; limiting the rule to Private silently breaks those users.
    `New-NetFirewallRule -DisplayName '${WEBUI_FIREWALL_RULE}' -Direction Inbound -Action Allow -Enabled True -Profile Any -Protocol TCP -LocalPort ${safePort} | Out-Null`,
  ].join('\r\n');
  await fs.promises.writeFile(scriptPath, `${elevatedScript}\r\n`, { encoding: 'utf8', mode: 0o600 });
  ...
  try {
    ...
    await fs.promises.unlink(scriptPath).catch((): void => undefined);
  }
  return isWebUIFirewallAuthorized(safePort);
}
...
export async function startDesktopWebUI(opts: { port?: number; allowRemote?: boolean }): Promise<DesktopWebUIHandle> {
  ...
  const handle = await startWebHost({
    app: {
      ...
      // getDataPath() returns ~/.winkgo[-dev] symlink on macOS to sidestep
...
export const restoreDesktopWebUIFromPreferences = async (): Promise<void> => {
  ...
  const permittedAllowRemote = allowRemote && winkGoCloudAuthService.hasCapability('remote.desktop');

  try {
    const handle = await startDesktopWebUI({ port: preferredPort, allowRemote: permittedAllowRemote });
    console.log(
