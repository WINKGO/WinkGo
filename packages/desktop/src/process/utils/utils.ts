...
import { DEFAULT_WORK_DIR_NAME, getPlatformServices } from '@/common/platform';
...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'winkgo');
};
 ...
 * Release builds use ~/.winkgo; dev builds use ~/.winkgo-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.winkgo，Dev 模式使用 ~/.winkgo-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const brandedDataPath = path.join(rootPath, DEFAULT_WORK_DIR_NAME);
  const legacyDataPath = path.join(rootPath, 'winkgo');
  // If a Windows process prevented the startup rename, keep the user's
  // existing workspace active rather than creating a blank branded folder.
  const dataPath = !existsSync(brandedDataPath) && existsSync(legacyDataPath) ? legacyDataPath : brandedDataPath;
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.winkgo'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.winkgo-config; dev builds use ~/.winkgo-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.winkgo-config，Dev 模式使用 ~/.winkgo-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.winkgo-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[WinkGo] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[WinkGo] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[WinkGo] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[WinkGo] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
