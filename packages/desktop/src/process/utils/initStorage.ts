...
const STORAGE_PATH = {
  config: 'winkgo-config.txt',
  chatMessage: 'winkgo-chat-message.txt',
  chat: 'winkgo-chat.txt',
  env: '.winkgo-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[WinkGo] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[WinkGo] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[WinkGo] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('winkgo.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'winkgo-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'winkgo-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'winkgo-chat-history', conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, 'winkgo-chat-history', 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[WinkGo] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
 ...
 * `crates/winkgo-app/assets/builtin-assistants/` and
 * `crates/winkgo-app/assets/builtin-skills/`) — neither is synced from
...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[WinkGo:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  //    now live in the backend SQLite catalog (see winkgo-assistant crate) and
  ...
  try {
    ...
    console.error('[WinkGo] Failed to ensure assistant dirs:', error);
  }
