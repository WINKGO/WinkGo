...
function classifyIncompleteInstallation(details: ErrorWithDetails['details']): BackendStartupFailureInfo | undefined {
  ...
  const missingBundledWinkGoCoreDir = !resourcesDirEntries.includes('bundled-winkgo-core/');
  const missingRuntimeDir = details.runtimeDirExists === false && typeof details.runtimeKey === 'string';
  const missingResources = missingBundledWinkGoCoreDir ? ['bundled-winkgo-core/'] : [];
  if (details.runtimeDirExists === false && typeof details.runtimeKey === 'string') {
    missingResources.push(`bundled-winkgo-core/${details.runtimeKey}/`);
  }
  ...
  if (missingManagedResourcesDir && typeof details.runtimeKey === 'string') {
    missingResources.push(`bundled-winkgo-core/${details.runtimeKey}/managed-resources/`);
  }
  ...
  if (missingRuntimeBinary && typeof details.runtimeKey === 'string' && typeof details.binaryName === 'string') {
    missingResources.push(`bundled-winkgo-core/${details.runtimeKey}/${details.binaryName}`);
  }
  ...
  return {
    incompleteInstallationKind:
      missingBundledWinkGoCoreDir || missingRuntimeDir || missingManagedResourcesDir
        ? 'missing_directory_resources'
        : 'missing_backend_binary',
    missingBackendBinary:
      missingBundledWinkGoCoreDir || missingRuntimeDir || missingManagedResourcesDir || missingRuntimeBinary,
    missingBundledWinkGoCoreDir,
...
// A transient concurrent-startup race (two winkgo_core instances briefly bootstrapping
