...
const migration_v8: IMigration = {
  ...
  up: (db) => {
    ...
    if (!columns.has('source')) {
      db.exec(`ALTER TABLE conversations ADD COLUMN source TEXT CHECK(source IN ('winkgo', 'telegram'))`);
...
const migration_v11: IMigration = {
  ...
  up: (db) => {
    ...
    db.exec(`UPDATE conversations SET source = NULL WHERE source IS NOT NULL AND source NOT IN ('winkgo', 'telegram')`);
    ...
    db.exec(`CREATE TABLE IF NOT EXISTS conversations_new (
        ...
        source TEXT CHECK(source IS NULL OR source IN ('winkgo', 'telegram')),
        created_at INTEGER NOT NULL,
        updated_at INTEGER NOT NULL,
...
const migration_v12: IMigration = {
  ...
  up: (db) => {
    ...
    // Clean up any invalid source values before copying
    db.exec(
      `UPDATE conversations SET source = NULL WHERE source IS NOT NULL AND source NOT IN ('winkgo', 'telegram', 'lark')`
    );
    ...
    db.exec(`CREATE TABLE IF NOT EXISTS conversations_new (
        ...
        model TEXT,
        status TEXT CHECK(status IN ('pending', 'running', 'finished')),
        source TEXT CHECK(source IS NULL OR source IN ('winkgo', 'telegram', 'lark')),
        created_at INTEGER NOT NULL,
        updated_at INTEGER NOT NULL,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
      )`);
...
const migration_v14: IMigration = {
  ...
  up: (db) => {
    ...
    // so DROP TABLE will NOT trigger ON DELETE CASCADE on the messages table.
    db.exec(
      `UPDATE conversations SET source = NULL WHERE source IS NOT NULL AND source NOT IN ('winkgo', 'telegram', 'lark', 'dingtalk')`
    );
    ...
    db.exec(`CREATE TABLE IF NOT EXISTS conversations_new (
        ...
        model TEXT,
        status TEXT CHECK(status IN ('pending', 'running', 'finished')),
        source TEXT CHECK(source IS NULL OR source IN ('winkgo', 'telegram', 'lark', 'dingtalk')),
...
/**
 * Migration v20 -> v21: Add 'winkgo_agent' to conversations type CHECK constraint
 */
const migration_v21: IMigration = {
  version: 21,
  name: "Add 'winkgo_agent' to conversations type CHECK",
  up: (db) => {
    db.exec(`CREATE TABLE IF NOT EXISTS conversations_new (
        id TEXT PRIMARY KEY,
        user_id TEXT NOT NULL,
        name TEXT NOT NULL,
        type TEXT NOT NULL CHECK(type IN ('gemini', 'acp', 'codex', 'openclaw-gateway', 'nanobot', 'remote', 'winkgo_agent')),
        extra TEXT NOT NULL,
        model TEXT,
        status TEXT CHECK(status IN ('pending', 'running', 'finished')),
        source TEXT,
    ...
    console.log("[Migration v21] Added 'winkgo_agent' to conversations type CHECK");
  },
  down: (db) => {
    // Remove winkgo_agent conversations before copying to table with stricter constraint
    db.exec(`DELETE FROM messages WHERE conversation_id IN (SELECT id FROM conversations WHERE type = 'winkgo_agent')`);
    db.exec(`DELETE FROM conversations WHERE type = 'winkgo_agent'`);
    ...
    console.log("[Migration v21] Rolled back: Removed 'winkgo_agent' from conversations type CHECK");
  },
};
