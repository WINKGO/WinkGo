...
const FORCE_DEV_AUTO_UPDATE_ENV = 'WINKGO_FORCE_DEV_AUTO_UPDATE';
...
class AutoUpdaterService extends EventEmitter {
  ...
  private ensureDevUpdateConfig(): void {
    try {
      ...
      const devConfig = [
        'provider: generic',
        `url: ${cdnFeedOptions.url}`,
        'updaterCacheDirName: com.winkgo.app',
        '',
      ].join('\n');
  ...
  private moveCwdOutOfInstallDirForWindowsHandoff(): void {
    ...
    try {
      const safeCwd = path.join(app.getPath('temp'), 'winkgo-updater-cwd');
   ...
   * while the downloaded archive contains the packaged app (com.winkgo.app). Squirrel.Mac
