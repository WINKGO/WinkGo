...
/**
 * Resolve a model-provided path against the trusted workspace and reject
 * lexical traversal as well as symlinks that escape outside the workspace.
 */
...
export async function processImageUri(imageUri: string, workspaceDir: string): Promise<ImageContent | null> {
  ...
  try {
    ...
    const possiblePaths = [imageUri, path.resolve(workspaceDir, imageUri)].filter(
      (possiblePath, index, paths) => paths.indexOf(possiblePath) === index
    );
    throw new Error(
...
export async function executeImageGeneration(
  ...
  try {
    ...
    const message = `Workspace directory not found: ${resolvedWorkspaceDir}`;
    return { success: false, text: message, error: message };
  }
  if (!stat.isDirectory()) {
    const message = `Workspace path is not a directory: ${resolvedWorkspaceDir}`;
    return { success: false, text: message, error: message };
  }
