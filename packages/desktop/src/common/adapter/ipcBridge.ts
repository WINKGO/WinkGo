 ...
 * calls routed to winkgo_core. Electron-native operations (window controls,
...
export const conversation = {
  ...
  createWithConversation: withResponseMap(
    httpPost<TChatConversation, { conversation: TChatConversation }>('/api/conversations/clone', (p) => {
      const isWinkGoAgent = p.conversation.type === 'winkgo_agent';
...
export interface IWinkGoSkillCatalogItem {
  id: string;
  displayName: string;
  description: string;
...
export interface IWinkGoSkillsCatalog {
  available: boolean;
  skills: IWinkGoSkillCatalogItem[];
}

export interface IWinkGoPreparedSkillImport {
  skillPath?: string;
}

export interface IWinkGoSkillBridgePlan {
  serverName: string;
  enabled: boolean;
...
export interface IWinkGoSmartHomePreferences {
...
export interface IWinkGoSmartHomePreferencesSaveRequest {
  homeAssistantUrl: string;
  accessToken?: string;
...
export type WinkGoAuthProvider = 'winkgo' | 'local' | 'google' | 'wechat';

export type WinkGoAuthUser = {
  id: string;
  username: string;
  phone?: string;
  provider: WinkGoAuthProvider;
  createdAt: string;
  lastLoginAt: string;
  loginCount: number;
};

export type WinkGoAuthSession = {
  authenticated: boolean;
  user: WinkGoAuthUser | null;
  edition: import('@/common/types/platform/winkGoEdition').WinkGoEditionSnapshot;
  oauth: {
    google: boolean;
    wechat: boolean;
  };
...
export type WinkGoAuthErrorCode =
  | 'invalidCredentials'
  | 'accountExists'
  | 'validationError'
  | 'licenseDenied'
...
export type WinkGoAuthResult = {
  success: boolean;
  user?: WinkGoAuthUser;
  message?: string;
  code?: WinkGoAuthErrorCode;
};

export type WinkGoAuthCredentials = {
  username: string;
  password: string;
  phone?: string;
...
export type WinkGoOrganizerRule = {
  id: string;
  name: string;
  keywords: string[];
};

export type WinkGoOrganizeRequest = {
  paths: string[];
  destinationRoot: string;
  mode: WinkGoOrganizerMode;
  autoRename: boolean;
  customRules: WinkGoOrganizerRule[];
  manualClassification?: string;
};

export type WinkGoOrganizeOperation = {
  source: string;
  destination: string;
  originalName: string;
  finalName: string;
...
export type WinkGoOrganizeFailure = {
  path: string;
  reason: string;
};

export type WinkGoOrganizeResult = {
  destinationRoot: string;
  operations: WinkGoOrganizeOperation[];
  failures: WinkGoOrganizeFailure[];
  skipped: string[];
};

export type WinkGoUndoResult = {
  restored: string[];
  failures: WinkGoOrganizeFailure[];
};
...
export type WinkGoFileShortcutStatus = {
  memo: boolean;
  fileShelf: boolean;
...
export type WinkGoFormatEngineStatus = {
  ffmpegAvailable: boolean;
  ffmpegPath: string | null;
  officeAvailable: boolean;
  officePath: string | null;
  officeEngine: 'LibreOffice' | 'WPS Office' | null;
  ncmAvailable: boolean;
};

export type WinkGoFormatConversionRequest = {
  jobId: string;
  preset: WinkGoFormatPreset;
  paths: string[];
  outputFolder: string;
};

export type WinkGoFormatConversionItem = {
  sourcePath: string;
  sourceName: string;
...
export type WinkGoFormatConversionReport = {
  jobId: string;
  preset: WinkGoFormatPreset;
  outputFolder: string;
  items: WinkGoFormatConversionItem[];
  error?: string;
};

export type WinkGoFormatConversionProgress = {
  jobId: string;
  preset: WinkGoFormatPreset;
  fileName: string;
  index: number;
...
export type WinkGoWindowsRuntimeState = {
  available: boolean;
  mediaEnabled: boolean;
  notificationEnabled: boolean;
  notificationAccess: WinkGoNotificationAccess;
  media: WinkGoMediaSnapshot | null;
  notification: WinkGoCapturedNotification | null;
};

export type WinkGoInspirationProviderId =
  | 'didi-ride'
  | 'meituan-life'
  | 'gaode-map'
  | 'mcdonalds-china'
...
export type WinkGoInspirationTestRecord = {
  ok: boolean;
  message: string;
  latencyMs: number;
  testedAtMs: number;
...
export type WinkGoInspirationProvider = {
  id: WinkGoInspirationProviderId;
  name: string;
  subtitle: string;
  phase: 'available' | 'queued';
  runtime: string;
  risk: string;
  enabled: boolean;
...
export type WinkGoInspirationSnapshot = {
  selectedProvider: WinkGoInspirationProviderId;
  providers: WinkGoInspirationProvider[];
  configPath: string;
  legacyCompatible: boolean;
};

export type WinkGoInspirationSaveRequest = {
...
export type WinkGoInspirationResult<T> = {
  success: boolean;
  data?: T;
  error?: string;
};

export type WinkGoInspirationActionResult = {
  snapshot: WinkGoInspirationSnapshot;
  message: string;
};

export type WinkGoMeituanLinkResult = WinkGoInspirationActionResult & {
  connected: boolean;
  authUrl: string;
  qrImageUrl: string;
};

export type WinkGoXiaozhiSecretKind = 'bridge' | 'hardware' | 'mobile';

export type WinkGoXiaozhiTestRecord = {
  ok: boolean;
  message: string;
...
export type WinkGoXiaozhiConfig = {
  ...
  lastSavedMs: number;
  hardwareLastTest: WinkGoXiaozhiTestRecord | null;
  mobileLastTest: WinkGoXiaozhiTestRecord | null;
};

export type WinkGoXiaozhiLocalProbe = {
  ok: boolean;
  label: string;
  detail: string;
  elapsedMs: number;
...
export type WinkGoRemoteGatewaySnapshot = {
  state: WinkGoRemoteGatewayState;
  enabled: boolean;
  connected: boolean;
  connecting: boolean;
  accountId: string;
...
export type WinkGoXiaozhiSnapshot = {
  config: WinkGoXiaozhiConfig;
  runtime: WinkGoXiaozhiLocalProbe;
  bridge: WinkGoXiaozhiLocalProbe;
  remoteGateway: WinkGoRemoteGatewaySnapshot;
...
export type WinkGoXiaozhiActionResult = {
  snapshot: WinkGoXiaozhiSnapshot;
  message: string;
};
...
export const application = {
  ...
  // The backend reads WINKGO_*_DIR env vars on boot, so it does not own this config.
...
// ---------------------------------------------------------------------------
// Original WINK GO Skills — Electron local metadata only, loaded on demand.
// ---------------------------------------------------------------------------

export const winkGoSkills = {
  listCatalog: bridge.buildProvider<IWinkGoSkillsCatalog, void>('winkgo-skills.list-catalog'),
  prepareImport: bridge.buildProvider<IWinkGoPreparedSkillImport, { skillId: string }>('winkgo-skills.prepare-import'),
  syncEnabled: bridge.buildProvider<IWinkGoSkillBridgePlan, { skillIds: string[] }>('winkgo-skills.sync-enabled'),
  getWechatPreferences: bridge.buildProvider<IWinkGoWechatPreferences, void>('winkgo-skills.get-wechat-preferences'),
  saveWechatPreferences: bridge.buildProvider<IWinkGoWechatPreferences, IWinkGoWechatPreferences>(
    'winkgo-skills.save-wechat-preferences'
  ),
  getSmartHomePreferences: bridge.buildProvider<IWinkGoSmartHomePreferences, void>(
    'winkgo-skills.get-smart-home-preferences'
  ),
...
export const winkGoAuth = {
  getSession: bridge.buildProvider<WinkGoAuthSession, void>('winkgo-auth.get-session'),
  login: bridge.buildProvider<WinkGoAuthResult, WinkGoAuthCredentials>('winkgo-auth.login'),
  register: bridge.buildProvider<WinkGoAuthResult, WinkGoAuthCredentials>('winkgo-auth.register'),
  logout: bridge.buildProvider<void, void>('winkgo-auth.logout'),
};

export const winkGoFiles = {
  getDefaultFolder: bridge.buildProvider<string, void>('winkgo-files.get-default-folder'),
...
export const winkGoFormat = {
  detectEngines: bridge.buildProvider<WinkGoFormatEngineStatus, void>('winkgo-format.detect-engines'),
  getDefaultOutputFolder: bridge.buildProvider<string, void>('winkgo-format.get-default-output-folder'),
  selectFiles: bridge.buildProvider<string[], { preset: WinkGoFormatPreset }>('winkgo-format.select-files'),
  chooseOutputFolder: bridge.buildProvider<string | null, { defaultPath?: string }>(
    'winkgo-format.choose-output-folder'
  ),
  openOutput: bridge.buildProvider<void, { path: string }>('winkgo-format.open-output'),
  startConversion: bridge.buildProvider<WinkGoFormatConversionReport, WinkGoFormatConversionRequest>(
    'winkgo-format.start-conversion'
  ),
  progress: bridge.buildEmitter<WinkGoFormatConversionProgress>('winkgo-format.progress'),
};
...
export const winkGoWindows = {
  ...
  mediaChanged: bridge.buildEmitter<WinkGoMediaSnapshot | null>('winkgo-windows.media-changed'),
  notificationReceived: bridge.buildEmitter<WinkGoCapturedNotification>('winkgo-windows.notification-received'),
};

export const winkGoInspiration = {
  getSnapshot: bridge.buildProvider<WinkGoInspirationResult<WinkGoInspirationSnapshot>, void>(
    'winkgo-inspiration.get-snapshot'
  ),
  saveProvider: bridge.buildProvider<WinkGoInspirationResult<WinkGoInspirationSnapshot>, WinkGoInspirationSaveRequest>(
    'winkgo-inspiration.save-provider'
  ),
  testProvider: bridge.buildProvider<
    WinkGoInspirationResult<WinkGoInspirationActionResult>,
    { providerId: WinkGoInspirationProviderId }
  >('winkgo-inspiration.test-provider'),
  startMeituanLink: bridge.buildProvider<WinkGoInspirationResult<WinkGoMeituanLinkResult>, void>(
    'winkgo-inspiration.start-meituan-link'
  ),
  completeMeituanLink: bridge.buildProvider<WinkGoInspirationResult<WinkGoInspirationActionResult>, void>(
    'winkgo-inspiration.complete-meituan-link'
  ),
};

export const winkGoXiaozhi = {
  getSnapshot: bridge.buildProvider<WinkGoInspirationResult<WinkGoXiaozhiSnapshot>, void>(
    'winkgo-xiaozhi.get-snapshot'
  ),
  saveConfig: bridge.buildProvider<WinkGoInspirationResult<WinkGoXiaozhiSnapshot>, WinkGoXiaozhiSaveRequest>(
    'winkgo-xiaozhi.save-config'
  ),
  testConnections: bridge.buildProvider<WinkGoInspirationResult<WinkGoXiaozhiActionResult>, void>(
    'winkgo-xiaozhi.test-connections'
  ),
  startRuntime: bridge.buildProvider<WinkGoInspirationResult<WinkGoXiaozhiActionResult>, void>(
    'winkgo-xiaozhi.start-runtime'
  ),
  refreshBindingCode: bridge.buildProvider<WinkGoInspirationResult<WinkGoXiaozhiSnapshot>, void>(
    'winkgo-xiaozhi.refresh-binding-code'
  ),
  authorizeFirewall: bridge.buildProvider<WinkGoInspirationResult<WinkGoXiaozhiSnapshot>, void>(
...
export const winkGoImages = {
  recoverGeneratedImage: bridge.buildProvider<WinkGoGeneratedImageRecoveryResult, { callId: string }>(
    'winkgo-images.recover-generated-image'
  ),
};
...
// ---------------------------------------------------------------------------
// Google subscription status (Google OAuth provider path, used by winkgo_agent)
// ---------------------------------------------------------------------------
...
export const webui = {
  ...
  authorizeFirewall: bridge.buildProvider<{ authorized: boolean }, { port: number }>('webui.authorize-firewall'),
