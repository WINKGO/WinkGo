...
/**
 * Legacy config keys that may still exist on disk from before WINK GO Core owned
 ...
 * backend, but all current writes should go through WINK GO Core-owned storage.
 */
 ...
 * 会话来源类型 - 标识会话创建的来源
 */
export type ConversationSource = 'winkgo' | 'telegram' | 'lark' | 'dingtalk' | 'weixin' | 'wecom' | (string & {});
...
interface IChatConversation<T, Extra> {
  ...
  /** 会话来源，默认为 winkgo / Conversation source, defaults to winkgo */
...
export type TChatConversation =
  ...
  // backend/crates/winkgo-common/src/enums.rs and the WinkGoAgent factory.
...
export interface IMcpServer {
  ...
  /** Built-in MCP server managed by WinkGo (hide edit/delete in UI) */
  builtin?: boolean;
}
...
export const BUILTIN_IMAGE_GEN_NAME = 'winkgo-image-generation';
export const BUILTIN_IMAGE_GEN_LEGACY_NAMES = ['WinkGo Image Generation', BUILTIN_IMAGE_GEN_ID] as const;
