...
function isBackendStartupFailureEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.['winkgo.failure'] === 'backend_startup';
}
...
function getInstallPathKind(resourcesPath: unknown): string | undefined {
  ...
  if (normalized.includes('\\appdata\\local\\programs\\winkgo\\resources')) {
    return 'user_local_programs';
  }
  if (
    normalized.includes('\\program files\\winkgo\\resources') ||
    normalized.includes('\\program files (x86)\\winkgo\\resources')
  ) {
...
export async function captureBackendStartupFailure(error: unknown): Promise<void> {
  ...
  Sentry.withScope((scope) => {
    scope.setTag('winkgo.failure', 'backend_startup');
    scope.setTag('winkgo.backend_startup.reason', failureInfo.reason);
    if (failureInfo.runtime) {
      scope.setTag('winkgo.backend_startup.runtime', failureInfo.runtime);
    }
    if (failureInfo.packageArch) {
      scope.setTag('winkgo.backend_startup.package_arch', failureInfo.packageArch);
    }
    if (failureInfo.deviceArch) {
      scope.setTag('winkgo.backend_startup.device_arch', failureInfo.deviceArch);
    }
    if (failureInfo.expectedDownloadArch) {
      scope.setTag('winkgo.backend_startup.expected_download_arch', failureInfo.expectedDownloadArch);
    }
    if (typeof failureInfo.isRosettaTranslated === 'boolean') {
      scope.setTag('winkgo.backend_startup.rosetta_translated', getBooleanTagValue(failureInfo.isRosettaTranslated));
    }
    if (typeof details?.stage === 'string') {
      scope.setTag('winkgo.backend_startup.stage', details.stage);
    }
    if (failureInfo.backendBoundaryCode) {
      scope.setTag('winkgo.backend_startup.boundary_code', failureInfo.backendBoundaryCode);
    }
    if (failureInfo.backendBoundaryStage) {
      scope.setTag('winkgo.backend_startup.boundary_stage', failureInfo.backendBoundaryStage);
    }
    if (failureInfo.localDataIssueKind) {
      scope.setTag('winkgo.backend_startup.local_data_issue_kind', failureInfo.localDataIssueKind);
    }
    if (failureInfo.incompleteInstallationKind) {
      scope.setTag('winkgo.backend_startup.incomplete_installation_kind', failureInfo.incompleteInstallationKind);
    }
    for (const [tag, value] of [
      ['winkgo.backend_startup.missing_bundled_dir', getBooleanTagValue(failureInfo.missingBundledWinkGoCoreDir)],
      ['winkgo.backend_startup.missing_runtime_dir', getBooleanTagValue(failureInfo.missingRuntimeDir)],
      ['winkgo.backend_startup.missing_binary', getBooleanTagValue(failureInfo.missingBackendBinary)],
      ['winkgo.backend_startup.missing_hub_dir', getBooleanTagValue(failureInfo.missingHubDir)],
      ['winkgo.backend_startup.missing_pet_states_dir', getBooleanTagValue(failureInfo.missingPetStatesDir)],
      ['winkgo.backend_startup.missing_pwa_dir', getBooleanTagValue(failureInfo.missingPwaDir)],
      ['winkgo.backend_startup.install_path_kind', getInstallPathKind(details?.resourcesPath)],
      ['winkgo.backend_startup.last_update_status', getString(autoUpdateDiagnostics?.lastEvent?.status)],
      [
        'winkgo.backend_startup.health_polling_delayed',
        getBooleanTagValue(
          typeof details?.healthCheckPollingDelayed === 'boolean' ? details.healthCheckPollingDelayed : undefined
        ),
      ],
      ['winkgo.backend_startup.health_attempts_bucket', getHealthAttemptBucket(details?.healthCheckAttempts)],
      [
        'winkgo.backend_startup.health_attempt_deficit_bucket',
        getHealthAttemptBucket(details?.healthCheckAttemptDeficit),
      ],
      ['winkgo.backend_startup.health_timeout_overrun_bucket', getDurationBucket(details?.healthCheckTimeoutOverrunMs)],
      ['winkgo.backend_startup.health_max_attempt_gap_bucket', getDurationBucket(details?.healthCheckMaxAttemptGapMs)],
      [
        'winkgo.backend_startup.seconds_since_quit_and_install',
    ...
    if (details) {
      scope.setContext('winkgo_core_startup', details);
      scope.setExtra('winkgo_core_startup', details);
    }
    scope.setContext('winkgo_core_startup_classification', { ...failureInfo });
    scope.setExtra('winkgo_core_startup_classification', failureInfo);
    scope.setContext('winkgo_core_install_diagnostics', installDiagnostics);
    scope.setExtra('winkgo_core_install_diagnostics', installDiagnostics);
