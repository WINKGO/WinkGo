...
import { createDesktopIslandWindow, destroyDesktopIslandWindow } from './process/winkgo/desktopIslandWindow';
...
if (!gotTheLock) {
  console.warn('[WinkGo] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    // Prefer additionalData (reliable on all platforms), fallback to argv scan
    const deepLinkUrl = (additionalData as { deepLinkUrl?: string })?.deepLinkUrl || argv.find(isWinkGoDeepLinkUrl);
    if (deepLinkUrl) {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[WinkGo] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      ...
      console.error('[WinkGo] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[WinkGo] runBackendMigrations completed');
    } catch (error) {
      console.error('[WinkGo] Backend migration hook threw:', error);
    }
  })();
};
...
function markBackendReady(backendPort: number, source: string): void {
  ...
  console.log(`[WinkGo] ${source} ready (port=${backendPort})`);
...
function resolveDebugBackendStartupFailure(): BackendStartupFailureInfo | null {
  const reason = process.env.WINKGO_DEBUG_BACKEND_STARTUP_FAILURE as BackendStartupFailureInfo['reason'] | undefined;
  if (!reason) {
    return null;
  }
  if ((app.isPackaged && !isE2ETestMode) || isWebUIMode || isResetPasswordMode) {
    console.warn('[WinkGo] Ignoring WINKGO_DEBUG_BACKEND_STARTUP_FAILURE outside desktop dev/e2e mode.');
    return null;
  }
  ...
  console.warn(`[WinkGo] Ignoring unknown WINKGO_DEBUG_BACKEND_STARTUP_FAILURE value: ${reason}`);
  return null;
}
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[WinkGo] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[WinkGo] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[WinkGo] Renderer did-finish-load');
      showWindow();
      scheduleBackendMigrations();
    });
  ...
  const disableAutoUpdater =
    process.env.WINKGO_DISABLE_AUTO_UPDATE === '1' || process.env.WINKGO_E2E_TEST === '1' || isCiRuntime;
  ...
  if (!app.isPackaged && rendererUrl) {
    ...
    mainWindow.loadURL(rendererUrl).catch((error) => {
      ...
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[WinkGo] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[WinkGo] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[WinkGo] loadFile failed:', error.message || error);
    });
  }
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[WinkGo] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[WinkGo] Main window closed');
  ...
  // Closing the workspace window must not tear down the local Agent/MCP
  // runtime. Explicit Quit from the tray still follows the normal shutdown.
...
const handleAppReady = async (): Promise<void> => {
  ...
  if (debugBackendStartupFailure) {
    ...
    // Start winkgo_core only after initializeProcess(). initStorage may open
  ...
  try {
    ...
    console.error('[WinkGo] Failed to restore zoom factor:', error);
  ...
  try {
    ...
    console.error('[WinkGo] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    try {
      // Inside Electron (`WinkGo --webui` or packaged `winkgo-web` mode that
      ...
      const handle = await startWebHost({
        ...
        // Expose the same WINKGO_{CACHE,WORK,LOG}_DIR env the desktop IPC path
        ...
        backend: {
          ...
          port: (() => {
            ...
            if (!port) {
              throw new Error('[WebUI] Cannot start: winkgo_core is not running (globalThis.__backendPort unset)');
            }
            return port;
          })(),
        },
    ...
    if (isE2ETestMode) {
      ...
      try {
        ...
        // The tray is the explicit lifecycle control for the background
        // Runtime, so it remains available even when the workspace is hidden.
        createOrUpdateTray();
      } catch {
    ...
    // 未保存语言时与渲染进程一致，跟随系统语言，避免中文界面搭配英文托盘菜单。
    try {
      const savedLanguage = await ProcessConfig.get('language');
      await setInitialLanguage(savedLanguage || app.getLocale());
...
// Register winkgo:// as the default protocol client
...
// macOS: handle winkgo:// URLs via the open-url event
...
// instance lock. A lock-losing instance must NOT spawn a competing winkgo_core
// backend — doing so races the first instance's winkgo_core over the same data
...
if (shouldRegisterBackendStartup(gotTheLock)) {
  void app
    ...
    .catch((error) => {
      ...
      console.error('[WinkGo] App initialization failed:', error);
      app.quit();
    });
}
...
installQuitCleanup({
  ...
  // Stop winkgo_core subprocess — backend shutdown kills all agent children
...
app.on('will-quit', () => {
  console.log('[WINK GO] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[WINK GO] quit (exitCode=${exitCode})`);
});
