...
declare global {
  interface Window {
    __WinkGoSafeResizeObserver__?: boolean;
    __WinkGoResizeObserverPatched__?: boolean;
  }
...
const patchResizeObserver = () => {
  ...
  if (!window.__WinkGoSafeResizeObserver__ && typeof ResizeObserver !== 'undefined') {
    ...
    window.__WinkGoSafeResizeObserver__ = true;
  }
};
...
const patchGlobalErrorFilters = () => {
  ...
  if (!window.__WinkGoResizeObserverPatched__) {
    ...
    window.__WinkGoResizeObserverPatched__ = true;
  }
};

const patchConsole = () => {
  // Console patch mirrors the listener filters so devtools logs stay clean（控制台同样做拦截，防止被重复警告淹没）.
  if (typeof console !== 'undefined' && !console.__WinkGoResizeObserverPatched__) {
    ...
    console.__WinkGoResizeObserverPatched__ = true;
  }
};
