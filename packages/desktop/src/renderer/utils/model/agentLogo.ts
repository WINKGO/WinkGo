...
import { brandManagedAgentForDisplay, isWinkGoBrandAsset } from '@/renderer/utils/model/winkGoBranding';
...
export async function fetchAgentLogos(): Promise<AgentLogoMap> {
  try {
    ...
    if (Array.isArray(agents)) {
      ...
      for (const rawAgent of agents as ManagedAgent[]) {
        const agent = brandManagedAgentForDisplay(rawAgent);
...
export function resolveAgentLogo(
  ...
  if (isWinkGoBrandAsset(opts.icon)) return opts.icon;
...
export function resolveAgentAvatar(
  ...
  if (isWinkGoBrandAsset(opts.icon)) return { kind: 'image', value: opts.icon };
