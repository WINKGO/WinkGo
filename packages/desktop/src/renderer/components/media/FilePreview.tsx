  ...
  if (isImage) {
    return (
      <div
        ...
        data-testid={isInline ? 'inline-local-image-preview' : undefined}
      >
        <div
          className={classNames(
            'overflow-hidden border-1 border-solid b-color-border-2 bg-bg-2',
            isInline ? 'rd-12px flex items-center justify-center' : 'rd-8px'
          )}
        >
          <Image
            src={imageUrl}
            alt={file_name}
            width={isInline ? undefined : 60}
            height={isInline ? undefined : 60}
            className={classNames(
              'cursor-pointer',
              isInline
                ? 'block max-w-full max-h-520px object-contain [&_.arco-image-img]:max-w-full [&_.arco-image-img]:max-h-520px [&_.arco-image-img]:object-contain'
                : 'object-cover'
            )}
          ...
          />
          {!imageUrl && (
            <div className={classNames('bg-bg-3', isInline ? 'w-full h-220px animate-pulse' : 'w-60px h-60px')}></div>
          )}
        </div>
        {!readonly && (
          <div
