...
export const UPDATE_AVAILABLE_EVENT = 'winkgo-update-available';
...
export const useUpdateNotificationController = () => {
  ...
  const pendingStartupAvailableRef = useRef<AvailableOutcome | null>(null);
  const startupWebsiteCheckStartedRef = useRef(false);
  ...
  const restoreDownloadedUpdate = useCallback(async () => {
    try {
      ...
      if (!res?.success || !res.data?.ready || !res.data.version) {
        ...
        const pendingStartupAvailable = pendingStartupAvailableRef.current;
        pendingAutoAvailableRef.current = null;
        pendingStartupAvailableRef.current = null;
        if (pendingAutoAvailable) {
          dispatchAutoAvailable(pendingAutoAvailable);
        } else if (pendingStartupAvailable) {
          presentAvailableOutcome(pendingStartupAvailable);
        }
        return;
      }
      pendingAutoAvailableRef.current = null;
  ...
  }, [dispatchAutoAvailable, presentAvailableOutcome]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('winkgo-open-update-modal', handleWindowOpen);
    ...
    return () => {
      removeOpenListener();
      window.removeEventListener('winkgo-open-update-modal', handleWindowOpen);
  ...
  useEffect(() => {
    if (startupWebsiteCheckStartedRef.current) return;
    startupWebsiteCheckStartedRef.current = true;
    ...
    const timer = window.setTimeout(() => {
      void runUpdateCheck({
        ...
        if (restoreDownloadedPendingRef.current) {
          pendingStartupAvailableRef.current = outcome;
          return;
        }
        presentAvailableOutcome(outcome);
      });
    }, STARTUP_WEBSITE_UPDATE_CHECK_DELAY_MS);
