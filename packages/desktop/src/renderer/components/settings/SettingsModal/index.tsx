...
import { LEGACY_ANCHOR_REMAP } from '@/renderer/pages/settings/settingsNavigation';
...
export const SubModal: React.FC<SubModalProps> = ({ visible, onCancel, title, children }) => {
  return (
    <WinkGoModal
      ...
      title={title}
    >
      <WinkGoScrollArea className='h-full px-20px pb-16px text-14px text-t-primary'>{children}</WinkGoScrollArea>
    </WinkGoModal>
  );
};
...
const SettingsModal: React.FC<SettingsModalProps> = ({ visible, onCancel, defaultTab = 'model' }) => {
  ...
  const desktopMenu = (
    <WinkGoScrollArea className='flex-shrink-0 b-color-border-2 scrollbar-hide' style={{ width: `${SIDEBAR_WIDTH}px` }}>
