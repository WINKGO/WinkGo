 ...
 * Legacy backend/custom-agent migration is handled by WINK GO Core during channel
...
export function getDefaultChannelAssistant(assistants: Assistant[]): Assistant | undefined {
  return (
    assistants.find((assistant) => assistant.source === 'generated' && isWinkGoAgentAssistant(assistant)) ||
    assistants.find((assistant) => isWinkGoAgentAssistant(assistant)) ||
    assistants[0]
  );
}
