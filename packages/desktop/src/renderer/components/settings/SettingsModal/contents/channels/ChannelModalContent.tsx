...
const ChannelModalContent: React.FC = () => {
    ...
    try {
      if (enabled) {
        if (!slackPluginStatus?.hasToken) {
          Message.warning(t('settings.assistant.slackTokensRequired'));
          return;
        }
        await channel.enablePlugin.invoke({ plugin_id: 'slack', config: {} });
        Message.success(t('settings.assistant.slackPluginEnabled'));
      } else {
        await channel.disablePlugin.invoke({ plugin_id: 'slack' });
        Message.success(t('settings.assistant.slackPluginDisabled'));
      }
      await loadPluginStatus();
    } catch (error) {
      Message.error(error instanceof Error ? error.message : String(error));
  ...
  const handleToggleDiscordPlugin = async (enabled: boolean) => {
    ...
    try {
      if (enabled) {
        ...
        await channel.enablePlugin.invoke({ plugin_id: 'discord', config: {} });
        Message.success(t('settings.assistant.discordPluginEnabled'));
      } else {
        await channel.disablePlugin.invoke({ plugin_id: 'discord' });
        Message.success(t('settings.assistant.discordPluginDisabled'));
      }
      await loadPluginStatus();
    } catch (error) {
      Message.error(error instanceof Error ? error.message : String(error));
  ...
  const channels: ChannelConfig[] = useMemo(() => {
    const telegramChannel: ChannelConfig = {
      ...
      description: t('settings.channels.telegramDesc', 'Chat with WINK GO assistant via Telegram'),
      status: 'active',
    ...
    const slackChannel: ChannelConfig = {
      ...
      description: t('settings.channels.slackDesc', 'Chat with WINK GO assistant via Slack'),
      status: 'active',
    ...
    const discordChannel: ChannelConfig = {
      ...
      description: t('settings.channels.discordDesc', 'Chat with WINK GO assistant via Discord'),
      status: 'active',
    ...
    const larkChannel: ChannelConfig = {
      ...
      description: t('settings.channels.larkDesc', 'Chat with WINK GO assistant via Lark or Feishu'),
      status: 'active',
    ...
    const dingtalkChannel: ChannelConfig = {
      ...
      description: t('settings.channels.dingtalkDesc', 'Chat with WINK GO assistant via DingTalk'),
      status: 'active',
    ...
    const weixinChannel: ChannelConfig = {
      ...
      description: t('settings.channels.weixinDesc', 'Chat with WINK GO assistant via WeChat'),
      status: 'active',
    ...
    const wecomChannel: ChannelConfig = {
      ...
      description: t('settings.channels.wecomDesc', 'Chat with WINK GO assistant via WeCom (Enterprise WeChat)'),
  ...
  const channelGuideText = t('settings.webui.featureChannelsDesc', {
    defaultValue: 'Connect Telegram, Lark, and DingTalk to interact with WINK GO from IM apps.',
  });
  ...
  return (
    <WinkGoScrollArea className={isPageMode ? 'h-full' : ''}>
