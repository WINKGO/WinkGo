...
const WebuiModalContent: React.FC = () => {
    ...
    const unsubscribe = webui.statusChanged.on((data) => {
      ...
      if (data.running) {
        setStatus((prev) => ({
          ...
          firewallAuthorized: data.firewallAuthorized ?? prev?.firewallAuthorized,
        }));
        if (data.networkUrl) {
    ...
    try {
      if (enabled) {
        ...
        setStatus((prev) => ({
          ...
          localUrl,
          networkUrl: remoteEnabled && responseIP ? `http://${responseIP}:${port}` : undefined,
          lanIP: responseIP,
          initialPassword: responsePassword || cachedPassword || prev?.initialPassword,
          firewallAuthorized: startResult.firewallAuthorized,
        }));
  ...
  const handleAuthorizeFirewall = async () => {
    setFirewallLoading(true);
    try {
      const result = await webui.authorizeFirewall.invoke({ port: status?.port || port });
      setStatus((prev) => (prev ? { ...prev, firewallAuthorized: result.authorized } : prev));
      if (result.authorized) {
        Message.success(t('settings.webui.firewallAuthorized', { defaultValue: 'Windows 防火墙已允许手机访问' }));
      } else {
        Message.warning(
          t('settings.webui.firewallNotAuthorized', {
            defaultValue: '防火墙授权未完成，请在 Windows 提示中选择“是”',
          })
        );
      }
    } catch (error) {
      console.error('[WebuiModal] Firewall authorization failed:', error);
      Message.error(
        t('settings.webui.firewallAuthorizationFailed', { defaultValue: 'Windows 防火墙授权失败，请重试' })
      );
    } finally {
  ...
  if (!isDesktop) {
    return (
      <div className='flex flex-col h-full w-full'>
        <WinkGoScrollArea className='flex-1 min-h-0 pb-16px' disableOverflow={isPageMode}>
        ...
        */}
        ...
        <div className='px-[12px] md:px-[28px] py-14px bg-2 rd-16px'>
                  ...
                  onClick={() =>
                    void talkToButler({
                      prompt: t('settings.talkToButler.prompt.setupRemote', {
                        defaultValue:
                          'Help me set up remote access so I can open WINK GO from my phone or over the internet.',
                      }),
                    })
                  }
                >
          ...
          {webuiEnabled && allowRemotePreference && (
            <PreferenceRow
              label={t('settings.webui.firewall', { defaultValue: 'Windows 防火墙' })}
              description={
                status?.firewallAuthorized
                  ? t('settings.webui.firewallReadyDesc', {
                      defaultValue: 'WebUI 端口已放行，局域网手机可以访问',
                    })
                  : t('settings.webui.firewallRequiredDesc', {
                      defaultValue: '当前端口尚未放行，手机可能无法连接',
                    })
              }
              extra={
                <span className={`text-12px ${status?.firewallAuthorized ? 'text-success' : 'text-warning'}`}>
                  {status?.firewallAuthorized
                    ? `✓ ${t('settings.webui.firewallReady', { defaultValue: '已放行' })}`
                    : t('settings.webui.firewallBlocked', { defaultValue: '待授权' })}
                </span>
              }
            >
              {status?.firewallAuthorized ? (
                <div className='h-32px min-w-104px px-14px inline-flex items-center justify-center gap-6px rd-8px border border-[rgba(var(--success-6),0.28)] bg-[rgba(var(--success-6),0.09)] text-[rgb(var(--success-6))] text-13px font-500 shadow-sm'>
                ...
                <Button
                  ...
                  className='!h-32px !min-w-112px !px-14px !rd-8px shadow-sm'
                  onClick={() => void handleAuthorizeFirewall()}
                >
                  {firewallLoading
                    ? t('settings.webui.authorizingFirewall', { defaultValue: '正在授权…' })
                    : t('settings.webui.authorizeFirewall', { defaultValue: '授权并放行' })}
                </Button>
              )}
            </PreferenceRow>
          )}
