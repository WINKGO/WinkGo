...
import { Alert, Button, Checkbox, Message, Modal, Switch } from '@arco-design/web-react';
...
const BrowserDataSection: React.FC = () => {
  ...
  const saveLoginPermission = useCallback(
    async (enabled: boolean, consentAccepted = false) => {
      setLoginSwitchLoading(true);
      try {
        await ipcBridge.systemSettings.setBrowserLoginPermission.invoke({ enabled, consentAccepted });
        await mutate('browser.login-permission');
        Message.success(
          t(enabled ? 'settings.browserData.loginPermissionEnabled' : 'settings.browserData.loginPermissionDisabled')
        );
        return true;
      } catch (error) {
        Message.error(error instanceof Error ? error.message : t('settings.browserData.loginPermissionFailed'));
        return false;
      } finally {
        setLoginSwitchLoading(false);
      }
    },
    [t]
  );

  const handleToggleLoginPermission = useCallback(
    (checked: boolean) => {
      if (checked) {
      ...
      void saveLoginPermission(false);
    },
    [saveLoginPermission]
  );

  const handleAcceptLoginDisclaimer = useCallback(async () => {
    if (!consentChecked) return;
    const saved = await saveLoginPermission(true, true);
    if (saved) {
      setConsentVisible(false);
      setConsentChecked(false);
    }
  }, [consentChecked, saveLoginPermission]);
  ...
  return (
    <div className='px-[12px] md:px-[32px] py-16px bg-2 rd-16px'>
      ...
      <PreferenceRow
        label={t('settings.browserData.loginPermissionLabel')}
        description={t('settings.browserData.loginPermissionDesc')}
      >
        <Switch
          checked={loginPermission?.enabled ?? false}
          loading={loginSwitchLoading || loginPermissionLoading}
          onChange={handleToggleLoginPermission}
        />
      </PreferenceRow>

      {loginPermission?.enabled && (
        <Alert type='warning' content={t('settings.browserData.loginPermissionActiveNotice')} className='mb-8px' />
      )}
      ...
      <Modal
        ...
        title={t('settings.browserData.loginDisclaimerTitle')}
        okText={t('settings.browserData.loginDisclaimerAccept')}
        cancelText={t('common.cancel')}
        okButtonProps={{ disabled: !consentChecked, loading: loginSwitchLoading }}
        onOk={handleAcceptLoginDisclaimer}
        onCancel={() => {
          if (loginSwitchLoading) return;
          setConsentVisible(false);
          setConsentChecked(false);
        }}
        unmountOnExit
      >
        <Alert type='warning' content={t('settings.browserData.loginDisclaimerLead')} className='mb-12px' />
        <ul className='pl-20px mb-16px text-13px leading-22px text-t-secondary list-disc'>
          <li>{t('settings.browserData.loginDisclaimerAccount')}</li>
          <li>{t('settings.browserData.loginDisclaimerManualSecrets')}</li>
          <li>{t('settings.browserData.loginDisclaimerResponsibility')}</li>
          <li>{t('settings.browserData.loginDisclaimerStillBlocked')}</li>
        </ul>
        <Checkbox checked={consentChecked} onChange={setConsentChecked}>
          {t('settings.browserData.loginDisclaimerCheckbox')}
        </Checkbox>
      </Modal>
    </div>
  );
