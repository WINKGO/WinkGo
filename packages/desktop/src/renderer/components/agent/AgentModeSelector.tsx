...
const AgentModeSelector: React.FC<AgentModeSelectorProps> = ({
  ...
  useEffect(
    () =>
      addTitlebarQuickActionVisibilityListener('permission', (visible) => {
        setDropdownVisible(visible && canInteract && !isLoading && runtimeConfig.setStatus.state !== 'setting');
      }),
    [canInteract, isLoading, runtimeConfig.setStatus.state]
  );
    ...
    const compactContent = (
      <span data-testid='mode-selector' data-current-mode={current_mode} className='inline-flex'>
        <RuntimeSelectorPill
          ...
          data-winkgo-quick-action='permission'
