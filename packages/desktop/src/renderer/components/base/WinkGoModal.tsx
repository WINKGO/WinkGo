...
/** WinkGoModal 组件 Props */
export interface WinkGoModalProps extends Omit<ModalProps, 'title' | 'footer'> {
...
const STD_HEADER_CLASS = 'winkgo-modal-std-header flex items-start justify-between gap-16px px-24px pt-20px pb-16px';
...
const STD_BODY_LAYOUT_CLASS = 'winkgo-modal-std-body min-h-0 flex-1 overflow-y-auto';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <WinkGoModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </WinkGoModal>
 *
 * // 预设尺寸 / Preset size
 * <WinkGoModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </WinkGoModal>
 *
...
const WinkGoModal: React.FC<WinkGoModalProps> = ({
  children,
  variant,
  size,
  header,
  ...
  const renderFooter = () => {
    ...
    if (footerConfig.render) {
      ...
      const footerClassName = classNames(
        ...
        isStandard && 'winkgo-modal-std-footer',
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={onCancel}
      className={classNames('winkgo-modal', isStandard && 'winkgo-modal-standard', className)}
      style={finalStyle}
      getPopupContainer={() => document.body}
    >
      <div
        className={classNames('winkgo-modal-wrapper', isStandard && 'flex flex-col min-h-0')}
        style={{ borderRadius: borderRadiusVal }}
      >
        {renderHeader()}
        <div
          className={classNames(
            'winkgo-modal-body-content',
...
WinkGoModal.displayName = 'WinkGoModal';

export default WinkGoModal;
