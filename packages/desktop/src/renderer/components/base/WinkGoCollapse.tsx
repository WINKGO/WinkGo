 ...
 */
export interface WinkGoCollapseProps {
 ...
 */
export interface WinkGoCollapseItemProps {
 ...
 */
const WinkGoCollapseItem: React.FC<WinkGoCollapseItemProps> = ({ children }) => <>{children}</>;
WinkGoCollapseItem.displayName = 'WinkGoCollapseItem';
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <WinkGoCollapse defaultActiveKey={['1']}>
 *   <WinkGoCollapse.Item name="1" header="面板1">
 *     内容1
 *   </WinkGoCollapse.Item>
 *   <WinkGoCollapse.Item name="2" header="面板2">
 *     内容2
 *   </WinkGoCollapse.Item>
 * </WinkGoCollapse>
 *
 * // 手风琴模式 / Accordion mode
 * <WinkGoCollapse accordion defaultActiveKey="1">
 *   <WinkGoCollapse.Item name="1" header="面板1">内容1</WinkGoCollapse.Item>
 *   <WinkGoCollapse.Item name="2" header="面板2">内容2</WinkGoCollapse.Item>
 * </WinkGoCollapse>
 *
 * // 自定义图标 / Custom icon
 * <WinkGoCollapse
 ...
 * ```
 */
const WinkGoCollapseComponent: React.FC<WinkGoCollapseProps> & { Item: typeof WinkGoCollapseItem } = ({
  children,
  className,
  defaultActiveKey,
  activeKey,
  ...
  const items = useMemo(() => {
    return React.Children.toArray(children).filter((child): child is React.ReactElement<WinkGoCollapseItemProps> => {
      return React.isValidElement(child) && child.type === WinkGoCollapseItem;
    });
  }, [children]);
...
WinkGoCollapseComponent.Item = WinkGoCollapseItem;

export default WinkGoCollapseComponent;
