...
const Layout: React.FC<{
  ...
  // The WINK GO brand block returns to the most recent chat while viewing Settings.
  // Outside Settings it remains a static identifier, so it cannot disrupt an active conversation.
  ...
  const handleBrandHome = useCallback(() => {
    ...
    try {
      target = sessionStorage.getItem('winkgo:last-non-settings-path');
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    ...
    const handleCheckUpdate = () => {
      window.dispatchEvent(new CustomEvent('winkgo-open-update-modal', { detail: { source: 'tray' } }));
    };
            ...
            >
              <ArcoLayout.Header
                className={classNames(
                  ...
                  {
                    'cursor-pointer group': collapsed || isSettingsRoute,
                  }
                )}
                role={isSettingsRoute ? 'button' : undefined}
                tabIndex={isSettingsRoute ? 0 : undefined}
                aria-label={isSettingsRoute ? t('common.back', { defaultValue: 'Back to Chat' }) : undefined}
                onClick={isSettingsRoute ? handleBrandHome : undefined}
                onKeyDown={
                  isSettingsRoute
                    ? (event) => {
              ...
              >
                <div
                  className={classNames('shrink-0 h-32px w-46px relative', {
                    '!size-24px': collapsed,
                  })}
                  onClick={isSettingsRoute ? undefined : onClick}
                >
                  <img
                ...
                <div className='text-16px text-t-primary collapsed-hidden font-semibold'>WINK GO</div>
                {isMobile && !collapsed && (
                  <button
                    type='button'
