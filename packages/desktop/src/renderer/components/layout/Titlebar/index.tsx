...
const Titlebar: React.FC<TitlebarProps> = ({ workspaceAvailable }) => {
  ...
  const appTitle = useMemo(() => 'WINK GO', []);
  ...
  useEffect(() => {
    if (!isSettingsRoute) {
      ...
      try {
        sessionStorage.setItem('winkgo:last-non-settings-path', path);
      } catch {
        // ignore
      }
      return;
    ...
    try {
      const stored = sessionStorage.getItem('winkgo:last-non-settings-path');
      if (stored) {
  ...
  return (
    ...
    >
      ...
      <div ref={toolbarRef} className='app-titlebar__toolbar'>
        ...
        <TitlebarQuickActions iconSize={iconSize} iconStrokeWidth={desktopIconStroke} isMobile={layout?.isMobile} />
        {showWorkspaceButton && (
          <button
            type='button'
            className={classNames('app-titlebar__button', layout?.isMobile && 'app-titlebar__button--mobile')}
