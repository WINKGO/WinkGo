...
const LocalAgents: React.FC = () => {
  ...
  const sortedOfficialAgents = useMemo(
    () =>
      officialAgents.toSorted((left, right) => {
        const leftIsWinkGoAgent = left.agent_type === 'winkgo_agent' || left.backend === 'winkgo_agent';
        const rightIsWinkGoAgent = right.agent_type === 'winkgo_agent' || right.backend === 'winkgo_agent';
        if (leftIsWinkGoAgent !== rightIsWinkGoAgent) {
          return leftIsWinkGoAgent ? -1 : 1;
        }
        // Strategic partner: pin Kimi right after the builtin winkgo_agent agent.
  ...
  const handleTestConnection = useCallback(
    async (agentId: string) => {
      try {
        ...
        const result = brandManagedAgentForDisplay(
  ...
  return (
    <div data-testid='agent-management-page' className='flex flex-col gap-16px'>
      ...
      <Alert
        data-testid='agent-detection-scope-note'
        type='info'
        showIcon
        content={t('settings.agentManagement.detectionScopeNote')}
        className='!rounded-10px !border-border-2 !bg-fill-1'
      />
