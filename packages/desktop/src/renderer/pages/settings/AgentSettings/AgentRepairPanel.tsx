...
const showSaveAndTestResult = (t: ReturnType<typeof useTranslation>['t'], result: ManagedAgent) => {
  const displayResult = brandManagedAgentForDisplay(result);
  switch (result.status) {
    case 'online':
      Message.success(t('settings.agentManagement.testConnectionOnline', { name: displayResult.name }));
      break;
    case 'missing':
      Message.warning(t('settings.agentManagement.testConnectionMissing', { name: displayResult.name }));
      break;
    case 'offline':
      Message.warning(
        formatManagedAgentDiagnosticMessage(t, result) ||
          (result.last_check_error_code === 'auth_required'
            ? t('settings.agentManagement.testConnectionAuth', { name: displayResult.name })
            : t('settings.agentManagement.testConnectionOffline', { name: displayResult.name }))
      );
      break;
    default:
      break;
...
const AgentRepairPanel: React.FC<AgentRepairPanelProps> = ({ agent, onSaved }) => {
  ...
  const envBlock = (
    <div>
      ...
      {/* What configuring env vars can fix — grounded in how WinkGo injects them
  ...
  return (
    <div className='mt-10px flex flex-col gap-12px rounded-10px bg-aou-1 px-12px py-12px'>
      ...
      {!isInternalWinkGoCli && isActionable && showPath ? pathBlock : null}
      {!isInternalWinkGoCli && isActionable ? envBlock : null}
      ...
      {!isInternalWinkGoCli && isActionable ? (
        <Button
          type='primary'
          size='large'
          disabled={isSaving}
