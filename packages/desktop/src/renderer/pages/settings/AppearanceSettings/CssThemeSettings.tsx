...
const CssThemeSettings: React.FC = () => {
  ...
  // Keep the Light theme available to the runtime, but omit its legacy
  // WinkGo-branded promotional cover from the visible gallery.
  // Virtual "Follow System" card follows Dark in the gallery.
  ...
  const displayThemes = useMemo(() => {
    ...
    const arr = themes.filter((theme) => theme.id !== LIGHT_THEME_ID);
    arr.splice(Math.min(1, arr.length), 0, systemCard);
    return arr;
  }, [themes, t]);
