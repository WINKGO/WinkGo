...
import { WINKGO_FILES_MARKER } from '@/common/config/constants';
...
import { brandLegacyTextForDisplay } from '@/renderer/utils/model/winkGoBranding';
...
const parseFileMarker = (content: string, canParseFileMarker: boolean): ParsedFileMarker => {
  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    if (lines[index].trim() === WINKGO_FILES_MARKER) {
...
const MessageText: React.FC<{ message: IMessageText; showCopyRow?: boolean }> = ({ message, showCopyRow = true }) => {
  ...
  const contentToRender = useMemo(() => {
    ...
    if (typeof content === 'string') {
      ...
      return isUserMessage ? content : brandLegacyTextForDisplay(content);
    }
    return content;
  ...
  const inlineImagePaths = useMemo(() => {
    if (isUserMessage || typeof text !== 'string') return [];

    const attachmentPaths = new Set(resolvedFiles);
    return extractLocalImagePaths(text)
      .map((file_path) => resolveMessageFilePath(file_path, conversationContext?.workspace))
      .filter((file_path) => !attachmentPaths.has(file_path));
  }, [conversationContext?.workspace, isUserMessage, resolvedFiles, text]);
  ...
  const displaySenderName = senderName ? brandLegacyTextForDisplay(senderName) : senderName;
      ...
      <div className={classNames('min-w-0 flex flex-col group', isUserMessage ? 'items-end' : 'items-start')}>
        ...
        >
          ...
          {inlineImagePaths.length > 0 && (
            <div className='mt-12px flex w-full flex-col gap-10px' data-testid='assistant-local-image-previews'>
              {inlineImagePaths.map((path) => (
                <FilePreview key={path} path={path} onRemove={() => undefined} readonly variant='inline' />
              ))}
            </div>
          )}
        </div>
