...
const MessageThinking: React.FC<{ message: IMessageThinking }> = ({ message }) => {
  ...
  const displayText = brandLegacyTextForDisplay(text);
  const displaySubject = subject ? brandLegacyTextForDisplay(subject) : subject;
  ...
  }, [displayText, isDone, expanded]);

  const summaryText = isDone
    ? `${t('conversation.thinking.complete', { defaultValue: 'Thought complete' })} · ${formatDuration(duration || 0)}`
    : `${displaySubject || t('conversation.thinking.label', { defaultValue: 'Thinking...' })} · ${formatElapsedTime(elapsedTime)}`;
