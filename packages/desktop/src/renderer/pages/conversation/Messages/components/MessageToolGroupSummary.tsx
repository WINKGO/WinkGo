...
const ToolImagePreview: React.FC<{ path: string }> = ({ path }) => {
  ...
  return (
    <div className='group relative m-l-20px m-t-10px w-fit max-w-560px overflow-hidden rounded-lg border bg-1 p-2'>
      {messageContext}
      <LocalImageView
        src={path}
        alt={getAcpImageFileName(path)}
        className='max-w-full max-h-480px object-contain rounded-md'
      />
...
const MessageToolGroupSummary: React.FC<{ messages: ToolMessage[] }> = ({ messages }) => {
  ...
  const [recoveredImagePaths, setRecoveredImagePaths] = useState<Record<string, string>>({});
  ...
  const displayTools = useMemo(
    () =>
      tools.map((tool) => ({
        ...tool,
        imagePath: tool.imagePath ?? recoveredImagePaths[tool.key],
      })),
    [recoveredImagePaths, tools]
  );

  useEffect(() => {
    let active = true;
    const recoverable = tools.filter((tool) => {
      if (!tool.imageRecoveryCallId || tool.imagePath || recoveryAttemptsRef.current.has(tool.key)) return false;
      recoveryAttemptsRef.current.add(tool.key);
      return true;
    });
    if (recoverable.length === 0) return () => undefined;

    void Promise.all(
      recoverable.map(async (tool) => {
        const result = await ipcBridge.winkGoImages.recoverGeneratedImage.invoke({
          callId: tool.imageRecoveryCallId!,
        });
        return { key: tool.key, path: result.path };
      })
    )
      .then((results) => {
        if (!active) return;
        setRecoveredImagePaths((current) => {
          const next = { ...current };
          for (const result of results) {
            if (result.path) next[result.key] = result.path;
          }
          return next;
        });
      })
      .catch((error) => {
        console.error('[MessageToolGroupSummary] Failed to recover generated image:', error);
      });
  ...
  return (
    <div className='tool-group-summary'>
      ...
      {displayTools
        .filter((item): item is NormalizedToolCall & { imagePath: string } => Boolean(item.imagePath))
        .map((item) => (
          <ToolImagePreview key={`${item.key}-image`} path={item.imagePath} />
        ))}
    </div>
  );
};
