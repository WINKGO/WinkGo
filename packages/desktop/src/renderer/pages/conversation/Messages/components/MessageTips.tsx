...
const MessageTips: React.FC<{ message: IMessageTips }> = ({ message }) => {
  ...
  const localizedTipBody = brandLegacyTextForDisplay(resolveAgentTipBody(content, code, params, t));
  ...
  if (structuredError) {
    ...
    const title = brandLegacyTextForDisplay(
      errorCode
    ...
    const body = brandLegacyTextForDisplay(
      errorCode
        ? t(
    ...
    const detailParts = [
      ...
      .filter(Boolean)
      .map((part) => brandLegacyTextForDisplay(part));
