...
const WinkGoAgentConversationPanel: React.FC<{
  conversation: WinkGoAgentConversation;
  ...
  const modelSelection = useWinkGoAgentModelSelection({
  ...
  const chatLayoutProps = {
    ...
    presetAssistant: presetAssistantInfo ? { ...presetAssistantInfo, id: winkgo_agentAssistantId } : undefined,
  };
...
const ChatConversation: React.FC<{
  ...
  const isWinkGoAgentConversation = conversation?.type === 'winkgo_agent';
  ...
  const acpConversation = isWinkGoAgentConversation ? undefined : conversation;
  ...
  const conversationNode = useMemo(() => {
    if (!conversation || isWinkGoAgentConversation) return null;
      ...
      case 'antigravity':
        return (
          <AcpChat
            ...
            initialModelId={
              (conversation.extra as { current_model_id?: string } | undefined)?.current_model_id
            }
            session_mode={conversation.extra?.session_mode}
  ...
  if (conversation && conversation.type === 'winkgo_agent') {
    return <WinkGoAgentConversationPanel key={conversation.id} conversation={conversation} sliderTitle={sliderTitle} />;
  }
