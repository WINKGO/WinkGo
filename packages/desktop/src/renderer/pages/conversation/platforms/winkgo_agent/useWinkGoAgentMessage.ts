...
export const useWinkGoAgentMessage = (
  ...
  useEffect(() => {
    return ipcBridge.conversation.responseStream.on((message) => {
      ...
      switch (message.type) {
        ...
        case 'finish':
          {
            logStreamTerminalObserved(conversation_id, message.turn_id, 'winkgo_agent', message.type);
            // winkgo_agent stream_end carries usage in data field
        ...
        case 'acp_permission':
          ...
          // Backend winkgo_agent emits wire type 'acp_permission' but the payload is
  ...
  useEffect(() => {
    ...
    void getConversationOrNull(conversation_id).then((res) => {
      ...
      if (res.type === 'winkgo_agent' && res.extra?.last_token_usage) {
