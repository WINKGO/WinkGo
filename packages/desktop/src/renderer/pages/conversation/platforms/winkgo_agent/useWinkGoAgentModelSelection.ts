...
export type WinkGoAgentModelSelection = {
...
export type UseWinkGoAgentModelSelectionOptions = {
  initialModel: TProviderWithModel | undefined;
  onSelectModel: (provider: IProvider, modelName: string) => Promise<boolean>;
};

export const useWinkGoAgentModelSelection = ({
  initialModel,
  onSelectModel,
}: UseWinkGoAgentModelSelectionOptions): WinkGoAgentModelSelection => {
  ...
  // WinkGoCore does not support Google Auth — filter it out
