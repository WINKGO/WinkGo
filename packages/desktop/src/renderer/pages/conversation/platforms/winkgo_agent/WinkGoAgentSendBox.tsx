...
import { useWinkGoAgentMessage } from './useWinkGoAgentMessage';
...
const useWinkGoAgentSendBoxDraft = getSendBoxDraftHook('winkgo_agent', {
  _type: 'winkgo_agent',
  atPath: [],
  content: '',
  uploadFile: [],
...
const useSendBoxDraft = (conversation_id: string) => {
  const { data, mutate } = useWinkGoAgentSendBoxDraft(conversation_id);
...
const WinkGoAgentSendBox: React.FC<{
  ...
  useEffect(() => {
    ...
    const storageKey = `winkgo_agent_initial_message_${conversation_id}`;
    const processedKey = `winkgo_agent_initial_processed_${conversation_id}`;
  ...
  const handleSheetModeChange = useCallback(
    async (mode: string) => {
      ...
      try {
        ...
        console.error('[WinkGoAgentSendBox] Failed to switch mode via sheet:', error);
  ...
  const handleStop = async (): Promise<void> => {
    ...
    try {
      ...
      console.warn('[WinkGoAgentSendBox] stop request failed', error);
        ...
        placeholder={
          current_model?.use_model
            ? t('acp.sendbox.placeholder', {
                backend: brandLegacyTextForDisplay(agent_name || WINK_GO_CLI_DISPLAY_NAME),
            ...
            {uploadFile.length > 0 && (
              <HorizontalFileList>
                {uploadFile.map((path) => (
                  <FilePreview
                    key={path}
                    data-testid={`winkgo_agent-file-tag-${uploadFile.indexOf(path)}`}
                    path={path}
                ...
                {atPath.map((item) => {
                  ...
                  if (!item.isFile) {
                    ...
                    return (
                      <Tag
                        key={item.path}
                        data-testid={`winkgo_agent-folder-tag-${folderIndex}`}
                        color='blue'
                        closable
                        onClose={() => {
...
export default WinkGoAgentSendBox;
