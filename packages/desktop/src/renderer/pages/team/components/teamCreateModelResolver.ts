 ...
 * `use_model: null`. Downstream, GeminiSendBox / WinkGoAgentSendBox gate the
...
function resolveBackendDefaultModel(assistant_backend?: string): Promise<string> {
  ...
  if (assistant_backend === 'winkgo_agent') {
    return resolveWinkGoAgentDefaultModel();
  }
...
async function resolveWinkGoAgentDefaultModel(): Promise<string> {
  return 'default';
}
