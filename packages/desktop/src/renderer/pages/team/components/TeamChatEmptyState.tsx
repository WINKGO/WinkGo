...
const TeamChatEmptyState: React.FC<Props> = ({
  ...
  const winkgo_agentDraft = useWinkGoAgentDraft(conversation_id);
  const setContentByKind = {
    acp: (text: string) => acpDraft.mutate((prev) => ({ ...prev, content: text })),
    winkgo_agent: (text: string) => winkgo_agentDraft.mutate((prev) => ({ ...prev, content: text })),
