...
const useWinkGoAgentDraft = getSendBoxDraftHook('winkgo_agent', {
  _type: 'winkgo_agent',
  atPath: [],
  content: '',
  uploadFile: [],
...
const TeamAddMemberPopover: React.FC<Props> = ({ children, disabled = false }) => {
  ...
  // 未知 Leader 会话时传空串（不写入）。按 Leader 后端选 acp/winkgo_agent 草稿。
  ...
  const winkgo_agentDraft = useWinkGoAgentDraft(leaderConversationId);
  ...
  const handleTellLeader = useCallback(() => {
    ...
    if (leader.assistant_backend === 'winkgo_agent') {
      winkgo_agentDraft.mutate((prev) => ({ ...prev, content: text }));
    } else {
      acpDraft.mutate((prev) => ({ ...prev, content: text }));
    }
    setVisible(false);
  }, [leader?.slot_id, leader?.assistant_backend, leaderConversationId, switchTab, t, acpDraft, winkgo_agentDraft]);
