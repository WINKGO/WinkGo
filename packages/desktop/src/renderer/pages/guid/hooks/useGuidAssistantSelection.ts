...
import { assistantRuntimeKey, isWinkGoAgentAssistant, type Assistant } from '@/common/types/agent/assistantTypes';
...
export function pickDefaultAssistantSelectionKey(assistants: Assistant[]): string | null {
  const enabledAssistants = assistants.filter((assistant) => assistant.enabled !== false);
  const preferred =
    enabledAssistants.find((assistant) => assistant.source === 'generated' && isWinkGoAgentAssistant(assistant)) ??
    enabledAssistants.find((assistant) => isWinkGoAgentAssistant(assistant)) ??
    enabledAssistants[0];
