...
import { WinkGoInlineSearchInput } from '@/renderer/components/base';
...
const GuidWorkspaceFootnote: React.FC<GuidWorkspaceFootnoteProps> = ({
  ...
  useEffect(
    () =>
      addTitlebarQuickActionVisibilityListener('workspace', (visible) => {
        if (!visible) {
          closeDropdown();
    ...
    [closeDropdown, handleBrowseWorkspace, openDropdown, recentWorkspaces.length]
  );
          ...
          <Tooltip content={workspaceDir} position='top'>
            <div className={styles.workspacePill}>
              <button
                ...
                data-winkgo-quick-action='workspace'
                onClick={toggleOpen}
              >
                <FolderIcon size={14} />
