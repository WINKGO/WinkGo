...
const GuidModelSelector: React.FC<GuidModelSelectorProps> = ({
  ...
  const [modelMenuVisible, setModelMenuVisible] = React.useState(false);

  React.useEffect(() => addTitlebarQuickActionVisibilityListener('model', setModelMenuVisible), []);
  ...
  if (isGeminiMode) {
    // Provider-grouped models (e.g. winkgo_agent). Build groups + a composite-id lookup
