...
import { ArrowRightUp, MindMapping } from '@icon-park/react';
...
const GuidPage: React.FC = () => {
  ...
  const modelSelection = useGuidModelSelection('winkgo_agent');
  ...
  useEffect(() => {
    ...
    const signature = JSON.stringify({
      ...
      availableModels: {
        ...
        winkgo_agent: modelSelection.modelList.map((provider) => ({
          id: provider.id,
    ...
    const applyAssistantDefaults = async () => {
      ...
      if (shouldApplyDefaultModel && effectiveBackend === 'winkgo_agent') {
  ...
  // Only winkgo_agent now — Gemini runs as a regular ACP backend with ACP-cached models.
  const PROVIDER_BASED_AGENTS = new Set(['winkgo_agent']);
