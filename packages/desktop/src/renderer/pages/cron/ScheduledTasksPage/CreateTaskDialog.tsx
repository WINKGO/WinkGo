...
import { assistantRuntimeKey, isWinkGoAgentAssistant } from '@/common/types/agent/assistantTypes';
...
const CreateTaskDialog: React.FC<CreateTaskDialogProps> = ({
  ...
  const isGeminiMode = resolvedBackend === 'gemini' || resolvedBackend === 'winkgo_agent';

  // Providers compatible with winkgo_agent (WinkGoCLI does not support Google Auth).
  // Computed independent of the current selection so assistant options backed
  // by winkgo_agent can be disabled when no provider is configured.
  const winkgo_agentProviders = useMemo(
    () => providers.filter((p) => !p.platform?.toLowerCase().includes('gemini-with-google-auth')),
    [providers]
  );
  const hasWinkGoAgentProvider = winkgo_agentProviders.length > 0;

  const filteredProviders = useMemo(
    () => (resolvedBackend === 'winkgo_agent' ? winkgo_agentProviders : providers),
    [resolvedBackend, providers, winkgo_agentProviders]
  );

  // Build Gemini current_model from model_id for GuidModelSelector.
  // For winkgo_agent edit mode, prefer the exact provider_id stored in model —
  ...
  const geminiCurrentModel = useMemo<TProviderWithModel | undefined>(() => {
    if (resolvedBackend !== 'winkgo_agent' || !model_id) return undefined;

    const editedProviderId =
      resolvedBackend === 'winkgo_agent' ? editJob?.metadata.agent_config?.model?.provider_id : undefined;
    if (editedProviderId) {
  ...
  const acpCachedModelInfo = useMemo<AcpModelInfo | null>(() => {
    if (!resolvedBackend || resolvedBackend === 'gemini' || resolvedBackend === 'winkgo_agent') return null;
    return buildAssistantModelInfo(selectedAssistantModels);
  }, [resolvedBackend, selectedAssistantModels]);

  // Auto-pick the first available model from /api/providers when winkgo_agent is
  // selected but none is set yet. Source of truth is the backend provider
  // list — do NOT read from any frontend-cached default.
  useEffect(() => {
    if (resolvedBackend !== 'winkgo_agent' || model_id) return;
    for (const provider of winkgo_agentProviders) {
  ...
  }, [resolvedBackend, model_id, winkgo_agentProviders, getAvailableModels]);
    ...
    try {
      ...
      if (canEditAgentConfig) {
        ...
        agent_config = resolveCronAgentConfig({
          ...
          presetAssistants,
          selectedWinkGoAgentProvider: geminiCurrentModel
            ? {
          ...
          winkgo_agentModelRequiredMessage: t('cron.page.form.winkgo_agentModelRequired'),
        }).agent_config;
      }
          ...
          >
            ...
            >
              {presetAssistants.map((assistant) => {
                ...
                const avatar = resolveAssistantAvatar(assistant.avatar);
                const disabled = isWinkGoAgentAssistant(assistant) && !hasWinkGoAgentProvider;
                return (
                  <Option key={assistant.id} value={assistant.id} disabled={disabled}>
                    <div
                      className='flex items-center gap-8px'
                      title={disabled ? t('cron.page.form.winkgo_agentNoProvider') : undefined}
                    >
                      {avatar.kind === 'image' ? (
                        <img src={avatar.value} alt={name} className='w-16px h-16px object-contain' />
                      ) : avatar.kind === 'emoji' ? (
                      ...
                      <span>{name}</span>
                      {disabled && (
                        <span className='text-12px text-t-tertiary'>{t('cron.page.form.winkgo_agentNoProvider')}</span>
                      )}
                    </div>
                  </Option>
                );
