...
/** Backend manifest id of the built-in WinkGo Butler assistant. */
const BUTLER_ASSISTANT_ID = 'winkgo-assistant';
 ...
 * select the WinkGo Butler, and pre-fill the chat input with a ready-made
      ...
      try {
        ...
        if (butler) {
          ...
          if (butler.enabled === false) {
            ...
            Message.success(
              t('settings.talkToButler.enabledToast', { defaultValue: 'Enabled the WINK GO Butler for you' })
            );
          }
        }
      } catch (error) {
