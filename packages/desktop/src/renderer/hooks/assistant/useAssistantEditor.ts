...
export const useAssistantEditor = ({
  ...
  const handleEdit = async (assistant: AssistantListItem) => {
    const useWinkGoCliIdentity = isWinkGoCliAssistant(assistant);
    setIsCreating(false);
    setActiveAssistantId(assistant.id);
    setEditVisible(true);
    setPromptViewMode(isBuiltinAssistant(assistant) ? 'preview' : 'edit');
    setEditName(useWinkGoCliIdentity ? WINK_GO_CLI_DISPLAY_NAME : assistant.name || '');
    setEditDescription(assistant.description ? brandLegacyTextForDisplay(assistant.description) : '');
    setEditAvatar(useWinkGoCliIdentity ? winkGoLogo : assistant.avatar || '');
    ...
    try {
      ...
      setEditAvatar(useWinkGoCliIdentity ? winkGoLogo : detail.profile.avatar || '');
      setEditAvatarPreview(undefined);
