...
const setDraft = <K extends DraftConversationType>(
  ...
  switch (type) {
    ...
    case 'winkgo_agent':
      if (draft) {
        store.winkgo_agent.set(conversation_id, draft as Extract<Draft, { _type: 'winkgo_agent' }>);
      } else {
        store.winkgo_agent.delete(conversation_id);
      }
      break;
    default:
      break;
...
const getDraft = <K extends DraftConversationType>(
  ...
  switch (type) {
    ...
    case 'winkgo_agent':
      return store.winkgo_agent.get(conversation_id) as Extract<Draft, { _type: K }>;
    default:
      return undefined;
  }
};
