...
import { brandManagedAgentsForDisplay } from '@/renderer/utils/model/winkGoBranding';
...
export const useManagedAgents = (): UseManagedAgentsResult => {
  ...
  const agents = useMemo(() => brandManagedAgentsForDisplay(data ?? []), [data]);
...
export const useManagedAgentRuntimeCatalog = (): ManagedAgent[] => {
  ...
  return useMemo(() => brandManagedAgentsForDisplay(data ?? []), [data]);
};
...
export async function getManagedAgents(): Promise<ManagedAgent[]> {
  ...
  return brandManagedAgentsForDisplay(data);
}
