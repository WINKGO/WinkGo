...
function normalizeAvatar(avatar: string | undefined): { logo: string; isEmoji: boolean; isFallback?: boolean } {
  if (avatar === winkGoLogo) {
    return { logo: winkGoLogo, isEmoji: false };
  }
...
function buildPresetInfoFromAssistant(assistant: Assistant, locale: string): PresetAssistantInfo {
  const displayAssistant = brandAssistantForDisplay(assistant);
  const localeKey = resolveLocaleKey(locale);
  const name =
    displayAssistant.name_i18n?.[localeKey] ||
    displayAssistant.name_i18n?.[locale] ||
    displayAssistant.name ||
    displayAssistant.id;
  const avatar = typeof displayAssistant.avatar === 'string' ? displayAssistant.avatar : '';
...
function buildPresetInfoFromConversationAssistant(
  ...
  const isWinkGoAgent = assistant.backend?.trim().toLowerCase() === 'winkgo_agent';
  const normalized = normalizeAvatar(isWinkGoAgent ? winkGoLogo : assistant.avatar);
  return {
    name: isWinkGoAgent ? WINK_GO_CLI_DISPLAY_NAME : assistant.name,
    logo: normalized.logo,
    isEmoji: normalized.isEmoji,
    isFallback: normalized.isFallback,
    ...
    const resolveLegacyRuntimeInfo = (): { info: PresetAssistantInfo; isLoading: false } | null => {
      ...
      const legacyBackend =
        typeof (conversation.extra as { backend?: unknown })?.backend === 'string'
          ...
          : '';
      if (legacyBackend.toLowerCase() === 'winkgo_agent') {
        const normalized = normalizeAvatar(winkGoLogo);
        return {
          info: {
            name: WINK_GO_CLI_DISPLAY_NAME,
            logo: normalized.logo,
            isEmoji: normalized.isEmoji,
            isFallback: normalized.isFallback,
