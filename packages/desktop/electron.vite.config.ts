...
function resolveWinkGoBuildEdition(): 'free' | 'pro' {
  const requestedEdition = String(process.env.WINKGO_EDITION || 'free')
    .trim()
    .toLowerCase();
  const isCiEnvironment = /^(1|true|yes)$/i.test(String(process.env.CI || '').trim());
  if (requestedEdition !== 'free' && requestedEdition !== 'pro') {
    throw new Error(`Unsupported WINK GO edition "${requestedEdition}". Set WINKGO_EDITION to "free" or "pro".`);
  }
  if (requestedEdition === 'pro' && (process.env.WINKGO_ALLOW_PRO_DEV_BUILD !== '1' || isCiEnvironment)) {
    throw new Error(
      'Pro Vite builds are development-only. Set WINKGO_ALLOW_PRO_DEV_BUILD=1 together with WINKGO_EDITION=pro on a local development machine.'
    );
  }
  ...
  return {
    main: {
      plugins: [
        ...
        // '@winkgo/web-host' excluded so its TS sources (which use ESM ".js" import specifiers)
        // are bundled by esbuild rather than left as `require('@winkgo/web-host')`, which Node
        // cannot resolve because the package ships no compiled .js files (workspace-only).
        externalizeDepsPlugin({ exclude: ['fix-path', '@winkgo/web-host'] }),
        ...(isDevelopment
          ? [
              {
      ...
      define: {
        ...
        'process.env.WINKGO_EDITION': JSON.stringify(winkGoEdition),
      },
    },
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another WinkGo clone is running),
        ...
        // Bind explicitly to IPv4. On Windows, `localhost` may bind only to ::1
        // while Electron resolves it to 127.0.0.1, causing a false
        // ERR_CONNECTION_REFUSED and an unexpected fallback to stale assets.
        host: '127.0.0.1',
        port: 5173,
      ...
      define: {
        'process.env.NODE_ENV': JSON.stringify(mode),
        'process.env.env': JSON.stringify(process.env.env),
        'process.env.WINKGO_MULTI_INSTANCE': JSON.stringify(process.env.WINKGO_MULTI_INSTANCE ?? ''),
        'process.env.SENTRY_DSN': JSON.stringify(process.env.SENTRY_DSN ?? ''),
        'process.env.WINKGO_EDITION': JSON.stringify(winkGoEdition),
        // Inject the real WinkGo version (root package.json) so renderer code
