...
// tarball layout:
//   winkgo-web/
//   ├── winkgo-web              ← bun-compiled standalone binary (process.execPath)
//   ├── package.json             ← for runtime version lookup
//   ├── bundled-winkgo-core/<plat-arch>/winkgo_core[.exe]
//   └── static/                  ← SPA assets
//
...
const isPackaged = (() => {
  ...
  return exeName === 'winkgo-web' || exeName === 'winkgo-web.exe';
})();

const BACKEND_BINARY = process.platform === 'win32' ? 'winkgo_core.exe' : 'winkgo_core';
const DEFAULT_PORT = 25808;
const RESET_COMMAND = isPackaged ? 'winkgo-web resetpass' : 'bun run resetpass';
...
function resolveBackendBinary(flags: Map<string, string | true>): string {
  ...
  const envOverride = process.env.WINKGO_BACKEND_BIN;
  if (envOverride) return path.resolve(envOverride);
  const platArch = `${process.platform}-${process.arch}`;
  const bundled = path.join(cliRoot, 'bundled-winkgo-core', platArch, BACKEND_BINARY);
  return bundled;
}
...
function resolveDataDir(flags: Map<string, string | true>): string {
  ...
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.WINKGO_DATA_DIR;
  if (envOverride) return path.resolve(envOverride);
  return path.join(os.homedir(), '.winkgo-web');
}

function resolveLogDir(flags: Map<string, string | true>, dataDir: string): string {
  const override = flags.get('log-dir');
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.WINKGO_LOG_DIR;
  if (envOverride) return path.resolve(envOverride);
...
function resolvePort(flags: Map<string, string | true>): number {
  ...
  const env = process.env.WINKGO_PORT ?? process.env.PORT;
...
function resolveAllowRemote(flags: Map<string, string | true>): boolean {
  ...
  const env = process.env.WINKGO_ALLOW_REMOTE ?? process.env.WINKGO_REMOTE;
  if (!env) return false;
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(staticDir)) {
    console.error(`[winkgo-web] static dir not found: ${staticDir}`);
    console.error(`  hint: pass --static-dir <path> pointing to the SPA build output`);
    process.exit(1);
  }

  console.log(`[winkgo-web] version    : ${version}`);
  console.log(`[winkgo-web] data dir   : ${dataDir}`);
  console.log(`[winkgo-web] log dir    : ${logDir}`);
  console.log(`[winkgo-web] static dir : ${staticDir}`);
  console.log(`[winkgo-web] backend bin: ${backendBin}`);
  console.log(`[winkgo-web] launching  : port=${port} allowRemote=${allowRemote}`);
  ...
  if (!backendAvailable) {
    ...
    console.warn('   To enable backend: download winkgo_core and set WINKGO_BACKEND_BIN.');
    console.warn('');
    ...
    console.log('');
    console.log('WinkGo WebUI (frontend only) is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
    if (autoOpenBrowser) {
      const openResult = openBrowserUrl(handle.localUrl);
      if (openResult.ok) {
        console.log(`[winkgo-web] opened ${handle.localUrl} in your browser.`);
      } else {
        console.warn(`[winkgo-web] could not open the browser automatically: ${openResult.reason}`);
      }
    }
    console.log('');
    ...
    console.log('');
    console.log('WinkGo WebUI is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    shuttingDown = true;
    console.log(`\n[winkgo-web] received ${signal}, stopping...`);
    try {
      if (currentHandle) await currentHandle.stop();
    } catch (err) {
      console.error('[winkgo-web] stop failed:', err);
    }
    process.exit(0);
  };
...
/**
 * `winkgo-web resetpass` — spin up the backend just long enough to POST
...
async function runResetPassword(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(backendBin)) {
    console.error(`[winkgo-web] backend binary not found: ${backendBin}`);
    console.error('  hint: pass --backend-bin <path> or set WINKGO_BACKEND_BIN');
    process.exit(1);
  }
  const dataDir = resolveDataDir(flags);
  fs.mkdirSync(dataDir, { recursive: true });
  ...
  console.log(`[winkgo-web] resetting admin password in ${dataDir}`);
  ...
  try {
    ...
    if (!ready) {
      console.error('[winkgo-web] backend did not become ready within 15s');
      process.exit(1);
    }
    ...
    if (!res.ok) {
      console.error(`[winkgo-web] /api/webui/reset-password returned ${res.status}`);
      process.exit(1);
    }
    ...
    if (!newPassword) {
      console.error('[winkgo-web] reset-password response missing new_password');
      process.exit(1);
    }
    console.log(`[winkgo-web] username: ${username}`);
    console.log(`[winkgo-web] new password: ${newPassword}`);
    console.log('[winkgo-web] existing sessions have been invalidated.');
  } finally {
    try {
      await handle.stop();
    } catch {
...
async function main(): Promise<void> {
  ...
  if (command === '--help' || command === 'help' || command === '-h') {
    console.log(`Usage: winkgo-web <command> [options]
...
Options for start:
  ...
  --data-dir <path>       Override data dir (default: ~/.winkgo-web)
...
Options for resetpass:
  --data-dir <path>       Which data dir to reset (default: ~/.winkgo-web)
  --backend-bin <path>    Override backend binary path

Environment variables:
  WINKGO_PORT, WINKGO_ALLOW_REMOTE, WINKGO_DATA_DIR, WINKGO_LOG_DIR,
  WINKGO_BACKEND_BIN, WINKGO_OPEN_BROWSER
`);
    return;
  }
  ...
  if (command !== 'start') {
    ...
    console.error('Usage: winkgo-web [start|resetpass|version|help]');
    process.exit(1);
  }
...
main().catch((err: Error) => {
  console.error('[winkgo-web] fatal:', err.message);
