...
open -a WinkGo --args --webui
```

### Method 2: Create Shell Script

Create `start-winkgo-webui.sh`:
...
```bash
chmod +x start-winkgo-webui.sh
./start-winkgo-webui.sh
```
...
```bash
chmod +x ~/.local/share/applications/winkgo-webui.desktop
```
...
Create `~/bin/start-winkgo-webui.sh`:
...
```bash
chmod +x ~/bin/start-winkgo-webui.sh
```
...
Create `/etc/systemd/system/winkgo-webui.service`:
...
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/WinkGo/winkgo --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable winkgo-webui.service
sudo systemctl start winkgo-webui.service

# Check status
sudo systemctl status winkgo-webui.service
```
...
WinkGo --no-sandbox --webui --port 8080
```
...
```bash
proot-distro login ubuntu -- bash -c "WinkGo --no-sandbox --webui --remote"
```
...
```cmd
WinkGo.exe --webui --remote
```

### macOS

```bash
/Applications/WinkGo.app/Contents/MacOS/WinkGo --webui --remote
```
...
which winkgo
# or
find /opt -name "winkgo" 2>/dev/null
```
...
```powershell
& "C:\Program Files\WinkGo\WinkGo.exe" --webui 2>&1 | Tee-Object -FilePath winkgo.log
```

**macOS/Linux:**

```bash
/path/to/winkgo --webui 2>&1 | tee winkgo.log
```
...
export WINKGO_PORT=8080

# Allow remote access without passing --remote
export WINKGO_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as WINKGO_ALLOW_REMOTE=true)
export WINKGO_HOST=0.0.0.0

# Then start the application
winkgo --webui

# You can also pass the port directly via CLI
winkgo --webui --port 8080
```
...
| Platform | Location                                                 |
| -------- | -------------------------------------------------------- |
| Windows  | `%APPDATA%/WinkGo/webui.config.json`                     |
| macOS    | `~/Library/Application Support/WinkGo/webui.config.json` |
| Linux    | `~/.config/WinkGo/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\WinkGo\WinkGo.exe" --resetpass

# Or for a specific user
"C:\Program Files\WinkGo\WinkGo.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/WinkGo.app/Contents/MacOS/WinkGo --resetpass

# Or for a specific user
/Applications/WinkGo.app/Contents/MacOS/WinkGo --resetpass username
```
...
# Or for a specific user
winkgo --resetpass username

# Or using full path
/opt/WinkGo/winkgo --resetpass
```
