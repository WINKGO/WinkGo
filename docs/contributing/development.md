...
If `which winkgo-core` prints nothing, add `export PATH="$HOME/.cargo/bin:$PATH"` to your shell profile (`~/.zshrc`, `~/.bashrc`, or your shell's equivalent), open a new terminal, and verify again.
...
Use `--force` when rebuilding local changes with the same WINK GO Core package version; otherwise Cargo may keep the already installed binary.
...
Use the Rust MSVC toolchain and install Microsoft C++ Build Tools. After installing or changing toolchains, open a new PowerShell window and rerun the WINK GO Core install command.
