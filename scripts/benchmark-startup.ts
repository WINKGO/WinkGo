 ...
 * parsing the electron-log file for [WinkGo:ready] / [WinkGo:init] /
 * [WinkGo:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [WinkGo:ready] marks
  ...
  // Parsed from [WinkGo:init] marks
  initTotalMs: number;
  // Parsed from [WinkGo:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'WinkGo-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'WinkGo', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'WinkGo', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'WinkGo', 'logs', `${today}.log`));
  }
...
// Matches: [WinkGo:ready] <label> +<ms>ms
// Matches: [WinkGo:init]  <label> +<ms>ms
// Matches: [WinkGo:process] <label> +<ms>ms
const MARK_REGEX = /\[WinkGo:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[WinkGo] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[WinkGo] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[WinkGo] Showing main window')) marks.logs.showingMainWindow = true;
  }
...
async function launchApp(timeoutMs: number, withMemory: boolean): Promise<ElectronApplication> {
  ...
  if (!fs.existsSync(mainEntry)) {
    ...
    execSync('npx electron-vite build', {
      cwd: projectRoot,
      stdio: 'inherit',
      env: { ...process.env, WINKGO_EDITION: 'free', WINKGO_ALLOW_PRO_DEV_BUILD: '0' },
    });
  }
