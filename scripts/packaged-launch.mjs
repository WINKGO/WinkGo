...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('WINK-GO.exe');
    await killProcessByName('WinkGo');
  ...
  const env = {
    ...process.env,
    WINKGO_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] WINKGO_EXTENSIONS_PATH: ${env.WINKGO_EXTENSIONS_PATH}`);
