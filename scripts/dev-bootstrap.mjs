...
const KILLABLE_NAMES = new Set(['electron', 'winkgo', 'winkgo.exe']);
...
function listLikelyConflictingProcesses() {
  try {
    if (isWindows()) {
      const output = run(
        "powershell -NoProfile -Command \"Get-Process | Where-Object { $_.ProcessName -in @('electron','WinkGo','node','bun') } | Select-Object ProcessName,Id | ConvertTo-Json -Compress\""
      );
      const parsed = output ? JSON.parse(output) : [];
      return Array.isArray(parsed) ? parsed : [parsed];
    }

    const output = run(`ps -A -o pid=,comm= | egrep "electron|WinkGo|node|bun" || true`);
    return output
      .split(/\r?\n/)
      .filter(Boolean)
      .map((line) => {
...
function cleanupByName() {
  ...
  for (const proc of processes) {
    ...
    if (!['electron', 'winkgo'].some((k) => rawName.includes(k))) continue;
    if (killPid(pid)) {
...
function launch(scriptName, withExtensions) {
  ...
  if (withExtensions) {
    env.WINKGO_EXTENSIONS_PATH = path.resolve(process.cwd(), 'examples');
    log(`WINKGO_EXTENSIONS_PATH=${env.WINKGO_EXTENSIONS_PATH}`);
  }
