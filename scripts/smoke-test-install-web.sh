...
export INSTALL_DIR="/tmp/winkgo-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/winkgo-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/winkgo-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/winkgo-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/winkgo-web"
    exit 1
fi
...
VERSION_OUTPUT=$(winkgo-web version 2>&1 || echo "")
