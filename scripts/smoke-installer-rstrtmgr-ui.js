...
function main() {
  ...
  const root = mkdtempSync(path.join(tmpdir(), 'winkgo-rm-ui-'));
  const installDir = path.join(root, 'install-dir');
  mkdirSync(installDir, { recursive: true });
  const lockedFile = path.join(installDir, 'locked-by-smoke.txt');
  writeFileSync(lockedFile, 'WinkGo Restart Manager UI smoke lock\n', 'utf8');

  let locker = null;
  const nsiPath = path.join(root, 'winkgo-rstrtmgr-ui-smoke.nsi');
  const exePath = path.join(root, 'winkgo-rstrtmgr-ui-smoke.exe');
  const logPath = path.join(
    process.env.TEMP || tmpdir(),
    `winkgo-installer-smoke-${new Date().toISOString().replace(/[-:]/g, '').replace(/\..+$/, '').replace('T', '-')}.log`
  );
  ...
  const nsi = `
Unicode true
Name "WinkGo Restart Manager UI Smoke"
OutFile "${nsisQuote(exePath)}"
RequestExecutionLevel user
SilentInstall normal
!define WINKGO_FALLBACK_LOG "winkgo-installer-smoke-fallback.log"
...
Section
  ...
  StrCpy $WinkGoSessionId "rstrtmgrui"
  StrCpy $WinkGoIsUpdated "1"
  InitPluginsDir
  BringToFront

  winkgo_query_lockers:
    !insertmacro WINKGO_QUERY_LOCKERS "${nsisQuote(lockedFile)}" $WinkGoLockerResult
