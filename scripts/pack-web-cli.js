...
const tarballName = `winkgo-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
// 1. Prepare bundled-winkgo-core
console.log('1. Preparing winkgo_core...');
prepareWinkGoCore({
  projectRoot,
  platform,
  arch,
...
const tarballContentDir = path.join(stagingDir, 'winkgo-web');
...
const executableName = platform === 'win32' ? 'winkgo-web.exe' : 'winkgo-web';
...
// `winkgo-web version` match the tarball filename.
...
// 7. Copy bundled-winkgo-core
const backendSrc = path.join(projectRoot, 'resources/bundled-winkgo-core', `${platform}-${arch}`);
const backendDest = path.join(tarballContentDir, 'bundled-winkgo-core', `${platform}-${arch}`);
if (!fs.existsSync(backendSrc)) {
  throw new Error(`Backend bundle dir missing at ${backendSrc}. Ensure prepareWinkGoCore succeeded.`);
}
