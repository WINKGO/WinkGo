...
if [ ! -d "$TEMP_DIR/winkgo-web" ]; then
  echo "❌ Missing winkgo-web directory"
  exit 1
fi

cd "$TEMP_DIR/winkgo-web"

# New layout (bun compile standalone binary):
#   winkgo-web/
#   ├── winkgo-web           ← single compiled executable (no bin/, no dist/, no node_modules)
#   ├── package.json         ← for version lookup
#   ├── static/              ← SPA assets
#   └── bundled-winkgo-core/<plat-arch>/...
for dir in static bundled-winkgo-core; do
  if [ ! -d "$dir" ]; then
...
for file in legal/LICENSE legal/NOTICE legal/THIRD_PARTY_NOTICES.md legal/THIRD_PARTY_DEPENDENCIES.json legal/THIRD_PARTY_LICENSES.txt legal/PRIVACY.md legal/TERMS.md; do
  if [ ! -s "$file" ]; then
...
grep -q "OfficeCLI" legal/THIRD_PARTY_NOTICES.md || { echo "❌ Third-party notices are missing OfficeCLI attribution"; exit 1; }
grep -q "Apache License 2.0" legal/THIRD_PARTY_NOTICES.md || { echo "❌ Third-party notices are missing the license marker"; exit 1; }
grep -q "Privacy Policy" legal/PRIVACY.md || { echo "❌ Privacy policy is invalid"; exit 1; }
grep -q "Terms of Service" legal/TERMS.md || { echo "❌ Terms of service are invalid"; exit 1; }

# 3. Check executable
echo ""
echo "3. Checking executable..."
if [ ! -x "winkgo-web" ]; then
  echo "❌ winkgo-web is not executable"
  exit 1
fi
echo "✓ winkgo-web is executable"

# 4. Test version command
echo ""
echo "4. Testing version command..."
VERSION=$(./winkgo-web version)
if [ -z "$VERSION" ]; then
...
BACKEND_DIR="bundled-winkgo-core/$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m | sed 's/aarch64/arm64/; s/x86_64/x64/')"
BACKEND_BINARY="$BACKEND_DIR/winkgo_core"
if [ ! -x "$BACKEND_BINARY" ]; then
  echo "❌ Backend binary missing or not executable: $BACKEND_BINARY"
  exit 1
fi
# winkgo_core has no --version flag. Read the pinned version from manifest.json
# (which prepareWinkGoCore writes at pack time) and use --help to confirm the
...
HTTP_PORT=25899
DATA_DIR="$(mktemp -d)/winkgo-web-data"
...
./winkgo-web start --port "$HTTP_PORT" --data-dir "$DATA_DIR" > /tmp/winkgo-web.log 2>&1 &
SERVER_PID=$!
...
for i in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:${HTTP_PORT}/" > /tmp/winkgo-web.html 2>/dev/null; then
    break
  fi
  sleep 1
done

if [ ! -s /tmp/winkgo-web.html ]; then
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP probe failed — no response body. Server log:"
  cat /tmp/winkgo-web.log
  exit 1
fi

# Look for the SPA shell signature — <html + <div id="root" or similar marker
if grep -q '<html' /tmp/winkgo-web.html && grep -qE '<(div id="root"|script)' /tmp/winkgo-web.html; then
  echo "✓ HTTP root returns SPA index ($(wc -c < /tmp/winkgo-web.html) bytes)"
else
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP root response does not look like SPA index:"
  head -20 /tmp/winkgo-web.html
  echo "---server log---"
  cat /tmp/winkgo-web.log
  exit 1
fi
...
if grep -q 'Backend binary not found' /tmp/winkgo-web.log; then
  ...
  for i in $(seq 1 20); do
    PASSWORD=$(grep -oE 'Generated initial admin password: [^ ]+' /tmp/winkgo-web.log | head -1 | sed 's/^Generated initial admin password: //')
    if [ -n "$PASSWORD" ]; then
      break
    fi
    sleep 1
