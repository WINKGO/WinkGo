...
$logPath = Join-Path $env:TEMP "winkgo-installer-smoke-$runId-log.jsonl"
...
@(
  ...
  '{"schemaVersion":1,"event":"failure","session":"smoke","version":"2.1.28","updated":true,"code":"E1003","phase":"atomic-failed","failedPath":"C:\\Users\\huang\\AppData\\Local\\Programs\\WinkGo","blockingProcesses":[],"fallbackReason":"restart-manager-no-process","message":"Windows did not identify a specific locking process. Close terminals, editors, and file managers opened in the install folder."}'
...
- File or folder: C:\Users\huang\AppData\Local\Programs\WinkGo
...
if ($status.copyText -notlike '*WinkGo installer failure E1003*') {
  throw 'copyText does not include failure heading'
}
if ($status.copyText -notlike '*To WinkGo Team*') {
