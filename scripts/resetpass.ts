 ...
 *   2. No webui running → spawn a short-lived winkgo_core against the same
 ...
 *   WINKGO_DATA_DIR=/x bun run resetpass
...
import { startBackend, stopBackend } from '@winkgo/web-host';
 ...
 * See the comment there for why the default is `~/.winkgo-web*` (not `~/.winkgo*`).
 */
 ...
 * desktop side). Callers can override with `--port` / `WINKGO_PORT` to match
 ...
 * pass --port / WINKGO_PORT if they launched webui on a non-default one.
 */
