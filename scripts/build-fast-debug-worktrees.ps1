param(
  ...
  [string]$WorktreeRoot = (Join-Path $PSScriptRoot '..\..\winkgo-build-worktrees'),
...
function New-BuildCommandFile([string]$WorktreePath, [string]$Version, [string]$Dsn, [string]$LocalWinkGoCoreBinary, [string]$LocalWinkGoCoreBundleDir) {
  ...
  $lines = @(
    ...
    '$env:WINKGO_DEBUG_AUTO_UPDATE_CURRENT_VERSION = ''' + $Version + '''',
    '$env:ELECTRON_BUILDER_COMPRESSION_LEVEL = ''1''',
    '$env:WINKGO_BACKEND_LOCAL_BINARY = ''' + ($LocalWinkGoCoreBinary -replace "'", "''") + '''',
    '$env:WINKGO_BACKEND_LOCAL_BUNDLE_DIR = ''' + ($LocalWinkGoCoreBundleDir -replace "'", "''") + '''',
...
function Start-BuildProcess([string]$WorktreePath, [string]$Version, [string]$Dsn, [string]$LogDir, [string]$LocalWinkGoCoreBinary, [string]$LocalWinkGoCoreBundleDir) {
  $scriptPath = New-BuildCommandFile $WorktreePath $Version $Dsn $LocalWinkGoCoreBinary $LocalWinkGoCoreBundleDir
...
$localWinkGoCoreBinary = Join-Path $repoRoot 'resources\bundled-winkgo-core\win32-x64\winkgo_core.exe'
$localWinkGoCoreBundleDir = Join-Path $repoRoot 'out\win-unpacked\resources\bundled-winkgo-core\win32-x64'
if (-not (Test-Path -LiteralPath (Join-Path $localWinkGoCoreBundleDir 'managed-resources') -PathType Container)) {
  $localWinkGoCoreBundleDir = Join-Path $env:LOCALAPPDATA 'Programs\WinkGo\resources\bundled-winkgo-core\win32-x64'
}
if (Test-Path -LiteralPath (Join-Path $localWinkGoCoreBundleDir 'managed-resources') -PathType Container) {
  $localWinkGoCoreBundleDir = (Resolve-Path -LiteralPath $localWinkGoCoreBundleDir).Path
  Write-Host "=== using local winkgo_core bundle fallback: $localWinkGoCoreBundleDir ==="
} else {
  $localWinkGoCoreBundleDir = ''
  Write-Warning 'Local winkgo_core bundle fallback was not found; builds may need to prepare managed resources.'
}
if (Test-Path -LiteralPath $localWinkGoCoreBinary -PathType Leaf) {
  $localWinkGoCoreBinary = (Resolve-Path -LiteralPath $localWinkGoCoreBinary).Path
  Write-Host "=== using local winkgo_core fallback: $localWinkGoCoreBinary ==="
} else {
  $localWinkGoCoreBinary = ''
  Write-Warning 'Local winkgo_core fallback was not found; builds may need to download winkgo_core.'
}
$worktrees = @()
$builds = @()
$results = @()
