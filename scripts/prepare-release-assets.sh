...
#
# Usage:
#   ./scripts/prepare-release-assets.sh [ARTIFACTS_DIR] [OUTPUT_DIR] [VERSION]
#
# Defaults:
...
VERSION_OVERRIDE="${3:-${MOCK_VERSION:-}}"

PRO_ARTIFACTS=$(find "$ARTIFACTS_DIR" -type f -iname 'WINK-GO-Pro-*' -print || true)
if [ -n "$PRO_ARTIFACTS" ]; then
  echo "::error::Pro release artifact is forbidden in the public release pipeline:"
  echo "$PRO_ARTIFACTS"
  exit 1
fi
...
while IFS= read -r file; do
  ...
  -name "winkgo-web-*.tar.gz" -o \
  -name "winkgo-web-*.tar.gz.sha256" \
\) | sort)
