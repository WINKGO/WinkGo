 ...
 *
 * Downloads the WinkGoHub index.json and all extension zip packages
 ...
 *   WINKGO_HUB_TAG    - Git tag to fetch from (default: 'dist-latest')
 *   WINKGO_HUB_SKIP   - Set to '1' to skip hub resource preparation
 */
...
async function prepareHubResources() {
  if (process.env.WINKGO_HUB_SKIP === '1') {
    console.log('[hub] Skipping hub resource preparation (WINKGO_HUB_SKIP=1)');
    return { skipped: true };
  }

  const tag = process.env.WINKGO_HUB_TAG || DEFAULT_TAG;
