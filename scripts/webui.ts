 ...
 *
 * Env vars:
 *   WINKGO_PORT           : static server port (default 33000)
 *   WINKGO_HOST           : listen host; set to 0.0.0.0 to imply --remote
 *   WINKGO_ALLOW_REMOTE   : "1"/"true" to expose to LAN
 *   WINKGO_DATA_DIR       : override userData path (default Electron-compatible)
 *   WINKGO_LOG_DIR        : override log dir (default <dataDir>/logs)
 *   WINKGO_STATIC_DIR     : override static dir (default out/renderer)
 *   WINKGO_BACKEND_BIN    : absolute path to winkgo_core binary (else PATH lookup)
 *   WINKGO_BACKEND_BUNDLED_DIR : dir containing bundled-winkgo-core/<plat-arch>/binary
 *   WINKGO_OPEN_BROWSER   : "1"/"true" to force open, "0"/"false" to disable
 */
...
/**
 * Resolve the directory where winkgo_core persists its SQLite DB.
 *
 * `bun run webui` runs **independently of the Electron desktop app** — it must
 * work on hosts that never installed WinkGo.app, and its default work dir must
 * NOT collide with Electron's.
 *
 *   --data-dir <path>       CLI override (highest priority)
 *   $WINKGO_DATA_DIR        env override (same effect)
 *   otherwise               ~/.winkgo-web         (production)
 *                           ~/.winkgo-web-dev     (dev, default)
 *                           ~/.winkgo-web-dev-2   (dev + WINKGO_MULTI_INSTANCE=1)
 *
 * Why a dedicated `-web` name, not the same `~/.winkgo[-dev]` that Electron
 * uses: on macOS, Electron's getDataPath() (packages/desktop/src/process/utils/
 * utils.ts) creates `~/.winkgo-dev` as a **symlink** to
 * `~/Library/Application Support/WinkGo-Dev/winkgo` so CLI tools (claude,
 ...
 * `.winkgo-web` keeps standalone webui's data dir off of the path Electron's
 * symlink needs.
 *
 * If the user wants the two to share data they opt-in explicitly via
 *   --data-dir ~/.winkgo-dev                     (or equivalent on other OSes)
...
function resolveBackendDataDir(): string {
  const override = getFlag('--data-dir') ?? process.env.WINKGO_DATA_DIR;
  ...
  const suffix =
    process.env.NODE_ENV === 'production' ? '' : process.env.WINKGO_MULTI_INSTANCE === '1' ? '-dev-2' : '-dev';
  const dir = path.join(os.homedir(), `.winkgo-web${suffix}`);
...
function resolveAllowRemote(): boolean {
  ...
  const host = process.env.WINKGO_HOST?.trim();
  if (host && ['0.0.0.0', '::', '::0'].includes(host)) return true;
  return parseBoolean(process.env.WINKGO_ALLOW_REMOTE ?? process.env.WINKGO_REMOTE);
}

function resolveStaticDir(): string {
  if (process.env.WINKGO_STATIC_DIR) return process.env.WINKGO_STATIC_DIR;
  const candidate = path.join(repoRoot, 'out', 'renderer');
  if (fs.existsSync(path.join(candidate, 'index.html'))) return candidate;
  throw new Error(`Renderer assets not found at ${candidate}. Run "bun run package" first, or set WINKGO_STATIC_DIR.`);
}
 ...
 *   $WINKGO_NO_BUILD=1        : env-level opt-out
 *   $WINKGO_STATIC_DIR is set : caller is pointing us at a prebuilt artifact dir
 */
function runPackageIfNeeded(): void {
  if (has('--no-build')) return;
  if (parseBoolean(process.env.WINKGO_NO_BUILD)) return;
  if (process.env.WINKGO_STATIC_DIR) return;
...
function resolveBackendBinary(): string {
  if (process.env.WINKGO_BACKEND_BIN) return process.env.WINKGO_BACKEND_BIN;

  const bundledBase = process.env.WINKGO_BACKEND_BUNDLED_DIR ?? path.join(repoRoot, 'resources', 'bundled-winkgo-core');
  ...
  throw new Error(
    `Cannot find "${BACKEND_BINARY}". Set WINKGO_BACKEND_BIN, put it on PATH, or place it at ${bundled}.`
  );
}
...
async function main(): Promise<void> {
  ...
  // This keeps `bun run webui` fully self-contained on hosts without WinkGo.app.
  const workDir = resolveBackendDataDir();
  const staticDir = resolveStaticDir();
  const backendBin = resolveBackendBinary();
  const logDir = process.env.WINKGO_LOG_DIR ?? path.join(workDir, 'logs');
