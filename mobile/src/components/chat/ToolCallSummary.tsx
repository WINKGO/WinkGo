...
function getStepItems(msg: TMessage): StepItem[] {
  if (msg.type === 'tool_group' && Array.isArray(msg.content)) {
    return msg.content.map((tool: any) => ({
      name: tool.description || tool.name || i18n.t('chat.toolCall'),
  ...
  if (msg.type === 'tool_call') {
    return [
      {
        name: msg.content?.name || i18n.t('chat.toolCall'),
  ...
  if (msg.type === 'acp_tool_call') {
    ...
    return [
      {
        name: update?.title || update?.kind || i18n.t('chat.toolCall'),
  ...
  if (msg.type === 'codex_tool_call') {
    return [
      {
        name: msg.content?.title || msg.content?.description || msg.content?.kind || i18n.t('chat.toolCall'),
        status: normalizeStatus(msg.content?.status),
      },
    ];
  }
  return [{ name: i18n.t('chat.toolCall'), status: 'pending' }];
}
