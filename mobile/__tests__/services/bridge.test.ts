...
function lastRequestId(): string {
  ...
  return lastCall?.[1]?.id as string;
}
...
describe('BridgeService', () => {
  ...
  describe('emit', () => {
    it('sends a fire-and-forget message via WebSocket', () => {
      ...
      expect(mockSend).toHaveBeenCalledWith('subscribe-chat:send', {
        id: expect.stringMatching(/^m_\d+_[a-z0-9]+$/),
        data: { text: 'hello' },
      });
    });
  ...
  describe('request', () => {
    it('sends request and resolves when server responds', async () => {
      ...
      simulateMessage(`subscribe.callback-get:sessions${requestId}`, [{ id: 1 }]);
    ...
    it('sends request with data', async () => {
      ...
      simulateMessage(`subscribe.callback-create:session${requestId}`, { id: 2 });
    ...
    it('cleans up listener after response', async () => {
      ...
      const requestId = lastRequestId();
      simulateMessage(`subscribe.callback-cleanup-test${requestId}`, 'result');
      await promise;
