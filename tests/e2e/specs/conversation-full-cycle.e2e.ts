...
async function selectCronDialogAgentByPattern(
  page: import('@playwright/test').Page,
  dialog: import('@playwright/test').Locator,
  preferredPatterns = [/Gemini/i, /Claude/i, /Codex/i, /WinkGo/i]
): Promise<string | null> {
...
test.describe('Conversation Full Cycle', () => {
  ...
  test('cron -- create task with assistant, verify detail, then delete', async ({ page }) => {
    ...
    const assistantOptions = page
      ...
      .filter({ hasText: /Claude|Codex|Gemini|WinkGo/ });
  ...
  const cronConversationAgents = ['claude', 'codex', 'gemini', 'winkgo_agent', 'opencode'] as const;
    ...
    test(`cron -- ${backend} conversation skill creates task with full-auto job mode`, async ({ page }) => {
      ...
      try {
        ...
        if (backend === 'winkgo_agent') {
          const preconditions = await resolveWinkGoAgentPreconditions(page);
