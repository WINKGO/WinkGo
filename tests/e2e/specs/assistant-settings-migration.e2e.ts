 ...
 * 1. seed a legacy `winkgo.db` using the pre-unification assistants schema
...
function resolveBackendBinary(): string {
  ...
  const candidates = [
    ...
    path.join(projectRoot, 'backend', 'target', 'debug', binaryName),
  ...
  throw new Error('winkgo_core binary not found for migration e2e');
}

function schemaPath(): string {
  return path.join(process.cwd(), 'backend', 'crates', 'winkgo-db', 'migrations', '001_initial_schema.sql');
}

function querySqliteValue(dataDir: string, sql: string): string {
  const dbPath = path.join(dataDir, 'winkgo-backend.db');
  return execFileSync('sqlite3', ['-readonly', dbPath, sql], { encoding: 'utf8' }).trim();
}

function seedLegacyDatabase(dataDir: string): void {
  const legacyDbPath = path.join(dataDir, 'winkgo.db');
...
test.describe('Assistant Settings Migration', () => {
  ...
  async function startBackend(): Promise<void> {
    ...
    backend = spawn(binary, ['--local', '--port', String(MIGRATION_BACKEND_PORT), '--data-dir', dataDir], {
      ...
      env: {
        ...process.env,
        RUST_LOG: 'warn',
        WINKGO_EXTENSIONS_PATH: path.join(process.cwd(), 'examples'),
      },
    });
    try {
  ...
  test.beforeEach(async () => {
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'winkgo-assistant-migration-'));
  ...
  test('legacy user assistant migrates into phase-1 defaults without losing existing config', async () => {
    ...
    expect(migrated?.preset_agent_type).toBe('winkgo_agent');
    ...
    expect(detail.engine.agent_backend).toBe('winkgo_agent');
