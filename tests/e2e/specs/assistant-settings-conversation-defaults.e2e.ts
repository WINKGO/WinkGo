...
import { getWinkGoAgentTestModels, type TProviderWithModel } from '../helpers/chatWinkGoAgent';
...
async function ensureWinkGoAgentTestModels(page: Page): Promise<EnsuredWinkGoAgentModels> {
  const existing = await getWinkGoAgentTestModels(page);
  if (existing?.modelA) {
    return {
...
function locateConversationModelButton(page: Page) {
  return page
    .locator('[data-testid="winkgo_agent-model-selector"], [data-testid="chat-model-selector"], .header-model-btn')
    .first();
}
...
test.describe('Assistant Settings Conversation Defaults', () => {
  ...
  test('fixed defaults are written into conversation assistant snapshots on create', async ({ page, electronApp }) => {
    ...
    try {
      ...
      const dbPath = path.join(userDataPath, 'winkgo', 'winkgo-backend.db');
  ...
  test('switching from auto to fixed seeds preferences and new conversations use fixed defaults', async ({
    ...
    const winkgo_agentModels = await ensureWinkGoAgentTestModels(page);
    ...
    try {
      await httpInvoke(page, 'PUT', `/api/assistants/${assistantId}`, {
        ...
        defaults: {
          model: {
            mode: 'fixed',
            value: winkgo_agentModels.modelA.useModel,
          },
          permission: {
            mode: 'fixed',
            value: 'default',
      ...
      await httpDelete(page, `/api/assistants/${assistantId}`).catch(() => {});
      if (winkgo_agentModels.cleanupProviderId) {
        await httpDelete(page, `/api/providers/${winkgo_agentModels.cleanupProviderId}`).catch(() => {});
      }
    }
  });
