...
/** Backend /api/teams/:id GET response shape — aligns with winkgo_core schema. */
  ...
  test('describes a preset and then spawns it as a teammate via the real TCP bridge', async ({ page }) => {
    ...
    try {
      ...
      try {
        ...
        expect(describeText).toContain('Backend: winkgo_agent');
        ...
        expect(spawned!.assistant_backend ?? spawned!.backend).toBe('winkgo_agent'); // preset backend wins
      } finally {
        client.close();
      }
    } finally {
