...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'winkgo-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'winkgo-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      WINKGO_EXTENSIONS_PATH: emptyExtensionsDir,
      WINKGO_EXTENSION_STATES_FILE: extensionStatesFile,
