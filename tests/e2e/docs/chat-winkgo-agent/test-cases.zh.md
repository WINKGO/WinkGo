# WinkGo CLI (winkgo_agent) E2E 测试用例

**版本**: Gate 2 初稿
**作者**: chat-winkgo_agent-designer
**日期**: 2026-04-22
**状态**: 待审核
...
**所有 winkgo_agent 测试用例的前置条件**:

1. **winkgo_agent binary 可用**：通过 `ipcBridge.fs.findWinkGoAgentBinary.invoke()` 验证，否则 skip 全部测试
2. **用户配置的模型列表至少 1 个可用 provider**：
   ...
   - 若无可用 provider：skip 全部 winkgo_agent 测试，原因："No non-Google-Auth provider with apiKey configured"

3. **测试数据准备**：临时工作目录 `/tmp/e2e-winkgo_agent-<timestamp>/`
...
| 维度       | 可选值                     | 说明                                                                                               |
| ---------- | -------------------------- | -------------------------------------------------------------------------------------------------- |
| 关联文件夹 | 无 / 单 / 多               | `atPath` 数组（源码：`WinkGoAgentSendBox.tsx:331-337`）                                            |
...
| 权限       | default / auto_edit / yolo | 来源：winkgo_agent runtime capabilities                                                            |
| 对话中操作 | 切换模型 / 切换权限 / 无   | 对话页 `WinkGoAgentModelSelector` / `AgentModeSelector`                                            |

### 1.4 清理约定

**命名模式**: 所有测试对话命名为 `E2E-winkgo_agent-<timestamp>-<scenario>`

**清理顺序**（每个用例 `afterEach` 执行）:

1. 停止 binary 进程：`ipcBridge.conversation.stopAgent.invoke(conversationId)`
2. 删除 DB 记录：`DELETE FROM conversations WHERE name LIKE 'E2E-winkgo_agent-%'`（级联删除 messages）
3. 删除临时目录：`fs.rm('/tmp/e2e-winkgo_agent-*', { recursive: true })`
4. 清理 sessionStorage：`sessionStorage.removeItem('winkgo_agent_initial_message_*')` + `sessionStorage.removeItem('winkgo_agent_initial_processed_*')`
...
- 同 §1.2 全局前置条件
- 调用 `getWinkGoAgentTestModels(page)` 获取 `modelA`（默认模型）
...
2. 选择 winkgo_agent agent（点击 `[data-agent-backend="winkgo_agent"]`）
3. 确认权限选择器显示 `default`（`AgentModeSelector` 默认值）
4. 输入测试消息："Hello, winkgo_agent! Please list files in current directory."
5. 点击发送按钮
6. 等待跳转至对话页（URL 匹配 `/conversation/winkgo_agent/*`）
...
FROM conversations
WHERE name LIKE 'E2E-winkgo_agent-%' AND id = ?;
-- 期望: type='winkgo_agent', mode='default', status='finished'

-- 2. 验证用户消息
SELECT id, type, position, content, status
FROM messages
WHERE conversation_id = ? AND position = 'right';
-- 期望: type='text', position='right', json_extract(content, '$.content') 包含 "Hello, winkgo_agent!"
...
- conversations name: `E2E-winkgo_agent-<timestamp>-minimal-path`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>-tc-a-01/`（workspace）
- sessionStorage: `winkgo_agent_initial_message_${conversationId}`, `winkgo_agent_initial_processed_${conversationId}`
...
- 同 §1.2 全局前置条件
- 临时工作目录存在测试文件夹：`/tmp/e2e-winkgo_agent-<timestamp>/test-folder/`
...
1. 创建临时目录 + 测试文件夹：
   ```bash
   mkdir -p /tmp/e2e-winkgo_agent-<timestamp>/test-folder/
   echo "sample content" > /tmp/e2e-winkgo_agent-<timestamp>/test-folder/sample.txt
   ```
2. 打开 guid 页，选择 winkgo_agent agent
3. 从文件树选择 `test-folder/`（触发 `emitter.emit('winkgo_agent.selected.file', [{ path, name, isFile: false }])`）
...
- conversations name: `E2E-winkgo_agent-<timestamp>-folder-single`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>/`（包含 test-folder）
...
- 同 §1.2 全局前置条件
- 测试文件存在：`/tmp/e2e-test-file.txt`（内容："Test file content for winkgo_agent E2E"）
...
1. 创建测试文件：
   ```bash
   echo "Test file content for winkgo_agent E2E" > /tmp/e2e-test-file.txt
   ```
2. 打开 guid 页，选择 winkgo_agent agent
3. 上传文件：
...
- conversations name: `E2E-winkgo_agent-<timestamp>-file-single`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>/`（workspace）
...
- 同 §1.2 全局前置条件
- 调用 `getWinkGoAgentTestModels(page)` 获取 `{ modelA, modelB }`
...
1. 打开 guid 页，选择 winkgo_agent agent
...
- conversations name: `E2E-winkgo_agent-<timestamp>-model-second`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
1. 打开 guid 页，选择 winkgo_agent agent
2. 打开权限选择器（`AgentModeSelector`，`data-testid="agent-mode-selector-winkgo_agent"`）
...
- conversations name: `E2E-winkgo_agent-<timestamp>-yolo-mode`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>/`（包含 binary 创建的 test.txt）
- sessionStorage: 同 TC-A-01
...
- conversations name: `E2E-winkgo_agent-<timestamp>-auto-edit-mode`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
2. 等待首条 AI 回复完成
3. 点击对话页模型选择器（`WinkGoAgentModelSelector`，`data-testid="winkgo_agent-model-selector"`）
...
- conversations name: `E2E-winkgo_agent-<timestamp>-switch-model`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
- conversations name: `E2E-winkgo_agent-<timestamp>-switch-permission-1`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
- conversations name: `E2E-winkgo_agent-<timestamp>-switch-permission-2`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
1. 创建测试数据：
   ```bash
   mkdir -p /tmp/e2e-winkgo_agent-<timestamp>/folder-a/
   echo "content A" > /tmp/e2e-winkgo_agent-<timestamp>/folder-a/file-a.txt
...
- conversations name: `E2E-winkgo_agent-<timestamp>-combo-folder-file-model`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>/`
...
- conversations name: `E2E-winkgo_agent-<timestamp>-multi-files`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>/`
...
1. 创建测试数据：
   ```bash
   mkdir -p /tmp/e2e-winkgo_agent-<timestamp>/folder-x/ /tmp/e2e-winkgo_agent-<timestamp>/folder-y/
   echo "X content" > /tmp/e2e-winkgo_agent-<timestamp>/folder-x/x.txt
   echo "Y content" > /tmp/e2e-winkgo_agent-<timestamp>/folder-y/y.txt
   ```
2. 打开 guid 页，选择 winkgo_agent agent
...
- conversations name: `E2E-winkgo_agent-<timestamp>-multi-folders`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>/`（包含 folder-x, folder-y）
- sessionStorage: 同 TC-A-01
...
- winkgo_agent binary **不可用**（通过环境变量 `WINKGO_CLI_PATH=/dev/null` 模拟）

**维度组合**: N/A（验证前置检查逻辑）

**操作步骤**:

1. 设置环境变量：`process.env.WINKGO_CLI_PATH = '/dev/null'`
2. 运行测试套件（或单个用例）
3. 验证测试被 skip（状态：skipped，原因："winkgo_agent binary not found"）
...
- 控制台输出 skip 原因："winkgo_agent binary not found, skipping E2E tests"
...
```typescript
// tests/e2e/helpers/chatWinkGoAgent.ts
export async function checkWinkGoAgentBinary(): Promise<boolean> {
  try {
...
test.beforeAll(async () => {
  const hasBinary = await checkWinkGoAgentBinary();
  if (!hasBinary) {
...
2. 手动触发文件夹选择事件（模拟选择不存在的路径）：
   ```typescript
   emitter.emit('winkgo_agent.selected.file', [
...
- conversations name: `E2E-winkgo_agent-<timestamp>-nonexistent-folder`
- temp dir: `/tmp/e2e-winkgo_agent-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
4. **并发对话场景**（暂缓至后续 Gate）：
   - 场景：同时打开 2 个 winkgo_agent 对话，轮流发送消息 → 验证 binary 进程隔离
...
1. **chat-winkgo_agent-engineer** review 本用例设计，评估实现工作量
...
| `src/renderer/pages/conversation/platforms/winkgo_agent/WinkGoAgentSendBox.tsx`          | 331-337  | `atPath` 状态（关联文件夹数组）                  |
| `src/renderer/pages/conversation/platforms/winkgo_agent/WinkGoAgentSendBox.tsx`          | 103-125  | 文件上传 handler                                 |
| `src/renderer/pages/conversation/platforms/winkgo_agent/WinkGoAgentSendBox.tsx`          | 206-212  | 发送消息（传递 `files` 参数）                    |
| `src/renderer/pages/conversation/platforms/winkgo_agent/useWinkGoAgentModelSelection.ts` | 36-40    | 过滤 Google Auth 模型                            |
| `src/renderer/pages/conversation/platforms/winkgo_agent/WinkGoAgentSendBox.tsx`          | —        | winkgo_agent runtime capabilities 转换为权限选项 |
| `backend/crates/winkgo-ai-agent/src/manager/winkgo_agent/agent.rs`                       | —        | 权限模式自动批准逻辑                             |
| `backend/crates/winkgo-ai-agent/src/manager/winkgo_agent/agent.rs`                       | —        | `set_mode()` 与会话 runtime 状态                 |
| `backend/crates/winkgo-ai-agent/src/manager/winkgo_agent/agent.rs`                       | —        | turn 结束、取消和等待逻辑                        |
...
FROM conversations
WHERE name LIKE 'E2E-winkgo_agent-%';
...
-- 清理所有 E2E 数据
DELETE FROM conversations WHERE name LIKE 'E2E-winkgo_agent-%';
```
