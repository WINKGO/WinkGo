...
- ✅ SQLite 数据库独立(`winkgo.db`)
- ✅ 扩展路径指向示例目录(`WINKGO_EXTENSIONS_PATH=examples/`)
...
// Returns:
// {
//   userSkillsDir: '~/.winkgo/skills/',     // My Skills 存储目录
...
- `/usr/local/share/winkgo-skills` — 系统级共享路径
...
- E2E 框架已设置 `WINKGO_EXTENSIONS_PATH=examples/`(`fixtures.ts:112`)
...
```typescript
// Setup: 创建临时目录
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'winkgo-e2e-custom-'));
...
| `auto-skill-card-${normalizeTestId(skill.name)}`      | 自动注入技能卡片            | line ~520 `<div className="skill-card">`        | `auto-skill-card-winkgo-skills`       |
