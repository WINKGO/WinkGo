...
```typescript
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'winkgo-e2e-state-'));
...
```typescript
WINKGO_EXTENSION_STATES_FILE: process.env.WINKGO_EXTENSION_STATES_FILE || e2eStateFile;
```
...
**Critical bottleneck**: `workers: 1` enforced + shared `winkgo.db` → parallel execution impossible without refactoring.
...
1. Extend `WINKGO_E2E_TEST` to include worker ID:
   ```typescript
   WINKGO_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
2. Modify `getDevAppName()` to return worker-specific name:
   ```typescript
   const workerId = process.env.WINKGO_E2E_TEST_WORKER_ID || '0';
   return `WinkGo-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/WinkGo-E2E-Worker-0/config/winkgo.db`
   - `~/Library/Application Support/WinkGo-E2E-Worker-1/config/winkgo.db`
...
**Complexity**: High (requires main process to read `WINKGO_DATABASE_PATH`, conflicts with userData convention)
