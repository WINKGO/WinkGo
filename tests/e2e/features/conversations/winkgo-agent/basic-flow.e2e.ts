/**
 * WinkGoAgent Chat E2E Tests - Basic Flow (P0 Priority)
 *
 * Test Cases Covered:
 ...
 *
 * Prerequisites:
 * - winkgo_agent binary available (via ipcBridge.fs.findWinkGoAgentBinary)
 * - User logged in
 * - At least 1 ACP model available (filtered Google Auth)
 *
 * Data-testid references:
 * - AgentPillBar: data-agent-backend="winkgo_agent"
 * - AgentModeSelector: data-testid="agent-mode-selector-winkgo_agent"
 * - WinkGoAgentSendBox: data-testid="winkgo_agent-sendbox"
 * - FileAttachButton: data-testid="winkgo_agent-attach-folder-btn"
 */
...
test.describe('WinkGoAgent Chat - Basic Flow (P0)', () => {
  // Set longer timeout for winkgo_agent tests (binary calls can be slow)
  test.setTimeout(240_000); // 4 minutes — allow 150s waitForWinkGoAgentReply + buffer

  let preconditions: { binary: string | null; models: WinkGoAgentTestModels | null };

  // Check winkgo_agent binary and provider availability before all tests
  ...
  test.afterEach(async ({ page }) => {
    ...
    await page.evaluate(() => {
      // Clear winkgo_agent-specific sessionStorage keys
  ...
  test('TC-A-01: should complete minimal conversation with no attachments', async ({ page }) => {
    ...
    const conversationName = `E2E-winkgo_agent-${timestamp}-minimal-path`;
    ...
    try {
      ...
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-01/01-guid-page-initial.png`);

      // Step 2: Create conversation via bridge (uses prioritized winkgo_agent-compatible provider)
      ...
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-01/02-conversation-created.png`);

      // Step 3: Send simple message
      await sendWinkGoAgentMessage(page, conversationId, 'Say hi in one word.');
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-01/03-message-sent.png`);

      // Step 4: Wait for AI reply
      await waitForWinkGoAgentReply(page, conversationId);
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-01/04-reply-completed.png`);
      ...
      expect(conversation.type).toBe('winkgo_agent');
      ...
      // Note: message.status is not set to 'finish' for winkgo_agent text messages (only conv.status matters)
  ...
  test('TC-A-02: should associate single folder and reference in message', async ({ page }) => {
    const timestamp = Date.now();
    const conversationName = `E2E-winkgo_agent-${timestamp}-folder-single`;
    ...
    try {
      ...
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-02/01-guid-page-before-create.png`);
      ...
      await sendWinkGoAgentMessage(page, conversationId, 'What files are in the attached folder?');

      // Screenshot 02: message sent
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-02/02-message-sent.png`);

      // Step 4: Wait for AI reply
      await waitForWinkGoAgentReply(page, conversationId);

      // Screenshot 03: AI reply completed
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-02/03-reply-completed.png`);
  ...
  test('TC-A-03: should upload single file and binary receives file parameter', async ({ page }) => {
    const timestamp = Date.now();
    const conversationName = `E2E-winkgo_agent-${timestamp}-file-single`;
    ...
    try {
      ...
      await fs.writeFile(testFilePath, 'Test file content for winkgo_agent E2E');

      // Screenshot 01: before creating conversation
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-03/01-before-create.png`);
      ...
      // with workspace containing the test file, which winkgo_agent can access
      const conversationId = await createWinkGoAgentConversationViaBridge(page, {
        name: conversationName,
        workspace: tempWorkspace.path,
        provider: preconditions.models!.modelA,
      ...
      // Screenshot 02: message sent
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-03/02-message-sent.png`);

      // Step 4: Wait for AI reply
      await waitForWinkGoAgentReply(page, conversationId);

      // Screenshot 03: reply completed
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-03/03-reply-completed.png`);
      ...
      // 2. Verify AI reply mentions file content (winkgo_agent can read workspace files)
      const aiMessages = messages.filter((m) => m.position === 'left' && m.type === 'text');
      expect(aiMessages.length).toBeGreaterThanOrEqual(1);
      ...
      // Note: Since winkgo_agent has workspace access, it should be able to read the file
