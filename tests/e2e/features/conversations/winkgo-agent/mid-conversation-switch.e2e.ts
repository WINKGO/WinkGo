/**
 * WinkGoAgent Chat E2E Tests - Mid-Conversation Switch (P1)
 *
 * Test Cases Covered:
...
test.describe('WinkGoAgent Chat - Mid-Conversation Switch (P1)', () => {
  ...
  test.skip('TC-A-08: should handle continuous switch (model → permission → model)', async ({ page }) => {
    // SKIP: Pending winkgo_agent binary investigation - runtime provider+mode switch causes silent hang on subsequent messages
    ...
    const conversationName = `E2E-winkgo_agent-${timestamp}-continuous-switch`;
    ...
    try {
      ...
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-08/01-created.png`);

      // Step 2: Send initial message
      await sendWinkGoAgentMessage(page, conversationId, 'Hello, initial message.');
      await waitForWinkGoAgentReply(page, conversationId);

      // Screenshot 02: initial reply
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-08/02-initial-reply.png`);
      ...
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-08/03-model-switched.png`);

      // Step 5: Switch permission (mode switch)
      const modeSelector = page.locator('[data-testid="agent-mode-selector-winkgo_agent"]');
      await expect(modeSelector).toBeVisible({ timeout: 10000 });
      await modeSelector.click();
      await page.waitForTimeout(500);

      const yoloOption = page.locator('[data-testid="winkgo_agent-mode-option-yolo"]');
      await expect(yoloOption).toBeVisible();
      await yoloOption.click();
      await page.waitForTimeout(1000);

      // Screenshot 04: permission switched
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-08/04-permission-switched.png`);
      ...
      const firstModel = page.locator(
        `[data-testid="winkgo_agent-model-option-${preconditions.models!.modelA.useModel}"]`
      );
      ...
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-08/05-model-switched-again.png`);

      // Step 7: Send message after all switches
      await sendWinkGoAgentMessage(page, conversationId, 'After all switches.');
      await waitForWinkGoAgentReply(page, conversationId);

      // Screenshot 06: final reply
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-08/06-final-reply.png`);
      ...
      // 3. Verify all AI replies exist (message.status check not applicable for winkgo_agent)
  ...
  test.skip('TC-A-09: should handle 3 rounds of conversation after model/permission switch', async ({ page }) => {
    // SKIP: Same root cause as TC-A-08 - winkgo_agent binary runtime state handling issue after model+permission switches
    // See tests/e2e/docs/chat-winkgo-agent/implementation-mapping.zh.md "Known Issues" section
    ...
    const timestamp = Date.now();
    const conversationName = `E2E-winkgo_agent-${timestamp}-multi-round`;
    ...
    try {
      ...
      // Step 2: Send initial message
      await sendWinkGoAgentMessage(page, conversationId, 'Round 1: Initial message.');
      await waitForWinkGoAgentReply(page, conversationId);

      // Screenshot 01: after round 1
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-09/01-round1.png`);
      ...
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-09/02-after-switches.png`);

      // Step 6: Round 2 - send and wait
      await sendWinkGoAgentMessage(page, conversationId, 'Round 2: After model and permission switch.');
      await waitForWinkGoAgentReply(page, conversationId);

      // Screenshot 03: after round 2
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-09/03-round2.png`);

      // Step 7: Round 3 - send and wait
      await sendWinkGoAgentMessage(page, conversationId, 'Round 3: Continue with switched config.');
      await waitForWinkGoAgentReply(page, conversationId);

      // Screenshot 04: after round 3
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-09/04-round3.png`);

      // Step 8: Round 4 (bonus) - send and wait
      await sendWinkGoAgentMessage(page, conversationId, 'Round 4: Final message.');
      await waitForWinkGoAgentReply(page, conversationId);

      // Screenshot 05: after round 4
      await takeScreenshot(page, `chat-winkgo_agent/tc-a-09/05-round4.png`);
