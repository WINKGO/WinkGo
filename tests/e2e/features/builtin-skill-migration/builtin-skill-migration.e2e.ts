 ...
 */
const REMOVED_AUTO_INJECT_NAME = 'winkgo-skills';
const REMOVED_AUTO_INJECT_DIR_NAME = 'winkgo-skills';
...
function resolveBackendBinary(): string {
  const candidates = [
    process.env.WINKGO_BACKEND_BINARY,
    path.join(os.homedir(), '.cargo', 'bin', 'winkgo_core'),
  ].filter((x): x is string => typeof x === 'string' && x.length > 0);
  for (const c of candidates) {
    if (fs.existsSync(c)) return c;
  }
  throw new Error('winkgo_core binary not found. Set WINKGO_BACKEND_BINARY or install to ~/.cargo/bin/winkgo_core.');
}
...
test.describe('Built-in Skill Migration (T3)', () => {
  ...
  test('S7: builtin skills in /api/skills expose an absolute, readable location for export', async ({ page }) => {
    ...
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'winkgo-e2e-s7-export-'));
    try {
  ...
  //
  // Run against a sibling `winkgo_core` process on port 25903 against a
  ...
  test.describe('Cold-start invariants (sibling backend)', () => {
    ...
    async function startBackend(): Promise<void> {
      ...
      const logPath = path.join(dataDir, 'sibling-winkgo_core.log');
      const logFd = fs.openSync(logPath, 'a');
      const parentEnv = { ...process.env };
      // Scrub any env vars that would leak main-Electron backend state.
      delete parentEnv.WINKGO_EXTENSIONS_PATH;
      delete parentEnv.WINKGO_EXTENSION_STATES_FILE;
      delete parentEnv.WINKGO_E2E_TEST;
      delete parentEnv.WINKGO_CDP_PORT;
      delete parentEnv.WINKGO_BUILTIN_SKILLS_PATH;
    ...
    test.beforeEach(() => {
      dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'winkgo-e2e-builtin-skill-'));
    });
    ...
    test('S8: legacy {cacheDir}/builtin-skills/ is gone after the current Electron boot', async () => {
      ...
      // `~/.winkgo-config` tree. Failing that we at least assert the
      ...
      const candidates = [
        path.join(os.homedir(), '.winkgo-config', 'builtin-skills'),
        path.join(os.homedir(), '.winkgo-config-dev', 'builtin-skills'),
      ];
      ...
      test.info().annotations.push({
        ...
        description:
          survivors.length === 0
            ? 'no legacy builtin-skills cache dirs detected under ~/.winkgo-config*'
