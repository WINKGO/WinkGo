...
### 2. Ensure `winkgo_core` is on PATH

The Electron main process spawns the `winkgo_core` binary during startup and
exposes its port to the renderer via `window.__backendPort`. In an unpackaged
E2E run, the binary must be reachable from the inherited `PATH` (`which` on
macOS/Linux, `where` on Windows). If it isn't, `__backendPort` will be `0` and
every HTTP call from the renderer (or from E2E helpers that use
`tests/e2e/helpers/httpBridge.ts`) will fail with `Failed to fetch`.

```bash
# Install the backend binary (builds to ~/.cargo/bin/winkgo_core)
cargo install --path backend/crates/winkgo-app
...
| ---------------------------- | ----- | ------------------------ |
| `WINKGO_E2E_TEST`            | `1`   | App recognizes test mode |
| `WINKGO_DISABLE_AUTO_UPDATE` | `1`   | No update checks         |
| `WINKGO_DISABLE_DEVTOOLS`    | `1`   | No DevTools windows      |
| `WINKGO_CDP_PORT`            | `0`   | CDP disabled             |
...
```bash
# Clean via database
sqlite3 "~/Library/Application Support/WinkGo-Dev/winkgo/winkgo.db" \
