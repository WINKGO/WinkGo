/**
 * E2E test helpers for winkgo_agent (WinkGo CLI) conversations.
 */
...
/**
 * WinkGoAgent test models structure.
 */
export interface WinkGoAgentTestModels {
  modelA: TProviderWithModel;
  modelB: TProviderWithModel | null;
}

/**
 * Resolve winkgo_agent binary path using `which winkgo_agent`.
 * @returns Binary path or null if not found
 */
export function resolveWinkGoAgentBinary(): string | null {
  try {
    const result = execSync('which winkgo_agent', { encoding: 'utf-8', timeout: 5000 }).trim();
...
/**
 * Get winkgo_agent test models (modelA + optional modelB).
 * @param page Playwright page
 * @returns Test models object or null if no compatible provider available
 */
export async function getWinkGoAgentTestModels(page: Page): Promise<WinkGoAgentTestModels | null> {
  try {
    const providers = (await httpGet<ProviderResponse[]>(page, '/api/providers')).map(normalizeProvider);
    if (!Array.isArray(providers)) return null;

    const isWinkGoAgentCompatible = (p: TProviderWithModel): boolean => {
      const platform = String(p.platform || '').toLowerCase();
      if (platform.includes('gemini-with-google-auth')) return false;
      // `gemini` (OpenAI-compat via /v1beta/openai) has a known winkgo_agent first-send
      ...
      // winkgo_agent appends `/v1/chat/completions`. Compatible when:
    ...
    const candidates = providers.filter(
      (p) =>
        p.enabled !== false && Array.isArray(p.model) && p.model.length > 0 && p.apiKey && isWinkGoAgentCompatible(p)
    );

    if (candidates.length === 0) return null;

    // Prefer more reliable winkgo_agent-compatible platforms first.
...
/**
 * Resolve winkgo_agent preconditions (binary + models).
 * @param page Playwright page
 * @returns Object with binary path and models, or null values if not available
 */
export async function resolveWinkGoAgentPreconditions(page: Page): Promise<{
  binary: string | null;
  models: WinkGoAgentTestModels | null;
}> {
  const binary = resolveWinkGoAgentBinary();
  const models = await getWinkGoAgentTestModels(page);
  return { binary, models };
}

/**
 * Create an winkgo_agent conversation via IPC bridge.
 * @param page Playwright page
 * @param opts Conversation options
 * @returns Conversation ID
 */
export async function createWinkGoAgentConversationViaBridge(
  page: Page,
  opts: {
    name?: string;
    workspace: string;
  ...
  const name = opts.name || `E2E-winkgo_agent-${timestamp}`;
  ...
  if (!result?.id) {
    throw new Error(
      `createWinkGoAgentConversationViaBridge: bridge returned no conversation id — check provider apiKey and platform fields`
    );
  }
  return result.id;
}
...
/**
 * Send a message in an winkgo_agent conversation via IPC bridge.
 * @param page Playwright page
 * @param conversationId Conversation ID
 * @param text Message text
 * @param opts Send options (files, etc.)
 */
export async function sendWinkGoAgentMessage(
  page: Page,
...
/**
 * Wait for winkgo_agent AI reply to finish.
 ...
 */
export async function waitForWinkGoAgentReply(page: Page, conversationId: string, timeoutMs = 150_000): Promise<void> {
  ...
  while (Date.now() < deadline) {
    ...
    if (aiTextMsgs.length > 0) {
      ...
      const conv = await getWinkGoAgentConversationDB(page, conversationId);
  ...
  const finalConv = await getWinkGoAgentConversationDB(page, conversationId);
  const finalMsgs = await getWinkGoAgentMessages(page, conversationId);
  console.error(`[waitForWinkGoAgentReply TIMEOUT] conv.status=${finalConv?.status}, msg count=${finalMsgs.length}`);
  for (const m of finalMsgs) {
    ...
    console.error(
      `[waitForWinkGoAgentReply TIMEOUT]   - pos=${m.position} type=${m.type} status=${m.status} preview="${preview}"`
    );
  }
  throw new Error(`WinkGoAgent reply timeout after ${timeoutMs}ms for conversation ${conversationId}`);
}

/**
 * Get winkgo_agent conversation from database via IPC bridge.
 * @param page Playwright page
 * @param conversationId Conversation ID
 * @returns Conversation object or null if not found
 */
export async function getWinkGoAgentConversationDB(page: Page, conversationId: string): Promise<any> {
  try {
...
/**
 * Get all messages for an winkgo_agent conversation from database.
 * @param page Playwright page
 * @param conversationId Conversation ID
 * @returns Array of message objects
 */
export async function getWinkGoAgentMessages(page: Page, conversationId: string): Promise<any[]> {
  try {
...
/**
 * Clean up all E2E winkgo_agent conversations from database.
 * Deletes conversations with name pattern 'E2E-winkgo_agent-%'.
 * Throws error if deletion fails (no silent failures).
 * @param page Playwright page
 */
export async function cleanupE2EWinkGoAgentConversations(page: Page): Promise<void> {
  ...
  const e2eConversations = conversations.filter((conv: any) => conv.name?.startsWith('E2E-winkgo_agent-'));
...
export function createTempWorkspace(scenario: string): { path: string; cleanup: () => void } {
  const timestamp = Date.now();
  const dirPath = `/tmp/e2e-chat-winkgo_agent-${scenario}-${timestamp}`;
...
/** Select an available winkgo_agent assistant on the guid page. */
export async function selectWinkGoAgent(page: Page): Promise<void> {
  await goToGuid(page);
  const assistantId = await selectAssistantForBackend(page, 'winkgo_agent');
  if (!assistantId) {
    throw new Error('No available winkgo_agent assistant found on guid page');
  }
}

/**
 * Select winkgo_agent model from dropdown.
 * Can be used on guid page or conversation page.
 * @param page Playwright page
 * @param modelId Model ID (e.g., 'claude-opus-4-7')
 */
export async function selectWinkGoAgentModel(page: Page, modelId: string): Promise<void> {
  const modelBtn = page.locator('[data-testid="winkgo_agent-model-selector"]');
  await modelBtn.waitFor({ state: 'visible', timeout: 10_000 });
  await modelBtn.click();

  const modelOption = page.locator(`[data-testid="winkgo_agent-model-option-${modelId}"]`);
...
/**
 * Select winkgo_agent mode (permission level) from dropdown.
 * @param page Playwright page
 * @param mode Mode value ('default', 'auto_edit', 'yolo')
 */
export async function selectWinkGoAgentMode(page: Page, mode: string): Promise<void> {
  const modeBtn = page.locator('[data-testid="agent-mode-selector-winkgo_agent"]');
  await modeBtn.waitFor({ state: 'visible', timeout: 10_000 });
  await modeBtn.click();

  const modeOption = page.locator(`[data-testid="winkgo_agent-mode-option-${mode}"]`);
...
/**
 * Attach a folder to winkgo_agent conversation by clicking the file attach button.
 ...
 */
export async function attachWinkGoAgentFolder(page: Page, folderPath: string): Promise<void> {
  ...
  const attachBtn = page.locator('[data-testid="winkgo_agent-attach-folder-btn"]');
  await attachBtn.waitFor({ state: 'visible', timeout: 10_000 });

  // Skip actual click for smoke test (would trigger file dialog)
  console.log(`[attachWinkGoAgentFolder] Attach button found (folder: ${folderPath})`);
}

/**
 * Upload files to winkgo_agent conversation using file input.
 * @param page Playwright page
 * @param filePaths Array of absolute file paths
 */
export async function uploadWinkGoAgentFiles(page: Page, filePaths: string[]): Promise<void> {
  const fileInput = page.locator('[data-testid="winkgo_agent-file-upload-input"]');
  await fileInput.setInputFiles(filePaths);

  // Wait for file preview tags to appear
  await page.waitForSelector('[data-testid^="winkgo_agent-file-tag-"]', { timeout: 5_000 });
}
