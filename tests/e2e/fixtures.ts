 ...
 *      (e.g. out/linux-unpacked/wink-go, out/mac-arm64/WINK GO.app, out/win-unpacked/WINK-GO.exe).
...
// Disposable userData root so WinkGoCore migrates a fresh DB per run instead of
...
const e2eUserDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'winkgo-e2e-userdata-'));
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/WINK-GO.exe  or  out/win-x64-unpacked/WINK-GO.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'WINK-GO.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/WinkGo.app/Contents/MacOS/WinkGo  or  out/mac/WinkGo.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'WinkGo');
        if (fs.existsSync(exe)) return { executablePath: exe, cwd: macDir };
      }
    }
  } else {
    // Linux: out/linux-unpacked/winkgo  (lowercase executable name)
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      const dirPath = path.join(outDir, dir);
      if (!fs.existsSync(dirPath)) continue;
      // Try common executable names
      for (const name of ['winkgo', 'WinkGo']) {
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    WINKGO_EXTENSIONS_PATH: process.env.WINKGO_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
