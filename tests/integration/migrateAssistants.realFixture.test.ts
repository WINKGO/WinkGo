 ...
 * winkgo_core binary using the user-provided fixtures
 * (`/Users/testuser/Downloads/winkgo-config.txt` + `Archive/*.md`).
 *
 ...
 *
 *   1. Decode the legacy `winkgo-config.txt` exactly as ConfigStorage would.
 *   2. Stage `Archive/*.md` as `<userData>/config/assistants/<id>.<locale>.md`.
 *   3. Spawn a real winkgo_core bound to a throw-away data-dir.
...
const FIXTURE_CONFIG = '/Users/testuser/Downloads/winkgo-config.txt';
const FIXTURE_ARCHIVE = '/Users/testuser/Downloads/Archive';
...
function resolveBackendBinary(): string {
  ...
  const candidates = [
    ...
    path.resolve(__dirname, '../../backend/target/debug', binaryName),
    path.join(os.homedir(), '.cargo', 'bin', binaryName),
  ].filter((x): x is string => typeof x === 'string' && x.length > 0);
  for (const c of candidates) {
    if (existsSync(c)) return c;
  }
  throw new Error('winkgo_core binary not found (set WINKGO_BACKEND_BINARY or build it)');
}
...
/**
 * Decode an `winkgo-config.txt` exactly the way `JsonFileBuilder` does:
...
describeIfFixtures('migrateAssistantsToBackend (real fixture)', () => {
  ...
  beforeEach(async () => {
    dataDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'winkgo-migrate-fixture-'));
