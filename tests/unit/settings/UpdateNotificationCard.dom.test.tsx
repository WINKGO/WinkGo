...
describe('UpdateNotificationCard', () => {
  beforeEach(() => {
    ...
    mocks.shellOpenExternalMock.mockResolvedValue({ success: true });
    mocks.updateCheckMock.mockResolvedValue({
      ...
      data: {
        ...
        latest: {
          ...
          assets: [],
          recommendedAsset: {
            name: 'WinkGo-2.1.14-mac-arm64.dmg',
            url: 'https://winkgo.top/releases/2.1.14/WINK-GO-2.1.14-mac-arm64.dmg',
            size: 123,
          },
        },
      },
    ...
    mocks.updateDownloadMock.mockImplementation(async (request: UpdateDownloadRequest) => ({
      ...
      data: {
        ...
        file_path: '/tmp/WinkGo-2.1.14-mac-arm64.dmg',
      },
    }));
  });
  ...
  it('checks winkgo.top after startup and automatically presents a newer website version', async () => {
    vi.useFakeTimers();
    mocks.updateCheckMock.mockResolvedValue({
      success: true,
      data: {
    ...
    expect(screen.getByTestId('update-notification-card')).toBeInTheDocument();
    expect(screen.getByText('2.1.15 → 2.1.16')).toBeInTheDocument();
    expect(screen.getByText('update.goToRelease')).toBeInTheDocument();
  });

  it('opens the WINK GO website when a newer manifest has no direct installer asset yet', async () => {
    mocks.updateCheckMock.mockResolvedValue({
      success: true,
      data: {
        currentVersion: '2.1.15',
    ...
    fireEvent.click(await screen.findByText('update.goToRelease'));
    await waitFor(() => {
      expect(mocks.shellOpenExternalMock).toHaveBeenCalledWith('https://winkgo.top/');
    });
    expect(mocks.updateDownloadMock).not.toHaveBeenCalled();
  });
  ...
  it('restores a cached completed auto-update on mount', async () => {
    mocks.autoUpdateRestoreDownloadedMock.mockResolvedValue({
      ...
      data: {
        ready: true,
        version: '2.1.14',
        filePath: '/cache/pending/WinkGo-2.1.14-mac.zip',
      },
    });
  ...
  it('shows consumed silent installer failure marker with retry, log, and feedback actions', async () => {
    const marker: InstallerLastFailureMarker = {
      ...
      retryCount: 3,
      instDir: 'D:\\WinkGo',
      logPath: 'C:\\Users\\me\\AppData\\Local\\Temp\\winkgo-installer-2.1.27-20260702-151830-ab12cd34ef56.log',
