...
const mocks = vi.hoisted(() => ({
  ...
  listWinkGoSkillsCatalog: vi.fn(),
  prepareWinkGoSkillImport: vi.fn(),
  importSkills: vi.fn(),
  deleteSkill: vi.fn(),
  showOpen: vi.fn(),
  messageError: vi.fn(),
...
vi.mock('@/common', () => ({
  ipcBridge: {
    ...
    winkGoSkills: {
      listCatalog: { invoke: mocks.listWinkGoSkillsCatalog },
      prepareImport: { invoke: mocks.prepareWinkGoSkillImport },
    },
  },
}));
...
describe('SkillsHubSettings', () => {
  ...
  beforeEach(() => {
    ...
    mocks.listWinkGoSkillsCatalog.mockResolvedValue({ available: true, skills: [] });
    mocks.prepareWinkGoSkillImport.mockResolvedValue({ skillPath: '/tmp/winkgo-skill-imports/codex' });
  });
  ...
  it('rebrands legacy product names in skill names and descriptions', async () => {
    mocks.listAvailableSkills.mockResolvedValue([
      {
        name: 'winkgo-config',
        description: 'Configure WinkGo assistants.',
        location: '/tmp/user-skills/winkgo-config',
        is_custom: true,
        source: 'custom',
      },
    ]);
    ...
    const card = await screen.findByTestId('my-skill-card-winkgo-config');
    expect(card).toHaveTextContent('WINK GO-config');
    expect(card).toHaveTextContent('Configure WINK GO assistants.');
    expect(card.textContent).not.toMatch(/WinkGo/i);
  });

  it('uses matching brand artwork for installed skills instead of initial-letter blocks', async () => {
    mocks.listAvailableSkills.mockResolvedValue([
      {
        name: 'codex',
        description: 'Codex desktop tools.',
        location: '/tmp/user-skills/codex',
        is_custom: true,
        source: 'custom',
      },
    ]);
    ...
    const card = await screen.findByTestId('my-skill-card-codex');
    const logo = screen.getByTestId('wink-go-skill-logo-codex');
    expect(card).toContainElement(logo);
    expect(logo).toHaveAttribute('aria-label', 'codex logo');
    expect(logo).not.toHaveTextContent('C');
  });

  it('lists and explicitly imports original WINK GO skills without starting a runtime', async () => {
    mocks.listWinkGoSkillsCatalog.mockResolvedValue({
      available: true,
      skills: [
        {
          id: 'codex',
          displayName: 'Codex',
          description: 'Independent Codex client and task entry.',
          capabilityLabels: ['Codex client', 'Tasks'],
          actionCount: 10,
        },
      ],
    });
    mocks.importSkills.mockResolvedValue({ skill_name: 'codex', skill_names: ['codex'] });
    ...
    fireEvent.click(screen.getByTestId('btn-import-wink-go-skill-codex'));
    await waitFor(() => expect(mocks.prepareWinkGoSkillImport).toHaveBeenCalledWith({ skillId: 'codex' }));
    await waitFor(() =>
      expect(mocks.importSkills).toHaveBeenCalledWith({
        skill_path: '/tmp/winkgo-skill-imports/codex',
      })
    );
  });
