...
const makeAppDataDir = () => {
  const dir = mkdtempSync(path.join(tmpdir(), 'winkgo-installer-last-failure-'));
  tempDirs.push(dir);
  return dir;
};
...
describe('installerLastFailure service', () => {
  it('consumes a valid installer-last-failure marker once from the AppData WINK GO directory', async () => {
    ...
    expect(markerPath).toBe(path.join(appDataDir, 'WINK GO', 'installer-last-failure.json'));
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        blockers: [{ pid: 1234, name: 'WinkGo.exe' }],
      })
    );
    ...
    expect(consumed?.blockers).toEqual([{ pid: 1234, name: 'WinkGo.exe' }]);
