...
describe('CdnGenericProvider', () => {
  it('resolves relative update files under the version directory', () => {
    ...
    const files = provider.resolveFiles({
      ...
      files: [
        {
          url: 'WINK-GO-2.1.14-mac-arm64.dmg',
          sha512: 'sha512-value',
        },
      ],
      path: 'WINK-GO-2.1.14-mac-arm64.dmg',
      sha512: 'sha512-value',
      releaseDate: '2026-06-08T00:00:00.000Z',
    } satisfies UpdateInfo);

    expect(files[0]?.url.href).toBe('https://winkgo.top/releases/2.1.14/WINK-GO-2.1.14-mac-arm64.dmg');
  });
});
