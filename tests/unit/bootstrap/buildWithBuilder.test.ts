...
`,
  ...
  it('defaults an unqualified build to Free and writes a matching Vite edition marker', () => {
    const outDir = resolve(repoRoot, 'out');
...
`,
  ...
  it('rejects skip-vite when the cached Vite output was built for another edition', () => {
    const outDir = resolve(repoRoot, 'out');
...
`,
  ...
  ])('prepares bundled WinkGoCore for $expectedArch with args $args', ({ args, expectedArch }) => {
