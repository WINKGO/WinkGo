...
describe('classifyBackendStartupFailure', () => {
  it('classifies missing GLIBC symbols as an incompatible backend runtime', () => {
    ...
    error.details = {
      stage: 'early_exit',
      stderrTail:
        "/opt/WinkGo/resources/bundled-winkgo-core/linux-x64/winkgo_core.bin: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.34' not found\n" +
        "/opt/WinkGo/resources/bundled-winkgo-core/linux-x64/winkgo_core.bin: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.32' not found",
    };
  ...
  it('classifies missing startup directory preparation as a startup directory failure', () => {
    const error = new Error('winkgo_core startup directory preparation failed') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
      stage: 'spawn',
      workDir: 'D:\\ai\\WinkGo\\工作目录',
      causeMessage: 'ENOENT: no such file or directory, mkdir D:\\ai\\WinkGo\\工作目录',
    };
  ...
  it('classifies startup directory permission failures separately from incomplete installs', () => {
    ...
    error.details = {
      stage: 'spawn',
      workDir: 'D:\\ai\\WinkGo\\工作目录',
      causeMessage: 'EPERM: operation not permitted, mkdir D:\\ai\\WinkGo\\工作目录',
    };
  ...
  it('does not classify post-resolution binary spawn ENOENT as a startup directory failure', () => {
    const error = new Error('winkgo_core process emitted an error before startup') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
      stage: 'spawn_error',
      binaryPath: 'D:\\apps\\WinkGo\\resources\\bundled-winkgo-core\\win32-x64\\winkgo_core.exe',
      causeMessage: 'spawn D:\\apps\\WinkGo\\resources\\bundled-winkgo-core\\win32-x64\\winkgo_core.exe ENOENT',
    };
  ...
  it('classifies packaged app resources missing from installation as incomplete installation', () => {
    ...
    expect(classifyBackendStartupFailure(error)).toEqual({
      ...
      missingPwaDir: true,
      missingResources: ['bundled-winkgo-core/', 'bundled-winkgo-core/win32-x64/'],
  ...
  it('classifies packaged runtime directories without the backend binary as incomplete installation', () => {
    ...
    expect(classifyBackendStartupFailure(error)).toEqual({
      ...
      incompleteInstallationKind: 'missing_directory_resources',
      missingBackendBinary: true,
      missingBundledWinkGoCoreDir: false,
      missingHubDir: false,
      missingPetStatesDir: false,
      missingPwaDir: false,
      missingResources: [
        'bundled-winkgo-core/win32-x64/managed-resources/',
        'bundled-winkgo-core/win32-x64/winkgo_core.exe',
      ],
