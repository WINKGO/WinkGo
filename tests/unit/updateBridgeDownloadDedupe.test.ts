...
vi.mock('electron', () => ({
  app: {
    ...
    getPath: vi.fn(() => '/tmp/winkgo-update-dedupe-test'),
    exit: vi.fn(),
    isPackaged: true,
  },
}));
...
describe('updateBridge manual download dedupe', () => {
  ...
  it('reuses the active manual download for the same URL, fallback URL, and file name', async () => {
    ...
    const request = {
      url: 'https://winkgo.top/releases/2.2.0/WinkGo-2.2.0-mac-arm64.dmg',
      fallbackUrl: 'https://www.winkgo.top/releases/2.2.0/WinkGo-2.2.0-mac-arm64.dmg',
      file_name: 'WinkGo-2.2.0-mac-arm64.dmg',
    };
