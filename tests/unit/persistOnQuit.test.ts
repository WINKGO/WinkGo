...
const flushMicrotasks = (): Promise<void> =>
  Array.from({ length: 12 }).reduce<Promise<void>>((chain) => chain.then(() => {}), Promise.resolve());
...
} from '@/process/utils/persistOnQuit';
...
describe('persistOnQuit', () => {
  ...
  it('continues quitting when a persistence write never settles', async () => {
    vi.useFakeTimers();
    trackPersistedWrite(new Promise<void>(() => {}));

    const preventDefault = vi.fn();
    fireAppEvent('before-quit', { preventDefault });
    await vi.advanceTimersByTimeAsync(PERSIST_ON_QUIT_TIMEOUT_MS);

    expect(preventDefault).toHaveBeenCalledTimes(1);
    expect(appQuit).toHaveBeenCalledTimes(1);

    const finalPreventDefault = vi.fn();
    fireAppEvent('before-quit', { preventDefault: finalPreventDefault });
