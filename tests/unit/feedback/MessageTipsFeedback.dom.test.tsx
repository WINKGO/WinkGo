...
describe('agent error locale copy', () => {
  ...
  it('does not label app-side errors as direct WinkGo ownership', () => {
    const localeDir = path.join(process.cwd(), 'packages/desktop/src/renderer/services/i18n/locales');
    ...
    for (const localeName of supportedLocaleNames) {
      ...
      expect(agentError.ownership.winkgo, localeName).not.toMatch(/WinkGo/);

      for (const [code, copy] of Object.entries<Record<string, string>>(agentError.codes)) {
        if (!code.startsWith('WINKGO_')) continue;

        expect(copy.title, `${localeName} ${code} title`).not.toMatch(/WinkGo/);
        expect(copy.body, `${localeName} ${code} body`).not.toMatch(/WinkGo/);
      }
    }
  });
