...
describe('agentLogo', () => {
  ...
  describe('resolveAgentLogo (backend lookup)', () => {
    ...
    it('does not expose local absolute paths as logo sources', () => {
      expect(resolveAgentLogo(LOGOS, { icon: '/Users/demo/.winkgo/agent-avatars/custom.png' })).toBeNull();
    });
  });
  ...
  describe('resolveAgentAvatar', () => {
    ...
    it('falls back to backend catalog logos when explicit local paths leak through', () => {
      expect(
        resolveAgentAvatar(LOGOS, { icon: '/Users/demo/.winkgo/agent-avatars/custom.png', backend: 'claude' })
      ).toEqual({
        kind: 'image',
    ...
    it('keeps the trusted WINK GO renderer asset as an image avatar', () => {
      expect(resolveAgentAvatar(LOGOS, { icon: winkGoLogo, backend: 'winkgo_agent' })).toEqual({
        kind: 'image',
        value: winkGoLogo,
      });
      expect(resolveAgentLogo(LOGOS, { icon: winkGoLogo, backend: 'winkgo_agent' })).toBe(winkGoLogo);
    });
  ...
  describe('fetchAgentLogos', () => {
    ...
    it('rebrands the internal winkgo_agent logo before publishing the shared catalog', async () => {
      bridgeMocks.getManagedAgents.mockResolvedValue([
        {
          id: 'agent-winkgo_agent',
          name: 'WinkGo CLI',
          ...
          installed: true,
          status: 'online',
          icon: '/api/assets/logos/brand/winkgo.svg',
        },
      ]);

      await expect(fetchAgentLogos()).resolves.toEqual({
        winkgo_agent: winkGoLogo,
        'agent-winkgo_agent': winkGoLogo,
      });
    });
  });
