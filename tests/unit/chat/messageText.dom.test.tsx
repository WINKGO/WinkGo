...
const mockFilePreview = vi.fn(({ path, variant }: { path: string; variant?: 'thumbnail' | 'inline' }) => (
  <div data-testid={variant === 'inline' ? 'inline-local-image-preview' : 'file-preview'}>{path}</div>
));
...
describe('MessageText attachment paths', () => {
  ...
  it('shows generated local image paths directly below assistant replies', () => {
    renderMessageText(
      '已生成图片，文件保存在：\n\n`C:\\Users\\Administrator\\.codex\\generated_images\\rabbit photo.png`'
    );

    expect(screen.getByTestId('inline-local-image-preview')).toHaveTextContent(
      'C:\\Users\\Administrator\\.codex\\generated_images\\rabbit photo.png'
    );
    expect(mockFilePreview).toHaveBeenCalledWith(
      expect.objectContaining({
        path: 'C:\\Users\\Administrator\\.codex\\generated_images\\rabbit photo.png',
        readonly: true,
        variant: 'inline',
      })
    );
  ...
  it('does not auto-preview local image paths written by the user', () => {
    renderMessageText('C:\\Users\\demo\\private\\photo.png', { position: 'right' });

    expect(screen.queryByTestId('inline-local-image-preview')).not.toBeInTheDocument();
  });

  it('does not duplicate a standard Markdown image preview', () => {
    renderMessageText('![generated](C:/Users/demo/generated/photo.png)');
  ...
  it('renders assistant marker mentions as full message text without file previews', () => {
    const content = '请不要使用 [[WINKGO_FILES]] 这种格式';
  ...
  it('shows legacy product names as WINK GO in assistant replies while preserving user text', () => {
    renderMessageText('你好！我是你的 WinkGo 管家。');
    expect(screen.getByTestId('message-text-content')).toHaveTextContent('你好！我是你的 WINK GO 管家。');
    expect(screen.queryByText(/WinkGo/i)).not.toBeInTheDocument();

    renderMessageText('WinkGo 是原项目名', { position: 'right', id: 'user-brand', msg_id: 'user-brand' });
    expect(screen.getAllByTestId('message-text-content').at(-1)).toHaveTextContent('WinkGo 是原项目名');
  });

  it('keeps assistant marker-tail markdown visible as message text', () => {
    const content = [
      '不是用 `[[WINKGO_FILES]]` 路径形式。',
      '',
      '[[WINKGO_FILES]]',
      '## 怎么解决',
      '- 路径引用...模型看不到像素',
    ...
    expect(messageContent).toHaveTextContent('不是用 `[[WINKGO_FILES]]` 路径形式。');
    expect(messageContent).toHaveTextContent('[[WINKGO_FILES]]');
  ...
  it('keeps assistant fenced-code marker text visible without file previews', () => {
    const content = ['```md', '[[WINKGO_FILES]]', 'uploads/photo.png', '```'].join('\n');
  ...
  it.each([
    ...
    const content = ['look', '', '[[WINKGO_FILES]]', ...tailLines].join('\n');
