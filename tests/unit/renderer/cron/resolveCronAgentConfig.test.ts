...
describe('resolveCronAgentConfig', () => {
  it('stores provider id for preset winkgo_agent assistants instead of literal winkgo_agent backend', () => {
    const result = resolveCronAgentConfig({
      ...
      presetAssistants: [
        assistant({
          ...
          agent: agent('agent-winkgo_agent', 'winkgo_agent'),
        }),
      ],
      ...
      getMode: () => 'yolo',
      winkgo_agentModelRequiredMessage: 'provider required',
    });
  ...
  it('omits backend for non-winkgo_agent assistants and lets the backend derive runtime identity', () => {
    const result = resolveCronAgentConfig({
...
function agent(_id: string, type: string, backend?: string): AssistantAgent {
  return {
    type,
    source: type === 'winkgo_agent' ? 'internal' : 'builtin',
    acp_backend: backend,
  };
}
