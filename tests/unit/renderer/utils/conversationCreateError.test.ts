...
describe('conversationCreateError', () => {
  ...
  it('extracts backend payloads from stringified BackendHttpError messages', () => {
    const error =
      'Backend POST /api/teams failed (400): {"success":false,"error":"Workspace path is unavailable: /Users/testuser/Documents/Archive . Make sure the selected workspace path exists and is accessible.","code":"WORKSPACE_PATH_UNAVAILABLE","details":{"workspace_path":"/Users/testuser/Documents/Archive ","operation":"create"}}';

    expect(normalizeConversationCreateErrorCode(error)).toBe('WORKSPACE_PATH_UNAVAILABLE');
    expect(getConversationCreateErrorMessage(error, t)).toBe(
      'The selected workspace path is unavailable. Make sure the workspace path "/Users/testuser/Documents/Archive " exists and is accessible.'
    );
  });
});
