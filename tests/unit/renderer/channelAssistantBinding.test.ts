...
describe('channel assistant binding helpers', () => {
  const assistants = [
    assistant({ id: 'bare-winkgo_agent', name: 'WinkGo CLI', source: 'generated', preset_agent_type: 'winkgo_agent' }),
  ...
  it('prefers the generated winkgo_agent assistant as the default selection', () => {
    expect(getDefaultChannelAssistant(assistants)?.id).toBe('bare-winkgo_agent');
  });
  ...
  it('falls back to the default assistant only when no binding was saved', () => {
    expect(resolveChannelAssistantId(undefined, assistants)).toBe('bare-winkgo_agent');
  });
