...
vi.mock('@/renderer/hooks/context/ConversationContext', () => ({
  useConversationContextSafe: () => ({ conversation_id: 'conversation-1', type: 'winkgo_agent' }),
}));
...
function createWarningMessage(content: string, id = 'warning-1'): IMessageTips {
  return {
    id,
    msg_id: id,
    conversation_id: 'conversation-1',
...
describe('MessageList', () => {
  ...
  it('hides Codex project-trust initialization notices from the conversation', () => {
    const trustNotice = createWarningMessage(
    ...
    render(<MessageList />, {
      wrapper: ({ children }) => <Wrapper messages={[trustNotice]}>{children}</Wrapper>,
    });

    expect(screen.queryByText('tips')).not.toBeInTheDocument();
  });

  it('keeps actionable warning messages visible', () => {
    const actionableWarning = createWarningMessage('Authentication expired. Please sign in again.');

    render(<MessageList />, {
      wrapper: ({ children }) => <Wrapper messages={[actionableWarning]}>{children}</Wrapper>,
    });
