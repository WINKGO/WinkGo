...
describe('WinkGoAgentModelSelector runtime options', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders the model list directly (no submenu) when thought level is unavailable', () => {
    render(<WinkGoAgentModelSelector selection={makeSelection()} />);

    expect(screen.getByTestId('winkgo_agent-model-selector')).toHaveTextContent('gpt-5.2');
