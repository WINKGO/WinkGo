...
describe('buildSendFailureError', () => {
  it('classifies 409 already-processing as WINKGO_CONVERSATION_BUSY (wait, not retry)', () => {
  ...
  it('classifies 409 already-running as WINKGO_CONVERSATION_BUSY', () => {
  ...
  it('classifies runtime shutdown conflict as non-retryable busy without raw internal diagnostics', () => {
    ...
    expect(result.code).toBe('WINKGO_CONVERSATION_BUSY');
  ...
  it('falls back to WINKGO_INTERNAL_ERROR for non-conflict 409 (different message)', () => {
    ...
    expect(result.code).toBe('WINKGO_INTERNAL_ERROR');
    expect(result.retryable).toBe(true);
  });

  it('falls back to WINKGO_INTERNAL_ERROR for non-HTTP errors', () => {
    const result = buildSendFailureError(new Error('boom'), 'boom');

    expect(result.code).toBe('WINKGO_INTERNAL_ERROR');
    expect(result.ownership).toBe('winkgo');
    expect(result.retryable).toBe(true);
  });
