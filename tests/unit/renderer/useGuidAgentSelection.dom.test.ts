...
describe('useGuidAssistantSelection', () => {
  ...
  it('restores the last selected guid assistant before falling back to the winkgo_agent default', async () => {
    mockAssistants = [
      assistantFixture({ id: 'bare-winkgo_agent', runtimeKey: 'winkgo_agent', source: 'generated', sortOrder: 1 }),
  ...
  it('persists manual guid assistant selections for the next visit', async () => {
    ...
    await waitFor(() => {
      expect(result.current.selectedAssistantId).toBe('bare-winkgo_agent');
    });
  ...
  it('reads winkgo_agent mode options from the managed agent catalog', async () => {
    mockAssistants = [
      {
        id: 'bare:632f31d2',
        source: 'generated',
    ...
    expect(result.current.selectedAssistantBackend).toBe('winkgo_agent');
    expect(result.current.selectedMode).toBe('default');
...
function assistantFixture({
  ...
  const isWinkGoAgent = runtimeKey === 'winkgo_agent';
  return {
    id,
    source,
    name: id,
    ...
    agent: isWinkGoAgent
      ? { type: 'winkgo_agent', source: 'internal' }
