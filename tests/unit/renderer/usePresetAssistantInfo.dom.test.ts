...
describe('usePresetAssistantInfo', () => {
  ...
  it('rebrands generated winkgo_agent conversation snapshots for team and conversation surfaces', () => {
    useSWRMock.mockImplementation((key: unknown) => {
      if (key === 'assistants') return { data: [], isLoading: false };
      if (key === 'extensions.acpAdapters') return { data: [], isLoading: false };
      return { data: undefined, isLoading: false };
    ...
    const conversation = {
      ...
      assistant: {
        ...
        source: 'generated',
        name: 'WinkGo CLI',
        avatar: '/api/assets/logos/brand/winkgo.svg',
        backend: 'winkgo_agent',
      },
    } as TChatConversation;
  ...
  it('restores local absolute assistant snapshot avatars from the backend assistant catalog', () => {
    ...
    const conversation = {
      ...
      assistant: {
        ...
        source: 'user',
        name: 'Local Avatar',
        avatar: '/Users/demo/.winkgo/assistant-avatars/assistant-local-avatar.jpg',
        backend: 'codex',
      },
    } as TChatConversation;
