...
const primeConversationCacheMock = vi.fn();
...
vi.mock('@/renderer/pages/conversation/utils/conversationCache', () => ({
  primeConversationCache: (...args: unknown[]) => primeConversationCacheMock(...args),
}));
...
describe('useGuidSend', () => {
  beforeEach(() => {
    ...
    primeConversationCacheMock.mockReset();
  });
  ...
  it('navigates immediately without waiting for assistant metadata revalidation', async () => {
    const deps = createDeps();
    const neverSettles = new Promise<void>(() => {});
    swrMutateMock.mockReturnValue(neverSettles);
    ...
    expect(primeConversationCacheMock).toHaveBeenCalledWith({ id: 'conv-1' });
  ...
  it('forwards local skill overrides for generated WinkGo CLI assistants through assistant conversation overrides', async () => {
    const deps = createDeps();
    deps.selectedAssistantId = 'bare:winkgo_agent';
    deps.selectedAssistantBackend = 'winkgo_agent';
    ...
    expect(payload.assistant?.id).toBe('bare:winkgo_agent');
    expect(payload.assistant?.conversation_overrides?.skill_ids).toEqual(['pdf-reader']);
    expect(payload.assistant?.conversation_overrides?.disabled_builtin_skill_ids).toEqual(['todo-tracker']);
    expect(payload.extra.session_mode).toBeUndefined();
  });

  it('does not write legacy preset_assistant_id for generated WinkGo CLI assistant conversations', async () => {
    const deps = createDeps();
    deps.selectedAssistantId = 'bare:winkgo_agent';
    deps.selectedAssistantBackend = 'winkgo_agent';
    deps.current_model = { provider_id: 'openai', model: 'gemini-2.5-pro', use_model: 'gemini-2.5-pro' } as never;
