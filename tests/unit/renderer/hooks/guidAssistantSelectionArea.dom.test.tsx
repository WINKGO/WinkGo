...
describe('AssistantSelectionArea', () => {
  it('maps available width to 6, 5, 4, 3, 2, then 1 visible assistant slots', () => {
    expect(resolveAssistantVisibleLimit(1000)).toBe(6);
    expect(resolveAssistantVisibleLimit(820)).toBe(5);
    expect(resolveAssistantVisibleLimit(680)).toBe(4);
    expect(resolveAssistantVisibleLimit(560)).toBe(3);
    expect(resolveAssistantVisibleLimit(460)).toBe(2);
  ...
  it('keeps the assistant picker visible after an assistant is selected', () => {
    render(
      <AssistantSelectionArea
        selectedAssistantId='bare-winkgo_agent'
    ...
    expect(screen.getByTestId('preset-pill-bare-winkgo_agent')).toBeInTheDocument();
  ...
  it('moves overflow assistants into a more dropdown', async () => {
    ...
    // (builtin). So the top row is [bare-winkgo_agent, user-research, user-review,
    ...
    expect(screen.queryByTestId('assistant-overflow-bare-winkgo_agent')).not.toBeInTheDocument();
  ...
  it('lays out the overflow dropdown as a grid matching the visible pill count', async () => {
    ...
    // The explicit cap keeps this overflow fixture at four columns.
  ...
  it('orders assistant pills by group then sort_order before applying overflow', () => {
    ...
    // Late 90); the official Writer sinks to the bottom group.
    expect(
      screen
        .getAllByRole('button')
        .slice(0, 4)
        .map((node) => node.textContent?.trim())
    ).toEqual(['WinkGo CLI', 'Early', 'Mid', 'Late']);
  });
  ...
  return [
    {
      ...
      enabled: true,
      sort_order: 10,
      preset_agent_type: 'winkgo_agent',
      enabled_skills: [],
      custom_skill_names: [],
