...
// Mirror WinkGoModal's real prop contract: header/footer may be config objects.
...
vi.mock('@renderer/components/base/WinkGoModal', () => {
...
describe('TeamCreateModal', () => {
  ...
  it('keeps blocked assistants visible and prevents selecting them', () => {
    ...
    expect(screen.getByTestId('team-create-agent-option-bare-winkgo_agent')).toBeInTheDocument();
    expect(screen.getByText('WINK GO 命令行')).toBeInTheDocument();
    expect(screen.queryByText('WinkGo CLI')).not.toBeInTheDocument();
  ...
  it('renders the reference two-column creation layout with details on the selected-member side', () => {
    ...
    expect(within(assistantPane).getByTestId('team-create-agent-option-bare-winkgo_agent')).toHaveClass('!h-44px');
    expect(within(assistantPane).getByTestId('team-create-agent-option-bare-winkgo_agent')).toHaveClass(
      'hover:!bg-fill-2'
    );
  ...
  it('passes assistant identity through when creating a team with an assistant leader', async () => {
    ...
    fireEvent.click(screen.getByTestId('team-create-agent-option-bare-winkgo_agent'));
    ...
    expect(resolveDefaultTeamAgentModelMock).toHaveBeenCalledWith({
      assistant_id: 'bare-winkgo_agent',
      assistant_backend: 'winkgo_agent',
    });
    expect(payload.agents[0]).toMatchObject({
      role: 'leader',
      assistant_id: 'bare-winkgo_agent',
      assistant_name: 'WINK GO 命令行',
    });
...
describe('TeamCreateModal · mobile (narrow screen)', () => {
  ...
  it('creates a team from the mobile flow with the first member as leader', async () => {
    ...
    expect(payload.agents[0]).toMatchObject({ role: 'leader', assistant_id: 'bare-winkgo_agent' });
  });
});
...
function assistants(): Assistant[] {
  return [
    assistant({
      ...
      name: 'WinkGo CLI',
      name_i18n: { 'zh-CN': 'WinkGo 命令行' },
      source: 'generated',
      agent_id: 'agent-winkgo_agent',
