...
describe('Layout sider brand Home button', () => {
  ...
  it('navigates to the recorded last non-settings path when clicked in a settings route', () => {
    ...
    sessionStorage.setItem('winkgo:last-non-settings-path', '/conversation/abc');
    renderLayout();
  ...
  it('falls back to /guid when the recorded path is itself a settings path', () => {
    currentPathname = '/settings/about';
    sessionStorage.setItem('winkgo:last-non-settings-path', '/settings/system');
    renderLayout();
  ...
  it('does not navigate when the wordmark is clicked in a non-settings route', () => {
    ...
    fireEvent.click(screen.getByText('WINK GO'));
    expect(navigate).not.toHaveBeenCalled();
  });
  ...
  it('returns to chat when the logo icon is clicked in a settings route', () => {
    currentPathname = '/settings/about';
    sessionStorage.setItem('winkgo:last-non-settings-path', '/conversation/abc');
    const { container } = renderLayout();

    const icon = container.querySelector('img[alt="WINK GO"]') as HTMLElement;
  ...
  it('opens the update notification directly for tray update checks', () => {
    ...
    window.addEventListener('winkgo-open-update-modal', openListener);
    ...
    try {
      ...
      window.removeEventListener('winkgo-open-update-modal', openListener);
    }
  });
});
