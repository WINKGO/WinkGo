...
vi.mock('@/renderer/pages/conversation/platforms/winkgo_agent/WinkGoAgentChat', () => ({
  __esModule: true,
  default: (props: unknown) => winkgo_agentChatMock(props),
}));
...
describe('TeamChatView', () => {
  ...
  it('passes loaded skills and MCP snapshot to WinkGoAgent team chat', async () => {
    usePresetAssistantInfoMock.mockReturnValue({ info: null });
    const mcpStatuses = [{ id: 'office', name: 'office', status: 'loaded' as const }];
    ...
    expect(await screen.findByTestId('mock-winkgo_agent-chat')).toBeInTheDocument();
    expect(winkgo_agentChatMock.mock.calls[0]?.[0]).toEqual(
      expect.objectContaining({
        loadedSkills: ['excel'],
        loadedMcpServers: ['office'],
        loadedMcpStatuses: mcpStatuses,
