...
describe('TeamWarmupOverlay failure states', () => {
  it('renders a single-member failure with its simplified error', () => {
    render(
      <TeamWarmupOverlay
        phase='error'
        assistants={[assistant('l', 'leader', 'Codex'), assistant('m', 'teammate', 'WinkGo CLI')]}
        runtimeStatus={runtime([
          ['l', 'ready'],
          ['m', 'failed', "Invalid request: Bad request: Provider 'winkgo_agent' not found"],
        ])}
        colorOf={colorOf}
        onRetry={() => {}}
      />
    ...
    expect(screen.getByText('Member WinkGo CLI failed to start')).toBeInTheDocument();
    // error is simplified (wrapper prefixes stripped)
    expect(screen.getByTestId('team-warmup-error')).toHaveTextContent("Provider 'winkgo_agent' not found");
  ...
  it('summarises multiple failures as a scrollable per-member list', () => {
    render(
      <TeamWarmupOverlay
        ...
        assistants={[
          ...
          assistant('a', 'teammate', 'WinkGo CLI'),
          assistant('c', 'teammate', 'CodeBuddy'),
        ]}
        runtimeStatus={runtime([
          ['l', 'failed', 'ACP error'],
          ['a', 'failed', "Bad request: Provider 'winkgo_agent' not found"],
          ['c', 'ready'],
        ])}
        colorOf={colorOf}
        onRetry={() => {}}
    ...
    expect(box).toHaveTextContent('WinkGo CLI');
    expect(box).toHaveTextContent("Provider 'winkgo_agent' not found");
