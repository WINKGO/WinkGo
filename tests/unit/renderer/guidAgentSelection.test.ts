...
describe('guid assistant selection helpers', () => {
  const assistants: Assistant[] = [
    ...
    assistant({ id: 'bare-winkgo_agent', source: 'generated', runtimeKey: 'winkgo_agent', sort_order: 10 }),
  ...
  it('does not accept legacy backend keys as assistant selection ids', () => {
    ...
    expect(resolveAssistantSelectionKey('winkgo_agent', assistants)).toBeUndefined();
  });

  it('defaults to the generated winkgo_agent assistant when available', () => {
    expect(pickDefaultAssistantSelectionKey(assistants)).toBe('bare-winkgo_agent');
  });
...
function assistant(
  ...
  const isWinkGoAgent = overrides.runtimeKey === 'winkgo_agent';
  return {
    id: overrides.id,
