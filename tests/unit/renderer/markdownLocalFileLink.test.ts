...
describe('resolveLocalFileLinkPath', () => {
  it('recognizes Windows absolute paths emitted as root-relative markdown links', () => {
    expect(resolveLocalFileLinkPath('/C:/Users/Administrator/AppData/Roaming/WinkGo/report.xlsx')).toBe(
      'C:/Users/Administrator/AppData/Roaming/WinkGo/report.xlsx'
    );
  });
  ...
  it('recognizes file-like POSIX absolute paths outside common home and temp roots', () => {
    expect(resolveLocalFileLinkPath('/opt/winkgo/outputs/report.xlsx')).toBe('/opt/winkgo/outputs/report.xlsx');
  });

  it('recognizes line suffixes without confusing Windows drive letters', () => {
    const reference = resolveLocalFileLinkReference('C:/Users/Administrator/AppData/Roaming/WinkGo/logs/app.log:1421');

    expect(reference).toEqual({
      filePath: 'C:/Users/Administrator/AppData/Roaming/WinkGo/logs/app.log',
      rawReference: 'C:/Users/Administrator/AppData/Roaming/WinkGo/logs/app.log:1421',
      line: 1421,
    });
    expect(resolveLocalFileLinkPath('C:/Users/Administrator/AppData/Roaming/WinkGo/logs/app.log:1421')).toBe(
      'C:/Users/Administrator/AppData/Roaming/WinkGo/logs/app.log'
    );
  });
  ...
  it('recognizes line and column suffixes without including the line in the file path', () => {
    const reference = resolveLocalFileLinkReference(
      'C:/Users/Administrator/AppData/Roaming/WinkGo/logs/app.log:1421:7'
    );

    expect(reference).toEqual({
      filePath: 'C:/Users/Administrator/AppData/Roaming/WinkGo/logs/app.log',
      rawReference: 'C:/Users/Administrator/AppData/Roaming/WinkGo/logs/app.log:1421:7',
      line: 1421,
      column: 7,
    });
    expect(resolveLocalFileLinkPath('C:/Users/Administrator/AppData/Roaming/WinkGo/logs/app.log:1421:7')).toBe(
      'C:/Users/Administrator/AppData/Roaming/WinkGo/logs/app.log'
    );
  });
  ...
  it('rejects unsupported hash line formats and remote hash links', () => {
    ...
    expect(resolveLocalFileLinkReference('https://winkgo.top/docs#L10')).toBeNull();
  ...
  it('does not treat normal web links or app routes as local files', () => {
    expect(resolveLocalFileLinkPath('https://winkgo.top/docs')).toBeNull();
    expect(resolveLocalFileLinkPath('/settings')).toBeNull();
  });

  it('formats local file paths as file URLs for browser link copying', () => {
    expect(toLocalFileHref('C:/Users/Administrator/AppData/Roaming/WinkGo/report.xlsx')).toBe(
      'file:///C:/Users/Administrator/AppData/Roaming/WinkGo/report.xlsx'
    );
...
describe('extractLocalImagePaths', () => {
  it('finds generated Windows and POSIX image paths in assistant text', () => {
    const content = [
      'Windows output:',
      '`C:\\Users\\Administrator\\.codex\\generated_images\\rabbit photo.png`',
      'POSIX output:',
      '/tmp/generated/cat.webp',
    ].join('\n');

    expect(extractLocalImagePaths(content)).toEqual([
      'C:\\Users\\Administrator\\.codex\\generated_images\\rabbit photo.png',
      '/tmp/generated/cat.webp',
    ]);
  });

  it('decodes file URLs and removes duplicate paths', () => {
    const content = ['file:///C:/Users/demo/generated/My%20Image.png', 'C:/Users/demo/generated/My Image.png'].join(
      '\n'
    );

    expect(extractLocalImagePaths(content)).toEqual(['C:/Users/demo/generated/My Image.png']);
  });

  it('ignores remote images and paths already rendered as Markdown images', () => {
    const content = [
      'https://example.com/remote.png',
      '![generated](C:/Users/demo/generated/local.png)',
      '<img src="/tmp/generated/already-rendered.webp" />',
    ].join('\n');

    expect(extractLocalImagePaths(content)).toEqual([]);
  });

  it('limits automatic previews when a tool prints many image paths', () => {
    const content = Array.from({ length: 12 }, (_, index) => `C:\\generated\\image-${index}.png`).join('\n');

    expect(extractLocalImagePaths(content)).toHaveLength(8);
  });
});
