...
- `/bump-version` — auto patch + latest WINK GO Core
- `/bump-version 2.2.0` — explicit WinkGo version + latest WINK GO Core
- `/bump-version 2.2.0 --core v0.1.12` — explicit both versions
- `/bump-version --skip-core` — pure frontend release (don't touch winkgoCoreVersion)
...
### Step 3: Determine WinkGo Target Version
...
Display: "Bumping WinkGo: {current} → {target}"

### Step 4: Query WINK GO Core Latest Release

**Skip entirely if `--skip-core` is set.**

```bash
gh release view --repo xuweihafeichangniu-lab/wink --json tagName,body
```

- If `--core <version>` provided → use that tag instead of latest
- Display the WINK GO Core version and ask user to confirm before continuing
- Also read current `winkgoCoreVersion` from `package.json` — if it already matches the queried version, warn the user and ask whether to proceed or use `--skip-core`

### Step 5: Verify WINK GO Core Artifacts

**Skip if `--skip-core`.**

```bash
gh release view <tag> --repo xuweihafeichangniu-lab/wink --json assets --jq '.assets[].name'
```

Verify all 7 expected assets exist:

- `winkgo-core-<tag>-x86_64-unknown-linux-gnu.tar.gz`
- `winkgo-core-<tag>-aarch64-unknown-linux-gnu.tar.gz`
- `winkgo-core-<tag>-x86_64-apple-darwin.tar.gz`
- `winkgo-core-<tag>-aarch64-apple-darwin.tar.gz`
- `winkgo-core-<tag>-x86_64-pc-windows-msvc.zip`
- `winkgo-core-<tag>-aarch64-pc-windows-msvc.zip`
- `winkgo-core-checksums.txt`

Missing → Stop: "WINK GO Core {tag} is missing artifacts: {list}. Wait for CI to complete or check for build failures."
...
- `"winkgoCoreVersion": "{old}"` → `"winkgoCoreVersion": "{new core tag}"` (skip if `--skip-core`)
...
#### 7c: Collect WINK GO Core Changes
...
## [{target}](https://github.com/xuweihafeichangniu-lab/wink/compare/v{previous}...v{target}) ({date YYYY-MM-DD})
...
### Core ([{core tag}](https://github.com/xuweihafeichangniu-lab/wink/releases/tag/{core tag}))
...
git commit -m "chore: bump version to {target} and winkgo-core to {core tag}"
...
```
 ...
 2. git pull --rebase
 3. Determine WinkGo target version (patch+1 or explicit)
 4. Query WINK GO Core latest release (or --core / --skip-core)
 5. Verify WINK GO Core artifacts (7 files)
 6. Edit package.json (version + winkgoCoreVersion)
 7. Generate CHANGELOG entry (frontend commits + WINK GO Core release body)
 8. lint + format + tsc
 9. vitest run
