...
preflight:
    ...
    $failed = $false
    Write-Host "=========================================="
    Write-Host "  WinkGo Build Preflight Check"
    Write-Host "=========================================="
    Write-Host ""
...
build-quick: preflight
    #!/usr/bin/env bash
    export NODE_OPTIONS="--max-old-space-size=8192"
    node scripts/build-with-builder.js auto --skip-native --edition free

# Build package only (no installer) - fastest iteration
build-package: preflight
    #!/usr/bin/env bash
    export NODE_OPTIONS="--max-old-space-size=8192"
    node scripts/build-with-builder.js auto --pack-only --skip-native --edition free

# Force full rebuild (clears cache)
build-force: preflight clean
    #!/usr/bin/env bash
    export NODE_OPTIONS="--max-old-space-size=8192"
    node scripts/build-with-builder.js auto --force --edition free
...
build-win-x64: preflight
    ...
    node scripts/build-with-builder.js x64 --win --x64 --edition free
...
build-win-arm64: preflight
    ...
    $env:MSVS_VERSION = "2022"
    $env:GYP_MSVS_VERSION = "2022"
    node scripts/build-with-builder.js arm64 --win --arm64 --edition free

# Build for Windows (auto-detect arch)
build-win: preflight
    #!/usr/bin/env pwsh
    Write-Host "Cleaning output directory..."
    Get-Process -Name "WinkGo","electron" -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
...
build-mac-arm64: preflight
    ...
    node scripts/build-with-builder.js arm64 --mac --arm64 --edition free
...
build-mac-x64: preflight
    ...
    export npm_config_target=$(node -p "require('./package.json').devDependencies.electron.replace(/[\^~]/g, '')")
    export npm_config_disturl="https://electronjs.org/headers"
    node scripts/build-with-builder.js x64 --mac --x64 --edition free
